...
from covalent_ext import real_covalent_confirmed_candidate_pocket_extraction_smoke as gate  # noqa: E402
        ...
        rows.append(
            {
                ...
                "decision": "pocket extraction smoke accepted" if passed else "pocket extraction smoke blocked",
...
def write_summary(manifest: dict[str, Any], path: str | Path) -> None:
    ...
    text = f"""# Real Covalent Confirmed Candidate Pocket Extraction Smoke v0 Summary
...
This step wrote pocket_atom_table with {manifest["pocket_atom_table_row_count"]} protein pocket atom rows.
This step wrote pocket extraction audit with {manifest["pocket_extraction_audit_row_count"]} rows.
...
This step did not run an atom_site text scan and did not use {parser_tools}.
...
def run() -> int:
    result = gate.build_real_covalent_confirmed_candidate_pocket_extraction_smoke_v0()
    write_csv(result["pocket_rows"], gate.POCKET_ATOM_TABLE_CSV, gate.POCKET_ATOM_COLUMNS)
    write_csv(result["audit_rows"], gate.POCKET_EXTRACTION_AUDIT_CSV, gate.POCKET_AUDIT_COLUMNS)
    ...
    print(
        "real_covalent_confirmed_candidate_pocket_extraction_smoke_v0_"
