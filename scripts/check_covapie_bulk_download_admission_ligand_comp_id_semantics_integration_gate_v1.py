#!/usr/bin/env python3
"""Verify deterministic Step14AU-D2 ligand component ID overlay outputs."""
...
from covalent_ext import covapie_bulk_download_admission_ligand_comp_id_semantics_integration_gate as gate  # noqa: E402
...
def _validate_helper_cases() -> None:
    class StringSubclass(str):
        pass

    assert gate._validate_d1_helpers()
    ...
    max_length = gate.evaluate_admit_003_ligand_comp_id("a" * 32)
    assert max_length["passed"] is True and max_length["canonical_ligand_comp_id"] == "A" * 32
    normalized = gate.evaluate_admit_003_ligand_comp_id("JuG")
    repeated = gate.evaluate_admit_003_ligand_comp_id(normalized["canonical_ligand_comp_id"])
...
def _validate_manifest_and_outputs(
    ...
    _validate_helper_cases()

    assert manifest["stage"] == gate.STAGE
    assert manifest["step_label"] == "Step14AU-D2"
    ...
    assert manifest["source_read_boundary"] == (
        "only_step14au_c2_and_step14au_d1_12_committed_metadata_outputs"
    )
    ...
    assert all(manifest[key] is True for key in (
        "artifact_reference_paths_not_recursively_opened", "all_source_boundary_checks_passed",
        "all_integration_lineage_checks_passed", "all_integrated_rule_checks_passed",
        "all_integrated_field_checks_passed", "all_integrated_context_checks_passed",
        "all_issue_transition_checks_passed", "all_safety_checks_passed", "all_checks_passed",
        "ligand_comp_id_semantics_integrated", "admit_003_rule_logic_ready",
        "candidate_record_id_semantics_integrated", "admit_001_rule_logic_ready",
        "pdb_identifier_semantics_integrated", "ready_for_admission_evaluator_interface_implementation",
    ...
    assert all(manifest[key] is False for key in (
        ...
        "raw_structure_read_current_step", "network_access_used_current_step",
        "external_component_registry_lookup_current_step",
    ))
    assert manifest["recommended_next_step"] == gate.RECOMMENDED_NEXT_STEP
    assert manifest["changed_rule_ids"] == ["ADMIT_003"]
    assert manifest["changed_field_names"] == ["ligand_comp_id"]
    assert manifest["changed_context_items"] == ["ligand_comp_id_contract"]
    ...
    assert (
        manifest["integrated_rule_count"], manifest["semantics_complete_rule_count"],
    ...
    assert (
        manifest["integrated_context_count"], manifest["evaluation_context_item_count"],
    ...
    assert gate._validate_lineage_rows(rules, fields, contexts, source)
...
def main() -> int:
    ...
    first = gate.run_covapie_bulk_download_admission_ligand_comp_id_semantics_integration_gate_v1(root)
    _validate_exact_output_files(root)
    first_hashes = {
        name: _hash(root / name) for name in (*gate.CSV_OUTPUTS, gate.MANIFEST_FILENAME)
    }
    second = gate.run_covapie_bulk_download_admission_ligand_comp_id_semantics_integration_gate_v1(root)
    _validate_exact_output_files(root)
    second_hashes = {
        name: _hash(root / name) for name in (*gate.CSV_OUTPUTS, gate.MANIFEST_FILENAME)
    }
    ...
    print("ligand_comp_id_semantics_integrated=true")
    print("admit_003_rule_logic_ready=true")
    ...
    print("ligand_comp_id_semantics_integration_outputs_byte_identical=true")
    print("covapie_bulk_download_admission_ligand_comp_id_semantics_integration_gate_v1_passed")
    return 0
