#!/usr/bin/env python3
"""Repository-state-neutral checker for the cumulative1000 2VS refresh V1."""
...
from covalent_ext import covapie_completed_human_decision_reconciliation_with_2vs_v1 as reconciliation  # noqa: E402
from covalent_ext import covapie_2vs_completed_decision_ingestion_and_task_label_availability_v1 as ingestion  # noqa: E402
from covalent_ext import covapie_cumulative1000_current_global_readiness_census_with_prf_v1 as predecessor  # noqa: E402
from covalent_ext import covapie_cumulative1000_current_global_readiness_census_with_2vs_v1 as subject  # noqa: E402
...
EXPECTED_CENSUS_SHA256 = "e0e4eb86d2961e2db2ca139ffe5492cfe9675b768826be85a3d0516b532ae24a"
EXPECTED_SUMMARY_SHA256 = "1b5cca68c2b81426cfae86921a666d8766dc40d31032c24ba90888f0b88588f7"
EXPECTED_BINDINGS_SHA256 = "77e22e96e118d4ebc7b468daee89694d37c4aa0a51f63ae937e4498d82724b59"
EXPECTED_TWO_VS_EVENT_IDS = (
...
EXPECTED_TWO_VS_STRUCTURAL_CELLS = {
...
FROZEN_BINDINGS = (
    ("PREDECESSOR_PRF_CENSUS_OWNER", "src/covalent_ext/covapie_cumulative1000_current_global_readiness_census_with_prf_v1.py", "repository_relative", 53480, "11c8fcadaa7fb25247de012ff5c9cb984006221969808dddfc87ba867286ad6f"),
    ("PREDECESSOR_PRF_MATERIALIZED_CENSUS", "data/derived/covalent_small/covapie_cumulative1000_current_global_readiness_census_with_prf_v1/covapie_cumulative1000_current_global_readiness_census_with_prf_v1.csv", "repository_relative", 506220, "a707cb60c8f788f9ad0e94e89c4038226cfa5f94c15b0afcfa6e36adca3c1b12"),
    ("PREDECESSOR_PRF_MATERIALIZED_SUMMARY", "data/derived/covalent_small/covapie_cumulative1000_current_global_readiness_census_with_prf_v1/covapie_cumulative1000_current_global_readiness_summary_with_prf_v1.json", "repository_relative", 14742, "82d4d36beb21efb2a588beaea9d3b9c61a6275596482e39ff45341e4cbe316f7"),
    ("PREDECESSOR_PRF_MATERIALIZED_MANIFEST", "data/derived/covalent_small/covapie_cumulative1000_current_global_readiness_census_with_prf_v1/covapie_cumulative1000_current_global_readiness_manifest_with_prf_v1.json", "repository_relative", 25586, "0e3b96b63577c536639143660361ad23744ccdf5327d62a2b4022c48001ff3d6"),
    ("TWO_VS_RECONCILIATION_SUCCESSOR", "src/covalent_ext/covapie_completed_human_decision_reconciliation_with_2vs_v1.py", "repository_relative", 11923, "e4ffb8300f40e3883560605cbf485481a42adc9321c7d602104c7467ad3df4f7"),
    ("TWO_VS_INGESTION_OWNER", "src/covalent_ext/covapie_2vs_completed_decision_ingestion_and_task_label_availability_v1.py", "repository_relative", 79775, "8812b4f9d0c77af4228b6f71ec9183867fec778311b290f8362a1164081e1409"),
    ("TWO_VS_EVENT_TASK_LABEL_AVAILABILITY", "data/derived/covalent_small/covapie_2vs_completed_decision_ingestion_and_task_label_availability_v1/covapie_2vs_event_task_label_availability_v1.csv", "repository_relative", 13276, "b5c08fecbd5a68408cea6b7ce4747734f3d6c3ea37d81aef939a634d6439c5cc"),
    ("TWO_VS_FORMAL_HUMAN_DECISION", "covapie-state/manual-review-aids/cumulative1000-high-yield-calibration-v1/2VS_COVAPIE_BULK_REVIEW_UNIT_5834329D9E2A1F22/formal-human-decision-v1/2vs_formal_human_decision_v1.json", "repository_parent_relative", 28640, "49f33bb2a21669ddb7ab8e98cfa710380e031b280855d5f3ebe6796cde2d06aa"),
)
...
def verify_frozen_bindings_v1(root: Path) -> list[dict[str, object]]:
    ...
    if ingestion.build_artifacts_v1(root)[ingestion.MATRIX] != _read(matrix_path, "2VS_SOURCE_DERIVED_MATRIX"):
        raise ValueError("2VS_MATRIX_NOT_SOURCE_DERIVED_FROM_FROZEN_FORMAL_DECISION")
    return verified
...
def independently_verify_delta_v1(root: Path, rows: list[dict[str, str]]) -> dict[str, object]:
    ...
    two_vs = set(EXPECTED_TWO_VS_EVENT_IDS)
    if (
        len(two_vs) != 8
        or not two_vs <= set(before)
        or [int(before[event_id]["scaleup_rank"]) for event_id in EXPECTED_TWO_VS_EVENT_IDS]
        != [848, 849, 850, 851, 859, 860, 861, 862]
    ):
        raise ValueError("PREDECESSOR_TWO_VS_IDENTITY_INVALID")
    expected_prior = {
    ...
    if any(
        ...
        for event_id in two_vs
    ):
        raise ValueError("PREDECESSOR_TWO_VS_STATE_INVALID")
    changed = {event_id for event_id in before if before[event_id] != after[event_id]}
    if changed != two_vs or len(changed) != 8:
        raise ValueError("DELTA_NOT_EXACT_TWO_VS8")
    if any(before[event_id] != after[event_id] for event_id in set(before) - two_vs):
        raise ValueError("NON_TWO_VS_ROW_DRIFT")
    if [int(after[event_id]["scaleup_rank"]) for event_id in EXPECTED_TWO_VS_EVENT_IDS] != [848, 849, 850, 851, 859, 860, 861, 862]:
        raise ValueError("TWO_VS_RANK_DRIFT")
    ...
    for event_id in two_vs:
        ...
        if changed_fields != expected_changed_fields:
            raise ValueError("TWO_VS_CHANGED_FIELD_SET_INVALID:" + event_id)
        if any(before[event_id][field] != after[event_id][field] for field in subject._STRUCTURAL_IDENTITY_FIELDS_V1):
            raise ValueError("TWO_VS_STRUCTURAL_DRIFT:" + event_id)
        row = after[event_id]
        if (
            row["review_unit_id"] != subject.TWO_VS_REVIEW_UNIT_ID_V1
            ...
            or row["human_review_authority_source"] != subject.TWO_VS_FORMAL_DECISION_SOURCE
            or row["chemistry_disposition"] != "POSITIVE"
            or row["chemistry_authority_source"] != subject.TWO_VS_EVENT_MATRIX_SOURCE
            ...
            raise ValueError("TWO_VS_SEMANTICS_INVALID:" + event_id)
        if any(
            row[field] != value
            for field, value in EXPECTED_TWO_VS_STRUCTURAL_CELLS.items()
        ):
            raise ValueError("TWO_VS_STRUCTURAL_COVERAGE_INVALID:" + event_id)
    ...
    checks = (
        selected(rows, "chemistry_disposition", "POSITIVE") == selected(frozen.rows, "chemistry_disposition", "POSITIVE") | two_vs,
        selected(rows, "chemistry_disposition", "UNRESOLVED") == selected(frozen.rows, "chemistry_disposition", "UNRESOLVED") - two_vs,
        selected(rows, "task_relevance_disposition", "RELEVANT") == selected(frozen.rows, "task_relevance_disposition", "RELEVANT") | two_vs,
        selected(rows, "task_relevance_disposition", "UNRESOLVED") == selected(frozen.rows, "task_relevance_disposition", "UNRESOLVED") - two_vs,
        selected(rows, "training_use_disposition", "EXCLUDE_FROM_TRAINING_ONLY") == selected(frozen.rows, "training_use_disposition", "EXCLUDE_FROM_TRAINING_ONLY") | two_vs,
...
def independently_verify_counts_v1(rows: list[dict[str, str]], summary: dict[str, object]) -> None:
    ...
    if role_counts != Counter({"STRICT_LINKER_PRESENT_V1": 31, "DIRECT_ATTACHMENT_OPTIONAL_LINKER_V1": 43}):
    ...
    if summary["reactive_pair"] != {  # type: ignore[index]
        ...
        "sample_level_authoritative_pair_count": 74,
    ...
    expected_role = {
        "role_partition_sample_authoritative_count": 74,
        "role_profile_counts": {
            "STRICT_LINKER_PRESENT_V1": 31,
            "DIRECT_ATTACHMENT_OPTIONAL_LINKER_V1": 43,
            "other": 0,
        },
        "canonical_mask_structural_labels_available_count": 74,
        "all_five_structurally_applicable_count": 31,
        "direct_profile_A_B3_C_count": 43,
        "unknown_role_row_count": 926,
    ...
    expected_exact5 = {
        "task_count": 5,
        "tasks": [
            {"task_id": 0, "semantic_name": "warhead_only", "display_alias": "A", "structurally_applicable_authoritative_role_count": 74},
            {"task_id": 1, "semantic_name": "linker_plus_warhead", "display_alias": "B", "structurally_applicable_authoritative_role_count": 31},
            {"task_id": 2, "semantic_name": "scaffold_plus_warhead", "display_alias": "B2", "structurally_applicable_authoritative_role_count": 31},
            {"task_id": 3, "semantic_name": "scaffold_only", "display_alias": "B3", "structurally_applicable_authoritative_role_count": 74},
            {"task_id": 4, "semantic_name": "scaffold_plus_linker_plus_warhead", "display_alias": "C", "structurally_applicable_authoritative_role_count": 74},
        ],
        "B3_present": True,
    ...
    if summary["training_stage"] != {  # type: ignore[index]
        ...
        "future_candidate_source_composition": {"FFQ": 4, "POA": 8, "G3H": 0, "ONL": 0, "PRF": 0, "2VS": 0},
    ...
    if summary["human_review"] != {  # type: ignore[index]
        ...
        "pending_event_count": 257,
        "current_pending_review_unit_count": 121,
    }:
        raise ValueError("HUMAN_REVIEW_SUMMARY_INVALID")
    expected_blockers = {
        "non_exclusive_counts_must_not_be_summed": True,
        "chemistry_unresolved": {"all_1000": 840},
        "pair_authority_absent": {"all_1000": 926, "within_positive_74": 0},
        "role_authority_absent": {"all_1000": 926, "within_positive_74": 0},
        "human_training_exclusion": {"within_positive_74": 45},
        "missing_split_authority": {"within_positive_74": 33, "within_include_29": 4},
        "missing_tensor_integration": {"within_positive_74": 33, "within_include_29": 0, "all_missing_are_training_excluded_population": True, "missing_source_composition": {"G3H": 8, "ONL": 9, "PRF": 8, "2VS": 8}},
        "missing_POST_training_authority": {"within_positive_74": 57, "within_include_29": 12},
        "missing_training_admission": {"within_positive_74": 69, "within_include_29": 24},
        "feature_semantics_pending": {"within_positive_74": 74},
    }
...
def independently_compute_top10_v1(root: Path, reconciled_rows: tuple[dict[str, str], ...]) -> list[dict[str, object]]:
    ...
    if len(candidates) != 121:
        raise ValueError("PENDING_QUEUE_NOT_EXACT121")
...
def verify_semantic_bindings_v1(root: Path, observed: tuple[dict[str, object], ...]) -> None:
    frozen = predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_prf_v1(root)
    if len(frozen.semantic_source_bindings) != 53:
        raise ValueError("PREDECESSOR_SEMANTIC_BINDING_COUNT_NOT_53")
    by_identity = {(row["path_namespace"], row["path"]): dict(row) for row in frozen.semantic_source_bindings}
    for role, relative, namespace, byte_count, sha256 in FROZEN_BINDINGS:
        if role == "PREDECESSOR_PRF_MATERIALIZED_MANIFEST":
            continue
    ...
    if observed != expected or len(observed) != 60:
...
def run_check_v1(root: Path = ROOT) -> dict[str, object]:
    ...
    fresh = subject.compute_covapie_cumulative1000_current_global_readiness_census_with_2vs_v1(root)
    if not subject.validate_covapie_cumulative1000_current_global_readiness_census_with_2vs_v1(fresh):
        raise ValueError("PUBLIC_VALIDATOR_DID_NOT_PASS")
    built = subject.build_covapie_cumulative1000_current_global_readiness_artifacts_with_2vs_v1(root)
    ...
    reconciled = reconciliation.reconcile_real_completed_human_decisions_with_2vs_v1(root)
    if (
        reconciled.review_summary["completed_positive_event_count"] != 57
        or reconciled.review_summary["completed_total_event_count"] != 81
        or reconciled.review_summary["unreviewed_event_count"] != 257
        or reconciled.review_summary["unreviewed_unit_count"] != 121
    ):
    ...
    if (
        expected_top[0]["review_unit_id"] != "COVAPIE_BULK_REVIEW_UNIT_9723B0F9EC07CC81"
        or expected_top[0]["ligand_component_ids"] != ["1F8"]
        or expected_top[0]["pdb_ids"] != ["3ORX"]
        or expected_top[0]["event_count"] != 8
    ):
        raise ValueError("NEXT_PRIORITY_NOT_1F8_EXACT8")
    verify_manifest_v1(root, materialized, fresh)

    with tempfile.TemporaryDirectory() as first, tempfile.TemporaryDirectory() as second:
        one = subject.materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_2vs_v1(root, Path(first))
        two = subject.materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_2vs_v1(root, Path(second))
    ...
    if (
        ...
        or boundary["next_priority_review_unit"] != "COVAPIE_BULK_REVIEW_UNIT_9723B0F9EC07CC81"
        or boundary["next_priority_review_ligand"] != "1F8"
    ...
    return {
        ...
        **delta,
        "refreshed_positive_count": 74,
        "pending_review_unit_count": 121,
...
def main() -> int:
    ...
    print("COVAPIE_CUMULATIVE1000_CURRENT_GLOBAL_READINESS_CENSUS_WITH_2VS_V1_CHECK:PASS")
