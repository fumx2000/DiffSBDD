#!/usr/bin/env python3
"""Independent fail-closed checker for the cumulative1000 with-GVE census V1."""
...
from covalent_ext import covapie_completed_human_decision_reconciliation_with_gve_v1 as reconciliation  # noqa: E402
from covalent_ext import covapie_cumulative1000_current_global_readiness_census_with_gve_v1 as subject  # noqa: E402
from covalent_ext import covapie_gve_completed_decision_ingestion_and_task_label_availability_v1 as ingestion  # noqa: E402


BASELINE_COMMIT = "9f13a9efba492cb3d38c0ec4c4777b5499005ca3"
EXPECTED_CENSUS_SHA256 = "540b072d45924688a640e5ca16484500b3815bc66267d73ceead81246b226557"
EXPECTED_SUMMARY_SHA256 = "3fc655a29f3674f951a75e9fc73198d2140ff86e7c58779d501d77b67dcd2d14"
EXPECTED_BINDINGS_SHA256 = "0468670c494a4610060d9538822ee4b6d058f9c2840a2da5087cf4d694ae5323"
FORBIDDEN_SUFFIXES = (
...
def verify_frozen_bindings_v1(root: Path) -> None:
    ...
    if len(payload) != 65255 or _sha(payload) != (
...
def independently_verify_matrix_v1(root: Path) -> list[dict[str, str]]:
    payload = _read(root / subject.GVE_EVENT_MATRIX_RELATIVE, "GVE_MATRIX")
    ...
    if (
        ...
        raise ValueError("GVE_MATRIX_IDENTITY_INVALID")
    expected = {
    ...
    for row in rows:
        if any(row[key] != value for key, value in expected.items()):
            raise ValueError("GVE_MATRIX_SEMANTICS_INVALID:" + row["canonical_event_id"])
        ...
        if (
            ...
            raise ValueError("GVE_MATRIX_EXACT5_INVALID")
    return rows
...
def independently_verify_delta_v1(
    ...
    if set(before) != set(after) or changed != exact4:
        raise ValueError("DELTA_NOT_EXACT_GVE_EXACT4")
    ...
    expected_after = {
        ...
        "human_review_completed": "true",
        "human_review_authority_source": subject.GVE_HUMAN_DECISION_SOURCE,
        "chemistry_disposition": "POSITIVE",
        "chemistry_authority_source": subject.GVE_EVENT_MATRIX_SOURCE,
        "positive_authority_source": subject.GVE_EVENT_MATRIX_SOURCE,
        "task_relevance_disposition": "NOT_RELEVANT",
        "task_relevance_authority_source": subject.GVE_EVENT_MATRIX_SOURCE,
    ...
    for event_id in exact4:
        ...
        if fields != subject._ACTUAL_CHANGED_GVE_FIELDS_V1:
            raise ValueError("GVE_CHANGED_FIELDS_NOT_EXACT12:" + event_id)
        if unchanged_authorized != subject._AUTHORIZED_BUT_UNCHANGED_GVE_FIELDS_V1:
            raise ValueError("GVE_AUTHORIZED_UNCHANGED_NOT_EXACT3:" + event_id)
        if fields & subject._FORBIDDEN_GVE_CHANGED_FIELDS_V1:
            raise ValueError("GVE_FORBIDDEN_FIELD_CHANGED:" + event_id)
        if any(after[event_id][key] != value for key, value in expected_after.items()):
            raise ValueError("GVE_FINAL_SEMANTICS_INVALID:" + event_id)
        if matrix_by_event[event_id]["chemistry_human_authoritative"] != "true":
            raise ValueError("GVE_POSITIVE_AUTHORITY_MISSING")
    legacy_1xd3 = {
        event_id for event_id in before if ":1XD3:" in event_id and ":GVE:" in event_id
    }
    rank322 = next(row["canonical_event_id"] for row in before_rows if row["scaleup_rank"] == "322")
    if any(before[event_id] != after[event_id] for event_id in legacy_1xd3 | {rank322}):
        raise ValueError("LEGACY_GVE_OR_RANK322_CHANGED")
    return {
        "changed_event_count": len(changed),
        "unchanged_event_count": len(rows) - len(changed),
        "authorized_overlay_field_count": len(subject._AUTHORIZED_GVE_OVERLAY_FIELDS_V1),
        "authorized_but_unchanged_field_count": len(subject._AUTHORIZED_BUT_UNCHANGED_GVE_FIELDS_V1),
        "actual_changed_field_count": len(subject._ACTUAL_CHANGED_GVE_FIELDS_V1),
    }
...
def independently_verify_counts_v1(
    ...
    if len(legacy) != 90 or orthogonal != set(ingestion.EXPECTED_EVENT_IDS):
    ...
    if (
        ...
        or blockers["pair_authority_absent"] != {"all_1000": 864, "within_chemistry_positive": 0}
        or blockers["role_authority_absent"] != {"all_1000": 868, "within_chemistry_positive": 4}
        or blockers["missing_tensor_integration"]["within_chemistry_positive"] != 95
        or blockers["missing_tensor_integration"]["within_training_include"] != 31
        or blockers["missing_tensor_integration"]["missing_source_composition"].get("GVE") != 4
    ...
    if summary["chemistry"]["positive_source_composition"].get("GVE") != 4:
        raise ValueError("GVE_POSITIVE_SOURCE_COMPOSITION_INVALID")
    if "GVE" in summary["training_stage"]["future_candidate_source_composition"]:
        raise ValueError("GVE_FUTURE_CANDIDATE_SOURCE_LEAK")
...
def independently_compute_top10_v1(
    ...
    if len(candidates) != 107 or top[0] != {
        "rank": 1,
...
def verify_manifest_v1(
    ...
    if (
        ...
        or manifest["semantic_source_binding_count"] != 150
    ...
    if (
        ...
        or contract["actual_changed_field_count_per_gve_row"] != 12
...
def run_check_v1(root: Path = ROOT) -> dict[str, object]:
    ...
    computation = subject.compute_covapie_cumulative1000_current_global_readiness_census_with_gve_v1(root)
    built = subject._build_artifacts_from_computation_v1(root, computation)
    rebuilt = subject.build_covapie_cumulative1000_current_global_readiness_artifacts_with_gve_v1(root)
    ...
    reconciled = reconciliation.reconcile_real_completed_human_decisions_with_gve_v1(root)
    if (
        len(reconciled.source_bindings) != 20
        or len(reconciled.normalized_facts) != 123
        or reconciled.review_summary["completed_total_event_count"] != 147
        or reconciled.review_summary["completed_total_unit_count"] != 24
    ):
        raise ValueError("GVE_RECONCILIATION_COUNTS_INVALID")
    ...
    for key in (
        "GVE_REVIEW_COMPLETED", "READY_FOR_LCY_REVIEW_PREPARATION",
    ...
    for key in (
        "LCY_REVIEW_STARTED", "NEXT_REVIEW_STARTED", "QUEUE_REFRESH", "TRAINING_STARTED",
        "READY_FOR_TRAINING", "new_human_authority_created", "new_pair_authority_created",
        "new_role_authority_created", "formal_validator_executed",
    ):
...
def main() -> int:
    ...
    print("WITH_GVE_CENSUS_SUCCESSOR_CANDIDATE_PASS=true")
    print("WITH_GVE_CENSUS_SCHEMA47_PRESERVED=true")
    print("GVE_EXACT4_OVERLAY_PASS=true")
    print("NON_GVE_ROWS_UNCHANGED_996_OF_996=true")
    print("AUTHORIZED_GVE_OVERLAY_EXACT15=true")
    print("ACTUAL_GVE_CHANGED_FIELDS_EXACT12=true")
    print("GVE_TASK_NOT_RELEVANT=true")
    print("GVE_CHEMISTRY_POSITIVE=true")
    print("GVE_TRAINING_NOT_APPLICABLE=true")
    ...
    print("TASK_NEGATIVE_CHEMISTRY_POSITIVE_POPULATION_EXACTLY_GVE_EXACT4=true")
    print("GLOBAL_TASK_NEGATIVE_RELAXATION_ALLOWED=false")
    print("GVE_PAIR_AUTHORITY_COUNT_136=true")
    print("GVE_ROLE_AUTHORITY_COUNT_132=true")
    print("GVE_MASK_AUTHORITY_COUNT_132=true")
    print("EXACT5_B3_PRESENT=true")
    print("SIXTH_TASK=false")
    print("SUMMARY_POPULATION_CODED_STALE_KEYS_ABSENT=true")
    print("SEMANTIC_SOURCE_BINDINGS_150=true")
    print("READY_FOR_LCY_REVIEW_PREPARATION=true")
    print("LCY_REVIEW_STARTED=false")
