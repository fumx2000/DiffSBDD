...
from covalent_ext import covapie_cys_sg_result_review_decision_application_gate as gate  # noqa: E402
...
def write_summary(manifest: dict[str, Any], path: str | Path) -> None:
    ...
    text = f"""# CovaPIE CYS/SG Result Review Decision Application Gate v0 Summary
...
applied_result_review_decision_count: `{manifest["applied_result_review_decision_count"]}`
ready_candidate_materialization_input_count: `{manifest["ready_candidate_materialization_input_count"]}`
blocked_carry_forward_applied_count: `{manifest["blocked_carry_forward_applied_count"]}`
...
def run() -> int:
    result = gate.run_covapie_cys_sg_result_review_decision_application_gate_v0()
    ...
    write_csv(result["ready_rows"], gate.READY_INPUT_MANIFEST_CSV, gate.READY_INPUT_COLUMNS)
    write_json(result["ready_rows"], gate.READY_INPUT_MANIFEST_JSON)
    ...
    print("covapie_cys_sg_result_review_decision_application_gate_v0_passed" if manifest["all_checks_passed"] else "covapie_cys_sg_result_review_decision_application_gate_v0_blocked")
    for key in [
        "applied_result_review_decision_count",
        "ready_candidate_materialization_input_count",
        "blocked_carry_forward_applied_count",
