"""Repository-state-neutral checker for CHT reconciliation successor V1."""
...
EXACT4_PATHS = (
    ...
    "scripts/"
    "check_covapie_completed_human_decision_reconciliation_with_cht_v1.py",
    "tests/"
    "test_covapie_completed_human_decision_reconciliation_with_cht_v1.py",
    "docs/covapie_completed_human_decision_reconciliation_with_cht_v1_guide.md",
)
CURRENT_CENSUS_ROOT = (
...
FROZEN_REPOSITORY_FILES = (
    ...
    (
        "CURRENT_GLOBAL_CENSUS_WITH_NEQ_OWNER",
        "src/covalent_ext/"
    ...
    (
        "CURRENT_GLOBAL_CENSUS_WITH_NEQ_CSV",
        CURRENT_CENSUS_ROOT
        + "covapie_cumulative1000_current_global_readiness_census_with_neq_v1.csv",
        521299,
        "8ffbb6df299321393b0aeba8945ed6a4de7f74ed659253d874494a7757e782f2",
    ),
    (
        "CURRENT_GLOBAL_CENSUS_WITH_NEQ_SUMMARY",
        CURRENT_CENSUS_ROOT
        + "covapie_cumulative1000_current_global_readiness_summary_with_neq_v1.json",
        15641,
        "08f61b61014658a51be184285aae478989717f67808ef0fe8e0dc09d068312d9",
    ),
    (
        "CURRENT_GLOBAL_CENSUS_WITH_NEQ_MANIFEST",
        CURRENT_CENSUS_ROOT
        + "covapie_cumulative1000_current_global_readiness_manifest_with_neq_v1.json",
        36162,
...
EXPECTED_SOURCE_FACT_COUNTS = (8, 16, 8, 9, 8, 8, 8, 7, 6, 5)
...
EXPECTED_FUTURE_CENSUS = {
    ...
    "direct_profile": 56,
    "warhead_only_A": 100,
    "linker_plus_warhead_B": 44,
    "scaffold_plus_warhead_B2": 44,
    "scaffold_only_B3": 100,
    "scaffold_plus_linker_plus_warhead_C": 100,
}
FORBIDDEN_SUFFIXES = (
...
def verify_candidate_exact4_v1(repo_root: Path) -> dict[str, object]:
    """Verify portable Exact4 files in candidate-untracked or tracked-clean state."""
...
def _verify_frozen_inputs(repo_root: Path) -> dict[str, object]:
    ...
    artifacts.append(
        _verify_file(
            root.parent / CHT_FORMAL_RELATIVE,
            label="CHT_FORMAL_HUMAN_DECISION",
            expected_byte_count=CHT_FORMAL_BYTE_COUNT,
            expected_sha256=CHT_FORMAL_SHA256,
        )
    )
...
def _verify_thin_successor_architecture(repo_root: Path) -> dict[str, object]:
    tree = ast.parse(_read_regular_file(repo_root / EXACT4_PATHS[0], "CHT_SUCCESSOR"))
    if subject.__all__ != EXPECTED_PUBLIC_API:
        raise ValueError("PUBLIC_API_NOT_MINIMAL_EXACT4")
    if subject.CHT_TRANSITION_ADAPTER_CREATED is not False:
        raise ValueError("CHT_TRANSITION_ADAPTER_FLAG_INVALID")
    function_names = {
    ...
    if any(
        ...
        raise ValueError("CHT_TRANSITION_HELPER_CREATED")
    generic_classes = {
    ...
    duplicated_generic_functions = function_names & {
        "_validate_source_binding",
        "_validate_fact",
        "_review_summary",
    }
    if duplicated_generic_functions:
    ...
    expected_once = {
        "neq_source_loader": "load_real_completed_decision_sources_with_neq_v1",
        "cht_ingestion_loader": "load_frozen_formal_decision_v1",
    ...
    if called.count("reconcile_real_completed_human_decisions_with_neq_v1") != 0:
        raise ValueError("NEQ_RESULT_RECONCILIATION_FORBIDDEN")
    return {
        ...
        **{key + "_ast_call_count": called.count(value) for key, value in expected_once.items()},
        "neq_reconciler_ast_call_count": 0,
    }
...
def _run_production_pipeline_counted(
    ...
    subject.neq_successor.load_real_completed_decision_sources_with_neq_v1 = counted_neq
    subject.cht_ingestion_owner.load_frozen_formal_decision_v1 = counted_cht
    ...
    try:
        result = subject.reconcile_real_completed_human_decisions_with_cht_v1(repo_root)
    finally:
        subject.neq_successor.load_real_completed_decision_sources_with_neq_v1 = original_neq
        subject.cht_ingestion_owner.load_frozen_formal_decision_v1 = original_cht
...
def _verify_generic_protections(
    ...
    for status in (generic.CURRENTLY_IN_PROGRESS, generic.COMPLETED_HUMAN_POSITIVE):
        ...
        for row in drifted:
            if row["canonical_event_id"] in cht_ids:
    ...
    row = next(row for row in mixed if row["canonical_event_id"] in cht_ids)
    ...
    return {
        "cht_in_progress_rejected": True,
        "cht_completed_rejected": True,
        "cht_mixed_status_rejected": True,
...
def _published_census_state(repo_root: Path) -> dict[str, object]:
    ...
    try:
        rows = tuple(csv.DictReader(io.StringIO(_read_regular_file(csv_path, "CURRENT_CENSUS").decode("utf-8"), newline="")))
    ...
    cht_ids = set(subject._CHT_EVENT_IDS)
    cht_rows = [row for row in rows if row.get("canonical_event_id") in cht_ids]
    if len(cht_rows) != 5 or any(
        row.get("review_unit_id") != subject._CHT_REVIEW_UNIT_ID
        ...
        raise ValueError("PUBLISHED_CURRENT_CENSUS_CHT_STATE_INVALID")
    counts = {
        "positive": sum(row.get("chemistry_disposition") == generic.CHEMISTRY_POSITIVE for row in rows),
        "relevant": sum(row.get("task_relevance_disposition") == generic.TASK_RELEVANT for row in rows),
        "training_include": sum(row.get("training_use_disposition") == generic.TRAINING_INCLUDE for row in rows),
        "training_exclude": sum(row.get("training_use_disposition") == generic.TRAINING_EXCLUDE for row in rows),
        "future_candidates": sum(row.get("future_training_admission_candidate") == "true" for row in rows),
...
def _verify_future_census_informational(
    ...
    if type(summary) is not dict or len(source.facts) != 5:
    ...
    if any(
        ...
        raise ValueError("FUTURE_CENSUS_CHT_DELTA_INVALID")
    ...
    observed = {
        "positive": summary["chemistry"]["POSITIVE"]["count"] + 5,
        "relevant": summary["task_relevance"]["RELEVANT"]["count"] + 5,
        "training_include": summary["training_use"]["INCLUDE"]["count"],
        "training_exclude": summary["training_use"][generic.TRAINING_EXCLUDE]["count"] + 5,
        "future_candidates": summary["training_stage"]["future_training_admission_candidate_count"],
        "sample_pair_authority": summary["reactive_pair"]["sample_level_authoritative_pair_count"] + 5,
        "sample_role_authority": summary["role"]["role_partition_sample_authoritative_count"] + 5,
        "strict_profile": summary["role"]["role_profile_counts"]["STRICT_LINKER_PRESENT_V1"] + 5,
        "direct_profile": summary["role"]["role_profile_counts"]["DIRECT_ATTACHMENT_OPTIONAL_LINKER_V1"],
        "warhead_only_A": task_counts["warhead_only"] + 5,
        "linker_plus_warhead_B": task_counts["linker_plus_warhead"] + 5,
        "scaffold_plus_warhead_B2": task_counts["scaffold_plus_warhead"] + 5,
        "scaffold_only_B3": task_counts["scaffold_only"] + 5,
        "scaffold_plus_linker_plus_warhead_C": task_counts["scaffold_plus_linker_plus_warhead"] + 5,
    }
...
def run_check_v1(repo_root: Path) -> dict[str, object]:
    """Run all read-only CHT reconciliation checks and return evidence."""
    ...
    source = subject.project_cht_completed_decision_v1(repo_root=root)
    if (
        type(source) is not generic.NormalizedDecisionSource
        or tuple(fact.canonical_event_id for fact in source.facts) != subject._CHT_EVENT_IDS
        or len(source.facts) != 5
        or any(
            fact.task_relevance_disposition != generic.TASK_RELEVANT
            or fact.chemistry_disposition != generic.CHEMISTRY_POSITIVE
            or fact.training_disposition != generic.TRAINING_EXCLUDE
        ...
        raise ValueError("CHT_SOURCE_PROJECTION_INVALID")
    bound = cht_ingestion_owner.load_frozen_formal_decision_v1(root)
    normalized_events = bound.get("normalized", {}).get("events", [])  # type: ignore[union-attr]
    if len(normalized_events) != 5 or any(event.get("training_admitted") is not False for event in normalized_events):
        raise ValueError("CHT_OWNER_VALIDATED_ADMISSION_STATE_INVALID")

    historical = generic.load_real_historical_reconciliation_v1(root)
    subject._prove_cht_original_unreviewed_prior_v1(historical)
    result, calls, captured_original, captured_adapted = _run_production_pipeline_counted(root)
    subject._prove_cht_rows_unchanged_after_onl_normalization_v1(captured_original, captured_adapted)

    sources = subject.load_real_completed_decision_sources_with_cht_v1(root)
    ...
    if (
        ...
        or len({item.binding.stable_identity for item in sources}) != 10
        or len({item.binding.review_unit_id for item in sources}) != 10
        or len(event_ids) != len(set(event_ids)) != 83
    ):
        raise ValueError("EXACT10_SOURCE_COMPOSITION_INVALID")
    if len(event_ids) != 83 or len(set(event_ids)) != 83:
        raise ValueError("EXACT10_SOURCE_FACT_COUNT_OR_COLLISION_INVALID")
    ...
    if dispositions != Counter({generic.TRAINING_INCLUDE: 19, generic.TRAINING_EXCLUDE: 64}):
        raise ValueError("NORMALIZED_TRAINING_DISPOSITIONS_INVALID")
    final_cht = {
        row["canonical_event_id"]: row
        for row in result.reconciled_rows
        if row["canonical_event_id"] in set(subject._CHT_EVENT_IDS)
    }
    expected_authority = json.dumps([source.binding.source_path], separators=(",", ":"), sort_keys=True)
    if len(final_cht) != 5 or any(
        ...
        for row in final_cht.values()
    ):
        raise ValueError("FINAL_CHT_RECONCILED_ROWS_INVALID")

    normal = generic.reconcile_completed_human_decisions_v1(captured_adapted, sources)
    reversed_result = generic.reconcile_completed_human_decisions_v1(captured_adapted, tuple(reversed(sources)))
    ...
    future = _verify_future_census_informational(source, published)
    pending = published["summary"]["top_pending_review_units_by_event_yield"]  # type: ignore[index]
    next_pending = pending[1]
    if (
        pending[0].get("review_unit_id") != subject._CHT_REVIEW_UNIT_ID
        or next_pending.get("review_unit_id") != "COVAPIE_BULK_REVIEW_UNIT_18734D9C06222450"
        or next_pending.get("ligand_component_ids") != ["OZJ"]
        or next_pending.get("event_count") != 4
        or next_pending.get("pdb_ids") != ["4CL8"]
    ):
        raise ValueError("EXPECTED_NEXT_PENDING_OZJ_INVALID")
    ...
    return {
        ...
        "onl_adapter_left_cht_unchanged": True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("source_bindings=10")
    print("normalized_facts=83")
    print("completed_positive=83")
    print("pending=231")
    print("published_global_positive=95")
    print("future_global_positive=100_INFORMATIONAL_ONLY")
    print("next_pending=OZJ_INFORMATIONAL_ONLY")
