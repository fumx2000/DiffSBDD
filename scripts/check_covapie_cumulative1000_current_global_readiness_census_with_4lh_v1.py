#!/usr/bin/env python3
"""Independent fail-closed checker for the with-4LH readiness census V1."""
...
from covalent_ext import (  # noqa: E402
    covapie_completed_human_decision_reconciliation_with_4lh_v1 as reconciliation,
)
from covalent_ext import (  # noqa: E402
    covapie_cumulative1000_current_global_readiness_census_with_4lh_v1 as subject,
)


BASELINE_COMMIT = "59da36c0f33318739ae155b997ec3e0bd7cc627a"
EXPECTED_CENSUS_SHA256 = "a8166f4c000dbeaa8c5672900fa1838748ad32c0b2810c1a132616d5c675e1aa"
EXPECTED_SUMMARY_SHA256 = "7ad61675927467d4f1b5ab7e54a42649815b98bb2835201397f30432fcf1716b"
EXPECTED_BINDINGS_SHA256 = "1004bb865d05dad16f548cca45855fe17ab92c0ee52e867e2a48dc8d5cf754dd"
FORBIDDEN_SUFFIXES = (
...
def independently_verify_delta_v1(
    ...
    if changed != target:
        raise ValueError("CHANGED_EVENTS_NOT_4LH_EXACT4")
    field_sets = {
        event: {
    ...
    if any(fields != subject._ACTUAL_CHANGED_4LH_FIELDS_V1 for fields in field_sets.values()):
...
def independently_verify_counts_v1(rows: Sequence[Mapping[str, str]]) -> dict[str, object]:
    ...
    expected = {
        "chemistry": Counter({"POSITIVE": 148, "NOT_ESTABLISHED": 90, "UNRESOLVED": 762}),
        "task": Counter({"RELEVANT": 137, "NOT_RELEVANT": 102, "UNRESOLVED": 761}),
        "training": Counter(
    ...
    status_counts = Counter(row["current_global_status"] for row in rows)
    if (
        status_counts["CURRENTLY_UNREVIEWED"] != 179
        or status_counts["COMPLETED_HUMAN_POSITIVE"] != 119
        or status_counts["COMPLETED_HUMAN_NEGATIVE"] != 70
    ):
    ...
    if profiles != Counter({subject.base.DIRECT_PROFILE: 88, subject.base.STRICT_PROFILE: 52}):
...
def verify_manifest_v1(
    ...
    if manifest.get("semantic_source_binding_count") != 168:
        raise ValueError("MANIFEST_BINDING_COUNT_INVALID")
    bindings = manifest.get("semantic_source_bindings")
    if type(bindings) is not list or len(bindings) != 168:
        raise ValueError("MANIFEST_BINDINGS_INVALID")
    if len({(item["path_namespace"], item["path"]) for item in bindings}) != 168:
...
def _expect_failure(callable_) -> None:
    try:
        callable_()
    except (subject.Cumulative1000CurrentGlobalReadinessCensusWith4LHError, ValueError):
        return
...
def check_semantic_probes_v1(
    ...
    target = subject.FOUR_LH_EXACT4_EVENT_IDS_V1[0]
    ...
    def validate(candidate) -> None:
        subject.validate_covapie_cumulative1000_current_global_readiness_census_with_4lh_v1(
            candidate,
    ...
    row_probes = (
        ...
        (target, {"post_geometry_source_evidence_available": "false"}),
    ...
    for changes in matrix_mutations:
        ...
        _expect_failure(lambda tampered=tampered: subject._validate_4lh_matrix_rows_v1(tampered))
...
def run_check_v1(root: Path = ROOT) -> dict[str, object]:
    ...
    computation = subject.compute_covapie_cumulative1000_current_global_readiness_census_with_4lh_v1(root)
    ...
    result = reconciliation.reconcile_real_completed_human_decisions_with_4lh_v1(root)
    matrix = subject._load_and_validate_4lh_event_matrix_v1(root)
...
def main() -> int:
    result = run_check_v1()
    print("4LH_CURRENT_GLOBAL_READINESS_CENSUS_V1_PASS=true")
    print("CENSUS_ROWS=1000")
    print("CENSUS_COLUMNS=47")
    print("CHANGED_EVENT_COUNT=4")
    print("UNCHANGED_EVENT_COUNT=996")
    print("CHEMISTRY_POSITIVE_COUNT=148")
    print("TASK_RELEVANT_COUNT=137")
    print("TASK_NOT_RELEVANT_COUNT=102")
    print("TRAINING_INCLUDE_COUNT=64")
    print("TRAINING_NOT_APPLICABLE_COUNT=102")
    print("PAIR_AUTHORITY_COUNT=148")
    print("ROLE_AUTHORITY_COUNT=140")
    print("CANONICAL_MASK_STRUCTURAL_LABEL_COUNT=140")
    print("DIRECT_PROFILE_A_B3_C_COUNT=88")
    print("TASK_NEGATIVE_CHEMISTRY_POSITIVE_POPULATION_COUNT=12")
    print("EXACT5_B3_PRESENT=true")
    print("SIXTH_TASK=false")
    print("NEXT_PRIORITY_REVIEW_LIGAND=TP2")
    print("NEXT_PRIORITY_REVIEW_RAW_PRIORITY_RANK=27")
