#!/usr/bin/env python3
"""Read-only checker for the 4M5 completed-decision reconciliation Exact4."""
...
BASELINE_COMMIT = "efc571483485e43194e6f39238c2ca55e3730abf"
...
PURPOSES = {
    EXACT4_PATHS[0]: "rich 4M5 validation, narrow projection, and reconciliation",
...
FROZEN_RUNTIME_LINEAGE = (
    ...
    (
        "WITH_CER_RECONCILIATION_PREDECESSOR",
        "src/covalent_ext/"
...
def _verify_architecture(root: Path) -> dict[str, object]:
    ...
    payload = _read_regular_file(root / EXACT4_PATHS[0], "FOUR_M5_RECONCILIATION")
    ...
    if classes != {"CompletedDecisionReconciliationWith4M5Error"}:
    ...
    expected_calls = {
        ...
        "load_real_completed_decision_sources_with_cer_v1": 1,
        "reconcile_real_completed_human_decisions_with_cer_v1": 1,
    ...
    if "validate_4m5_formal_human_decision_v1" in payload.decode("utf-8"):
...
def _verify_rich_authority(bound: dict[str, object]) -> dict[str, object]:
    ...
    expected_binding = {
        "path": four_m5_ingestion_owner.FORMAL_DECISION_RELATIVE.as_posix(),
    ...
    if not all(type(value) is dict for value in (pair, role, training, pre, canonical)):
        raise ValueError("FOUR_M5_RICH_AUTHORITY_SHAPE_INVALID")
    ...
    if (
        ...
        raise ValueError("FOUR_M5_RICH_AUTHORITY_SEMANTICS_INVALID")
    return {
        "event_count": 4,
        "review_unit_id": four_m5_ingestion_owner.EXPECTED_REVIEW_UNIT_ID,
        "ranks": tuple(four_m5_ingestion_owner.EXPECTED_RANKS),
        "protein_reactive_atom": "SG",
        "ligand_reactive_atom": "C15",
        "role_profile": four_m5_ingestion_owner.EXPECTED_ROLE_PROFILE,
        "W_L_S_counts": (9, 0, 16),
        ...
        "PRE_mapping_status": four_m5_ingestion_owner.PRE_MAPPING_STATUS,
...
def _verify_projection(
    ...
    if (
        ...
        or source.binding.byte_count != 29089
        ...
        raise ValueError("FOUR_M5_GENERIC_PROJECTION_INVALID_OR_RICH_LEAKAGE")
    return {
...
def _verify_sources(
    ...
    before = cer_predecessor.load_real_completed_decision_sources_with_cer_v1(root)
    after = subject.load_real_completed_decision_sources_with_4m5_v1(root)
    ...
    if (
        ...
        != PREDECESSOR_SOURCE_FACT_COUNTS
        or len(before_ids) != 107
        or len(set(before_ids)) != 107
        or len({source.binding.review_unit_id for source in before}) != 16
        or len({source.binding.stable_identity for source in before}) != 16
    ):
        raise ValueError("PREDECESSOR_SOURCE_CHAIN_NOT_EXACT16_107")
    if (
        len(after) != 17
        or tuple(len(source.facts) for source in after)
        != SUCCESSOR_SOURCE_FACT_COUNTS
        or len(after_ids) != 111
        or len(set(after_ids)) != 111
        or len({source.binding.review_unit_id for source in after}) != 17
        or len({source.binding.stable_identity for source in after}) != 17
        or after[:-1] != before
        or set(four_m5_ingestion_owner.EXPECTED_EVENT_IDS) & set(before_ids)
    ):
        raise ValueError("SUCCESSOR_SOURCE_CHAIN_NOT_EXACT17_111")
    return before, after
...
def _verify_historical_and_onl(
    ...
    if len(target) != 4 or any(
        ...
        raise ValueError("FOUR_M5_HISTORICAL_PRIOR_INVALID")
    adapted = (
...
def _verify_reconciliation(root: Path) -> generic.ReconciliationResult:
    before = (
        cer_predecessor.reconcile_real_completed_human_decisions_with_cer_v1(root)
    )
    after = subject.reconcile_real_completed_human_decisions_with_4m5_v1(root)
    ...
    if before.review_summary != BEFORE_SUMMARY or after.review_summary != AFTER_SUMMARY:
        raise ValueError("FOUR_M5_REVIEW_SUMMARY_DELTA_INVALID")
    before_by_event = {
    ...
    if changed != target_ids:
        raise ValueError("FOUR_M5_CHANGED_ROWS_NOT_EXACT4")
    for event_id in target_ids:
        ...
        if (
            ...
            raise ValueError("FOUR_M5_EXACT_FOUR_FIELD_TRANSITION_INVALID")
    if any(
        before_by_event[event_id] != after_by_event[event_id]
        for event_id in set(before_by_event) - target_ids
    ):
        raise ValueError("FOUR_M5_NON_TARGET_ROW_CHANGED")
    if len(after.normalized_facts) != 111 or len(after.source_bindings) != 17:
        raise ValueError("FOUR_M5_RECONCILIATION_RESULT_COMPOSITION_INVALID")
    return after
...
def _expect_subject_failure(callable_: object, token: str) -> None:
    ...
    except subject.CompletedDecisionReconciliationWith4M5Error as error:
...
def _verify_fail_closed_probes(
    ...
    for key, replacement in decision_mutations.items():
        ...
        changed["formal"]["human_authorization"][key] = replacement  # type: ignore[index]
        _expect_subject_failure(
            lambda changed=changed: subject._project_validated_4m5_binding_v1(
                changed
            ),
            "FOUR_M5_D1_D5_DECISIONS_INVALID",
        )
    ...
    for section, key, replacement in rich_mutations:
        changed = copy.deepcopy(bound)
        changed["formal"][section][key] = replacement  # type: ignore[index]
        _expect_subject_failure(
            lambda changed=changed: subject._project_validated_4m5_binding_v1(
                changed
            ),
    ...
    for mutation in ("missing", "duplicate", "extra"):
        ...
        _expect_subject_failure(
            ...
            "FOUR_M5_FORMAL_IDENTITY_NOT_EXACT4",
        )
    ...
    for row in wrong_prior:
        if row["canonical_event_id"] == four_m5_ingestion_owner.EXPECTED_EVENT_IDS[0]:
            ...
            break
    _expect_subject_failure(
        lambda: subject._prove_4m5_original_unreviewed_prior_v1(wrong_prior),
    ...
    for row in changed_adapted:
        if row["canonical_event_id"] == four_m5_ingestion_owner.EXPECTED_EVENT_IDS[0]:
            ...
            break
    _expect_subject_failure(
        lambda: subject._prove_4m5_rows_unchanged_after_onl_normalization_v1(
            historical, changed_adapted
        ),
        "ONL_ADAPTER_CHANGED_FOUR_M5_ROW",
    )
    return {
        "D1_D5_mutations_fail": True,
        "rich_boundary_mutations_fail": True,
        "missing_duplicate_extra_fail": True,
        "prior_state_mutation_fails": True,
        "ONL_4M5_mutation_fails": True,
    }
...
def _verify_no_outputs_or_transients(root: Path) -> None:
    ...
    if forbidden_root.exists():
        raise ValueError("MATERIALIZED_FOUR_M5_RECONCILIATION_ROOT_FORBIDDEN")
    ignored = set(
        filter(
            None,
            _git(
...
def run_check_v1(repo_root: Path = ROOT) -> dict[str, object]:
    """Run every read-only 4M5 reconciliation candidate gate."""
    ...
    bound = four_m5_ingestion_owner.load_frozen_formal_decision_v1(root)
    rich = _verify_rich_authority(bound)
    projection_source = subject._project_validated_4m5_binding_v1(bound)
    ...
    return {
        ...
        "event_collisions": 0,
        "FOUR_M5_rows_changed_by_ONL": 0,
        "FOUR_M5_changed_rows": 4,
        "non_FOUR_M5_unchanged_rows": 334,
        "review_summary": result.review_summary,
        "FOUR_M5_training_dispositions": dict(dispositions),
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("predecessor_sources=16 predecessor_facts=107")
    print("successor_sources=17 successor_facts=111 collisions=0")
    print("FOUR_M5_changed_rows=4 non_FOUR_M5_unchanged_rows=334")
    print("positive=107/16 completed=135/21 unreviewed=203/110")
