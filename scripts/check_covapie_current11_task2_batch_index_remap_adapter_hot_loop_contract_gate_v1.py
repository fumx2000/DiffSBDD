#!/usr/bin/env python3
"""Check the Current11 Task2 remap adapter hot-loop contract gate V1."""
...
def _owner_and_helper_validation(repo_root: Path) -> int:
    for spec in gate.OWNER_SPECS.values():
        ...
        if (
            ...
            _fail()
    for helper_name, expected_signature in gate.ADAPTER_PURE_HELPER_SIGNATURES.items():
        helper = getattr(adapter_owner, helper_name, None)
        ...
        if observed != expected_signature:
            _fail()
    if (
        tuple(adapter_owner._OUTPUT_FIELD_ORDER) != gate.OUTPUT17_FIELD_ORDER
        or tuple(adapter_owner._INPUT_FIELD_ORDER) != gate.INPUT_FIELD_ORDER
    ):
        _fail()
    return len(gate.ADAPTER_PURE_HELPER_SIGNATURES)
...
def _verify_artifacts(
    ...
    *,
    ...
    acquisition = context.get("clean_tracked_successor_authority_acquisition")
    cache = context.get("cache_and_lifetime")
    counts = runtime.get("fast_per_batch_structural_acceptance_counts")
    readiness = report.get("readiness")
    if lifecycle == "precommit-untracked":
        expected_context_ready = False
        expected_context_blocker = "hot_loop_contract_gate_not_published"
    elif lifecycle == "clean-tracked-successor":
        expected_context_ready = True
        expected_context_blocker = "NONE"
    else:
        _fail()
    if (
        manifest.get("architecture_name") != gate.ARCHITECTURE_NAME
        or manifest.get("artifact_names") != list(gate.ARTIFACT_NAMES)
        ...
        is not True
        or manifest.get("existing_slow_public_adapter_api_unchanged") is not True
        or manifest.get("existing_slow_public_adapter_exact2_semantics_unchanged")
        is not True
        or manifest.get("adapter_source_refactor_required") is not False
        or manifest.get("shared_kernel_refactor_required") is not False
        ...
        != [dict(row) for row in gate.FUTURE_PUBLIC_APIS]
        or context.get("future_product_public_api_count") != 2
        or context.get("public_context_class_allowed") is not False
        or context.get("logical_context_field_order")
        != list(gate.LOGICAL_CONTEXT_FIELD_ORDER)
        or context.get("logical_context_field_count") != 20
        or type(acquisition) is not dict
        or acquisition.get("reconciliation_public_build_count") != 1
        or acquisition.get("successor_public_build_count") != 1
        or acquisition.get("context_builder_direct_B2_public_build_count") != 0
        or acquisition.get("adapter_contract_exact6_count") != 0
        or type(cache) is not dict
        or cache.get("context_freshness_model") != gate.CONTEXT_FRESHNESS_MODEL
        or cache.get("pickle_contract_defined") is not False
        or any(
            cache.get(field) is not False
            for field in (
                "global_cache_allowed",
                "lru_cache_allowed",
                "hidden_singleton_context_allowed",
                "cross_process_shared_cache_allowed",
            )
        )
        or runtime.get("runtime_target") != gate.RUNTIME_TARGET
        or runtime.get("output17_field_order")
        != list(gate.OUTPUT17_FIELD_ORDER)
        or runtime.get("output17_field_count") != 17
        ...
        is not True
        or runtime.get("historical_private_failure_runtime_golden") is not False
        or runtime.get("failure_normalization_forbidden") is not True
        or runtime.get("old_adapter_report_returned_by_fast_path") is not False
        ...
        != gate.CONTEXT_FRESHNESS_MODEL
        or authority.get("one_context_per_process_or_ddp_rank_per_authority_snapshot")
        is not True
        or authority.get("current_compiler_context_uses_successor_authority")
        is not False
        ...
        or report.get("frozen_adapter_private_pure_helper_count") != 6
        or report.get("lightweight_success_case_count") != 3
        or report.get("lightweight_failure_case_count") != 2
        ...
        is not expected_context_ready
        or readiness.get("context_runtime_blocker")
        != expected_context_blocker
        or readiness.get(
...
def _main(argv: Sequence[str] | None = None) -> int:
    ...
    helper_count = _owner_and_helper_validation(repository)
    lifecycle = _repository_lifecycle(repository)
    before = _snapshot(repository, state)
    first = gate.build_covapie_current11_task2_batch_index_remap_adapter_hot_loop_contract_gate_v1(
    ...
    summary = {
        ...
        "negative_matrix_case_count": len(gate.NEGATIVE_CASES),
        "frozen_adapter_private_pure_helper_count": helper_count,
        ...
        "ready_for_remap_adapter_context_runtime_implementation": readiness[
            "ready_for_remap_adapter_context_runtime_implementation"
        ],
        "context_runtime_blocker": readiness["context_runtime_blocker"],
    ...
    if (
        ...
        is not True
        or summary["ready_for_remap_adapter_context_runtime_implementation"]
        is not expected_context_ready
        or summary["context_runtime_blocker"] != expected_context_blocker
