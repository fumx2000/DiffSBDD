#!/usr/bin/env python3
"""Read-only checker for the GVE completed-decision reconciliation Exact4."""
...
BASELINE_COMMIT = "6f8ebbc28d5b2dcdc87099e26874580d4168b3db"
...
EXACT4_PATHS = (
    ...
    "scripts/"
    "check_covapie_completed_human_decision_reconciliation_with_gve_v1.py",
    "tests/"
    "test_covapie_completed_human_decision_reconciliation_with_gve_v1.py",
    "docs/"
    "covapie_completed_human_decision_reconciliation_with_gve_v1_guide.md",
)
PURPOSES = {
    EXACT4_PATHS[0]: "rich GVE validation, narrow projection, and reconciliation",
...
FROZEN_RUNTIME_LINEAGE = (
    ...
    (
        "WITH_SR2_RECONCILIATION_PREDECESSOR",
        "src/covalent_ext/"
...
def _verify_architecture(root: Path) -> dict[str, object]:
    ...
    payload = _read_regular_file(root / EXACT4_PATHS[0], "GVE_RECONCILIATION")
    ...
    if classes != {"CompletedDecisionReconciliationWithGVEError"}:
    ...
    expected_calls = {
        ...
        "load_real_completed_decision_sources_with_sr2_v1": 1,
        "reconcile_real_completed_human_decisions_with_sr2_v1": 1,
    ...
    text = payload.decode("utf-8")
    if "validate_gve_formal_human_decision_v1" in text or "import subprocess" in text:
...
def _verify_rich_authority(bound: dict[str, object]) -> dict[str, object]:
    ...
    pair = formal["sample_reactive_pair_authority"]
    role = formal["sample_role_boundary"]
    training = formal["training_boundary"]
    pre = formal["PRE_boundary"]
    post = formal["POST_boundary"]
    tasks = formal["canonical_Exact5_and_sample_boundary"]
    if not all(
        isinstance(value, dict)
        for value in (pair, role, training, pre, post, tasks)
    ):
    ...
    return {
        ...
        "completed_lane": gve_ingestion_owner.EXPECTED_COMPLETED_LANE,
        "pair": "SG-CB",
...
def _verify_projection(
    ...
    subject._validate_projected_gve_source_v1(source)
    ...
    if any(
        ...
        raise ValueError("GVE_GENERIC_PROJECTION_INVALID")
    if (
...
def _verify_sources(
    ...
    if (
        len(before) != 19
        or len(before_facts) != 119
        or len({source.binding.stable_identity for source in before}) != 19
        or len(after) != 20
        or len(after_facts) != 123
        or len({source.binding.stable_identity for source in after}) != 20
        or after[:-1] != before
        or len({fact.canonical_event_id for fact in after_facts}) != 123
        or after[-1].binding.review_unit_id
        != gve_ingestion_owner.EXPECTED_REVIEW_UNIT_ID
    ):
        raise ValueError("SOURCE_CHAIN_19_119_TO_20_123_INVALID")
    return before, after


def _verify_reconciliation(
    root: Path,
    before_sources: tuple[generic.NormalizedDecisionSource, ...],
    ...
    after = subject.reconcile_real_completed_human_decisions_with_gve_v1(root)
...
def _expect_subject_failure(callable_: object, token: str) -> None:
    ...
    except subject.CompletedDecisionReconciliationWithGVEError as error:
...
def _verify_fail_closed_probes(
    ...
    if not isinstance(missing_formal, dict):
        raise ValueError("FAIL_CLOSED_FORMAL_NOT_OBJECT")
    missing_events = missing_formal["event_level_formal_decisions"]
    if not isinstance(missing_events, list):
        raise ValueError("FAIL_CLOSED_EVENTS_NOT_LIST")
    missing_events.pop()
    _expect_subject_failure(
        lambda: subject._validate_rich_gve_semantics_v1(missing),
        "GVE_FORMAL_EVENT_COVERAGE_NOT_EXACT4",
    )
    ...
    for changes in fact_mutations:
        ...
        _expect_subject_failure(
            lambda changed=changed: subject._validate_projected_gve_source_v1(
                replace(projection, facts=(changed, *projection.facts[1:]))
            ),
            "GVE_SOURCE_PROJECTION_INVALID",
        )
    wrong_namespace = replace(
        projection.binding, path_namespace="project_parent_relative"
    )
    _expect_subject_failure(
        lambda: subject._validate_projected_gve_source_v1(
            replace(projection, binding=wrong_namespace)
        ),
        "GVE_SOURCE_PROJECTION_IDENTITY_INVALID",
    )
    matrix_path = (
    ...
    matrix_binding = replace(projection.binding, source_path=matrix_path)
    matrix_facts = tuple(
        replace(fact, source_binding_path=matrix_path) for fact in projection.facts
    )
    _expect_subject_failure(
        lambda: subject._validate_projected_gve_source_v1(
            replace(projection, binding=matrix_binding, facts=matrix_facts)
        ),
        "GVE_SOURCE_PROJECTION_IDENTITY_INVALID",
    )

    collision_rows = [dict(row) for row in before.reconciled_rows]
    collision_rows[0]["canonical_event_id"] = (
        "COVAPIE_CYS_SG_EVENT_V1:1XD3:A:CYS:1-:SG:X:GVE:CB"
    )
    _expect_subject_failure(
        lambda: subject._prove_gve_predecessor_historical_state_v1(
            collision_rows
        ),
        "GVE_GENERIC_REVIEW_UNIT_COLLISION",
    )
    ...
    _expect_subject_failure(
        ...
        "GVE_RECONCILIATION_ROW_ORDER_OR_SCHEMA_CHANGED",
    )
    return {
        ...
        "census_operation_rejected": True,
        "missing_event_rejected": True,
        "completed_lane_leak_rejected": True,
        "positive_legacy_status_rejected": True,
        ...
        "ingestion_matrix_authority_rejected": True,
        "legacy_1XD3_collision_rejected": True,
        "non_GVE_mutation_rejected": True,
...
def _verify_no_outputs_or_transients(root: Path) -> None:
    forbidden_roots = (
        ...
        root / "data/derived/covalent_small/with_gve_reconciliation",
    )
    if any(path.exists() for path in forbidden_roots):
        raise ValueError("MATERIALIZED_GVE_RECONCILIATION_ROOT_FORBIDDEN")
    ignored = set(
        filter(
            None,
            _git(
...
def run_check_v1(repo_root: Path = ROOT) -> dict[str, object]:
    """Run every read-only GVE reconciliation candidate gate."""
    ...
    bound = gve_ingestion_owner.load_frozen_formal_decision_v1(root)
    rich = _verify_rich_authority(bound)
    projection_source = subject._project_validated_gve_binding_v1(bound)
    projection = _verify_projection(projection_source)
    before_sources, after_sources = _verify_sources(root)
    before, after = _verify_reconciliation(root, before_sources)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        ...
        or report["census_crossfield_fix_performed"] is not False
        or report["census_crossfield_debt_preserved"] is not True
        or report["reconciliation_complete_in_memory"] is not True
        or report["reconciliation_outputs_created"] != 0
        or report["ready_for_with_GVE_census_crossfield_audit"] is not True
    ):
    ...
    print("GVE_RECONCILIATION_CANDIDATE_PASS=true")
    print("GVE_GENERIC_EXACT11_PROJECTION_PASS=true")
    print("GVE_GENERIC_LEGACY_STATUS_NEGATIVE=true")
    print("GVE_GENERIC_TASK_NOT_RELEVANT=true")
    print("GVE_GENERIC_CHEMISTRY_POSITIVE=true")
    print("GVE_GENERIC_TRAINING_NOT_APPLICABLE=true")
    print("GVE_GENERIC_HUMAN_TRAINING_EXCLUDED=false")
    print("GVE_CHEMISTRY_POSITIVE_BUT_TASK_DOMAIN_NEGATIVE_PRESERVED=true")
    print("RICH_AUTHORITY_LEAKAGE_INTO_GENERIC_FACT=false")
    print("GENERIC_SOURCE_NAMESPACE_REPOSITORY_PARENT_RELATIVE=true")
    print("SOURCE_CHAIN_20_123=true")
    print("STABLE_SOURCE_IDENTITIES_20=true")
    print("GVE_EXACT4_CHANGED=true")
    print("NON_GVE_ROWS_UNCHANGED_334_OF_334=true")
    print("COMPLETED_POSITIVE_115_EVENTS_18_UNITS=true")
    print("COMPLETED_NEGATIVE_32_EVENTS_6_UNITS=true")
    print("COMPLETED_TOTAL_147_EVENTS_24_UNITS=true")
    print("UNREVIEWED_191_EVENTS_107_UNITS=true")
    print("IN_PROGRESS_ZERO=true")
    print("GVE_RAW_PRIORITY_RANK23_EXACT4=true")
    print("GVE_HISTORICAL_1XD3_COLLISION=false")
    print("RECONCILIATION_COMPLETE_IN_MEMORY=true")
    print("RECONCILIATION_OUTPUT_FILES=0")
    print("WITH_GVE_CENSUS_CROSSFIELD_AUDIT_REQUIRED_LATER=true")
    print("CENSUS_CROSSFIELD_FIX_PERFORMED=false")
    print("READY_FOR_WITH_GVE_CENSUS_CROSSFIELD_AUDIT=true")
