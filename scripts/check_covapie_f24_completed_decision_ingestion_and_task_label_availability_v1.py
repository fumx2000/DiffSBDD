#!/usr/bin/env python3
"""Fail-closed checker for F24 completed-decision ingestion V1."""
...
from covalent_ext import (  # noqa: E402
    covapie_f24_completed_decision_ingestion_and_task_label_availability_v1 as owner,
)


ERROR = "COVAPIE_F24_INGESTION_CHECK_FAILED"
FORBIDDEN_SUFFIXES = (
...
def _observed_f24_candidate_paths() -> set[str]:
    paths = [
        *(REPO_ROOT / "src/covalent_ext").glob("covapie_f24_*"),
        *(REPO_ROOT / "scripts").glob("check_covapie_f24_*"),
        *(REPO_ROOT / "tests").glob("test_covapie_f24_*"),
    ]
...
def check_candidate_inventory() -> tuple[list[dict[str, object]], str]:
    ...
    observed = _observed_f24_candidate_paths()
    if observed - expected_set:
        fail("UNEXPECTED_F24_CANDIDATE_FILE")
...
def check_independent_semantics(artifacts: dict[str, bytes]) -> None:
    ...
    if any(approval.get(key) != value for key, value in expected_d.items()):
        fail("SNAPSHOT_D1_D5_INVALID")
    role = snapshot.get("role_partition", {})
    if (
        role.get("chemical_warhead_atom_ids") != ["C1", "C2", "C8", "O2", "O6"]
        or role.get("warhead_role_atom_ids") != ["C1", "C2", "C4", "C8", "O2", "O5", "O6"]
        or role.get("sets_are_intentionally_distinct") is not True
        or role.get("selected_candidate_index_0based") is not None
        or role.get("role_profile") != "DIRECT_ATTACHMENT_OPTIONAL_LINKER_V1"
        or role.get("minimal_seed_authority_available") is not False
    ):
        fail("SNAPSHOT_CHEMICAL_ROLE_OR_SEED_INVALID")
    if len(rows) != 4 or [int(row["scaleup_rank"]) for row in rows] != [593, 594, 595, 596]:
        fail("MATRIX_EXACT4_INVALID")
    if len({row["canonical_event_id"] for row in rows}) != 4:
        fail("MATRIX_EVENT_COLLAPSE_OR_DUPLICATE")
    distances = ["1.833648", "1.671136", "1.893800", "1.599498"]
    for row, distance in zip(rows, distances):
        ...
        if (
            ...
            or row["selected_role_candidate_index_0based"] != ""
    ...
    verify_binding_records(manifest["preparation_exact6_bindings"], 6)
    verify_binding_records(manifest["immutable_semantic_owner_bindings"], 2)
