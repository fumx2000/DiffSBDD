...
from covalent_ext import covapie_small_pilot_manifest_rerun_gate as gate  # noqa: E402
...
def write_summary(manifest: dict[str, Any], path: str | Path) -> None:
    ...
    text = f"""# CovaPIE Small Pilot Manifest Rerun Gate v0 Summary

Step 14Y reruns the small pilot manifest from Step 14X ready candidate records.

The small pilot manifest is a candidate list for future sample preparation. It is not a sample index, not a final dataset, and not a training sample.

small_pilot_manifest_row_count: `{manifest["small_pilot_manifest_row_count"]}`
...
def run() -> int:
    result = gate.run_covapie_small_pilot_manifest_rerun_gate_v0()
    write_csv(result["precondition_rows"], gate.PRECONDITION_AUDIT_CSV, gate.PRECONDITION_COLUMNS)
    write_csv(result["pilot_rows"], gate.PILOT_MANIFEST_CSV, gate.PILOT_MANIFEST_COLUMNS)
    write_json(result["pilot_rows"], gate.PILOT_MANIFEST_JSON)
    write_csv(result["traceability_rows"], gate.TRACEABILITY_AUDIT_CSV, gate.TRACEABILITY_COLUMNS)
    write_csv(result["blocked_rows"], gate.BLOCKED_EXCLUSION_AUDIT_CSV, gate.BLOCKED_COLUMNS)
    ...
    print("covapie_small_pilot_manifest_rerun_gate_v0_passed" if manifest["all_checks_passed"] else "covapie_small_pilot_manifest_rerun_gate_v0_blocked")
    for key in [
        "small_pilot_manifest_row_count",
