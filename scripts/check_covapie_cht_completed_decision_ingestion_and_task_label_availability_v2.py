#!/usr/bin/env python3
"""Independent fail-closed checker for the additive CHT V2 successor."""
...
BASELINE_HEAD = "baab1358bcc8f776df20d8dc76ed476d51ba27f3"
BASELINE_TREE = "fb176b92e4662bb76c9b6cb593d0003ef595fee1"
BASELINE_SUBJECT = "add CovaPIE NEQ source binding successor v2"
...
TEST_RELATIVE = (
    "tests/"
    "test_covapie_cht_completed_decision_ingestion_and_task_label_availability_v2.py"
)
GUIDE_RELATIVE = (
    "docs/"
    "covapie_cht_completed_decision_ingestion_and_task_label_availability_v2_guide.md"
)
EXACT4_PATHS = (
    PRODUCTION_RELATIVE,
    CHECKER_RELATIVE,
...
def _verify_public_api() -> None:
    ...
    if not issubclass(subject.CHTSourceBindingV2Error, ValueError):
...
def _verify_production_ast(root: Path) -> dict[str, object]:
    text = _read_regular(root / PRODUCTION_RELATIVE, "CHT_V2_PRODUCTION").decode(
        "utf-8"
    )
    tree = ast.parse(text)
    ...
    cht_v1_calls = {
        ...
        and call.func.value.id == "cht_v1"
    }
    if cht_v1_calls & _FORBIDDEN_CHT_V1_CALLS:
        raise ValueError("FORBIDDEN_CHT_V1_ACTIVE_SOURCE_PATH_CALLED")
    neq_v1_calls = {
        call.func.attr
        for call in ast.walk(tree)
        if isinstance(call, ast.Call)
    ...
    if neq_v1_calls:
        raise ValueError("NEQ_V1_ACTIVE_REFERENCE_FORBIDDEN")
    neq_v2_calls = {
        call.func.attr
        for call in ast.walk(tree)
        if isinstance(call, ast.Call)
        and isinstance(call.func, ast.Attribute)
        and isinstance(call.func.value, ast.Name)
        and call.func.value.id == "neq_v2"
    }
    if "verify_published_neq_v1_projection_v2" not in neq_v2_calls:
        raise ValueError("PUBLISHED_NEQ_V2_PRECEDENT_NOT_ACTIVE")
    forbidden_tokens = (
        "stat.S_IMODE",
        "st_mode",
        "0o7777",
    ...
    return {
        ...
        "cht_v1_active_source_gate_called": False,
        "neq_v1_active_source_gate_called": False,
        "neq_v2_successor_called": True,
        "reused_cht_v1_function_names": sorted(cht_v1_calls),
    }


def _verify_cht_v1_pure_call_graph(root: Path) -> None:
    production_tree = ast.parse(
        _read_regular(root / PRODUCTION_RELATIVE, "CHT_V2_PRODUCTION").decode(
            "utf-8"
        )
    )
    roots = {
    ...
    v1_tree = ast.parse(
        _read_regular(root / cht_v1.SOURCE_RELATIVE, "CHT_V1_OWNER").decode(
            "utf-8"
        )
    )
    reachable = _reachable_functions(_function_nodes(v1_tree), roots)
    if reachable & _FORBIDDEN_CHT_V1_CALLS:
        raise ValueError("REUSED_CHT_V1_HELPER_REACHES_SOURCE_OR_MUTATION_GATE")


def _verify_neq_v2_precedent_call_graph(root: Path) -> None:
    text = _read_regular(root / subject.NEQ_V2_RELATIVE, "NEQ_V2_OWNER").decode(
        "utf-8"
    )
    tree = ast.parse(text)
    calls = {
    ...
    if calls & _FORBIDDEN_NEQ_V1_CALLS:
        raise ValueError("NEQ_V2_PRECEDENT_REACHES_NEQ_V1_SOURCE_GATE")
    ...
    if "_verify_source" not in reachable:
        raise ValueError("NEQ_V2_BOUND_SOURCE_PATH_NOT_ACTIVE")


def _verify_v1_static_false_failure_contract(root: Path) -> bool:
    text = _read_regular(root / cht_v1.SOURCE_RELATIVE, "CHT_V1_OWNER").decode(
        "utf-8"
    )
    tree = ast.parse(text)
...
def _verify_frozen_v1_bytes(root: Path) -> dict[str, str]:
    bindings = (
        *subject._FROZEN_CHT_V1_CODE_BINDINGS,
        *subject._PUBLISHED_CHT_V1_OUTPUT_BINDINGS,
    )
...
def _verify_neq_v2_dependency(root: Path) -> dict[str, str]:
    observed: dict[str, str] = {}
...
def _exercise_mode_regressions(root: Path) -> dict[str, bool]:
    nonexec = cht_v1.FROZEN_REVIEW_PACKAGE_BINDINGS[0]
    executable = cht_v1.FROZEN_REVIEW_PACKAGE_BINDINGS[-1]
    baseline = subject.load_frozen_cht_authority_v2(repo_root=root)
    result: dict[str, bool] = {}
    with tempfile.TemporaryDirectory(prefix="covapie_cht_v2_modes_") as name:
        temporary = Path(name)
        for binding, modes, pass_modes, label in (
            ...
            for mode in modes:
                ...
                if mode in pass_modes:
                    observed = subject.load_frozen_cht_authority_v2(
                        repo_root=root,
                    ...
                    except subject.CHTSourceBindingV2Error as error:
...
def _exercise_source_failure_regressions(root: Path) -> dict[str, bool]:
    ...
    relative = binding[0]
    result: dict[str, bool] = {}
    with tempfile.TemporaryDirectory(prefix="covapie_cht_v2_failures_") as name:
        temporary = Path(name)
...
def _verify_scientific_semantics(bound: dict[str, object]) -> None:
    ...
    if (
        ...
        or [event["pdb_id"] for event in events] != [row[2] for row in cht_v1.EXPECTED_EVENTS]
        or any(event["cys_residue_id"] != "CYS:450-" for event in events)
        ...
        or any(event["ligand_reactive_atom"] != "C4" for event in events)
        or any(event["selected_role_candidate_index_0based"] != 2 for event in events)
        or any(
            event["formal_event_training_use_decision"]
        ...
        or approval["D6_scientific_context"] != cht_v1.EXPECTED_D6
        or role["role_profile"] != "STRICT_LINKER_PRESENT_V1"
        or role["exact_heavy_atom_ids"] != ["C4", "C5", "C6", "C7", "C8", "N1", "O6"]
        ...
        raise ValueError("CHT_V2_SCIENTIFIC_EQUIVALENCE_INVALID")
    ...
    if (
        ...
        raise ValueError("CHT_V2_CANONICAL_EXACT5_INVALID")
...
def run_check_v2(repo_root: Path = ROOT) -> dict[str, object]:
    ...
    _verify_cht_v1_pure_call_graph(root)
    _verify_neq_v2_precedent_call_graph(root)
    _verify_b1_dependency(root)
    neq_v2_bindings = _verify_neq_v2_dependency(root)
    v1_bindings = _verify_frozen_v1_bytes(root)
    bound = subject.load_frozen_cht_authority_v2(repo_root=root)
    _verify_scientific_semantics(bound)
    artifacts = subject.verify_published_cht_v1_projection_v2(repo_root=root)
    ...
    if upstream != {
        ...
        "NEQ_V2_sha256": subject.NEQ_V2_SHA256,
        "NEQ_V2_published_commit": subject.NEQ_V2_PUBLISHED_COMMIT,
        ...
        raise ValueError("NEQ_V2_UPSTREAM_PRECEDENT_INVALID")
    result = {
        "lifecycle": lifecycle,
        "exact4": exact4,
        "cht_v1_bindings": v1_bindings,
        "neq_v2_bindings": neq_v2_bindings,
        "cht_v1_bytes_preserved": True,
        "cht_v1_artifacts_preserved": tuple(artifacts) == cht_v1.OUTPUT_FILENAMES,
        "b1_bound_source_helper_used": ast_result["b1_bound_source_helper_used"],
        "neq_v2_successor_bound": ast_result["neq_v2_successor_called"],
        "neq_v2_projection_exercised": upstream["NEQ_V2_projection_actually_called"],
        "neq_v1_scientific_matrix_preserved": upstream[
            "NEQ_V1_scientific_matrix_preserved"
        ],
        "cht_v1_source_gate_active": ast_result["cht_v1_active_source_gate_called"],
        ...
        "ready_for_v2_b2_4": True,
        "ready_for_training": False,
    }
    required_true = (
        "cht_v1_bytes_preserved",
        "cht_v1_artifacts_preserved",
        "b1_bound_source_helper_used",
        "neq_v2_successor_bound",
        "neq_v2_projection_exercised",
        "neq_v1_scientific_matrix_preserved",
