...
from covalent_ext import real_covalent_confirmed_candidate_atom_site_coordinate_extraction_smoke as gate  # noqa: E402
        ...
        rows.append(
            {
                ...
                "decision": "coordinate extraction smoke accepted" if passed else "coordinate extraction smoke blocked",
...
def write_summary(manifest: dict[str, Any], path: str | Path) -> None:
    ...
    text = f"""# Real Covalent Confirmed Candidate Atom Site Coordinate Extraction Smoke v0 Summary
...
Endpoint coordinates extracted: {manifest["extracted_coordinate_row_count"]}.
...
def run() -> int:
    result = gate.build_real_covalent_confirmed_candidate_atom_site_coordinate_extraction_smoke_v0()
    write_csv(build_report_rows(result), gate.REPORT_CSV, REPORT_COLUMNS)
    write_csv(result["coordinate_rows"], gate.EXTRACTED_COORDINATES_CSV, gate.EXTRACTED_COORDINATE_COLUMNS)
    write_json(result["manifest"], gate.MANIFEST_JSON)
    write_summary(result["manifest"], gate.SUMMARY_MD)
    print(
        "real_covalent_confirmed_candidate_atom_site_coordinate_extraction_smoke_v0_"
