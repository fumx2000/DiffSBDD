#!/usr/bin/env python3
"""Independent fail-closed checker for the cumulative1000 CHT refresh V1."""
...
EXPECTED_CENSUS_SHA256 = "b51bff3d31d910fa4990a1482e0d3b05364fed86a9cf503de833ddf8851f6384"
EXPECTED_SUMMARY_SHA256 = "b2130b1f0b9cf36455f1bf00e6e5c32e9a4ef250f18bb25f7a902af67c79e0b3"
EXPECTED_BINDINGS_SHA256 = "93250ea26a28e2b98016a842dd869aa641f99a8e8068c4582f8347777a1ac725"
EXPECTED_CHT_EVENT_IDS = (
    "COVAPIE_CYS_SG_EVENT_V1:4V3F:A:CYS:450-:SG:J:CHT:C4",
    "COVAPIE_CYS_SG_EVENT_V1:4V3F:B:CYS:450-:SG:P:CHT:C4",
    "COVAPIE_CYS_SG_EVENT_V1:4V3F:D:CYS:450-:SG:Z:CHT:C4",
    "COVAPIE_CYS_SG_EVENT_V1:5A2D:A:CYS:450-:SG:E:CHT:C4",
    "COVAPIE_CYS_SG_EVENT_V1:5A2D:B:CYS:450-:SG:H:CHT:C4",
)
...
FROZEN_BINDINGS = (
    (
        ...
        "src/covalent_ext/covapie_cumulative1000_current_global_readiness_census_with_neq_v1.py",
        "repository_relative", 63106,
        "ce1a94f1376c18c11e02d821133789dc75f643c2447f2f9d4091b5026f9e6dd4",
    ),
    (
        "PREDECESSOR_NEQ_MATERIALIZED_CENSUS",
        "data/derived/covalent_small/covapie_cumulative1000_current_global_readiness_census_with_neq_v1/covapie_cumulative1000_current_global_readiness_census_with_neq_v1.csv",
        "repository_relative", 521299,
        "8ffbb6df299321393b0aeba8945ed6a4de7f74ed659253d874494a7757e782f2",
    ),
    (
        "PREDECESSOR_NEQ_MATERIALIZED_SUMMARY",
        "data/derived/covalent_small/covapie_cumulative1000_current_global_readiness_census_with_neq_v1/covapie_cumulative1000_current_global_readiness_summary_with_neq_v1.json",
        "repository_relative", 15641,
        "08f61b61014658a51be184285aae478989717f67808ef0fe8e0dc09d068312d9",
    ),
    (
        "PREDECESSOR_NEQ_MATERIALIZED_MANIFEST",
        "data/derived/covalent_small/covapie_cumulative1000_current_global_readiness_census_with_neq_v1/covapie_cumulative1000_current_global_readiness_manifest_with_neq_v1.json",
        "repository_relative", 36162,
    ...
    (
        ...
        "repository_parent_relative", 33307,
...
SEMANTIC_ADDITIVE_BINDINGS = tuple(
    ...
    if item[0] != "PREDECESSOR_NEQ_MATERIALIZED_MANIFEST"
)
...
def verify_frozen_bindings_v1(root: Path) -> list[dict[str, object]]:
    ...
    if built[ingestion.MATRIX] != _read(matrix_path, "CHT_SOURCE_DERIVED_MATRIX"):
        raise ValueError("CHT_MATRIX_NOT_SOURCE_DERIVED_FROM_FORMAL_DECISION")
    formal = json.loads(
        _read(
    ...
    if formal.get("schema_version") != "covapie_cht_exact5_formal_human_decision_v1":
        raise ValueError("CHT_FORMAL_SCHEMA_INVALID")
    return verified


def independently_verify_matrix_v1(root: Path) -> list[dict[str, str]]:
    payload = _read(root / subject.CHT_EVENT_MATRIX_RELATIVE, "CHT_MATRIX")
    reader = csv.DictReader(io.StringIO(payload.decode("utf-8"), newline=""))
    rows = [dict(row) for row in reader]
    if tuple(reader.fieldnames or ()) != ingestion.MATRIX_HEADER:
        raise ValueError("CHT_MATRIX_HEADER_INVALID")
    if len(rows) != 5 or tuple(row["canonical_event_id"] for row in rows) != EXPECTED_CHT_EVENT_IDS:
        raise ValueError("CHT_MATRIX_NOT_EXACT5")
    if [int(row["scaleup_rank"]) for row in rows] != [913, 914, 915, 958, 959]:
        raise ValueError("CHT_MATRIX_RANKS_INVALID")
    if Counter(row["pdb_id"] for row in rows) != Counter({"4V3F": 3, "5A2D": 2}):
        raise ValueError("CHT_MATRIX_PDB_COUNTS_INVALID")
    expected = {
    ...
    for row in rows:
        if any(row[field] != value for field, value in expected.items()):
            raise ValueError("CHT_MATRIX_SEMANTICS_INVALID:" + row["canonical_event_id"])
        ...
        if (
            ...
            raise ValueError("CHT_MATRIX_EXACT5_INVALID")
    return rows
...
def independently_verify_delta_v1(
    ...
    cht = set(EXPECTED_CHT_EVENT_IDS)
    if [int(before[event_id]["scaleup_rank"]) for event_id in EXPECTED_CHT_EVENT_IDS] != [
        913, 914, 915, 958, 959,
    ]:
        raise ValueError("PREDECESSOR_CHT_IDENTITY_INVALID")
    expected_prior = {
    ...
    if any(
        ...
        raise ValueError("PREDECESSOR_CHT_STATE_INVALID")
    changed = {event_id for event_id in before if before[event_id] != after[event_id]}
    if changed != cht:
        raise ValueError("DELTA_NOT_EXACT_CHT_EXACT5")
    if any(before[event_id] != after[event_id] for event_id in set(before) - cht):
        raise ValueError("NON_CHT_ROW_DRIFT")
    for event_id in cht:
        changed_fields = {
        ...
        if changed_fields != EXPECTED_CHANGED_FIELDS:
            raise ValueError("CHT_CHANGED_FIELD_SET_INVALID:" + event_id)
        row = after[event_id]
        expected_after = {
            "current_global_status": "COMPLETED_HUMAN_POSITIVE",
            "current_review_status": "COMPLETED_HUMAN_POSITIVE",
            "human_review_completed": "true",
            "human_review_authority_source": subject.CHT_FORMAL_DECISION_SOURCE,
            "chemistry_disposition": "POSITIVE",
            "chemistry_authority_source": subject.CHT_EVENT_MATRIX_SOURCE,
            "positive_authority_source": subject.CHT_EVENT_MATRIX_SOURCE,
            "task_relevance_disposition": "RELEVANT",
            "task_relevance_authority_source": subject.CHT_EVENT_MATRIX_SOURCE,
        ...
        if any(row[field] != value for field, value in expected_after.items()):
            raise ValueError("CHT_SEMANTICS_INVALID:" + event_id)
        if any(row[field] != value for field, value in EXPECTED_CHT_STRUCTURAL_CELLS.items()):
            raise ValueError("CHT_STRUCTURAL_COVERAGE_INVALID:" + event_id)
    return {
...
def independently_verify_counts_v1(
    ...
    if Counter(row["current_global_status"] for row in rows) != Counter(
        {
            "CURRENTLY_UNREVIEWED": 231,
            "CURRENTLY_IN_PROGRESS": 0,
            "COMPLETED_HUMAN_POSITIVE": 83,
    ...
    if (
        ...
        or pair["sample_level_authoritative_pair_count"] != 100
        ...
        or pair["cht_sample_authority_contribution_count"] != 5
        or pair["cht_model_bound_target_contribution_count"] != 0
    ):
        raise ValueError("PAIR_SUMMARY_INVALID")
    if summary["role"] != {  # type: ignore[index]
        "role_partition_sample_authoritative_count": 100,
        "role_profile_counts": {
            "STRICT_LINKER_PRESENT_V1": 44,
            "DIRECT_ATTACHMENT_OPTIONAL_LINKER_V1": 56,
            "other": 0,
        },
        "canonical_mask_structural_labels_available_count": 100,
        "all_five_structurally_applicable_count": 44,
        "direct_profile_A_B3_C_count": 56,
        "unknown_role_row_count": 900,
    ...
    expected_blockers = {
        ...
        "chemistry_unresolved": {"all_1000": 814},
        "pair_authority_absent": {"all_1000": 900, "within_positive_100": 0},
        "role_authority_absent": {"all_1000": 900, "within_positive_100": 0},
        "human_training_exclusion": {"within_positive_100": 64},
        ...
        "feature_semantics_pending": {"within_positive_100": 100},
    }
    ...
    if (
        stage["training_use_include_count"] != 36
        or stage["future_training_admission_candidate_count"] != 19
        ...
        or stage["future_candidate_source_composition"]["CHT"] != 0
    ):
...
def independently_compute_top10_v1(
    ...
    if len(candidates) != 117:
        raise ValueError("PENDING_QUEUE_NOT_EXACT117")
...
def verify_semantic_bindings_v1(
    ...
    if len(frozen.semantic_source_bindings) != 81:
        raise ValueError("PREDECESSOR_SEMANTIC_BINDING_COUNT_NOT_81")
    ...
    if observed != expected or len(observed) != 88:
        raise ValueError("SEMANTIC_BINDINGS_NOT_PREDECESSOR_PLUS_CHT_INPUTS")
...
def run_check_v1(root: Path = ROOT) -> dict[str, object]:
    ...
    fresh = subject.compute_covapie_cumulative1000_current_global_readiness_census_with_cht_v1(
        root
    )
    if not subject.validate_covapie_cumulative1000_current_global_readiness_census_with_cht_v1(
        fresh
    ):
    ...
    reconciled = reconciliation.reconcile_real_completed_human_decisions_with_cht_v1(root)
    ...
    if not (
        expected_top[0]["review_unit_id"] == "COVAPIE_BULK_REVIEW_UNIT_18734D9C06222450"
        and expected_top[0]["ligand_component_ids"] == ["OZJ"]
        and expected_top[0]["pdb_ids"] == ["4CL8"]
        and expected_top[0]["event_count"] == 4
        and all(item["ligand_component_ids"] != ["CHT"] for item in expected_top)
    ):
        raise ValueError("NEXT_PRIORITY_NOT_OZJ_EXACT4")
    ...
    if (
        ...
        or boundary["next_priority_review_unit"] != "COVAPIE_BULK_REVIEW_UNIT_18734D9C06222450"
        or boundary["next_priority_review_ligand"] != "OZJ"
...
def main() -> int:
    ...
    print("COVAPIE_CUMULATIVE1000_CURRENT_GLOBAL_READINESS_CENSUS_WITH_CHT_V1_CHECK:PASS")
