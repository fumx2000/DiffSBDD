#!/usr/bin/env python3
"""Independent fail-closed checker for the cumulative1000 4M5 refresh V1."""
...
from covalent_ext import covapie_completed_human_decision_reconciliation_with_4m5_v1 as reconciliation  # noqa: E402
...
BASELINE_COMMIT = "32553f90aa072f13e100e0a2171201712791b257"
EXPECTED_CENSUS_SHA256 = "817f549811bde5a9d39bb9314c3052f54900a2edd2ac63df64c354ed50de905f"
EXPECTED_SUMMARY_SHA256 = "27235cf91bc8a5c987b3be75994b436d522a6e1825bc5626d074a1d830212432"
EXPECTED_BINDINGS_SHA256 = "bb903129877a35822027b59195c829216c2970b8858d26dde9671be1781af988"
EXPECTED_CHANGED_FIELDS = subject._AUTHORIZED_FOUR_M5_OVERLAY_FIELDS_V1
FROZEN_BINDINGS = (
    *subject._ADDITIVE_SOURCE_SPECS_V1,
    (
        "PREDECESSOR_WITH_CER_MATERIALIZED_MANIFEST",
...
def independently_verify_matrix_v1(root: Path) -> list[dict[str, str]]:
    payload = _read(root / subject.FOUR_M5_EVENT_MATRIX_RELATIVE, "FOUR_M5_MATRIX")
    built = ingestion.build_artifacts_v1(root)
    if built[ingestion.MATRIX] != payload:
        raise ValueError("FOUR_M5_MATRIX_NOT_SOURCE_DERIVED")
    reader = csv.DictReader(io.StringIO(payload.decode("utf-8"), newline=""))
    rows = [dict(row) for row in reader]
    if (
        ...
        or [int(row["scaleup_rank"]) for row in rows] != [973, 974, 975, 976]
    ):
        raise ValueError("FOUR_M5_MATRIX_IDENTITY_INVALID")
    expected = {
    ...
    for row in rows:
        if any(row[field] != value for field, value in expected.items()):
            raise ValueError("FOUR_M5_MATRIX_SEMANTICS_INVALID:" + row["canonical_event_id"])
        ...
        if (
            ...
            raise ValueError("FOUR_M5_MATRIX_EXACT5_INVALID")
    return rows
...
def independently_verify_delta_v1(
    ...
    if set(before) != set(after) or changed != exact4:
        raise ValueError("DELTA_NOT_EXACT_FOUR_M5_EXACT4")
    if any(before[event_id] != after[event_id] for event_id in set(before) - exact4):
        raise ValueError("NON_FOUR_M5_ROW_CHANGED")
    for event_id in exact4:
        ...
        if fields != EXPECTED_CHANGED_FIELDS:
            raise ValueError("FOUR_M5_CHANGED_FIELD_SET_INVALID:" + event_id)
        expected = {
            "current_global_status": "COMPLETED_HUMAN_POSITIVE",
            "current_review_status": "COMPLETED_HUMAN_POSITIVE",
            "human_review_completed": "true",
            "human_review_authority_source": subject.FOUR_M5_HUMAN_DECISION_SOURCE,
            "chemistry_disposition": "POSITIVE",
            "chemistry_authority_source": subject.FOUR_M5_EVENT_MATRIX_SOURCE,
            "positive_authority_source": subject.FOUR_M5_EVENT_MATRIX_SOURCE,
            "task_relevance_disposition": "RELEVANT",
            "task_relevance_authority_source": subject.FOUR_M5_EVENT_MATRIX_SOURCE,
...
def independently_verify_counts_v1(
    ...
    if profiles != Counter({"DIRECT_ATTACHMENT_OPTIONAL_LINKER_V1": 72, "STRICT_LINKER_PRESENT_V1": 52}):
    ...
    expected_blockers = {
        "chemistry_unresolved": {"all_1000": 786},
        "pair_authority_absent": {"all_1000": 876, "within_positive_124": 0},
        "role_authority_absent": {"all_1000": 876, "within_positive_124": 0},
        "human_training_exclusion": {"within_positive_124": 68},
        "missing_split_authority": {"within_positive_124": 83, "within_include_56": 31},
        "missing_POST_training_authority": {"within_positive_124": 107, "within_include_56": 39},
        "missing_training_admission": {"within_positive_124": 119, "within_include_56": 51},
        "feature_semantics_pending": {"within_positive_124": 124},
    }
    ...
    if (
        tensor["within_positive_124"] != 83
        or tensor["within_include_56"] != 27
        or tensor["missing_source_composition"].get("4M5") != 4
    ):
...
def independently_compute_top10_v1(
    ...
    if len(candidates) != 110 or any(item[2] == subject.FOUR_M5_REVIEW_UNIT_ID_V1 for item in candidates):
        raise ValueError("FOUR_M5_PENDING_OR_PENDING_COUNT_INVALID")
    return [
        {
            "rank": rank,
...
def verify_manifest_v1(manifest: dict[str, object], computation: object) -> None:
    ...
    if (
        ...
        or len(manifest["semantic_source_bindings"]) != 132
        ...
        or manifest["refresh_contract"]["changed_field_count_per_four_m5_row"] != 18
...
def run_check_v1(root: Path = ROOT) -> dict[str, object]:
    ...
    computation = subject.compute_covapie_cumulative1000_current_global_readiness_census_with_4m5_v1(root)
    built = subject._build_artifacts_from_computation_v1(root, computation)
    rebuilt = subject.build_covapie_cumulative1000_current_global_readiness_artifacts_with_4m5_v1(root)
    ...
    reconciled = reconciliation.reconcile_real_completed_human_decisions_with_4m5_v1(root)
    if (
        len(reconciled.normalized_facts) != 111
        or len({fact.source_binding_path for fact in reconciled.normalized_facts}) != 17
        or reconciled.review_summary["completed_positive_event_count"] != 107
        or reconciled.review_summary["unreviewed_event_count"] != 203
    ):
        raise ValueError("RECONCILIATION_EXACT17_111_COUNTS_INVALID")
    ...
    return {
        ...
        **delta,
        ...
        "four_m5_pending": False,
        "pending_review_unit_count": 110,
        "next_pending": top[0],
