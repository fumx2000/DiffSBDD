#!/usr/bin/env python3
"""Independent fail-closed checker for the cumulative1000 CER refresh V1."""
...
EXPECTED_CENSUS_SHA256 = "9da898c15da0dad45dd37a33ca2665e5e1634d588b566f824fe688c6cc49e71a"
EXPECTED_SUMMARY_SHA256 = "12187f4067b5b5f07e364c6e4f9fe74a7b44e0819d4afcaa24c1b6db1fb12d2b"
EXPECTED_BINDINGS_SHA256 = "4107486a5fb2e3086302f0064f0a1dff7d351697c74ffe18fe86e2a22d3f14fe"
EXPECTED_CER_EVENT_IDS = (
...
FROZEN_BINDINGS = (
    (
        ...
        "src/covalent_ext/covapie_cumulative1000_current_global_readiness_census_with_1n0_v1.py",
        "repository_relative", 65131,
        "8b5df9a718d9a42eb9372c68a466deb2b595fc6285dc02719b68390c96041ee5",
        False,
    ),
    (
        "PREDECESSOR_WITH_1N0_MATERIALIZED_CENSUS",
        "data/derived/covalent_small/covapie_cumulative1000_current_global_readiness_census_with_1n0_v1/covapie_cumulative1000_current_global_readiness_census_with_1n0_v1.csv",
        "repository_relative", 533426,
        "ac63ced99e77212e5952b41169369c5e5c77967f9409e2e1fec25f99808eaf35",
        False,
    ),
    (
        "PREDECESSOR_WITH_1N0_MATERIALIZED_SUMMARY",
        "data/derived/covalent_small/covapie_cumulative1000_current_global_readiness_census_with_1n0_v1/covapie_cumulative1000_current_global_readiness_summary_with_1n0_v1.json",
        "repository_relative", 17728,
        "516ab4c1ed9196c2233695566be9976d8f9f8dc5b13bb88b364b15eee8d08459",
        False,
    ),
    (
        "PREDECESSOR_WITH_1N0_MATERIALIZED_MANIFEST",
        "data/derived/covalent_small/covapie_cumulative1000_current_global_readiness_census_with_1n0_v1/covapie_cumulative1000_current_global_readiness_manifest_with_1n0_v1.json",
        "repository_relative", 53527,
    ...
    (
        ...
        "repository_relative", 32847,
        "a3b5332654bbee87503737f6e8b077022232e4eb1a92fb48606bd761e30c6fba",
        False,
    ),
    (
        "CER_INGESTION_OWNER",
        "src/covalent_ext/covapie_cer_completed_decision_ingestion_and_task_label_availability_v1.py",
        "repository_relative", 81662,
        "bc190935f661cd94e7dd5cf4f48782d6c99fba4455224ef3c73a3d1411b84e54",
        False,
    ),
    (
        "CER_EVENT_TASK_LABEL_AVAILABILITY",
        "data/derived/covalent_small/covapie_cer_completed_decision_ingestion_and_task_label_availability_v1/covapie_cer_event_task_label_availability_v1.csv",
        "repository_relative", 10331,
...
SEMANTIC_ADDITIVE_BINDINGS = tuple(
    ...
    if item[0] != "PREDECESSOR_WITH_1N0_MATERIALIZED_MANIFEST"
)
...
def verify_frozen_bindings_v1(root: Path) -> list[dict[str, object]]:
    ...
    for filename, (byte_count, sha256) in FROZEN_INGESTION_OUTPUTS.items():
        ...
        payload = _read(path, "CER_INGESTION_OUTPUT:" + filename)
        if len(payload) != byte_count or _sha(payload) != sha256:
            raise ValueError("CER_INGESTION_OUTPUT_BINDING_INVALID:" + filename)
        if built[filename] != payload:
            raise ValueError("CER_INGESTION_OUTPUT_NOT_SOURCE_DERIVED:" + filename)
    return verified


def independently_verify_matrix_v1(root: Path) -> list[dict[str, str]]:
    payload = _read(root / subject.CER_EVENT_MATRIX_RELATIVE, "CER_MATRIX")
    reader = csv.DictReader(io.StringIO(payload.decode("utf-8"), newline=""))
    rows = [dict(row) for row in reader]
    if tuple(reader.fieldnames or ()) != ingestion.MATRIX_HEADER:
        raise ValueError("CER_MATRIX_HEADER_INVALID")
    if len(rows) != 4 or tuple(row["canonical_event_id"] for row in rows) != EXPECTED_CER_EVENT_IDS:
        raise ValueError("CER_MATRIX_NOT_EXACT4")
    if [int(row["scaleup_rank"]) for row in rows] != [52, 53, 54, 55]:
        raise ValueError("CER_MATRIX_RANKS_INVALID")
    if Counter(row["pdb_id"] for row in rows) != Counter({"1FJ8": 4}):
        raise ValueError("CER_MATRIX_PDB_COUNTS_INVALID")
    if [
        (row["protein_chain_or_asym"], row["ligand_chain_or_asym"])
        for row in rows
    ] != [("A", "E"), ("B", "F"), ("C", "G"), ("D", "H")]:
        raise ValueError("CER_MATRIX_CONTEXTS_COLLAPSED_OR_DRIFTED")
    if {row["cys_residue_id"] for row in rows} != {"CYS:163-"}:
        raise ValueError("CER_MATRIX_CYS_IDENTITIES_INVALID")
    expected = {
        ...
        "pair_authority_scope": ingestion.AUTHORITY_SCOPE,
        ...
        "authority_scope": ingestion.AUTHORITY_SCOPE,
    ...
    for row in rows:
        if any(row[field] != value for field, value in expected.items()):
            raise ValueError("CER_MATRIX_SEMANTICS_INVALID:" + row["canonical_event_id"])
        ...
        if (
            ...
            raise ValueError("CER_MATRIX_EXACT5_INVALID")
    return rows
...
def independently_verify_delta_v1(
    ...
    cer = set(EXPECTED_CER_EVENT_IDS)
    if [int(before[event_id]["scaleup_rank"]) for event_id in EXPECTED_CER_EVENT_IDS] != [
        52, 53, 54, 55,
    ]:
        raise ValueError("PREDECESSOR_CER_IDENTITY_INVALID")
    expected_prior = {
    ...
    if any(
        ...
        raise ValueError("PREDECESSOR_CER_STATE_INVALID")
    changed = {event_id for event_id in before if before[event_id] != after[event_id]}
    if changed != cer:
        raise ValueError("DELTA_NOT_EXACT_CER_EXACT4")
    if any(before[event_id] != after[event_id] for event_id in set(before) - cer):
        raise ValueError("NON_CER_ROW_DRIFT")
    for event_id in cer:
        changed_fields = {
        ...
        if changed_fields != EXPECTED_CHANGED_FIELDS:
            raise ValueError("CER_CHANGED_FIELD_SET_INVALID:" + event_id)
        row = after[event_id]
        expected_after = {
            "current_global_status": "COMPLETED_HUMAN_POSITIVE",
            "current_review_status": "COMPLETED_HUMAN_POSITIVE",
            "human_review_completed": "true",
            "human_review_authority_source": subject.CER_HUMAN_DECISION_SOURCE,
            "chemistry_disposition": "POSITIVE",
            "chemistry_authority_source": subject.CER_EVENT_MATRIX_SOURCE,
            "positive_authority_source": subject.CER_EVENT_MATRIX_SOURCE,
            "task_relevance_disposition": "RELEVANT",
            "task_relevance_authority_source": subject.CER_EVENT_MATRIX_SOURCE,
        ...
        if any(row[field] != value for field, value in expected_after.items()):
            raise ValueError("CER_SEMANTICS_INVALID:" + event_id)
        if any(row[field] != value for field, value in EXPECTED_CER_STRUCTURAL_CELLS.items()):
            raise ValueError("CER_STRUCTURAL_COVERAGE_INVALID:" + event_id)
    return {
...
def independently_verify_counts_v1(
    ...
    if Counter(row["current_global_status"] for row in rows) != Counter(
        {
            "CURRENTLY_UNREVIEWED": 207,
            "CURRENTLY_IN_PROGRESS": 0,
            "COMPLETED_HUMAN_POSITIVE": 103,
    ...
    if (
        ...
        or pair["sample_level_authoritative_pair_count"] != 120
        ...
        or pair["cer_sample_authority_contribution_count"] != 4
        or pair["cer_model_bound_target_contribution_count"] != 0
    ):
        raise ValueError("PAIR_SUMMARY_INVALID")
    if summary["role"] != {  # type: ignore[index]
        "role_partition_sample_authoritative_count": 120,
        "role_profile_counts": {
            "STRICT_LINKER_PRESENT_V1": 52,
            "DIRECT_ATTACHMENT_OPTIONAL_LINKER_V1": 68,
            "other": 0,
        },
        "canonical_mask_structural_labels_available_count": 120,
        "all_five_structurally_applicable_count": 52,
        "direct_profile_A_B3_C_count": 68,
        "unknown_role_row_count": 880,
    ...
    expected_blockers = {
        ...
        "chemistry_unresolved": {"all_1000": 790},
        "pair_authority_absent": {"all_1000": 880, "within_positive_120": 0},
        "role_authority_absent": {"all_1000": 880, "within_positive_120": 0},
        "human_training_exclusion": {"within_positive_120": 68},
        ...
        "feature_semantics_pending": {"within_positive_120": 120},
    }
    ...
    if (
        stage["training_use_include_count"] != 52
        or stage["future_training_admission_candidate_count"] != 35
        ...
        or stage["future_candidate_source_composition"].get("CER") != 4
    ):
...
def independently_compute_top10_v1(
    ...
    if len(candidates) != 111:
        raise ValueError("PENDING_QUEUE_NOT_EXACT111")
    if any(unit == subject.CER_REVIEW_UNIT_ID_V1 for _n, _p, unit, _r, _s in candidates):
        raise ValueError("CER_REVIEW_UNIT_STILL_PENDING")
...
def verify_semantic_bindings_v1(
    ...
    if len(frozen.semantic_source_bindings) != 120:
        raise ValueError("PREDECESSOR_SEMANTIC_BINDING_COUNT_NOT_120")
    ...
    for role, relative, namespace, byte_count, sha256, expected_executable in SEMANTIC_ADDITIVE_BINDINGS:
        ...
        additive.append(row)
    expected = (*frozen.semantic_source_bindings, *additive)
    if observed != expected or len(observed) != 126:
        raise ValueError("SEMANTIC_BINDINGS_NOT_PREDECESSOR_PLUS_CER_INPUTS")
    ...
    if any(
        frozenset(row)
        not in {frozenset(legacy_keys), frozenset(legacy_keys | {"expected_executable"})}
        for row in observed
    ):
        raise ValueError("NUMERIC_POSIX_OR_AMBIGUOUS_SEMANTIC_IDENTITY")
    if any(
        row["path"] == ingestion.FORMAL_DECISION_RELATIVE.as_posix()
        and row["artifact_role"].startswith("CER")
        for row in observed
    ):
        raise ValueError("DIRECT_CER_FORMAL_BINDING_FORBIDDEN")
...
def verify_manifest_v1(artifacts: dict[str, bytes], fresh: object) -> None:
    ...
    if manifest["source_binding_policy_contract"] != {
        ...
        raise ValueError("MANIFEST_SOURCE_BINDING_POLICY_INVALID")
...
BASELINE_COMMIT = "df86a9cebc70f78f3cc384604cd2fd74f2e56463"
...
def check_lifecycle_simulations_v1() -> dict[str, bool]:
    expected = set(subject.EXACT7_PATHS_V1)
    future = {
        "src/covalent_ext/synthetic_future_census_v1.py",
        "data/derived/covalent_small/synthetic_future_summary_v1.json",
    }
    candidate = {
        "placement": "CANDIDATE_UNTRACKED",
        "head": BASELINE_COMMIT,
        "origin": BASELINE_COMMIT,
        "ahead": 0,
        "behind": 0,
    ...
    tracked = {
        ...
        "head": "synthetic-head",
        "origin": "synthetic-origin-between",
        "ahead": 2,
        "behind": 0,
        "baseline_is_ancestor_of_head": True,
        "baseline_is_ancestor_of_origin": True,
    ...
    _classify_repository_lifecycle_v1(
        **{**tracked, "origin": "synthetic-head", "ahead": 0}
    )

    def fails(**updates: object) -> bool:
        try:
    ...
    if not all(
        (
            fails(baseline_is_ancestor_of_head=False),
            fails(baseline_is_ancestor_of_origin=False),
            fails(origin_is_ancestor_of_head=False),
            fails(behind=1),
            fails(baseline_to_head_changed_paths=tuple(expected - {next(iter(expected))})),
            fails(origin="diverged", origin_is_ancestor_of_head=False),
        )
    ):
...
def verify_git_and_cache_safety_v1(root: Path) -> dict[str, object]:
    ...
    for directory, names, files in os.walk(root):
        ...
        forbidden_ignored_count += sum(
            filename.endswith((".pyc", ".tmp", ".part", ".log"))
...
def _verify_prewrite_safety_v1(root: Path) -> dict[str, bool]:
    with tempfile.TemporaryDirectory() as temporary:
        ...
        except subject.Cumulative1000CurrentGlobalReadinessCensusWithCERError:
            pass
        else:
...
def run_check_v1(root: Path = ROOT) -> dict[str, object]:
    ...
    fresh = subject.compute_covapie_cumulative1000_current_global_readiness_census_with_cer_v1(
        root
    )
    if not subject.validate_covapie_cumulative1000_current_global_readiness_census_with_cer_v1(
        fresh
    ):
    ...
    rebuilt = subject.build_covapie_cumulative1000_current_global_readiness_artifacts_with_cer_v1(
        root
    )
    if built != rebuilt:
    ...
    reconciled = reconciliation.reconcile_real_completed_human_decisions_with_cer_v1(root)
    ...
    if not (
        expected_top[0]["review_unit_id"] == "COVAPIE_BULK_REVIEW_UNIT_9E98765987D25C42"
        and expected_top[0]["ligand_component_ids"] == ["4M5"]
        and expected_top[0]["pdb_ids"] == ["5AZT", "5AZV"]
        and expected_top[0]["rank"] == 1
        and expected_top[0]["raw_priority_rank"] == 20
        and expected_top[0]["event_count"] == 4
        and all(item["ligand_component_ids"] != ["CER"] for item in expected_top)
    ):
        raise ValueError("NEXT_PRIORITY_NOT_4M5")
    ...
    if (
        ...
        or boundary["CER_CENSUS_SOURCE_BINDING_V2_CLEAN_FROM_BIRTH"] is not True
        or boundary["separate_CER_census_V2_successor_required"] is not False
        or boundary["QUEUE_REFRESH"] is not False
        or boundary["CENSUS_REFRESH"] is not True
        or boundary["priority_queue_file_modified"] is not False
        or boundary["priority_queue_file_created"] is not False
        or boundary["next_priority_review_unit"] != "COVAPIE_BULK_REVIEW_UNIT_9E98765987D25C42"
        or boundary["next_priority_review_ligand"] != "4M5"
        or boundary["next_priority_review_event_count"] != 4
        or boundary["next_priority_review_current_pending_rank"] != 1
        or boundary["next_priority_review_raw_priority_rank"] != 20
        or boundary["next_review_started"] is not False
        or boundary["NEXT_REVIEW_STARTED"] is not False
        or boundary["CER_REVIEW_COMPLETED"] is not True
        ...
        or boundary["formal_decision_read_directly"] is not False
        or boundary["formal_decision_bound_directly"] is not False
        or boundary["formal_validator_executed"] is not False
        or boundary["Step12D"]
    ...
    return {
        ...
        **safety,
        "refreshed_positive_count": 120,
        "task_relevant_count": 121,
        "training_include_count": 52,
        "training_exclude_count": 68,
        "future_candidate_count": 35,
        "formal_training_admitted_count": 5,
        "current_runtime_model_usable_count": 17,
        "pending_review_unit_count": 111,
