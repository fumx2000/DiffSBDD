#!/usr/bin/env python3
"""Fail-closed checker for OZJ Exact4 completed-decision ingestion V1."""
...
def verify_candidate_exact7_v1(repo_root: Path) -> dict[str, object]:
    ...
    expected = (
        ...
        "scripts/check_covapie_ozj_completed_decision_ingestion_and_task_label_availability_v1.py",
        "tests/test_covapie_ozj_completed_decision_ingestion_and_task_label_availability_v1.py",
        "data/derived/covalent_small/covapie_ozj_completed_decision_ingestion_and_task_label_availability_v1/covapie_ozj_completed_human_decision_snapshot_v1.json",
        ...
        "data/derived/covalent_small/covapie_ozj_completed_decision_ingestion_and_task_label_availability_v1/covapie_ozj_completed_decision_ingestion_summary_v1.json",
        "data/derived/covalent_small/covapie_ozj_completed_decision_ingestion_and_task_label_availability_v1/covapie_ozj_completed_decision_ingestion_manifest_v1.json",
    )
...
def _verify_two_directory_determinism(repo_root: Path) -> dict[str, object]:
    with tempfile.TemporaryDirectory(
        prefix="covapie_ozj_ingest_a_"
    ) as first_name:
        with tempfile.TemporaryDirectory(
            prefix="covapie_ozj_ingest_b_"
        ) as second_name:
            ...
            if first != second or any(
                ...
                raise ValueError(
                    "DETERMINISTIC_DOUBLE_MATERIALIZATION_MISMATCH"
                )
            return {
                "deterministic_double_materialization_byte_identical": True,
                "artifact_sha256": {
                    name: _sha(first[name])
                    for name in subject.OUTPUT_FILENAMES
                },
            }
...
def run_check_v1(repo_root: Path) -> dict[str, object]:
    ...
    if (
        binding["byte_count"] != 28914
        or binding["sha256"]
        != "0b14271a4541e69d768e28b6433c87b8b22c21505f6e3bdf075bb94381c3c606"
        or binding["schema_version"]
        != "covapie_ozj_exact4_formal_human_decision_v1"
        or binding["review_unit_id"]
        != "COVAPIE_BULK_REVIEW_UNIT_18734D9C06222450"
        or binding["reviewer_id"] != "fmx"
        or binding["attestor_id"] != "fmx"
        or binding["approved_at_utc"] != "2026-08-28T14:03:16Z"
        or binding["approved"] is not True
        or binding["unsigned"] is not False
    ):
        raise ValueError("FORMAL_OZJ_BINDING_INVALID")
    if (
        ...
        or len(bound["include_semantic_precedent_bindings"]) != 3
    ...
    if (
        len(events) != 4
        or len({event["canonical_event_id"] for event in events}) != 4
        ...
        raise ValueError("EXACT4_COVERAGE_INVALID")
    if len(matrix) != 4 or [
        int(row["scaleup_rank"]) for row in matrix
    ] != list(subject.EXPECTED_RANKS):
        raise ValueError("EXACT4_MATRIX_ORDER_INVALID")
    for index, row in enumerate(matrix):
        ...
        if (
            ...
            != subject.FUTURE_STATUS
            or row["future_training_candidate_derived_by_ingestion"]
            != "true"
            or row["future_training_candidate_is_training_admission"]
            != "false"
            or row["training_admitted"] != "false"
            or row["current_runtime_model_usable"] != "false"
            or row["source_CAF_OAD_bond_order"] != "DOUB"
        ):
            raise ValueError("OZJ_EVENT_OR_INCLUDE_PROJECTION_INVALID")

    role = snapshot["selected_role_partition"]
    tasks = snapshot["canonical_task_contract"]
    formal_training = snapshot["formal_training_use_source_boundary"]
    ...
    if (
        ...
        or role["reactive_CAF_in_warhead"] is not True
        ...
        raise ValueError("CANDIDATE1_STRICT_EXACT5_INVALID")
    if (
        formal_training["formal_event_training_use_decision"] != "INCLUDE"
        or formal_training["future_training_admission_candidate"] is not None
        or formal_training["future_training_admission_candidate_status"]
        != subject.FORMAL_FUTURE_STATUS
        or formal_training["formal_training_admitted"] is not False
        or ingestion["candidate_for_future_training_admission"] is not True
        or ingestion["future_training_admission_status"]
        != subject.FUTURE_STATUS
        or ingestion["future_training_candidate_derived_by_ingestion"]
        is not True
        or ingestion["future_training_candidate_is_training_admission"]
        is not False
        or ingestion["training_admitted"] is not False
        or ingestion["training_materialization_allowed_now"] is not False
        or ingestion["current_runtime_model_usable"] is not False
    ):
        raise ValueError("INCLUDE_NULL_TO_DERIVED_TRUE_BOUNDARY_INVALID")
    if (
        topology["source_CAF_OAD_bond_order"] != "DOUB"
        ...
        raise ValueError(
            "TOPOLOGY_GEOMETRY_OR_REUSABLE_BOUNDARY_INVALID"
        )
    ...
    if (
        ...
        or _sha(artifacts[subject.MATRIX])
        != subject._EXPECTED_MATRIX_SHA256_V1
        or _sha(artifacts[subject.SUMMARY])
    ...
    if (
        ...
        or summary["task_relevant_count"] != 4
        or summary["chemistry_positive_count"] != 4
        or summary["human_training_INCLUDE_count"] != 4
        or summary["future_training_admission_candidate_count"] != 4
        or summary["training_admitted_count"] != 0
        or summary["published_global_positive_count_remains"] != 100
        or summary["published_task_relevant_count_remains"] != 101
        or summary["published_training_INCLUDE_count_remains"] != 36
        or summary["published_training_EXCLUDE_count_remains"] != 64
        or summary["published_future_training_candidate_count_remains"] != 19
        or summary["current_published_OZJ_status"] != "CURRENTLY_UNREVIEWED"
        or future["chemistry_positive"] != 104
        or future["task_relevant"] != 105
        or future["training_INCLUDE"] != 40
        or future["training_EXCLUDE"] != 64
        or future["future_candidates"] != 23
        or future["sample_pair"] != 104
        or future["sample_role"] != 104
        or future["STRICT"] != 48
        or future["DIRECT"] != 56
        or future["A"] != 104
        or future["B"] != 48
        ...
        raise ValueError(
            "CURRENT_OR_INFORMATIONAL_FUTURE_CENSUS_BOUNDARY_INVALID"
        )
    ...
    result = {
        ...
        "candidate1_strict_role_projection_verified": True,
        "strict_profile_applicable_tasks_exact_0_1_2_3_4": True,
        "include_null_to_derived_true_separation_verified": True,
        ...
        "future_104_40_64_23_informational_only_verified": True,
    ...
    true_fields = (
        ...
        "architecture_precedents_verified",
        "include_semantic_precedent_verified", "exact4_unique_complete",
        "candidate1_strict_role_projection_verified",
        "strict_profile_applicable_tasks_exact_0_1_2_3_4",
        "include_null_to_derived_true_separation_verified",
        ...
        "future_104_40_64_23_informational_only_verified",
        "no_model_or_training_work_verified",
        "feature_semantics_audit_required_later",
        "ready_for_OZJ_reconciliation_successor",
    )
    false_fields = (
...
def main() -> int:
    ...
    keys = (
        ...
        "formal_decision_binding_verified", "formal_decision_byte_count",
        "formal_decision_sha256", "formal_decision_schema", "reviewer_id",
        "attestor_id", "approved_at_utc",
        ...
        "human_training_INCLUDE_count", "training_use_allowed_count",
