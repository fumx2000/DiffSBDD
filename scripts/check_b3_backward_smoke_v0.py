    ...
    return [
        {
            ...
            "section": "step11p_precondition",
            "status": "passed" if manifest["step11p_validated"] else "blocked",
            "evidence": _json_text(sections["step11p_precondition"]),
            "decision": "Step 11P B3 pretrained masked loss evidence is accepted.",
        ...
        },
        {
            ...
            "decision": "Canonical B3 target/context counts remain scaffold-only versus linker+warhead context.",
            "blocking_reasons": blockers,
            "recommended_next_step": recommended,
        },
        {
        ...
        },
        {
            "stage": STAGE,
            "previous_stage": PREVIOUS_STAGE,
            "section": "gradient_stats",
            "status": "passed" if manifest["finite_nonzero_grad_exists"] else "blocked",
            "evidence": _json_text(sections["gradient_stats"]),
            "decision": "Gradient statistics are finite and nonzero.",
            "blocking_reasons": blockers,
            "recommended_next_step": recommended,
        },
        {
            ...
            "previous_stage": PREVIOUS_STAGE,
            "section": "decision",
            "status": "passed" if manifest["b3_backward_smoke_passed"] else "blocked",
            "evidence": _json_text(sections["decision"]),
            "decision": "B3 backward smoke allows moving to a single update smoke.",
            "blocking_reasons": blockers,
            "recommended_next_step": recommended,
        },
        {
            ...
            "decision": "No optimizer, checkpoint/model/tensor dump, forbidden artifact, or protected source edit occurred.",
            "blocking_reasons": blockers,
            "recommended_next_step": recommended,
        },
    ]
...
def write_summary(result: dict[str, Any], output_md: str | Path) -> None:
    ...
    lines = [
        ...
        f"It uses canonical long-form `{MASK_LEVEL}` only with `{INPUT_SOURCE}`.",
        ...
        f"- {BWD}_success: {str(manifest[BWD + '_success']).lower()}",
        ...
        "",
        "## Decision",
        f"- b3_backward_smoke_passed: {str(manifest['b3_backward_smoke_passed']).lower()}",
        f"- b3_backward_gradient_contract_proven: {str(manifest['b3_backward_gradient_contract_proven']).lower()}",
        f"- b3_finite_nonzero_gradient_proven: {str(manifest['b3_finite_nonzero_gradient_proven']).lower()}",
        f"- b3_single_optimizer_step_smoke_allowed: {str(manifest['b3_single_optimizer_step_smoke_allowed']).lower()}",
...
def run(device: str = "cpu") -> int:
    result = build_b3_backward_smoke_v0(device=device)
    write_csv(build_report_rows(result), REPORT_CSV, REPORT_COLUMNS)
    write_json(result["manifest"], MANIFEST_JSON)
    write_csv(result["gradient_table_rows"], GRADIENT_TABLE_CSV, GRADIENT_TABLE_COLUMNS)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run B3 backward smoke v0.")
