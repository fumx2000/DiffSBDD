#!/usr/bin/env python3
"""Fail-closed checker for 1F8 Exact8 completed-decision ingestion V1."""
...
def verify_candidate_exact7_v1(repo_root: Path) -> dict[str, object]:
    ...
    expected = (
        "src/covalent_ext/covapie_1f8_completed_decision_ingestion_and_task_label_availability_v1.py",
        "scripts/check_covapie_1f8_completed_decision_ingestion_and_task_label_availability_v1.py",
        "tests/test_covapie_1f8_completed_decision_ingestion_and_task_label_availability_v1.py",
        "data/derived/covalent_small/covapie_1f8_completed_decision_ingestion_and_task_label_availability_v1/covapie_1f8_completed_human_decision_snapshot_v1.json",
        "data/derived/covalent_small/covapie_1f8_completed_decision_ingestion_and_task_label_availability_v1/covapie_1f8_event_task_label_availability_v1.csv",
        "data/derived/covalent_small/covapie_1f8_completed_decision_ingestion_and_task_label_availability_v1/covapie_1f8_completed_decision_ingestion_summary_v1.json",
        "data/derived/covalent_small/covapie_1f8_completed_decision_ingestion_and_task_label_availability_v1/covapie_1f8_completed_decision_ingestion_manifest_v1.json",
    )
...
def _verify_two_directory_determinism(repo_root: Path) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="covapie_1f8_ingest_a_") as first_name:
        with tempfile.TemporaryDirectory(prefix="covapie_1f8_ingest_b_") as second_name:
...
def run_check_v1(repo_root: Path) -> dict[str, object]:
    ...
    if tuple(
        event["canonical_event_id"] for event in bound["normalized"]["events"]
    ...
    if any(
        ...
        or row["selected_connection_id"] != subject.EXPECTED_EVENTS[index][9]
        ...
        raise ValueError("1F8_EVENT_PROVENANCE_INVALID")

    pair = snapshot["reactive_pair"]
    if (
        pair["protein_reactive_atom"] != "SG"
        or pair["ligand_reactive_atom"] != "SD"
        or pair["ligand_reactive_atom_element"] != "S"
        ...
        raise ValueError("1F8_REACTIVE_PAIR_PROJECTION_INVALID")
    ...
    if (
        ...
        or role["warhead_atoms"] != ["SD"]
        or role["linker_atoms"] != ["C15", "C16", "N17"]
        ...
        or role["linker_atoms_on_scaffold_warhead_paths"] is not True
        or role["machine_selected"] is not False
        or role["machine_recommended_candidate"] is not None
    ):
        raise ValueError("1F8_CANDIDATE7_ROLE_PROJECTION_INVALID")
    ...
    if (
        context["engineered_target_site"] != "PDK1_T148C"
        or context["native_cysteine_site"] is not False
        or context["medicinal_covalent_inhibitor_context"] is not True
        or context["allosteric_inhibitor_context"] is not True
        or context["disulfide_trapping_context"] is not True
        or context["observed_retained_fragment_context"] is not True
        or context["event_specific_disposition_exception"] is not False
    ):
        raise ValueError("1F8_SCIENTIFIC_CONTEXT_PROJECTION_INVALID")
    observed = snapshot["observed_graph_PRE_boundary"]
    if (
        observed["observed_reactive_atom"] != "SD"
        or observed["observed_reactive_atom_element"] != "S"
        or observed["observed_graph_represents_retained_fragment"] is not True
        or observed["observed_graph_is_complete_authoritative_PRE_reagent"] is not False
        or observed["authoritative_PRE_precursor_topology"] is not None
        or observed["authoritative_complete_PRE_disulfide_reagent_topology"] is not None
        or observed["PRE_precursor_reconstruction_performed"] is not False
        or observed["complete_PRE_warhead_topology_authority_created"] is not False
    ):
        raise ValueError("1F8_OBSERVED_RETAINED_FRAGMENT_PRE_BOUNDARY_INVALID")
    ...
    result = {
        ...
        "candidate7_strict_role_projection_verified": True,
        "global_canonical_exact5_unchanged": True,
        "strict_profile_applicable_tasks_exact_all_five": True,
        "scientific_context_projection_verified": True,
        "observed_retained_fragment_PRE_boundary_verified": True,
        ...
        "PRE_precursor_topology_authority_count": summary["PRE_precursor_topology_authority_count"],
        "complete_PRE_disulfide_reagent_authority_count": summary["complete_PRE_disulfide_reagent_authority_count"],
    ...
    required_true = (
        ...
        "candidate7_strict_role_projection_verified", "global_canonical_exact5_unchanged",
        "strict_profile_applicable_tasks_exact_all_five",
        "scientific_context_projection_verified",
        "observed_retained_fragment_PRE_boundary_verified",
        ...
        "ready_for_1F8_reconciliation_successor",
    )
    required_false = (
    ...
    if result["published_global_positive_count_remains"] != 74:
...
def main() -> int:
    ...
    for key in (
        ...
        "role_partition_human_authority_count", "candidate7_strict_role_projection_verified",
        "global_canonical_exact5_unchanged",
        "strict_profile_applicable_tasks_exact_all_five",
        "scientific_context_projection_verified",
        "observed_retained_fragment_PRE_boundary_verified",
        "POST_source_evidence_count", "POST_geometry_training_authority_count",
        "PRE_geometry_authority_count", "PRE_precursor_topology_authority_count",
        "complete_PRE_disulfide_reagent_authority_count",
        "reaction_family_target_count", "warhead_rule_target_count",
        "warhead_type_target_count", "training_excluded_positive_count",
        "training_include_count", "future_training_admission_candidate_count",
