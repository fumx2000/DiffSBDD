#!/usr/bin/env python3
"""Verify deterministic Step14AU-B2 PDB semantics overlay outputs."""
...
def _validate_manifest_and_outputs(manifest: dict[str, object], root: Path, first_hashes: dict[str, str]) -> None:
    """Validate current on-disk evidence independently of manifest declarations."""
    ...
    assert gate._validate_source_rows(gate._source_boundary_rows())
    assert gate._validate_source_semantics(source)
    assert manifest["stage"] == gate.STAGE
    assert manifest["step_label"] == "Step14AU-B2"
    ...
    assert manifest["source_read_boundary"] == "only_step14au_a_and_step14au_b1_12_committed_metadata_outputs"
    ...
    assert len(manifest["output_sha256"]) == 5
    ...
    assert all(manifest[key] is True for key in (
        "all_source_boundary_checks_passed", "all_integration_lineage_checks_passed",
        "all_integrated_rule_checks_passed", "all_integrated_field_checks_passed",
        "all_integrated_context_checks_passed", "all_issue_transition_checks_passed",
        "all_safety_checks_passed", "all_checks_passed", "pdb_identifier_semantics_integrated",
        "admit_002_rule_logic_ready", "ready_for_admission_evaluator_interface_implementation",
    ))
    ...
    assert manifest["recommended_next_step"] == "resolve_next_covapie_bulk_download_admission_semantics_blocker"
    assert manifest["changed_rule_ids"] == ["ADMIT_002"]
    assert manifest["changed_field_names"] == ["pdb_id"]
    assert manifest["changed_context_items"] == ["pdb_id_format_contract"]
    ...
    assert manifest["evaluation_context_item_count"] == 18
    ...
    assert all(row["issue_origin"] == "step14au_a_remaining_semantics_blocker" for row in issues)
    assert all(row["integration_transition"] == "unchanged" for row in issues)
    assert not any(row["issue_origin"] == "step14au_b2_integration_gate_failure" for row in issues)
    assert gate.SAFETY_COLUMNS == tuple(safety[0].keys())
    ...
    assert gate.SOURCE_COLUMNS == ("source_relative_path", "tracked_by_git", "regular_file", "symlink", "sha256_expected", "sha256_observed", "source_boundary_passed")


def main() -> int:
    root = REPO_ROOT / gate.DEFAULT_OUTPUT_ROOT
    first = gate.run_covapie_bulk_download_admission_pdb_identifier_semantics_integration_gate_v1(root)
    first_hashes = {name: _hash(root / name) for name in (*gate.CSV_OUTPUTS, gate.MANIFEST_FILENAME)}
    second = gate.run_covapie_bulk_download_admission_pdb_identifier_semantics_integration_gate_v1(root)
    ...
    print("pdb_identifier_semantics_integrated=true")
    ...
    print("pdb_identifier_semantics_integration_outputs_byte_identical=true")
    print("covapie_bulk_download_admission_pdb_identifier_semantics_integration_gate_v1_passed")
    return 0
