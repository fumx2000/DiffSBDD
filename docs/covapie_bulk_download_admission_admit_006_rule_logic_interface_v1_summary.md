...
This increment implements the standalone, pure-in-memory ADMIT_006 evaluator:
...
`Admit006EvaluationResult` is a frozen dataclass with these exact ordered fields:
...
is blocked with `COVALENT_EVENT_EVIDENCE_NOT_EXPLICIT`. Malformed, unknown, or
...
The historical design reason `covalent_event_evidence_missing` remains reserved
for a future adapter's missing-field or missing-value path and is never emitted
by this standalone evaluator.
...
Expected semantics come from a separately implemented local oracle and are also
cross-checked against the committed predecessor truth contract. The new
evaluator does not call the predecessor design classifier. AST transitive
call-graph checks constrain the evaluator closure to `evaluate_admit_006`,
...
1. ADMIT_006 precondition manifest: readiness baseline.
2. Evidence-source enum design source: normative Exact3/Exact2 definitions and predecessor-oracle identity.
3. Evidence-source enum manifest: committed counts, precedence, readiness, and hashes.
4. Evidence-source truth matrix: complete committed Exact37 expectations.
5. Evidence-source issue inventory: ordered Exact11 preservation baseline.
6. ADMIT_006/007 responsibility matrix: explicit/distance rule ownership and reason boundary.
7. Unified result schema contract: result and outcome-routing invariants.
8. Unified evaluator/context routing matrix: ADMIT_006 dependencies and unsupported runtime state.
9. Current unified runtime source: literal Exact5 registry and absent ADMIT_006 registration.
10. ADMIT_005 standalone source: established frozen-result/materialization structure.
...
`UNIFIED_ADMISSION_RULE_COVERAGE_INCOMPLETE` remains open, but ADMIT_006 is
still present in its `affected_rules` with ADMIT_007 through ADMIT_015. A
...
byte-for-byte from the committed enum-contract predecessor.
...
ADMIT_006 adapter-contract design, and the mandatory pre-training feature
semantics audit. Adapter, runtime, ADMIT_007, all-rule coverage, provider,
real-evaluation, bulk-download, and training readiness remain false.

The recommended next step is
`design_covapie_admit_006_unified_adapter_contract_v1`. This increment stops
before that step.
