...
OUTPUT_ROOT = Path("data/derived/covalent_small/covapie_final_dataset_qa_gate_v0")
PRECONDITION_AUDIT_CSV = OUTPUT_ROOT / "covapie_final_dataset_qa_precondition_audit.csv"
SCHEMA_ORDER_QA_CSV = OUTPUT_ROOT / "covapie_final_dataset_schema_order_qa_audit.csv"
CSV_JSON_CONSISTENCY_QA_CSV = OUTPUT_ROOT / "covapie_final_dataset_csv_json_consistency_qa_audit.csv"
ROW_LINEAGE_QA_CSV = OUTPUT_ROOT / "covapie_final_dataset_row_lineage_qa_audit.csv"
MASK_DISTRIBUTION_QA_CSV = OUTPUT_ROOT / "covapie_final_dataset_mask_distribution_qa_audit.csv"
FEATURE_BLOCKER_QA_CSV = OUTPUT_ROOT / "covapie_final_dataset_feature_blocker_qa_audit.csv"
READINESS_QA_CSV = OUTPUT_ROOT / "covapie_final_dataset_readiness_qa_audit.csv"
BOUNDARY_SAFETY_CSV = OUTPUT_ROOT / "covapie_final_dataset_qa_boundary_safety.csv"
GIT_SAFETY_CSV = OUTPUT_ROOT / "covapie_final_dataset_qa_git_safety.csv"
MANIFEST_JSON = OUTPUT_ROOT / "covapie_final_dataset_qa_gate_manifest.json"
SUMMARY_MD = Path("docs/covapie_final_dataset_qa_gate_v0_summary.md")

step13bn = step13bo.step13bn
step13bm = step13bo.step13bm
step13bl = step13bo.step13bl
step13bk = step13bo.step13bk
step13bj = step13bo.step13bj
step13bi = step13bo.step13bi
step13bh = step13bo.step13bh
step13bg = step13bo.step13bg
step13bf = step13bo.step13bf
step13be = step13bo.step13be
step13bd = step13bo.step13bd

CANONICAL_MASK_TASK_NAMES = step13bo.CANONICAL_MASK_TASK_NAMES
CANONICAL_MASK_TASK_ALIASES = step13bo.CANONICAL_MASK_TASK_ALIASES
METADATA_CSV_SHA256 = step13bo.METADATA_CSV_SHA256
...
SCHEMA_ORDER_QA_COLUMNS = [
    "field_index",
    "final_dataset_field_name",
    "expected_field_name",
    "observed_field_name",
    "source_schema_contract_found",
    "csv_field_order_matches",
    "json_field_order_matches",
    "no_missing_field",
    "no_extra_field",
    "schema_order_qa_passed",
    "qa_comment",
]
CSV_JSON_CONSISTENCY_QA_COLUMNS = [
    "audit_item",
    "expected_status",
    "observed_status",
    "csv_row_count",
    ...
    "json_row_count",
    "json_field_count",
    "normalized_csv_json_identical",
    "csv_json_consistency_qa_passed",
    "qa_comment",
]
ROW_LINEAGE_QA_COLUMNS = [
    ...
    "sample_id_matches_source",
    "split_unit_matches_step13bk",
    "final_dataset_smoke_row_materialized_previous_step",
    "final_dataset_smoke_preview_rewritten_current_step",
    "real_final_dataset_materialized",
    "dataloader_ready",
    "ready_for_training",
...
FEATURE_BLOCKER_QA_COLUMNS = [
    "blocker_item",
    "expected_status",
    "observed_status",
    "blocker_preserved_in_all_rows",
...
def _project_rows(rows: list[dict[str, Any]], fields: list[str]) -> list[dict[str, str]]:
...
def _event_to_splits(rows: list[dict[str, str]]) -> dict[str, set[str]]:
...
def _forbidden_named_artifact_exists(root: Path = OUTPUT_ROOT) -> bool:
    forbidden = {
        ...
        "covapie_final_dataset_smoke_preview.csv",
        "covapie_final_dataset_smoke_preview.json",
        "sample_index.csv",
        "sample_index.json",
...
def build_precondition_rows() -> list[dict[str, Any]]:
    ...
    checks = [
        ("step13bo_manifest_exists", step13bo.MANIFEST_JSON, "exists", step13bo.MANIFEST_JSON.exists(), step13bo.MANIFEST_JSON.exists()),
        ("step13bo_stage", step13bo.MANIFEST_JSON, step13bo.STAGE, manifest13bo.get("stage"), manifest13bo.get("stage") == step13bo.STAGE),
        ("step13bo_all_checks_passed", step13bo.MANIFEST_JSON, "true", manifest13bo.get("all_checks_passed"), manifest13bo.get("all_checks_passed") is True),
        ("step13bo_ready_for_final_dataset_qa_gate", step13bo.MANIFEST_JSON, "true", manifest13bo.get("ready_for_covapie_final_dataset_qa_gate"), manifest13bo.get("ready_for_covapie_final_dataset_qa_gate") is True),
        ("step13bo_ready_for_dataloader_smoke", step13bo.MANIFEST_JSON, "false", manifest13bo.get("ready_for_covapie_dataloader_smoke"), manifest13bo.get("ready_for_covapie_dataloader_smoke") is False),
        ("step13bo_ready_for_training", step13bo.MANIFEST_JSON, "false", manifest13bo.get("ready_for_training"), manifest13bo.get("ready_for_training") is False),
        ("step13bo_ready_to_train_now", step13bo.MANIFEST_JSON, "false", manifest13bo.get("ready_to_train_now"), manifest13bo.get("ready_to_train_now") is False),
        ...
        ("step13bo_schema_order_smoke_audit_passed", step13bo.SCHEMA_ORDER_AUDIT_CSV, "true", manifest13bo.get("schema_order_smoke_audit_passed"), manifest13bo.get("schema_order_smoke_audit_passed") is True),
        ("step13bo_row_lineage_smoke_audit_passed", step13bo.ROW_LINEAGE_AUDIT_CSV, "true", manifest13bo.get("row_lineage_smoke_audit_passed"), manifest13bo.get("row_lineage_smoke_audit_passed") is True),
        ("step13bo_mask_distribution_smoke_audit_passed", step13bo.MASK_DISTRIBUTION_AUDIT_CSV, "true", manifest13bo.get("mask_distribution_smoke_audit_passed"), manifest13bo.get("mask_distribution_smoke_audit_passed") is True),
        ("step13bo_feature_blocker_smoke_audit_passed", step13bo.FEATURE_BLOCKER_AUDIT_CSV, "true", manifest13bo.get("feature_blocker_smoke_audit_passed"), manifest13bo.get("feature_blocker_smoke_audit_passed") is True),
        ...
        ("step13bk_split_unit_preview_row_count", step13bk.SPLIT_UNIT_SMOKE_PREVIEW_CSV, "4", len(_csv_rows(step13bk.SPLIT_UNIT_SMOKE_PREVIEW_CSV)), len(_csv_rows(step13bk.SPLIT_UNIT_SMOKE_PREVIEW_CSV)) == 4),
...
def build_schema_order_rows(preview_rows: list[dict[str, str]], json_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    missing = set(schema_fields) - set(csv_fields)
    extra = set(csv_fields) - set(schema_fields)
    rows = []
    for index, expected in enumerate(schema_fields, start=1):
        observed = csv_fields[index - 1] if index - 1 < len(csv_fields) else ""
        json_observed = json_projected_fields[index - 1] if index - 1 < len(json_projected_fields) else ""
        passed = expected == observed == json_observed and not missing and not extra
        rows.append(
            {
                "field_index": index,
                "final_dataset_field_name": expected,
                "expected_field_name": expected,
                "observed_field_name": observed,
                "source_schema_contract_found": True,
                "csv_field_order_matches": expected == observed,
                "json_field_order_matches": expected == json_observed,
                "no_missing_field": not missing,
                "no_extra_field": not extra,
                "schema_order_qa_passed": passed,
                "qa_comment": "CSV order and schema-projected JSON field order match Step 13BN contract",
            }
        )
    return rows
...
def build_csv_json_consistency_rows(preview_rows: list[dict[str, str]], json_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    csv_column_count = len(preview_rows[0]) if preview_rows else 0
    json_field_count = len(projected_json[0]) if projected_json else 0
    passed = len(preview_rows) == 20 and csv_column_count == 45 and len(json_rows) == 20 and json_field_count == 45 and identical
    return [
        {
            "audit_item": "final_dataset_smoke_preview_csv_json_consistency",
            "expected_status": "20_csv_rows_45_columns_20_json_rows_45_fields_identical_after_schema_projection",
            "observed_status": f"{len(preview_rows)}_csv_rows_{csv_column_count}_csv_columns_{len(json_rows)}_json_rows_{json_field_count}_json_fields",
            "csv_row_count": len(preview_rows),
            "csv_column_count": csv_column_count,
            "json_row_count": len(json_rows),
            "json_field_count": json_field_count,
            "normalized_csv_json_identical": identical,
            "csv_json_consistency_qa_passed": passed,
            "qa_comment": "JSON is compared after schema projection without rewriting the Step 13BO preview",
        }
    ]
...
def build_row_lineage_rows(preview_rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for row in preview_rows:
        ...
        sample_found = row["sample_id"] in sample_ids
        split_found = row["split_unit_id"] == split_by_sample.get(row["sample_id"])
        passed = (
        ...
        rows.append(
            {
                ...
                "source_sample_index_row_found": sample_found,
                "source_split_unit_found": split_found,
                "parent_event_group_bound_to_one_split_unit": event_bound,
                "sample_id_matches_source": sample_found,
                "split_unit_matches_step13bk": split_found,
                "final_dataset_smoke_row_materialized_previous_step": True,
                "final_dataset_smoke_preview_rewritten_current_step": False,
                "real_final_dataset_materialized": False,
                "dataloader_ready": False,
                "ready_for_training": False,
                "row_lineage_qa_passed": passed,
                "qa_comment": "Step 13BO smoke row lineage validated without rewriting preview or materializing real final dataset",
            }
        )
    return rows
        ...
        rows.append(
            {
                ...
                "qa_comment": "five canonical masks preserved in Step 13BO smoke preview",
            }
        )
    return rows
    ...
    return [
        {
            ...
            "blocker_qa_passed": checks[blocker],
            "qa_comment": "training blocker preserved after final dataset smoke preview QA",
        }
...
def build_readiness_rows() -> list[dict[str, Any]]:
    checks = [
        ...
        ("generic_final_dataset_not_written", "true", True),
        ("final_dataset_smoke_preview_not_rewritten_current_step", "true", True),
        ("dataloader_smoke_not_written", "true", True),
        ("training_artifacts_not_written", "true", True),
        ("feature_semantics_known_for_training_false", "true", True),
        ("unknown_atom_feature_policy_finalized_for_training_false", "true", True),
        ("ready_for_dataloader_interface_design_gate", "true", True),
        ("ready_for_training_false", "true", True),
    ]
    return [
        {
            "readiness_item": item,
            "expected_status": expected,
            "observed_status": "true" if passed else "false",
            "readiness_qa_passed": passed,
            "qa_comment": "final dataset QA permits dataloader interface design gate only" if item == "ready_for_dataloader_interface_design_gate" else "readiness boundary preserved",
        }
...
def build_boundary_rows() -> list[dict[str, Any]]:
    statuses = {
        ...
        "read_step13bo_smoke_audits": "executed_derived_csv_json_read_only",
        ...
        "final_dataset_smoke_preview_write_current_step": "not_executed_current_step_already_completed_previous_step",
        "real_final_dataset_write": "blocked_current_step",
        "generic_final_dataset_smoke_write": "blocked_current_step",
    ...
    return [
        {
            ...
            "qa_comment": "final dataset QA boundary respected",
        }
...
def build_git_safety_rows() -> list[dict[str, Any]]:
    checks = [
        ...
        ("no_final_dataset_smoke_preview_rewritten_current_step", str(OUTPUT_ROOT), "true", not _forbidden_named_artifact_exists()),
...
def run_covapie_final_dataset_qa_gate_v0() -> dict[str, Any]:
    ...
    preview_rows = _csv_rows(step13bo.SMOKE_PREVIEW_CSV)
    json_rows = _load_json(step13bo.SMOKE_PREVIEW_JSON)
    schema_rows = build_schema_order_rows(preview_rows, json_rows)
    consistency_rows = build_csv_json_consistency_rows(preview_rows, json_rows)
    ...
    manifest = {
        ...
        "step13bo_final_dataset_smoke_validated": all(_bool(row["precondition_passed"]) for row in precondition_rows),
        "source_preview_row_count": len(preview_rows),
        "source_preview_column_count": len(preview_rows[0]) if preview_rows else 0,
        "source_preview_json_row_count": len(json_rows),
        ...
        "schema_order_qa_row_count": len(schema_rows),
        "csv_json_consistency_qa_row_count": len(consistency_rows),
        "row_lineage_qa_row_count": len(lineage_rows),
        "mask_distribution_qa_row_count": len(mask_rows),
        "feature_blocker_qa_row_count": len(blocker_rows),
        "readiness_qa_row_count": len(readiness_rows),
        "schema_order_qa_passed": all(_bool(row["schema_order_qa_passed"]) for row in schema_rows),
        "csv_json_consistency_qa_passed": all(_bool(row["csv_json_consistency_qa_passed"]) for row in consistency_rows),
        "row_lineage_qa_passed": all(_bool(row["row_lineage_qa_passed"]) for row in lineage_rows),
        "mask_distribution_qa_passed": all(_bool(row["mask_distribution_qa_passed"]) for row in mask_rows),
        "feature_blocker_qa_passed": all(_bool(row["blocker_qa_passed"]) for row in blocker_rows),
        ...
        "final_dataset_smoke_preview_written_previous_step": True,
    ...
    manifest["all_checks_passed"] = all(
        [
            manifest["step13bo_final_dataset_smoke_validated"],
            manifest["source_preview_row_count"] == 20,
            manifest["source_preview_column_count"] == 45,
            manifest["source_preview_json_row_count"] == 20,
            manifest["source_unique_event_count"] == 4,
            manifest["source_unique_split_unit_count"] == 4,
            manifest["source_canonical_mask_task_count"] == 5,
            manifest["schema_order_qa_row_count"] == 45,
            manifest["csv_json_consistency_qa_row_count"] == 1,
            manifest["row_lineage_qa_row_count"] == 20,
            manifest["mask_distribution_qa_row_count"] == 5,
            manifest["feature_blocker_qa_row_count"] == 13,
            manifest["readiness_qa_row_count"] == 10,
            manifest["schema_order_qa_passed"],
            manifest["csv_json_consistency_qa_passed"],
            manifest["row_lineage_qa_passed"],
            manifest["mask_distribution_qa_passed"],
            manifest["feature_blocker_qa_passed"],
            manifest["readiness_qa_passed"],
            manifest["boundary_safety_passed"],
            manifest["git_safety_passed"],
            manifest["final_dataset_smoke_preview_written_previous_step"],
            not manifest["final_dataset_smoke_preview_written_current_step"],
            ...
            manifest["ready_for_covapie_dataloader_interface_design_gate"],
            not manifest["ready_for_covapie_dataloader_smoke"],
            not manifest["ready_for_training"],
            not manifest["ready_to_train_now"],
        ]
    )
    manifest["blocking_reasons"] = [] if manifest["all_checks_passed"] else ["final_dataset_qa_gate_failed"]
    return {
