"""Additive cumulative1000 readiness census refresh for published I12 Exact4.

This successor consumes the frozen 2A2-refreshed census plus already-published
I12 ingestion and reconciliation authority. It deep-copies the predecessor
rows and overlays only I12 Exact4. It creates no human, chemistry, pair, role,
...
from . import covapie_completed_human_decision_reconciliation_with_i12_v1 as i12_reconciliation
from . import covapie_cumulative1000_current_global_readiness_census_with_2a2_v1 as predecessor
from . import covapie_i12_completed_decision_ingestion_and_task_label_availability_v1 as i12_ingestion
...
__all__ = (
    "Cumulative1000CurrentGlobalReadinessCensusWithI12Error",
    "compute_covapie_cumulative1000_current_global_readiness_census_with_i12_v1",
    "validate_covapie_cumulative1000_current_global_readiness_census_with_i12_v1",
    "build_covapie_cumulative1000_current_global_readiness_artifacts_with_i12_v1",
    "materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_i12_v1",
)


SCHEMA_VERSION = "covapie_cumulative1000_current_global_readiness_census_with_i12_v1"
STAGE = "COVAPIE_CUMULATIVE1000_CURRENT_GLOBAL_READINESS_CENSUS_WITH_I12_V1"
ERROR_TOKEN = "COVAPIE_CUMULATIVE1000_CURRENT_GLOBAL_READINESS_CENSUS_WITH_I12_V1_ERROR"

OUTPUT_DIRECTORY_RELATIVE = Path(
    "data/derived/covalent_small/"
    "covapie_cumulative1000_current_global_readiness_census_with_i12_v1"
)
CENSUS_FILE = "covapie_cumulative1000_current_global_readiness_census_with_i12_v1.csv"
SUMMARY_FILE = "covapie_cumulative1000_current_global_readiness_summary_with_i12_v1.json"
MANIFEST_FILE = "covapie_cumulative1000_current_global_readiness_manifest_with_i12_v1.json"
...
CHECKER_RELATIVE = Path(
    "scripts/check_covapie_cumulative1000_current_global_readiness_census_with_i12_v1.py"
)
TEST_RELATIVE = Path(
    "tests/test_covapie_cumulative1000_current_global_readiness_census_with_i12_v1.py"
)
GUIDE_RELATIVE = Path(
    "docs/covapie_cumulative1000_current_global_readiness_census_with_i12_v1_guide.md"
)
...
I12_EXACT4_EVENT_IDS_V1 = i12_ingestion.EXPECTED_EVENT_IDS
I12_EXACT4_RANKS_V1 = i12_ingestion.EXPECTED_RANKS
I12_REVIEW_UNIT_ID_V1 = i12_ingestion.EXPECTED_REVIEW_UNIT_ID
I12_DIRECT_TASK_IDS_CELL_V1 = "[0,3,4]"
...
I12_RECONCILIATION_OWNER_RELATIVE = Path(
    "src/covalent_ext/covapie_completed_human_decision_reconciliation_with_i12_v1.py"
)
I12_INGESTION_OWNER_RELATIVE = i12_ingestion.SOURCE_RELATIVE
I12_EVENT_MATRIX_RELATIVE = i12_ingestion.OUTPUT_ROOT_RELATIVE / i12_ingestion.MATRIX
PRIORITY_QUEUE_RELATIVE = predecessor.PRIORITY_QUEUE_RELATIVE

I12_EVENT_MATRIX_SOURCE = I12_EVENT_MATRIX_RELATIVE.as_posix()
...
I12_HUMAN_DECISION_SOURCE = i12_ingestion.FORMAL_DECISION_RELATIVE.as_posix()

_EXPECTED_GLOBAL_STATUS_COUNTS_V1 = {
    generic.CURRENTLY_UNREVIEWED: 215,
...
_AUTHORIZED_I12_OVERLAY_FIELDS_V1 = frozenset(
    {
...
_EXPECTED_I12_STRUCTURAL_CELLS_V1 = {
...
_ADDITIVE_SOURCE_SPECS_V1 = (
    ...
    (
        ...
        I12_RECONCILIATION_OWNER_RELATIVE,
        "repository_relative",
        25975,
...
class Cumulative1000CurrentGlobalReadinessCensusWithI12Error(ValueError):
    """Raised unless the additive I12 refresh is exactly source-derived."""


def _fail(reason: str) -> NoReturn:
    raise Cumulative1000CurrentGlobalReadinessCensusWithI12Error(
...
def _verify_additive_sources(root: Path) -> tuple[dict[str, object], ...]:
    ...
    try:
        verify_bound_source_v2(
            ...
            label="PREDECESSOR_2A2_MANIFEST:" + PREDECESSOR_MANIFEST_RELATIVE.as_posix(),
            expected_executable=_PREDECESSOR_MANIFEST_SPEC_V1[2],
        )
    except SourceBindingPolicyV2Error as error:
        raise Cumulative1000CurrentGlobalReadinessCensusWithI12Error(
            f"{ERROR_TOKEN}:PREDECESSOR_2A2_MANIFEST_BINDING_INVALID"
        ) from error
    return tuple(bindings)


def _validate_i12_matrix_rows_v1(
    rows: Sequence[Mapping[str, str]],
) -> tuple[dict[str, str], ...]:
    normalized = tuple(dict(row) for row in rows)
    if (
        len(normalized) != 4
        or any(tuple(row) != i12_ingestion.MATRIX_HEADER for row in normalized)
        or tuple(row["canonical_event_id"] for row in normalized)
        != I12_EXACT4_EVENT_IDS_V1
        or tuple(int(row["scaleup_rank"]) for row in normalized)
        != I12_EXACT4_RANKS_V1
        or len({row["canonical_event_id"] for row in normalized}) != 4
    ):
        _fail("I12_EVENT_MATRIX_IDENTITY_NOT_EXACT4")
    if Counter(row["pdb_id"] for row in normalized) != Counter(
        {"1WOF": 2, "2AMP": 2}
    ):
        _fail("I12_EVENT_MATRIX_PDB_IDENTITY_NOT_EXACT_1WOF_2AMP")
    if {row["cys_residue_id"] for row in normalized} != {
        "CYS:145-",
        "CYS:144-",
    }:
        _fail("I12_EVENT_MATRIX_CYS_IDENTITY_INVALID")
    if tuple(
        (row["protein_chain_or_asym"], row["ligand_chain_or_asym"])
        for row in normalized
    ) != (("A", "C"), ("B", "D"), ("A", "C"), ("B", "D")):
        _fail("I12_EVENT_MATRIX_CONTEXTS_COLLAPSED_OR_DRIFTED")
    expected_cells = {
        ...
        "direct_profile_applicable_task_ids_json": I12_DIRECT_TASK_IDS_CELL_V1,
        ...
        "future_training_admission_status": i12_ingestion.FUTURE_STATUS,
        ...
        "PRE_status": i12_ingestion.PRE_STATUS,
        ...
        "authority_source": i12_ingestion.AUTHORITY_SOURCE,
        "authority_scope": i12_ingestion.AUTHORITY_SCOPE,
    ...
    for row in normalized:
        ...
        if any(row[key] != value for key, value in expected_cells.items()):
            _fail("I12_EVENT_MATRIX_SEMANTICS_INVALID:" + event_id)
        try:
            applicability = json.loads(row["canonical_task_applicability_json"])
        except json.JSONDecodeError as error:
            raise Cumulative1000CurrentGlobalReadinessCensusWithI12Error(
                f"{ERROR_TOKEN}:I12_EXACT5_JSON_INVALID:{event_id}"
            ) from error
        if (
            len(applicability) != 5
            ...
            != list(i12_ingestion.WARHEAD_ROLE)
            or json.loads(row["linker_atoms_json"])
            != list(i12_ingestion.LINKER_ROLE)
            or json.loads(row["scaffold_atoms_json"])
            != list(i12_ingestion.SCAFFOLD_ROLE)
        ):
            _fail("I12_EVENT_MATRIX_EXACT5_INVALID:" + event_id)
    return normalized


def _load_and_validate_i12_event_matrix_v1(
    root: Path,
) -> tuple[dict[str, str], ...]:
    payload = _read_regular_file(root / I12_EVENT_MATRIX_RELATIVE, "I12_EVENT_MATRIX")
    if len(payload) != 10005 or _sha256(payload) != _ADDITIVE_SOURCE_SPECS_V1[5][4]:
        _fail("I12_EVENT_MATRIX_BINDING_INVALID")
    source_derived = i12_ingestion.build_artifacts_v1(root)
    if source_derived[i12_ingestion.MATRIX] != payload:
        _fail("I12_EVENT_MATRIX_NOT_SOURCE_DERIVED")
    try:
        reader = csv.DictReader(io.StringIO(payload.decode("utf-8"), newline=""))
    except UnicodeDecodeError as error:
        raise Cumulative1000CurrentGlobalReadinessCensusWithI12Error(
            f"{ERROR_TOKEN}:I12_EVENT_MATRIX_NOT_UTF8"
        ) from error
    if tuple(reader.fieldnames or ()) != i12_ingestion.MATRIX_HEADER:
        _fail("I12_EVENT_MATRIX_HEADER_INVALID")
    return _validate_i12_matrix_rows_v1(tuple(dict(row) for row in reader))


def _validate_i12_reconciliation_v1(root: Path) -> generic.ReconciliationResult:
    result = i12_reconciliation.reconcile_real_completed_human_decisions_with_i12_v1(
        root
    )
    if result.review_summary != {
        ...
        _fail("I12_RECONCILIATION_SUMMARY_INVALID")
    if len(result.normalized_facts) != 99 or Counter(
        fact.training_disposition for fact in result.normalized_facts
    ) != Counter({generic.TRAINING_INCLUDE: 31, generic.TRAINING_EXCLUDE: 68}):
        _fail("I12_RECONCILIATION_NORMALIZED_FACTS_INVALID")
    rows = [
        row
        for row in result.reconciled_rows
        if row["raw_review_unit_id"] == I12_REVIEW_UNIT_ID_V1
    ]
    if (
        len(rows) != 4
        or {row["canonical_event_id"] for row in rows}
        != set(I12_EXACT4_EVENT_IDS_V1)
        or any(
            row["current_review_status"] != generic.COMPLETED_HUMAN_POSITIVE
            for row in rows
        )
    ):
        _fail("I12_RECONCILIATION_EXACT4_INVALID")
    facts = [
        fact
        for fact in result.normalized_facts
        if fact.canonical_event_id in set(I12_EXACT4_EVENT_IDS_V1)
    ]
    if len(facts) != 4 or any(
        ...
        _fail("I12_RECONCILIATION_EXACT4_SEMANTICS_INVALID")
    return result


def _assert_predecessor_i12_state_v1(
    computation: base.Cumulative1000CurrentGlobalReadinessComputationV1,
) -> None:
    i12_set = set(I12_EXACT4_EVENT_IDS_V1)
    rows = [
        row for row in computation.rows if row["canonical_event_id"] in i12_set
    ]
    if len(rows) != 4 or tuple(
        int(row["scaleup_rank"]) for row in rows
    ) != I12_EXACT4_RANKS_V1:
        _fail("PREDECESSOR_I12_EXACT4_IDENTITY_INVALID")
    expected = {
        "current_global_status": generic.CURRENTLY_UNREVIEWED,
        "priority_review_in_scope": "true",
        "review_unit_id": I12_REVIEW_UNIT_ID_V1,
    ...
    if any(any(row[key] != value for key, value in expected.items()) for row in rows):
        _fail("PREDECESSOR_I12_STATE_INVALID")
    if any(
        any(
            row[field] != value
            for field, value in _EXPECTED_I12_STRUCTURAL_CELLS_V1.items()
        )
        for row in rows
    ):
        _fail("PREDECESSOR_I12_STRUCTURAL_COVERAGE_INVALID")
    ...
    if chemistry[base.CHEMISTRY_POSITIVE] != 112:
        _fail("PREDECESSOR_POSITIVE_COUNT_NOT_112")
    if relevance[base.TASK_RELEVANT] != 113:
        _fail("PREDECESSOR_TASK_RELEVANT_COUNT_NOT_113")
...
def _overlay_i12_exact4_v1(
    predecessor_rows: Sequence[Mapping[str, str]],
    matrix_rows: Sequence[Mapping[str, str]],
) -> tuple[dict[str, str], ...]:
    validated_matrix = _validate_i12_matrix_rows_v1(matrix_rows)
    matrix_by_event = {
    ...
    for row in rows:
        ...
        if (
            ...
            or row["ligand_component_id"] != "I12"
            or row["review_unit_id"] != I12_REVIEW_UNIT_ID_V1
        ):
            _fail("I12_MATRIX_PREDECESSOR_IDENTITY_MISMATCH:" + event_id)
        row.update(
            {
                "current_global_status": generic.COMPLETED_HUMAN_POSITIVE,
                "current_review_status": generic.COMPLETED_HUMAN_POSITIVE,
                "human_review_completed": "true",
                "human_review_authority_source": I12_HUMAN_DECISION_SOURCE,
                "chemistry_disposition": base.CHEMISTRY_POSITIVE,
                "chemistry_authority_source": I12_EVENT_MATRIX_SOURCE,
                "positive_authority_source": I12_EVENT_MATRIX_SOURCE,
                "task_relevance_disposition": base.TASK_RELEVANT,
                "task_relevance_authority_source": I12_EVENT_MATRIX_SOURCE,
                ...
                "structurally_applicable_task_ids_json": I12_DIRECT_TASK_IDS_CELL_V1,
                "training_use_include": "true",
...
def _top_pending_review_units_v1(
    ...
    first = top[0]
    if not (
        first["review_unit_id"] == "COVAPIE_BULK_REVIEW_UNIT_80FE8023FD901B01"
        and first["raw_priority_rank"] == 18
        and first["event_count"] == 4
        and first["ligand_component_ids"] == ["1N0"]
        and first["pdb_ids"] == ["4JWS", "4JWU", "4JX1"]
...
def _build_summary_v1(
    ...
    source_composition["I12"] = sum(
        row["positive_authority_source"] == I12_EVENT_MATRIX_SOURCE for row in rows
    )
    summary["chemistry"] = {
    ...
    summary["reactive_pair"].update(
        {
            ...
            "i12_sample_authority_contribution_count": 4,
            "i12_model_bound_target_contribution_count": 0,
    ...
    summary["blockers"] = {
        ...
        "missing_tensor_integration": {
            ...
            "missing_source_composition": {
                **summary["blockers"]["missing_tensor_integration"][
                ...
                "I12": sum(
                    row["positive_authority_source"] == I12_EVENT_MATRIX_SOURCE
                    for row in missing_tensor_rows
                ),
            },
        },
        ...
        "feature_semantics_pending": {"within_positive_116": len(positive_rows)},
    }
    summary["top_pending_review_units_by_event_yield"] = top_pending
    summary["authority_boundary"].update(
        {
            "next_priority_review_unit": "COVAPIE_BULK_REVIEW_UNIT_80FE8023FD901B01",
            "next_priority_review_ligand": "1N0",
            "next_priority_review_event_count": 4,
            "next_priority_review_current_pending_rank": 1,
            "next_priority_review_raw_priority_rank": 18,
            "next_review_started": False,
            "I12_REVIEW_COMPLETED": True,
            ...
            "I12_CENSUS_SOURCE_BINDING_V2_CLEAN_FROM_BIRTH": True,
            "separate_I12_census_V2_successor_required": False,
            "QUEUE_REFRESH": False,
            "READY_FOR_TRAINING": False,
    ...
    summary["authority_boundary"].pop("I12_REVIEW_STARTED", None)
    return summary
...
def _verify_predecessor_semantic_bindings_v1(
    ...
    if len(bindings) != 108:
        _fail("PREDECESSOR_SEMANTIC_BINDING_COUNT_NOT_108")
    if (
...
def compute_covapie_cumulative1000_current_global_readiness_census_with_i12_v1(
    repo_root: Path,
) -> base.Cumulative1000CurrentGlobalReadinessComputationV1:
    """Compute the exact additive I12 refresh entirely from frozen sources."""
    ...
    _assert_predecessor_i12_state_v1(frozen)
    _verify_predecessor_semantic_bindings_v1(frozen.semantic_source_bindings)
    reconciliation = _validate_i12_reconciliation_v1(root)
    matrix_rows = _load_and_validate_i12_event_matrix_v1(root)
    rows = _overlay_i12_exact4_v1(frozen.rows, matrix_rows)
...
def validate_covapie_cumulative1000_current_global_readiness_census_with_i12_v1(
    computation: object,
    *,
    ...
    frozen = (
        predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_2a2_v1(
            root
        )
    ...
    i12_set = set(I12_EXACT4_EVENT_IDS_V1)
    if changed != i12_set or len(changed) != 4:
        _fail("PREDECESSOR_DELTA_NOT_EXACT_I12_EXACT4")
    if any(
        refreshed_by_event[event_id] != frozen_by_event[event_id]
        for event_id in seen - i12_set
    ):
        _fail("NON_I12_ROW_CHANGED")
    ...
    for event_id in I12_EXACT4_EVENT_IDS_V1:
        ...
        if changed_fields != _AUTHORIZED_I12_OVERLAY_FIELDS_V1:
            _fail("I12_CHANGED_FIELD_SET_INVALID:" + event_id)
        if any(
            before[field] != after[field] for field in _STRUCTURAL_IDENTITY_FIELDS_V1
        ):
            _fail("I12_STRUCTURAL_EVIDENCE_CHANGED:" + event_id)
        if any(after[field] != value for field, value in expected_i12.items()):
            _fail("I12_REFRESHED_SEMANTICS_INVALID:" + event_id)

    previous_sets = _sets_for_algebra_v1(frozen.rows)
    current_sets = _sets_for_algebra_v1(rows)
    if previous_sets["chemistry_positive"] & i12_set:
        _fail("I12_PREDECESSOR_POSITIVE_INTERSECTION_NOT_EMPTY")
    if not (
        current_sets["chemistry_positive"]
        == previous_sets["chemistry_positive"] | i12_set
        and current_sets["chemistry_unresolved"]
        == previous_sets["chemistry_unresolved"] - i12_set
        and current_sets["task_relevant"] == previous_sets["task_relevant"] | i12_set
        and current_sets["task_unresolved"]
        == previous_sets["task_unresolved"] - i12_set
        and current_sets["training_include"]
        == previous_sets["training_include"] | i12_set
        and current_sets["training_unresolved"]
        == previous_sets["training_unresolved"] - i12_set
        ...
        == previous_sets["future_candidate"] | i12_set
        ...
        _fail("I12_EXACT_SET_ALGEBRA_INVALID")
    ...
    for binding in bindings:
        ...
        if binding.get("artifact_role") in additive_roles:
    ...
    if bindings != expected_bindings or len(bindings) != 114:
...
def build_covapie_cumulative1000_current_global_readiness_artifacts_with_i12_v1(
    repo_root: Path,
) -> dict[str, bytes]:
    ...
    root = repo_root.resolve()
    computation = compute_covapie_cumulative1000_current_global_readiness_census_with_i12_v1(
        root
    )
...
def materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_i12_v1(
    repo_root: Path,
    ...
    artifacts = build_covapie_cumulative1000_current_global_readiness_artifacts_with_i12_v1(
        root
    )
    try:
