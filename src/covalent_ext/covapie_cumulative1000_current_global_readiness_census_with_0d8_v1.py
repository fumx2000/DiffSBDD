"""Additive cumulative1000 readiness census successor for published 0D8 Exact4.

This metadata-only owner consumes the published with-LCY census, the published
0D8 ingestion matrix, and the published with-0D8 generic reconciliation.  It
deep-copies the frozen 1000 rows and overlays only the four exact 0D8 event
...
from . import covapie_completed_human_decision_reconciliation_with_0d8_v1 as reconciliation_owner
from . import covapie_cumulative1000_current_global_readiness_census_with_lcy_v1 as predecessor
...
SCHEMA_VERSION = "covapie_cumulative1000_current_global_readiness_census_with_0d8_v1"
STAGE = "COVAPIE_CUMULATIVE1000_CURRENT_GLOBAL_READINESS_CENSUS_WITH_0D8_V1"
ERROR_TOKEN = "COVAPIE_CUMULATIVE1000_CURRENT_GLOBAL_READINESS_CENSUS_WITH_0D8_V1_ERROR"

OUTPUT_DIRECTORY_RELATIVE = Path(
    "data/derived/covalent_small/"
    "covapie_cumulative1000_current_global_readiness_census_with_0d8_v1"
)
CENSUS_FILE = "covapie_cumulative1000_current_global_readiness_census_with_0d8_v1.csv"
SUMMARY_FILE = "covapie_cumulative1000_current_global_readiness_summary_with_0d8_v1.json"
MANIFEST_FILE = "covapie_cumulative1000_current_global_readiness_manifest_with_0d8_v1.json"
PRODUCTION_RELATIVE = Path(
    "src/covalent_ext/covapie_cumulative1000_current_global_readiness_census_with_0d8_v1.py"
)
CHECKER_RELATIVE = Path(
    "scripts/check_covapie_cumulative1000_current_global_readiness_census_with_0d8_v1.py"
)
TEST_RELATIVE = Path(
    "tests/test_covapie_cumulative1000_current_global_readiness_census_with_0d8_v1.py"
)
GUIDE_RELATIVE = Path(
    "docs/covapie_cumulative1000_current_global_readiness_census_with_0d8_v1_guide.md"
)
EXACT7_PATHS_V1 = (
...
ZERO_D8_EXACT4_EVENT_IDS_V1 = ingestion.EXPECTED_EVENT_IDS
ZERO_D8_EXACT4_RANKS_V1 = ingestion.EXPECTED_RANKS
ZERO_D8_REVIEW_UNIT_ID_V1 = ingestion.EXPECTED_REVIEW_UNIT_ID
ZERO_D8_HUMAN_DECISION_SOURCE = ingestion.FORMAL_DECISION_RELATIVE.as_posix()
ZERO_D8_EVENT_MATRIX_RELATIVE = ingestion.OUTPUT_ROOT_RELATIVE / ingestion.MATRIX
ZERO_D8_EVENT_MATRIX_SOURCE = ZERO_D8_EVENT_MATRIX_RELATIVE.as_posix()
...
ZERO_D8_RECONCILIATION_OWNER_RELATIVE = reconciliation_owner.SOURCE_RELATIVE
ZERO_D8_INGESTION_OWNER_RELATIVE = ingestion.SOURCE_RELATIVE
...
NEXT_PENDING_REVIEW_UNIT_ID_V1 = "COVAPIE_BULK_REVIEW_UNIT_C4EFE734A5B0CF57"
NEXT_PENDING_EVENT_IDS_V1 = (
    "COVAPIE_CYS_SG_EVENT_V1:4Z16:A:CYS:909-:SG:E:4LH:CAP",
    "COVAPIE_CYS_SG_EVENT_V1:4Z16:B:CYS:909-:SG:F:4LH:CAP",
    "COVAPIE_CYS_SG_EVENT_V1:4Z16:C:CYS:909-:SG:G:4LH:CAP",
    "COVAPIE_CYS_SG_EVENT_V1:4Z16:D:CYS:909-:SG:H:4LH:CAP",
)

_AUTHORIZED_0D8_OVERLAY_FIELDS_V1 = frozenset(
    {
...
_AUTHORIZED_BUT_UNCHANGED_0D8_FIELDS_V1 = frozenset(
    {"future_training_admission_candidate", "human_training_excluded", "training_use_include"}
)
_ACTUAL_CHANGED_0D8_FIELDS_V1 = (
    _AUTHORIZED_0D8_OVERLAY_FIELDS_V1 - _AUTHORIZED_BUT_UNCHANGED_0D8_FIELDS_V1
)

_EXPECTED_GLOBAL_STATUS_COUNTS_V1 = dict(predecessor._EXPECTED_GLOBAL_STATUS_COUNTS_V1)
_EXPECTED_GLOBAL_STATUS_COUNTS_V1[generic.CURRENTLY_UNREVIEWED] = 183
...
_EXPECTED_REFRESHED_CENSUS_SHA256_V1: str | None = (
    "dd7cb0e923dcfdfe464b9ffc4cf0b17c569fa8c3ca33ac23fbda7103dbe9d273"
)
_EXPECTED_REFRESHED_SUMMARY_SHA256_V1: str | None = (
    "479528564feb0ab67685408aab2e404162d48474331492688d986fae0bf2a4bc"
)
_EXPECTED_REFRESHED_SEMANTIC_SOURCE_BINDINGS_SHA256_V1: str | None = (
    "f421e36b39fa90c91798ac53b5ccf3fe967334fe78ea89f52e54b946b4d47620"
)

_SHA_PATTERN = re.compile(r"^[0-9a-f]{64}$")
_ADDITIVE_SOURCE_SPECS_V1 = (
    (
        "PREDECESSOR_WITH_LCY_CENSUS_OWNER", PREDECESSOR_OWNER_RELATIVE,
        "repository_relative", 72462,
        "68590ac83ba11798802bf339aedefc0df5823afba1432a5150790be741f785b0", False,
    ),
    (
        "PREDECESSOR_WITH_LCY_MATERIALIZED_CENSUS", PREDECESSOR_CENSUS_RELATIVE,
        "repository_relative", 545586,
        "a393fc8e2419d354f73863b389a64a12874ec500282b61986bfefe51f10b12ce", False,
    ),
    (
        "PREDECESSOR_WITH_LCY_MATERIALIZED_SUMMARY", PREDECESSOR_SUMMARY_RELATIVE,
        "repository_relative", 20179,
        "edd88fce900fd8f77f1b1647b39b6a2adb54e184361c0edc5d1111d5346e7ada", False,
    ),
    (
        "0D8_RECONCILIATION_OWNER", ZERO_D8_RECONCILIATION_OWNER_RELATIVE,
        "repository_relative", 28275,
        "a5f804db8f8fedcfed7e8035e15ea496179ba9c347985c571ca49ac05b38a8d1", False,
    ),
    (
        "0D8_INGESTION_OWNER", ZERO_D8_INGESTION_OWNER_RELATIVE,
        "repository_relative", 106540,
        "1be00e2d03d7eb709fbe3ba11c577bd48308a77c03c795d77226da50599b2579", False,
    ),
    (
        "0D8_EVENT_TASK_LABEL_AVAILABILITY", ZERO_D8_EVENT_MATRIX_RELATIVE,
        "repository_relative", 10290,
        "18fdc265115fcdf3b195160866f715c12bc3514e828d62ce40518799f9496835", False,
    ),
)
_PREDECESSOR_MANIFEST_SPEC_V1 = (
    70882,
    "b6e87c05076a1cc08345025afb25f44b6453b62d77e0ec1ca8b4380b9255fdd7",
    False,
)


class Cumulative1000CurrentGlobalReadinessCensusWith0D8Error(ValueError):
    """Raised unless the 0D8 successor is exactly source-derived."""


def _fail(reason: str) -> NoReturn:
    raise Cumulative1000CurrentGlobalReadinessCensusWith0D8Error(f"{ERROR_TOKEN}:{reason}")
...
def _read_regular_file(path: Path, label: str) -> bytes:
    ...
    try:
        ...
        raise Cumulative1000CurrentGlobalReadinessCensusWith0D8Error(
...
def _verify_additive_sources(root: Path) -> tuple[dict[str, object], ...]:
    ...
    _verify_bound(
        root, "PREDECESSOR_WITH_LCY_MANIFEST", PREDECESSOR_MANIFEST_RELATIVE,
...
def _verify_predecessor_bindings(
    ...
    manifest = json.loads(
        _read_regular_file(root / PREDECESSOR_MANIFEST_RELATIVE, "PREDECESSOR_WITH_LCY_MANIFEST")
    )
    ...
    if (
        ...
        or len(manifested) != 156
        or digest != "1382e6a7bb4b3a01496c17afba6ccb27bff843ff814911c17c04b8119bf57f14"
    ):
        _fail("PREDECESSOR_SEMANTIC_BINDINGS_NOT_MANIFEST_DERIVED_EXACT156")


def _validate_0d8_matrix_rows_v1(
    ...
    if (
        ...
        or tuple(row["canonical_event_id"] for row in normalized) != ZERO_D8_EXACT4_EVENT_IDS_V1
        or tuple(int(row["scaleup_rank"]) for row in normalized) != ZERO_D8_EXACT4_RANKS_V1
        or len({row["canonical_event_id"] for row in normalized}) != 4
    ):
        _fail("0D8_EVENT_MATRIX_IDENTITY_NOT_EXACT4")
    ...
    if actual_contexts != expected_contexts:
        _fail("0D8_EVENT_MATRIX_CONTEXT_DRIFT")
    expected_cells = {
        "review_unit_id": ZERO_D8_REVIEW_UNIT_ID_V1,
        "pdb_id": "4V37",
        "model_number": "1",
        ...
        "warhead_atoms_json": "[\"C8\",\"OH\"]",
        "linker_atoms_json": "[]",
        "scaffold_atoms_json": "[\"C7\",\"CA3\",\"N3\"]",
    ...
    for row in normalized:
        ...
        if any(row[key] != value for key, value in expected_cells.items()):
            _fail("0D8_EVENT_MATRIX_SEMANTICS_INVALID:" + event_id)
        try:
            applicability = json.loads(row["canonical_task_applicability_json"])
        except json.JSONDecodeError as error:
            raise Cumulative1000CurrentGlobalReadinessCensusWith0D8Error(
                f"{ERROR_TOKEN}:0D8_MATRIX_JSON_INVALID:{event_id}"
            ) from error
        if (
            len(applicability) != 5
            ...
            _fail("0D8_EVENT_MATRIX_EXACT5_BOUNDARY_INVALID:" + event_id)
    return normalized


def _load_and_validate_0d8_event_matrix_v1(root: Path) -> tuple[dict[str, str], ...]:
    payload = _read_regular_file(root / ZERO_D8_EVENT_MATRIX_RELATIVE, "0D8_EVENT_MATRIX")
    if len(payload) != 10290 or _sha256(payload) != (
        "18fdc265115fcdf3b195160866f715c12bc3514e828d62ce40518799f9496835"
    ):
        _fail("0D8_EVENT_MATRIX_BINDING_INVALID")
    try:
        reader = csv.DictReader(io.StringIO(payload.decode("utf-8"), newline=""))
    except UnicodeDecodeError as error:
        raise Cumulative1000CurrentGlobalReadinessCensusWith0D8Error(
            f"{ERROR_TOKEN}:0D8_EVENT_MATRIX_NOT_UTF8"
        ) from error
    if tuple(reader.fieldnames or ()) != ingestion.MATRIX_HEADER:
        _fail("0D8_EVENT_MATRIX_HEADER_INVALID")
    return _validate_0d8_matrix_rows_v1(tuple(dict(row) for row in reader))


def _validate_0d8_reconciliation_v1(root: Path) -> generic.ReconciliationResult:
    result = reconciliation_owner.reconcile_real_completed_human_decisions_with_0d8_v1(root)
    expected_summary = {
    ...
    if (
        ...
        or len(result.source_bindings) != 22
        or len(result.normalized_facts) != 131
        or len({binding.stable_identity for binding in result.source_bindings}) != 22
    ):
        _fail("0D8_RECONCILIATION_EXACT22_131_INVALID")
    target = set(ZERO_D8_EXACT4_EVENT_IDS_V1)
    facts = [fact for fact in result.normalized_facts if fact.canonical_event_id in target]
    if len(facts) != 4 or any(
        fact.review_unit_id != ZERO_D8_REVIEW_UNIT_ID_V1
        ...
        or fact.source_binding_path != ZERO_D8_HUMAN_DECISION_SOURCE
        for fact in facts
    ):
        _fail("0D8_RECONCILIATION_EXACT4_ORTHOGONALITY_INVALID")
    ...
    if len(target_rows) != 4 or any(
        ...
        _fail("0D8_RECONCILIATION_TARGET_STATUS_INVALID")
    return result


def _assert_predecessor_0d8_state_v1(
    ...
    if (
        ...
        _fail("PREDECESSOR_WITH_LCY_CENSUS_IDENTITY_INVALID")
    target = set(ZERO_D8_EXACT4_EVENT_IDS_V1)
    ...
    if (
        len(rows) != 4
        or tuple(int(row["scaleup_rank"]) for row in rows) != ZERO_D8_EXACT4_RANKS_V1
        or any(any(row[key] != value for key, value in expected.items()) for row in rows)
    ):
        _fail("PREDECESSOR_0D8_EXACT4_STATE_INVALID")


def _overlay_0d8_exact4_v1(
    predecessor_rows: Sequence[Mapping[str, str]],
    matrix_rows: Sequence[Mapping[str, str]],
    reconciliation: generic.ReconciliationResult,
) -> tuple[dict[str, str], ...]:
    matrix_by_event = {
        row["canonical_event_id"]: row for row in _validate_0d8_matrix_rows_v1(matrix_rows)
    }
    facts = {
    ...
    if set(facts) != set(ZERO_D8_EXACT4_EVENT_IDS_V1):
        _fail("0D8_RECONCILIATION_MATRIX_IDENTITY_MISMATCH")
    ...
    for row in rows:
        ...
        if (
            ...
            or row["review_unit_id"] != ZERO_D8_REVIEW_UNIT_ID_V1
        ):
            _fail("0D8_MATRIX_PREDECESSOR_IDENTITY_MISMATCH:" + event_id)
        row.update(
            {
                ...
                "chemistry_authority_source": ZERO_D8_EVENT_MATRIX_SOURCE,
                "positive_authority_source": ZERO_D8_EVENT_MATRIX_SOURCE,
                "task_relevance_disposition": fact.task_relevance_disposition,
                "task_relevance_authority_source": ZERO_D8_EVENT_MATRIX_SOURCE,
...
def _top_pending_review_units_v1(
    ...
    if len(candidates) != 105 or any(
        unit == ZERO_D8_REVIEW_UNIT_ID_V1 for _n, _p, unit, _row, _s in candidates
    ):
...
def _build_summary_v1(
    ...
    summary["refresh_delta"] = {
        "frozen_predecessor_positive_count": 140,
        "0D8_exact4_delta_count": 4,
    ...
    source_composition["0D8"] = sum(
        row["positive_authority_source"] == ZERO_D8_EVENT_MATRIX_SOURCE for row in rows
    )
    summary["chemistry"] = {
    ...
    summary["reactive_pair"].update(
        {
            ...
            "0d8_sample_authority_contribution_count": sum(
                row["positive_authority_source"] == ZERO_D8_EVENT_MATRIX_SOURCE
                and row["reactive_pair_sample_authoritative"] == "true" for row in rows
            ),
            "0d8_training_target_contribution_count": sum(
                row["positive_authority_source"] == ZERO_D8_EVENT_MATRIX_SOURCE
    ...
    summary["training_stage"]["future_candidate_source_composition"].pop("0D8", None)
    missing_source_composition = dict(
        summary["blockers"]["missing_tensor_integration"]["missing_source_composition"]
    )
    missing_source_composition["0D8"] = sum(
        row["positive_authority_source"] == ZERO_D8_EVENT_MATRIX_SOURCE
    ...
    boundary.update(
        {
            ...
            "next_priority_review_ligand": "4LH",
            ...
            "0D8_CENSUS_SOURCE_BINDING_V2_CLEAN_FROM_BIRTH": True,
        }
    )
    return summary
...
def compute_covapie_cumulative1000_current_global_readiness_census_with_0d8_v1(
    repo_root: Path,
) -> base.Cumulative1000CurrentGlobalReadinessComputationV1:
    """Compute the additive 0D8 successor entirely from frozen published sources."""

    root = repo_root.resolve()
    additive_bindings = _verify_additive_sources(root)
    frozen = predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_lcy_v1(root)
    _assert_predecessor_0d8_state_v1(frozen, root)
    _verify_predecessor_bindings(root, frozen.semantic_source_bindings)
    reconciliation = _validate_0d8_reconciliation_v1(root)
    matrix_rows = _load_and_validate_0d8_event_matrix_v1(root)
    rows = _overlay_0d8_exact4_v1(frozen.rows, matrix_rows, reconciliation)
...
def validate_covapie_cumulative1000_current_global_readiness_census_with_0d8_v1(
    computation: object,
    *,
    ...
    """Fail closed on every 0D8 identity, authority, mask, and training boundary."""
    ...
    if (
        len(_AUTHORIZED_0D8_OVERLAY_FIELDS_V1) != 19
        or len(_AUTHORIZED_BUT_UNCHANGED_0D8_FIELDS_V1) != 3
        or len(_ACTUAL_CHANGED_0D8_FIELDS_V1) != 16
        or _AUTHORIZED_BUT_UNCHANGED_0D8_FIELDS_V1 - _AUTHORIZED_0D8_OVERLAY_FIELDS_V1
    ):
        _fail("AUTHORIZED_0D8_OVERLAY_CONTRACT_INVALID")

    root = Path(__file__).resolve().parents[2]
    frozen = predecessor_computation or predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_lcy_v1(root)
    reconciliation = reconciliation_result or _validate_0d8_reconciliation_v1(root)
    validated_matrix = (
        _validate_0d8_matrix_rows_v1(matrix_rows)
        if matrix_rows is not None else _load_and_validate_0d8_event_matrix_v1(root)
    )
    _assert_predecessor_0d8_state_v1(frozen, root)
    _verify_predecessor_bindings(root, frozen.semantic_source_bindings)
    ...
    exact4 = set(ZERO_D8_EXACT4_EVENT_IDS_V1)
    ...
    for event_id in ZERO_D8_EXACT4_EVENT_IDS_V1:
        changed_fields = {
            field for field in CENSUS_COLUMNS_V1
            if before[event_id][field] != after[event_id][field]
        }
        unchanged_authorized = {
            field for field in _AUTHORIZED_0D8_OVERLAY_FIELDS_V1
            if before[event_id][field] == after[event_id][field]
        }
        if changed_fields != _ACTUAL_CHANGED_0D8_FIELDS_V1:
            _fail("0D8_CHANGED_FIELD_SET_NOT_EXACT16:" + event_id)
        if unchanged_authorized != _AUTHORIZED_BUT_UNCHANGED_0D8_FIELDS_V1:
            _fail("0D8_AUTHORIZED_BUT_UNCHANGED_SET_NOT_EXACT3:" + event_id)
        if any(after[event_id][field] != value for field, value in expected_after.items()):
            _fail("0D8_REFRESHED_SEMANTICS_INVALID:" + event_id)
    ...
    expected_orthogonal = (
        set(GVE_EXACT4_EVENT_IDS_V1) | set(LCY_EXACT4_EVENT_IDS_V1) | exact4
    )
    ...
    for row in rows:
        ...
        if event_id in exact4:
            ...
            if (
                ...
                _fail("0D8_MATRIX_CENSUS_SEMANTIC_MISMATCH:" + event_id)
    ...
    if Counter(row["training_use_disposition"] for row in rows) != Counter(
        {
            "INCLUDE": 60, "EXCLUDE_FROM_TRAINING_ONLY": 72,
            "NOT_APPLICABLE": 102, "UNRESOLVED": 766,
        }
    ):
    ...
    if profiles != Counter({base.DIRECT_PROFILE: 84, base.STRICT_PROFILE: 52}):
    ...
    expected_blockers = {
        "pair_authority_absent": {"all_1000": 856, "within_chemistry_positive": 0},
        "role_authority_absent": {"all_1000": 864, "within_chemistry_positive": 8},
        ...
        "feature_semantics_pending": {"within_chemistry_positive": 144},
    }
    ...
    if (
        blockers["chemistry_unresolved"] != {"all_1000": 766}
        or tensor["within_chemistry_positive"] != 103
        or tensor["within_training_include"] != 31
        or tensor["missing_source_composition"].get("0D8") != 4
        or summary["chemistry"]["positive_source_composition"].get("0D8") != 4
        or "0D8" in summary["training_stage"]["future_candidate_source_composition"]
        or summary["reactive_pair"]["0d8_sample_authority_contribution_count"] != 4
        or summary["reactive_pair"]["0d8_training_target_contribution_count"] != 0
    ):
        _fail("SUMMARY_0D8_SOURCE_COMPOSITION_INVALID")
...
def _build_artifacts_from_computation_v1(
    ...
    manifest = {
        ...
        "predecessor_manifest_validation_binding": {
            "artifact_role": "PREDECESSOR_WITH_LCY_MANIFEST_VALIDATION_IDENTITY",
        ...
        "refresh_contract": {
            ...
            "actual_changed_field_count_per_0d8_row": 16,
            "semantic_source_binding_count": 162,
            "predecessor_semantic_source_binding_count": 156,
...
def build_covapie_cumulative1000_current_global_readiness_artifacts_with_0d8_v1(
    repo_root: Path,
) -> dict[str, bytes]:
    ...
    root = repo_root.resolve()
    computation = compute_covapie_cumulative1000_current_global_readiness_census_with_0d8_v1(root)
...
def materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_0d8_v1(
    repo_root: Path, output_directory: Path | None = None,
) -> dict[str, bytes]:
    """Write only the authorized Exact3 files under the exact 0D8 output root."""
    ...
    artifacts = build_covapie_cumulative1000_current_global_readiness_artifacts_with_0d8_v1(root)
