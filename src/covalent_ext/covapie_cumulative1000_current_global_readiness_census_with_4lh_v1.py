"""Additive cumulative1000 readiness census successor for published 4LH Exact4.

This metadata-only owner consumes the published with-0D8 census, the published
4LH ingestion matrix, and the published with-4LH generic reconciliation.  It
deep-copies the frozen 1000 rows and overlays only the four exact 4LH event
...
from . import covapie_completed_human_decision_reconciliation_with_4lh_v1 as reconciliation_owner
from . import covapie_cumulative1000_current_global_readiness_census_with_0d8_v1 as predecessor
from .covapie_source_binding_policy_v2 import SourceBindingPolicyV2Error, verify_bound_source_v2


__all__ = (
    "Cumulative1000CurrentGlobalReadinessCensusWith4LHError",
    "compute_covapie_cumulative1000_current_global_readiness_census_with_4lh_v1",
    "validate_covapie_cumulative1000_current_global_readiness_census_with_4lh_v1",
    "build_covapie_cumulative1000_current_global_readiness_artifacts_with_4lh_v1",
    "materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_4lh_v1",
)

SCHEMA_VERSION = "covapie_cumulative1000_current_global_readiness_census_with_4lh_v1"
STAGE = "COVAPIE_CUMULATIVE1000_CURRENT_GLOBAL_READINESS_CENSUS_WITH_4LH_V1"
ERROR_TOKEN = "COVAPIE_CUMULATIVE1000_CURRENT_GLOBAL_READINESS_CENSUS_WITH_4LH_V1_ERROR"

OUTPUT_DIRECTORY_RELATIVE = Path(
    "data/derived/covalent_small/"
    "covapie_cumulative1000_current_global_readiness_census_with_4lh_v1"
)
CENSUS_FILE = "covapie_cumulative1000_current_global_readiness_census_with_4lh_v1.csv"
SUMMARY_FILE = "covapie_cumulative1000_current_global_readiness_summary_with_4lh_v1.json"
MANIFEST_FILE = "covapie_cumulative1000_current_global_readiness_manifest_with_4lh_v1.json"
PRODUCTION_RELATIVE = Path(
    "src/covalent_ext/covapie_cumulative1000_current_global_readiness_census_with_4lh_v1.py"
)
CHECKER_RELATIVE = Path(
    "scripts/check_covapie_cumulative1000_current_global_readiness_census_with_4lh_v1.py"
)
TEST_RELATIVE = Path(
    "tests/test_covapie_cumulative1000_current_global_readiness_census_with_4lh_v1.py"
)
GUIDE_RELATIVE = Path(
    "docs/covapie_cumulative1000_current_global_readiness_census_with_4lh_v1_guide.md"
)
EXACT7_PATHS_V1 = (
...
FOUR_LH_EXACT4_EVENT_IDS_V1 = ingestion.EXPECTED_EVENT_IDS
FOUR_LH_EXACT4_RANKS_V1 = ingestion.EXPECTED_RANKS
FOUR_LH_REVIEW_UNIT_ID_V1 = ingestion.EXPECTED_REVIEW_UNIT_ID
FOUR_LH_HUMAN_DECISION_SOURCE = ingestion.FORMAL_DECISION_RELATIVE.as_posix()
FOUR_LH_EVENT_MATRIX_RELATIVE = ingestion.OUTPUT_ROOT_RELATIVE / ingestion.MATRIX
FOUR_LH_EVENT_MATRIX_SOURCE = FOUR_LH_EVENT_MATRIX_RELATIVE.as_posix()
...
FOUR_LH_RECONCILIATION_OWNER_RELATIVE = reconciliation_owner.SOURCE_RELATIVE
FOUR_LH_INGESTION_OWNER_RELATIVE = ingestion.SOURCE_RELATIVE
...
NEXT_PENDING_REVIEW_UNIT_ID_V1 = "COVAPIE_BULK_REVIEW_UNIT_C750E9F706F9E0AF"
...
_AUTHORIZED_4LH_OVERLAY_FIELDS_V1 = frozenset(
    {
...
_AUTHORIZED_BUT_UNCHANGED_4LH_FIELDS_V1 = frozenset(
    {"human_training_excluded"}
)
_ACTUAL_CHANGED_4LH_FIELDS_V1 = (
    _AUTHORIZED_4LH_OVERLAY_FIELDS_V1 - _AUTHORIZED_BUT_UNCHANGED_4LH_FIELDS_V1
)

_EXPECTED_GLOBAL_STATUS_COUNTS_V1 = dict(predecessor._EXPECTED_GLOBAL_STATUS_COUNTS_V1)
_EXPECTED_GLOBAL_STATUS_COUNTS_V1[generic.CURRENTLY_UNREVIEWED] = 179
_EXPECTED_GLOBAL_STATUS_COUNTS_V1[generic.COMPLETED_HUMAN_POSITIVE] = 119
_EXPECTED_GLOBAL_STATUS_COUNTS_V1[generic.COMPLETED_HUMAN_NEGATIVE] = 70
_EXPECTED_BOOLEAN_COUNTS_V1 = dict(predecessor._EXPECTED_BOOLEAN_COUNTS_V1)
_EXPECTED_BOOLEAN_COUNTS_V1.update(
    {
        "reactive_pair_sample_authoritative": 148,
        "role_partition_sample_authoritative": 140,
        "canonical_mask_structural_labels_available": 140,
...
_EXPECTED_REFRESHED_CENSUS_SHA256_V1: str | None = (
    "a8166f4c000dbeaa8c5672900fa1838748ad32c0b2810c1a132616d5c675e1aa"
)
_EXPECTED_REFRESHED_SUMMARY_SHA256_V1: str | None = (
    "7ad61675927467d4f1b5ab7e54a42649815b98bb2835201397f30432fcf1716b"
)
_EXPECTED_REFRESHED_SEMANTIC_SOURCE_BINDINGS_SHA256_V1: str | None = (
    "1004bb865d05dad16f548cca45855fe17ab92c0ee52e867e2a48dc8d5cf754dd"
)

_SHA_PATTERN = re.compile(r"^[0-9a-f]{64}$")
_ADDITIVE_SOURCE_SPECS_V1 = (
    (
        "PREDECESSOR_WITH_0D8_CENSUS_OWNER", PREDECESSOR_OWNER_RELATIVE,
        "repository_relative", 70158,
        "5529b963bd2792eb66bd9073a67fd8c0c7c08f1bb29a64faf5459e4c52b2f71c", False,
    ),
    (
        "PREDECESSOR_WITH_0D8_MATERIALIZED_CENSUS", PREDECESSOR_CENSUS_RELATIVE,
        "repository_relative", 547666,
        "dd7cb0e923dcfdfe464b9ffc4cf0b17c569fa8c3ca33ac23fbda7103dbe9d273", False,
    ),
    (
        "PREDECESSOR_WITH_0D8_MATERIALIZED_SUMMARY", PREDECESSOR_SUMMARY_RELATIVE,
        "repository_relative", 20778,
        "479528564feb0ab67685408aab2e404162d48474331492688d986fae0bf2a4bc", False,
    ),
    (
        "4LH_RECONCILIATION_OWNER", FOUR_LH_RECONCILIATION_OWNER_RELATIVE,
        "repository_relative", 30628,
        "911396f60c985d6c32e1dcbff025a89d5bbcf07b2fed11a99e774272f7128391", False,
    ),
    (
        "4LH_INGESTION_OWNER", FOUR_LH_INGESTION_OWNER_RELATIVE,
        "repository_relative", 72317,
        "d0baeb39806d9da79989aea90f3d96419ea9f794c8bdef55202405086560f425", False,
    ),
    (
        "4LH_EVENT_TASK_LABEL_AVAILABILITY", FOUR_LH_EVENT_MATRIX_RELATIVE,
        "repository_relative", 10647,
        "5f72b7f78d5a2859b340c5592fb26b95c7e7a1a4ba3e6ba4174159adee209284", False,
    ),
)
_PREDECESSOR_MANIFEST_SPEC_V1 = (
    73718,
    "36910e9779603b5545953ee503ed8573dbc1bb55926d095e0d5d4f84b9075ca6",
    False,
)


class Cumulative1000CurrentGlobalReadinessCensusWith4LHError(ValueError):
    """Raised unless the 4LH successor is exactly source-derived."""


def _fail(reason: str) -> NoReturn:
    raise Cumulative1000CurrentGlobalReadinessCensusWith4LHError(f"{ERROR_TOKEN}:{reason}")
...
def _read_regular_file(path: Path, label: str) -> bytes:
    ...
    try:
        ...
        raise Cumulative1000CurrentGlobalReadinessCensusWith4LHError(
...
def _verify_additive_sources(root: Path) -> tuple[dict[str, object], ...]:
    ...
    _verify_bound(
        root, "PREDECESSOR_WITH_0D8_MANIFEST", PREDECESSOR_MANIFEST_RELATIVE,
...
def _verify_predecessor_bindings(
    ...
    manifest = json.loads(
        _read_regular_file(root / PREDECESSOR_MANIFEST_RELATIVE, "PREDECESSOR_WITH_0D8_MANIFEST")
    )
    ...
    if (
        ...
        or len(manifested) != 162
        or digest != "f421e36b39fa90c91798ac53b5ccf3fe967334fe78ea89f52e54b946b4d47620"
    ):
        _fail("PREDECESSOR_SEMANTIC_BINDINGS_NOT_MANIFEST_DERIVED_EXACT162")


def _validate_4lh_matrix_rows_v1(
    ...
    if (
        ...
        or tuple(row["canonical_event_id"] for row in normalized) != FOUR_LH_EXACT4_EVENT_IDS_V1
        or tuple(int(row["scaleup_rank"]) for row in normalized) != FOUR_LH_EXACT4_RANKS_V1
        or len({row["canonical_event_id"] for row in normalized}) != 4
    ):
        _fail("4LH_EVENT_MATRIX_IDENTITY_NOT_EXACT4")
    ...
    if actual_contexts != expected_contexts:
        _fail("4LH_EVENT_MATRIX_CONTEXT_DRIFT")
    expected_cells = {
        "review_unit_id": FOUR_LH_REVIEW_UNIT_ID_V1,
        "pdb_id": "4Z16",
        "model_number": "1",
        "cys_residue_id": "CYS:909-",
        "ligand_component_id": "4LH",
        ...
        "warhead_atoms_json": "[\"CAP\",\"CAQ\",\"CBE\",\"OAE\",\"NBA\"]",
        "linker_atoms_json": "[]",
        "W_L_S_counts_json": "[5,0,31]",
        ...
        "supporting_PRE_source_graph_count": "1",
    ...
    for row in normalized:
        ...
        if any(row[key] != value for key, value in expected_cells.items()):
            _fail("4LH_EVENT_MATRIX_SEMANTICS_INVALID:" + event_id)
        try:
            applicability = json.loads(row["canonical_task_applicability_json"])
            warhead_atoms = json.loads(row["warhead_atoms_json"])
            linker_atoms = json.loads(row["linker_atoms_json"])
            scaffold_atoms = json.loads(row["scaffold_atoms_json"])
        except json.JSONDecodeError as error:
            raise Cumulative1000CurrentGlobalReadinessCensusWith4LHError(
                f"{ERROR_TOKEN}:4LH_MATRIX_JSON_INVALID:{event_id}"
            ) from error
        if (
            len(applicability) != 5
            ...
            or warhead_atoms != list(ingestion.WARHEAD_ATOMS)
            or linker_atoms != []
            or scaffold_atoms != list(ingestion.SCAFFOLD_ATOMS)
            or [len(warhead_atoms), len(linker_atoms), len(scaffold_atoms)] != [5, 0, 31]
            or row["global_canonical_task_count"] != "5"
            or row["B3_present"] != "true"
            or row["sixth_task"] != "false"
        ):
            _fail("4LH_EVENT_MATRIX_EXACT5_BOUNDARY_INVALID:" + event_id)
    return normalized


def _load_and_validate_4lh_event_matrix_v1(root: Path) -> tuple[dict[str, str], ...]:
    payload = _read_regular_file(root / FOUR_LH_EVENT_MATRIX_RELATIVE, "4LH_EVENT_MATRIX")
    if len(payload) != 10647 or _sha256(payload) != (
        "5f72b7f78d5a2859b340c5592fb26b95c7e7a1a4ba3e6ba4174159adee209284"
    ):
        _fail("4LH_EVENT_MATRIX_BINDING_INVALID")
    try:
        reader = csv.DictReader(io.StringIO(payload.decode("utf-8"), newline=""))
    except UnicodeDecodeError as error:
        raise Cumulative1000CurrentGlobalReadinessCensusWith4LHError(
            f"{ERROR_TOKEN}:4LH_EVENT_MATRIX_NOT_UTF8"
        ) from error
    if tuple(reader.fieldnames or ()) != ingestion.MATRIX_HEADER:
        _fail("4LH_EVENT_MATRIX_HEADER_INVALID")
    return _validate_4lh_matrix_rows_v1(tuple(dict(row) for row in reader))


def _validate_4lh_reconciliation_v1(root: Path) -> generic.ReconciliationResult:
    result = reconciliation_owner.reconcile_real_completed_human_decisions_with_4lh_v1(root)
    expected_summary = {
    ...
    if (
        ...
        or len(result.source_bindings) != 23
        or len(result.normalized_facts) != 135
        or len({binding.stable_identity for binding in result.source_bindings}) != 23
    ):
        _fail("4LH_RECONCILIATION_EXACT23_135_INVALID")
    target = set(FOUR_LH_EXACT4_EVENT_IDS_V1)
    facts = [fact for fact in result.normalized_facts if fact.canonical_event_id in target]
    if len(facts) != 4 or any(
        fact.review_unit_id != FOUR_LH_REVIEW_UNIT_ID_V1
        ...
        or fact.source_binding_path != FOUR_LH_HUMAN_DECISION_SOURCE
        for fact in facts
    ):
        _fail("4LH_RECONCILIATION_EXACT4_FACTS_INVALID")
    ...
    if len(target_rows) != 4 or any(
        ...
        _fail("4LH_RECONCILIATION_TARGET_STATUS_INVALID")
    return result


def _assert_predecessor_4lh_state_v1(
    ...
    if (
        ...
        _fail("PREDECESSOR_WITH_0D8_CENSUS_IDENTITY_INVALID")
    target = set(FOUR_LH_EXACT4_EVENT_IDS_V1)
    ...
    if (
        len(rows) != 4
        or tuple(int(row["scaleup_rank"]) for row in rows) != FOUR_LH_EXACT4_RANKS_V1
        or any(any(row[key] != value for key, value in expected.items()) for row in rows)
    ):
        _fail("PREDECESSOR_4LH_EXACT4_STATE_INVALID")


def _overlay_4lh_exact4_v1(
    predecessor_rows: Sequence[Mapping[str, str]],
    matrix_rows: Sequence[Mapping[str, str]],
    reconciliation: generic.ReconciliationResult,
) -> tuple[dict[str, str], ...]:
    matrix_by_event = {
        row["canonical_event_id"]: row for row in _validate_4lh_matrix_rows_v1(matrix_rows)
    }
    facts = {
    ...
    if set(facts) != set(FOUR_LH_EXACT4_EVENT_IDS_V1):
        _fail("4LH_RECONCILIATION_MATRIX_IDENTITY_MISMATCH")
    ...
    for row in rows:
        ...
        if (
            ...
            or row["review_unit_id"] != FOUR_LH_REVIEW_UNIT_ID_V1
        ):
            _fail("4LH_MATRIX_PREDECESSOR_IDENTITY_MISMATCH:" + event_id)
        row.update(
            {
                ...
                "chemistry_authority_source": FOUR_LH_EVENT_MATRIX_SOURCE,
                "positive_authority_source": FOUR_LH_EVENT_MATRIX_SOURCE,
                "task_relevance_disposition": fact.task_relevance_disposition,
                "task_relevance_authority_source": FOUR_LH_EVENT_MATRIX_SOURCE,
...
def _top_pending_review_units_v1(
    ...
    if len(candidates) != 104 or any(
        unit == FOUR_LH_REVIEW_UNIT_ID_V1 for _n, _p, unit, _row, _s in candidates
    ):
...
def _build_summary_v1(
    ...
    summary["refresh_delta"] = {
        "frozen_predecessor_positive_count": 144,
        "4LH_exact4_delta_count": 4,
    ...
    source_composition["4LH"] = sum(
        row["positive_authority_source"] == FOUR_LH_EVENT_MATRIX_SOURCE for row in rows
    )
    summary["chemistry"] = {
    ...
    summary["reactive_pair"].update(
        {
            ...
            "4lh_sample_authority_contribution_count": sum(
                row["positive_authority_source"] == FOUR_LH_EVENT_MATRIX_SOURCE
                and row["reactive_pair_sample_authoritative"] == "true" for row in rows
            ),
            "4lh_training_target_contribution_count": sum(
                row["positive_authority_source"] == FOUR_LH_EVENT_MATRIX_SOURCE
    ...
    summary["training_stage"]["future_candidate_source_composition"]["4LH"] = sum(
        row["positive_authority_source"] == FOUR_LH_EVENT_MATRIX_SOURCE
    ...
    missing_source_composition["4LH"] = sum(
        row["positive_authority_source"] == FOUR_LH_EVENT_MATRIX_SOURCE
    ...
    boundary.update(
        {
            ...
            "next_priority_review_ligand": "TP2",
            ...
            "4LH_CENSUS_SOURCE_BINDING_V2_CLEAN_FROM_BIRTH": True,
        }
    )
    return summary
...
def compute_covapie_cumulative1000_current_global_readiness_census_with_4lh_v1(
    repo_root: Path,
) -> base.Cumulative1000CurrentGlobalReadinessComputationV1:
    """Compute the additive 4LH successor entirely from frozen published sources."""

    root = repo_root.resolve()
    additive_bindings = _verify_additive_sources(root)
    frozen = predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_0d8_v1(root)
    _assert_predecessor_4lh_state_v1(frozen, root)
    _verify_predecessor_bindings(root, frozen.semantic_source_bindings)
    reconciliation = _validate_4lh_reconciliation_v1(root)
    matrix_rows = _load_and_validate_4lh_event_matrix_v1(root)
    rows = _overlay_4lh_exact4_v1(frozen.rows, matrix_rows, reconciliation)
...
def validate_covapie_cumulative1000_current_global_readiness_census_with_4lh_v1(
    computation: object,
    *,
    ...
    """Fail closed on every 4LH identity, authority, mask, and training boundary."""
    ...
    if (
        len(_AUTHORIZED_4LH_OVERLAY_FIELDS_V1) != 19
        or len(_AUTHORIZED_BUT_UNCHANGED_4LH_FIELDS_V1) != 1
        or len(_ACTUAL_CHANGED_4LH_FIELDS_V1) != 18
        or _AUTHORIZED_BUT_UNCHANGED_4LH_FIELDS_V1 - _AUTHORIZED_4LH_OVERLAY_FIELDS_V1
    ):
        _fail("AUTHORIZED_4LH_OVERLAY_CONTRACT_INVALID")

    root = Path(__file__).resolve().parents[2]
    frozen = predecessor_computation or predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_0d8_v1(root)
    reconciliation = reconciliation_result or _validate_4lh_reconciliation_v1(root)
    validated_matrix = (
        _validate_4lh_matrix_rows_v1(matrix_rows)
        if matrix_rows is not None else _load_and_validate_4lh_event_matrix_v1(root)
    )
    _assert_predecessor_4lh_state_v1(frozen, root)
    _verify_predecessor_bindings(root, frozen.semantic_source_bindings)
    ...
    exact4 = set(FOUR_LH_EXACT4_EVENT_IDS_V1)
    ...
    for event_id in FOUR_LH_EXACT4_EVENT_IDS_V1:
        changed_fields = {
            field for field in CENSUS_COLUMNS_V1
            if before[event_id][field] != after[event_id][field]
        }
        unchanged_authorized = {
            field for field in _AUTHORIZED_4LH_OVERLAY_FIELDS_V1
            if before[event_id][field] == after[event_id][field]
        }
        if changed_fields != _ACTUAL_CHANGED_4LH_FIELDS_V1:
            _fail("4LH_CHANGED_FIELD_SET_NOT_EXACT18:" + event_id)
        if unchanged_authorized != _AUTHORIZED_BUT_UNCHANGED_4LH_FIELDS_V1:
            _fail("4LH_AUTHORIZED_BUT_UNCHANGED_SET_NOT_EXACT1:" + event_id)
        if any(after[event_id][field] != value for field, value in expected_after.items()):
            _fail("4LH_REFRESHED_SEMANTICS_INVALID:" + event_id)
    ...
    expected_orthogonal = (
        set(GVE_EXACT4_EVENT_IDS_V1)
        | set(LCY_EXACT4_EVENT_IDS_V1)
        | set(ZERO_D8_EXACT4_EVENT_IDS_V1)
    )
    if orthogonal != expected_orthogonal or len(orthogonal) != 12:
        _fail("TASK_NEGATIVE_CHEMISTRY_POSITIVE_POPULATION_NOT_EXACT12")
    legacy = {
        row["canonical_event_id"] for row in rows
        if (
            row["task_relevance_disposition"], row["chemistry_disposition"],
    ...
    if Counter(row["training_use_disposition"] for row in rows) != Counter(
        {
            ...
            "NOT_APPLICABLE": 102, "UNRESOLVED": 762,
        }
    ):
    ...
    if profiles != Counter({base.DIRECT_PROFILE: 88, base.STRICT_PROFILE: 52}):
    ...
    if applicability != Counter({0: 140, 1: 52, 2: 52, 3: 140, 4: 140}):
    ...
    expected_blockers = {
        "pair_authority_absent": {"all_1000": 852, "within_chemistry_positive": 0},
        "role_authority_absent": {"all_1000": 860, "within_chemistry_positive": 8},
        "human_training_exclusion": {"within_chemistry_positive": 72},
        "missing_split_authority": {
            "within_chemistry_positive": 107, "within_training_include": 39,
        },
        "missing_POST_training_authority": {
            "within_chemistry_positive": 131, "within_training_include": 47,
        },
        "missing_training_admission": {
            "within_chemistry_positive": 143, "within_training_include": 59,
        },
        "feature_semantics_pending": {"within_chemistry_positive": 148},
    }
    ...
    if (
        blockers["chemistry_unresolved"] != {"all_1000": 762}
        or tensor["within_chemistry_positive"] != 107
        or tensor["within_training_include"] != 35
        or tensor["missing_source_composition"].get("4LH") != 4
        or summary["chemistry"]["positive_source_composition"].get("4LH") != 4
        or summary["training_stage"]["future_candidate_source_composition"].get("4LH") != 4
        or summary["reactive_pair"]["4lh_sample_authority_contribution_count"] != 4
        or summary["reactive_pair"]["4lh_training_target_contribution_count"] != 0
    ):
        _fail("SUMMARY_4LH_SOURCE_COMPOSITION_INVALID")
...
def _build_artifacts_from_computation_v1(
    ...
    manifest = {
        ...
        "predecessor_manifest_validation_binding": {
            "artifact_role": "PREDECESSOR_WITH_0D8_MANIFEST_VALIDATION_IDENTITY",
        ...
        "refresh_contract": {
            ...
            "actual_changed_field_count_per_4lh_row": 18,
            "semantic_source_binding_count": 168,
            "predecessor_semantic_source_binding_count": 162,
...
def build_covapie_cumulative1000_current_global_readiness_artifacts_with_4lh_v1(
    repo_root: Path,
) -> dict[str, bytes]:
    ...
    root = repo_root.resolve()
    computation = compute_covapie_cumulative1000_current_global_readiness_census_with_4lh_v1(root)
...
def materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_4lh_v1(
    repo_root: Path, output_directory: Path | None = None,
) -> dict[str, bytes]:
    """Write only the authorized Exact3 files under the exact 4LH output root."""
    ...
    artifacts = build_covapie_cumulative1000_current_global_readiness_artifacts_with_4lh_v1(root)
