"""Design-only contract gate for the future ADMIT_006 unified adapter.
...
PROJECT = "CovaPIE"
STEP = "ADMIT_006 unified adapter contract design gate v1"
STAGE = "covapie_bulk_download_admission_admit_006_unified_adapter_contract_design_gate_v1"
EXPECTED_BASE_COMMIT = "b26a7eb2e3856bd0993ce4a797afa0d46c514a25"
EXPECTED_BASE_SUBJECT = "add CovaPIE standalone ADMIT_006 rule logic interface v1"
MANIFEST_SCHEMA_VERSION = "covapie_admit_006_unified_adapter_contract_manifest_v1"
RECOMMENDED_NEXT_STEP = "implement_covapie_unified_dispatch_runtime_with_admit_001_to_006_v1"
REPO_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_OUTPUT_ROOT = Path("data/derived/covalent_small") / STAGE

ADMISSION_RULE_ID = "ADMIT_006"
ADMISSION_RULE_NAME = "explicit_covalent_event_evidence"
ADAPTER_ID = "covapie_admit_006_unified_adapter_v1"
FORMAL_EVALUATOR_NAME = "evaluate_admit_006"
...
FUTURE_REGISTERED_RULE_ORDER = tuple(f"ADMIT_{index:03d}" for index in range(1, 7))
CURRENT_REGISTERED_RULE_ORDER = FUTURE_REGISTERED_RULE_ORDER[:5]
...
CANDIDATE_MAPPING_REASON = "ADMIT_006_CANDIDATE_RECORD_MAPPING_INVALID"
SOURCE_TYPE_REASON = "ADMIT_006_UNIFIED_ADAPTER_SOURCE_TYPE_INVALID"
SOURCE_INVARIANT_REASON = "ADMIT_006_UNIFIED_ADAPTER_SOURCE_INVARIANT_INVALID"
CONTEXT_REASONS = {
    "batch_context": "ADMIT_006_BATCH_CONTEXT_MUST_BE_NONE",
    "evaluation_context": "ADMIT_006_EVALUATION_CONTEXT_MAPPING_REQUIRED",
    "evaluation_context_key": "ADMIT_006_ALLOWED_COVALENT_EVIDENCE_CLASSES_REQUIRED",
    "download_result_context": "ADMIT_006_DOWNLOAD_RESULT_CONTEXT_MUST_BE_NONE",
    "stage_authorization_context": "ADMIT_006_STAGE_AUTHORIZATION_CONTEXT_MUST_BE_NONE",
}
EXECUTION_PRECEDENCE = (
    ...
    "formal_evaluate_admit_006_exactly_once",
...
SOURCE_PATHS = tuple(Path(value) for value in (
    ...
    "data/derived/covalent_small/covapie_bulk_download_admission_admit_006_rule_logic_interface_v1/covapie_admit_006_rule_logic_interface_manifest.json",
    "data/derived/covalent_small/covapie_bulk_download_admission_admit_006_rule_logic_interface_v1/covapie_admit_006_rule_logic_interface_contract.csv",
    "data/derived/covalent_small/covapie_bulk_download_admission_admit_006_rule_logic_interface_v1/covapie_admit_006_rule_logic_interface_truth_matrix.csv",
    "data/derived/covalent_small/covapie_bulk_download_admission_admit_006_rule_logic_interface_v1/covapie_admit_006_rule_logic_interface_issue_readiness_inventory.csv",
    ...
    "data/derived/covalent_small/covapie_bulk_download_admission_unified_dispatch_runtime_with_admit_001_to_005_v1/covapie_admit_001_to_005_runtime_manifest.json",
...
SOURCE_SHA256 = dict(zip(SOURCE_PATHS, (
    ...
    "b19c251567e2d1b56d425e6dbcdca1a77c7238c026b7786b00ade429682ab792",
    "793d754f81a54bb785e74665893651e02c5bb12d01d877d43ff3007cc2fc8b02",
...
CONTRACT_FILENAME = "covapie_admit_006_unified_adapter_contract.csv"
ROUTING_FILENAME = "covapie_admit_006_candidate_projection_and_context_routing_matrix.csv"
TRUTH_FILENAME = "covapie_admit_006_unified_result_projection_truth_matrix.csv"
SAFETY_FILENAME = "covapie_admit_006_unified_adapter_safety_audit.csv"
ISSUE_FILENAME = "covapie_admit_006_unified_adapter_issue_readiness_inventory.csv"
MANIFEST_FILENAME = "covapie_admit_006_unified_adapter_contract_manifest.json"
...
@dataclass(frozen=True)
class Admit006EvaluationDesignRecord:
...
def _validate_predecessors(snapshot: FrozenSourceSnapshot) -> dict[str, Any]:
    ...
    standalone_manifest = _json(snapshot, 1)
    contract_fields, contract_rows = _csv(snapshot, 2)
    truth_fields, truth_rows = _csv(snapshot, 3)
    issue_fields, issue_rows = _csv(snapshot, 4)
    enum_manifest = _json(snapshot, 6)
    responsibility_fields, responsibility_rows = _csv(snapshot, 7)
    result_fields, result_rows = _csv(snapshot, 8)
    routing_fields, routing_rows = _csv(snapshot, 9)
    runtime_manifest = _json(snapshot, 11)
    standalone_tree = ast.parse(_bytes(snapshot, SOURCE_PATHS[0]).decode())
    runtime_tree = ast.parse(_bytes(snapshot, SOURCE_PATHS[10]).decode())
    ...
    if coverage["status"] != "open" or "ADMIT_006" not in coverage["affected_rules"].split("|"):
    ...
    if enum_manifest.get("normative_enum_members") != list(CANONICAL_ENUM_MEMBERS):
        raise ValueError("Exact3 changed")
    if enum_manifest.get("allowed_covalent_evidence_classes") != list(ALLOWED_CLASSES):
        raise ValueError("Exact2 changed")
    ...
    if runtime_manifest.get("registered_rule_ids") != list(CURRENT_REGISTERED_RULE_ORDER):
        raise ValueError("current Exact5 manifest changed")
    if _literal_registry_keys(runtime_tree) != CURRENT_REGISTERED_RULE_ORDER:
        raise ValueError("current Exact5 registry changed")
    top_functions = {node.name for node in runtime_tree.body if isinstance(node, ast.FunctionDef)}
    if "_evaluate_registered_admit_006" in top_functions or "evaluate_all_rules" in top_functions:
    ...
    if "build_design_state" not in {n.name for n in precedent_tree.body if isinstance(n, ast.FunctionDef)}:
        raise ValueError("ADMIT_005 design precedent shape changed")
    return {"truth_rows": truth_rows, "issue_rows": issue_rows}


def _source_values(source: Admit006EvaluationDesignRecord) -> dict[str, object]:
...
def _source_invariants_valid(source: Admit006EvaluationDesignRecord) -> bool:
...
def validate_source_shape_and_invariants_for_design(source: object) -> SourceValidationDecision:
    if type(source) is not Admit006EvaluationDesignRecord:
        return SourceValidationDecision(False, "UNIFIED_ADMISSION_RULE_ADAPTER_NOT_READY", SOURCE_TYPE_REASON, False)
    if tuple(field.name for field in fields(source)) != STANDALONE_RESULT_FIELDS or not _source_invariants_valid(source):
...
def validate_source_oracle_equivalence_for_design(source: object, expected: object) -> SourceValidationDecision:
    ...
    if not prevalidated.accepted:
        return prevalidated
    if type(expected) is not Admit006EvaluationDesignRecord or not _source_invariants_valid(expected) or source != expected:
        return SourceValidationDecision(False, "UNIFIED_ADMISSION_RULE_ADAPTER_NOT_READY", SOURCE_INVARIANT_REASON, False)
    return SourceValidationDecision(True, "", "", True)


def classify_admit_006_independent_design(scalar: object, context: object) -> Admit006EvaluationDesignRecord:
    ...
    return Admit006EvaluationDesignRecord(
...
def _source_from_truth_row(row: Mapping[str, str]) -> Admit006EvaluationDesignRecord:
...
def project_exact10_to_exact13_for_design(source: Admit006EvaluationDesignRecord) -> UnifiedAdmissionEvaluationDesignRecord:
...
def _contract_rows() -> list[dict[str, str]]:
    definitions = [
        ...
        ("registration", "future_exact6_order", "|".join(FUTURE_REGISTERED_RULE_ORDER)),
        ("registration", "current_exact5_order", "|".join(CURRENT_REGISTERED_RULE_ORDER)),
        ...
        ("routing", "routed_context_value", "original_identity_no_prevalidation"),
        ...
        ("candidate", "required_field", CANDIDATE_FIELDS[0]),
        ...
        ("source", "exact_type", "type(source)_is_Admit006EvaluationResult_subclass_rejected"),
        ("source", "exact10_fields", "|".join(STANDALONE_RESULT_FIELDS)),
        ("source", "invariants", "complete_cross_field_validation_before_oracle"),
        ("oracle", "identity", INDEPENDENT_ORACLE_NAME),
        ("oracle", "equivalence", "same_original_objects_exactly_once_complete_ordered_exact10_equality"),
        ...
        ("stop", "runtime_registry", "current_exact5_unmodified_admit_006_not_registered"),
...
def _routing_rows() -> list[dict[str, str]]:
    invalid_context_result = project_exact10_to_exact13_for_design(
        classify_admit_006_independent_design(CANONICAL_ENUM_MEMBERS[0], [])
    )
    ...
    cases = [
        ...
        ("context", "context_value_identity", "required value identity sentinel", "same_object_passed", "", "", 1, 1, "accessed_after_context"),
        ...
        ("candidate", "scalar_identity", "scalar object identity sentinel", "same_object_passed", "", "", 1, 1, "value_read_once"),
    ]
...
def _truth_row(case_id: str, group: str, behavior: str, reason: str = "", *, source: object = None,
    ...
    return {"case_id": case_id, "case_group": group, "behavior": behavior,
            ...
            "source_exact10_json": _json_record(source) if type(source) is Admit006EvaluationDesignRecord else "",
            "oracle_exact10_json": _json_record(oracle) if type(oracle) is Admit006EvaluationDesignRecord else "",
...
class _SourceSubclass(Admit006EvaluationDesignRecord):
    pass
...
def _truth_rows(predecessor_rows: Sequence[Mapping[str, str]]) -> list[dict[str, str]]:
    ...
    for row in predecessor_rows:
        ...
        oracle = classify_admit_006_independent_design(scalar, context)
    ...
    valid = classify_admit_006_independent_design(CANONICAL_ENUM_MEMBERS[0], ALLOWED_CLASSES)
    ...
    for name, scalar, context in special:
        source = classify_admit_006_independent_design(scalar, context)
    ...
    rows.extend((
        _truth_row("BOUNDARY_current_exact5", "runtime_boundary", "registry_unchanged", ""),
        _truth_row("BOUNDARY_admit006_not_registered", "runtime_boundary", "not_registered", ""),
    ))
...
def _safety_rows() -> list[dict[str, str]]:
    positive = (
        ...
        "blocked_passthrough", "context_invalid_partial_canonical_projection", "current_exact5_verified",
    )
    negative = (
        "adapter_handler_implementation", "registry_modification", "admit_006_registration", "admit_007_work",
...
READINESS = {
    "admit_006_standalone_evaluator_available": True,
    "admit_006_candidate_projection_contract_frozen": True,
    "admit_006_context_routing_contract_frozen": True,
    "admit_006_missing_field_value_contract_frozen": True,
    "admit_006_unified_adapter_contract_frozen": True,
    "admit_006_source_result_validation_contract_frozen": True,
    "admit_006_source_oracle_equivalence_contract_frozen": True,
    "admit_006_unified_result_projection_contract_frozen": True,
    "admit_006_blocked_passthrough_contract_frozen": True,
    "admit_006_context_invalid_partial_canonical_projection_frozen": True,
    "ready_for_unified_dispatch_runtime_with_admit_001_to_006_implementation": True,
    "feature_semantics_audit_required_before_training": True,
    "admit_006_unified_adapter_implemented": False,
    "admit_006_registered_in_engine": False,
    "current_exact5_runtime_modified": False,
    ...
    "admit_007_to_015_registered_in_engine": False,
...
def build_design_state(repo_root: Path = REPO_ROOT, head_ref: str = "HEAD") -> dict[str, Any]:
    ...
    return {
        ...
        "issue_bytes": _bytes(snapshot, SOURCE_PATHS[4]),
    }
...
def run_covapie_bulk_download_admission_admit_006_unified_adapter_contract_design_gate_v1(
...
if __name__ == "__main__":
    result = run_covapie_bulk_download_admission_admit_006_unified_adapter_contract_design_gate_v1()
