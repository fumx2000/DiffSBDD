"""Additive cumulative1000 readiness census successor for published LCY Exact4.

The successor consumes the frozen with-GVE census, the published LCY ingestion
matrix, and the published with-LCY reconciliation.  It deep-copies the 1000
predecessor rows and overlays only the four frozen LCY event identities.  The
historical base census validator is intentionally not used on final rows: its
``NOT_RELEVANT => NOT_ESTABLISHED`` rule predates the explicitly authorized LCY
...
from . import covapie_completed_human_decision_reconciliation_with_lcy_v1 as lcy_reconciliation
from . import covapie_cumulative1000_current_global_readiness_census_with_gve_v1 as predecessor
from . import covapie_lcy_completed_decision_ingestion_and_task_label_availability_v1 as lcy_ingestion
...
__all__ = (
    "Cumulative1000CurrentGlobalReadinessCensusWithLCYError",
    "compute_covapie_cumulative1000_current_global_readiness_census_with_lcy_v1",
    "validate_covapie_cumulative1000_current_global_readiness_census_with_lcy_v1",
    "build_covapie_cumulative1000_current_global_readiness_artifacts_with_lcy_v1",
    "materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_lcy_v1",
)


SCHEMA_VERSION = "covapie_cumulative1000_current_global_readiness_census_with_lcy_v1"
STAGE = "COVAPIE_CUMULATIVE1000_CURRENT_GLOBAL_READINESS_CENSUS_WITH_LCY_V1"
ERROR_TOKEN = "COVAPIE_CUMULATIVE1000_CURRENT_GLOBAL_READINESS_CENSUS_WITH_LCY_V1_ERROR"

OUTPUT_DIRECTORY_RELATIVE = Path(
    "data/derived/covalent_small/"
    "covapie_cumulative1000_current_global_readiness_census_with_lcy_v1"
)
CENSUS_FILE = "covapie_cumulative1000_current_global_readiness_census_with_lcy_v1.csv"
SUMMARY_FILE = "covapie_cumulative1000_current_global_readiness_summary_with_lcy_v1.json"
MANIFEST_FILE = "covapie_cumulative1000_current_global_readiness_manifest_with_lcy_v1.json"

PRODUCTION_RELATIVE = Path(
    "src/covalent_ext/"
    "covapie_cumulative1000_current_global_readiness_census_with_lcy_v1.py"
)
CHECKER_RELATIVE = Path(
    "scripts/check_covapie_cumulative1000_current_global_readiness_census_with_lcy_v1.py"
)
TEST_RELATIVE = Path(
    "tests/test_covapie_cumulative1000_current_global_readiness_census_with_lcy_v1.py"
)
GUIDE_RELATIVE = Path(
    "docs/covapie_cumulative1000_current_global_readiness_census_with_lcy_v1_guide.md"
)
EXACT7_PATHS_V1 = (
...
LCY_EXACT4_EVENT_IDS_V1 = lcy_ingestion.EXPECTED_EVENT_IDS
LCY_EXACT4_RANKS_V1 = lcy_ingestion.EXPECTED_RANKS
LCY_REVIEW_UNIT_ID_V1 = lcy_ingestion.EXPECTED_REVIEW_UNIT_ID
LCY_HUMAN_DECISION_SOURCE = lcy_ingestion.FORMAL_DECISION_RELATIVE.as_posix()
LCY_EVENT_MATRIX_RELATIVE = lcy_ingestion.OUTPUT_ROOT_RELATIVE / lcy_ingestion.MATRIX
LCY_EVENT_MATRIX_SOURCE = LCY_EVENT_MATRIX_RELATIVE.as_posix()
...
LCY_RECONCILIATION_OWNER_RELATIVE = Path(
    "src/covalent_ext/covapie_completed_human_decision_reconciliation_with_lcy_v1.py"
)
LCY_INGESTION_OWNER_RELATIVE = lcy_ingestion.SOURCE_RELATIVE
PRIORITY_QUEUE_RELATIVE = predecessor.PRIORITY_QUEUE_RELATIVE

GVE_EXACT4_EVENT_IDS_V1 = predecessor.GVE_EXACT4_EVENT_IDS_V1
LCY_SAME_COMPONENT_CONTROL_EVENT_ID_V1 = (
    "COVAPIE_CYS_SG_EVENT_V1:3A2G:A:CYS:102-:SG:G:LCY:C1"
)

NEXT_PENDING_REVIEW_UNIT_ID_V1 = "COVAPIE_BULK_REVIEW_UNIT_BF1809E89D22D405"
...
_EXPECTED_GLOBAL_STATUS_COUNTS_V1 = {
    generic.CURRENTLY_UNREVIEWED: 187,
    generic.CURRENTLY_IN_PROGRESS: 0,
    generic.COMPLETED_HUMAN_POSITIVE: 115,
    generic.COMPLETED_HUMAN_NEGATIVE: 66,
...
_AUTHORIZED_LCY_OVERLAY_FIELDS_V1 = frozenset(
    {
...
_AUTHORIZED_BUT_UNCHANGED_LCY_FIELDS_V1 = frozenset(
    {
        "future_training_admission_candidate",
        "human_training_excluded",
        "training_use_include",
...
_ACTUAL_CHANGED_LCY_FIELDS_V1 = (
    _AUTHORIZED_LCY_OVERLAY_FIELDS_V1 - _AUTHORIZED_BUT_UNCHANGED_LCY_FIELDS_V1
)
_FORBIDDEN_LCY_CHANGED_FIELDS_V1 = frozenset(
    {
...
_EXPECTED_REFRESHED_SUMMARY_SHA256_V1: str | None = (
    "edd88fce900fd8f77f1b1647b39b6a2adb54e184361c0edc5d1111d5346e7ada"
)
_EXPECTED_REFRESHED_SEMANTIC_SOURCE_BINDINGS_SHA256_V1: str | None = (
    "1382e6a7bb4b3a01496c17afba6ccb27bff843ff814911c17c04b8119bf57f14"
)

_SHA_PATTERN = re.compile(r"^[0-9a-f]{64}$")
_ADDITIVE_SOURCE_SPECS_V1 = (
    (
        "PREDECESSOR_WITH_GVE_CENSUS_OWNER",
        PREDECESSOR_OWNER_RELATIVE,
        "repository_relative",
        70129,
        "28e80ebae0ab061d87dcae3343276fc329953980c10887e587b0dfc7fd50ee0e",
        False,
    ),
    (
        "PREDECESSOR_WITH_GVE_MATERIALIZED_CENSUS",
    ...
    (
        "PREDECESSOR_WITH_GVE_MATERIALIZED_SUMMARY",
    ...
    (
        "LCY_RECONCILIATION_OWNER",
        LCY_RECONCILIATION_OWNER_RELATIVE,
        "repository_relative",
        38419,
    ...
    (
        "LCY_EVENT_TASK_LABEL_AVAILABILITY",
        LCY_EVENT_MATRIX_RELATIVE,
        "repository_relative",
        10030,
        "402ad7a822263a656e5bd362e142c29079e54d38cc2b894b1617d060cd72bce7",
        False,
    ),
)
...
class Cumulative1000CurrentGlobalReadinessCensusWithLCYError(ValueError):
    """Raised unless the additive LCY successor is exactly source-derived."""


def _fail(reason: str) -> NoReturn:
    raise Cumulative1000CurrentGlobalReadinessCensusWithLCYError(
...
def _validate_lcy_matrix_rows_v1(
    ...
    if actual_contexts != expected_contexts:
        _fail("LCY_EVENT_MATRIX_CONTEXT_DRIFT")
    expected_cells = {
        "review_unit_id": LCY_REVIEW_UNIT_ID_V1,
        "model_number": "1",
        "ligand_component_id": "LCY",
        "completed_lane": lcy_ingestion.EXPECTED_COMPLETED_LANE,
        ...
        "PRE_mapping_status": lcy_ingestion.PRE_MAPPING_STATUS,
        "PRE_status": lcy_ingestion.PRE_STATUS,
    ...
    for row in normalized:
        ...
        if any(row[key] != value for key, value in expected_cells.items()):
            _fail("LCY_EVENT_MATRIX_SEMANTICS_INVALID:" + event_id)
        try:
            applicability = json.loads(row["canonical_task_applicability_json"])
        except json.JSONDecodeError as error:
            raise Cumulative1000CurrentGlobalReadinessCensusWithLCYError(
                f"{ERROR_TOKEN}:LCY_MATRIX_JSON_INVALID:{event_id}"
            ) from error
        if (
            len(applicability) != 5
            ...
            _fail("LCY_EVENT_MATRIX_EXACT5_BOUNDARY_INVALID:" + event_id)
    return normalized


def _load_and_validate_lcy_event_matrix_v1(
    root: Path,
) -> tuple[dict[str, str], ...]:
    payload = _read_regular_file(root / LCY_EVENT_MATRIX_RELATIVE, "LCY_EVENT_MATRIX")
    if len(payload) != 10030 or _sha256(payload) != (
        "402ad7a822263a656e5bd362e142c29079e54d38cc2b894b1617d060cd72bce7"
    ):
        _fail("LCY_EVENT_MATRIX_BINDING_INVALID")
    source_derived = lcy_ingestion.build_artifacts_v1(root)
    if source_derived[lcy_ingestion.MATRIX] != payload:
        _fail("LCY_EVENT_MATRIX_NOT_SOURCE_DERIVED")
    try:
        reader = csv.DictReader(io.StringIO(payload.decode("utf-8"), newline=""))
    except UnicodeDecodeError as error:
        raise Cumulative1000CurrentGlobalReadinessCensusWithLCYError(
            f"{ERROR_TOKEN}:LCY_EVENT_MATRIX_NOT_UTF8"
        ) from error
    if tuple(reader.fieldnames or ()) != lcy_ingestion.MATRIX_HEADER:
        _fail("LCY_EVENT_MATRIX_HEADER_INVALID")
    return _validate_lcy_matrix_rows_v1(tuple(dict(row) for row in reader))


def _validate_lcy_reconciliation_v1(root: Path) -> generic.ReconciliationResult:
    result = lcy_reconciliation.reconcile_real_completed_human_decisions_with_lcy_v1(root)
    expected_summary = {
    ...
    if (
        ...
        or len(result.source_bindings) != 21
        or len(result.normalized_facts) != 127
        or len({binding.stable_identity for binding in result.source_bindings}) != 21
    ):
        _fail("LCY_RECONCILIATION_EXACT21_127_INVALID")
    target = set(LCY_EXACT4_EVENT_IDS_V1)
    facts = [fact for fact in result.normalized_facts if fact.canonical_event_id in target]
    if len(facts) != 4 or any(
        fact.review_unit_id != LCY_REVIEW_UNIT_ID_V1
        ...
        or fact.source_binding_path != LCY_HUMAN_DECISION_SOURCE
        for fact in facts
    ):
        _fail("LCY_RECONCILIATION_EXACT4_ORTHOGONALITY_INVALID")
    ...
    if len(target_rows) != 4 or any(
        ...
        _fail("LCY_RECONCILIATION_TARGET_STATUS_INVALID")
    return result


def _assert_predecessor_lcy_state_v1(
    ...
    if (
        ...
        _fail("PREDECESSOR_WITH_GVE_CENSUS_IDENTITY_INVALID")
    target = set(LCY_EXACT4_EVENT_IDS_V1)
    ...
    if (
        len(rows) != 4
        or tuple(int(row["scaleup_rank"]) for row in rows) != LCY_EXACT4_RANKS_V1
        or any(any(row[key] != value for key, value in expected.items()) for row in rows)
    ):
        _fail("PREDECESSOR_LCY_EXACT4_STATE_INVALID")
    if (
        Counter(row["chemistry_disposition"] for row in computation.rows)
        != Counter({"POSITIVE": 136, "NOT_ESTABLISHED": 90, "UNRESOLVED": 774})
        or Counter(row["task_relevance_disposition"] for row in computation.rows)
        != Counter({"RELEVANT": 133, "NOT_RELEVANT": 94, "UNRESOLVED": 773})
        ...
        _fail("PREDECESSOR_WITH_GVE_DISTRIBUTION_INVALID")


def _overlay_lcy_exact4_v1(
    predecessor_rows: Sequence[Mapping[str, str]],
    matrix_rows: Sequence[Mapping[str, str]],
    reconciliation: generic.ReconciliationResult,
) -> tuple[dict[str, str], ...]:
    matrix_by_event = {
        row["canonical_event_id"]: row for row in _validate_lcy_matrix_rows_v1(matrix_rows)
    }
    facts = {
    ...
    if set(facts) != set(LCY_EXACT4_EVENT_IDS_V1):
        _fail("LCY_RECONCILIATION_MATRIX_IDENTITY_MISMATCH")
    ...
    for row in rows:
        ...
        if (
            ...
            or row["review_unit_id"] != LCY_REVIEW_UNIT_ID_V1
        ):
            _fail("LCY_MATRIX_PREDECESSOR_IDENTITY_MISMATCH:" + event_id)
        row.update(
            {
                ...
                "chemistry_authority_source": LCY_EVENT_MATRIX_SOURCE,
                "positive_authority_source": LCY_EVENT_MATRIX_SOURCE,
                "task_relevance_disposition": fact.task_relevance_disposition,
                "task_relevance_authority_source": LCY_EVENT_MATRIX_SOURCE,
...
def _top_pending_review_units_v1(
    ...
    if len(candidates) != 106 or any(
        unit == LCY_REVIEW_UNIT_ID_V1 for _n, _p, unit, _row, _s in candidates
    ):
...
def _build_summary_v1(
    ...
    summary["refresh_delta"] = {
        "frozen_predecessor_positive_count": 136,
        "LCY_exact4_delta_count": 4,
    ...
    source_composition["LCY"] = sum(
        row["positive_authority_source"] == LCY_EVENT_MATRIX_SOURCE for row in rows
    )
    summary["chemistry"] = {
    ...
    summary["reactive_pair"].update(
        {
            ...
            "lcy_sample_authority_contribution_count": sum(
                row["positive_authority_source"] == LCY_EVENT_MATRIX_SOURCE
                and row["reactive_pair_sample_authoritative"] == "true"
                for row in rows
            ),
            "lcy_training_target_contribution_count": sum(
                row["positive_authority_source"] == LCY_EVENT_MATRIX_SOURCE
    ...
    summary["training_stage"]["future_candidate_source_composition"].pop("LCY", None)
    missing_source_composition = dict(
        summary["blockers"]["missing_tensor_integration"]["missing_source_composition"]
    )
    missing_source_composition["LCY"] = sum(
        row["positive_authority_source"] == LCY_EVENT_MATRIX_SOURCE
    ...
    boundary.update(
        {
            ...
            "next_priority_review_ligand": "0D8",
            ...
            "LCY_CENSUS_SOURCE_BINDING_V2_CLEAN_FROM_BIRTH": True,
            "separate_LCY_census_V2_successor_required": False,
        }
    )
    for stale in (
        "READY_FOR_GVE_REVIEW_PREPARATION",
        "GVE_REVIEW_STARTED",
        "READY_FOR_LCY_REVIEW_PREPARATION",
        "LCY_REVIEW_STARTED",
    ):
...
def compute_covapie_cumulative1000_current_global_readiness_census_with_lcy_v1(
    repo_root: Path,
) -> base.Cumulative1000CurrentGlobalReadinessComputationV1:
    """Compute the additive LCY successor entirely from frozen sources."""

    root = repo_root.resolve()
    additive_bindings = _verify_additive_sources(root)
    frozen = predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_gve_v1(root)
    _assert_predecessor_lcy_state_v1(frozen, root)
    _verify_predecessor_bindings(root, frozen.semantic_source_bindings)
    reconciliation = _validate_lcy_reconciliation_v1(root)
    matrix_rows = _load_and_validate_lcy_event_matrix_v1(root)
    rows = _overlay_lcy_exact4_v1(frozen.rows, matrix_rows, reconciliation)
...
def validate_covapie_cumulative1000_current_global_readiness_census_with_lcy_v1(
    computation: object,
    *,
    ...
    """Fail closed on every LCY identity, authority, and cross-field boundary."""
    ...
    if (
        len(_AUTHORIZED_LCY_OVERLAY_FIELDS_V1) != 15
        or len(_AUTHORIZED_BUT_UNCHANGED_LCY_FIELDS_V1) != 3
        or len(_ACTUAL_CHANGED_LCY_FIELDS_V1) != 12
        or _AUTHORIZED_BUT_UNCHANGED_LCY_FIELDS_V1
        - _AUTHORIZED_LCY_OVERLAY_FIELDS_V1
    ):
        _fail("AUTHORIZED_LCY_OVERLAY_CONTRACT_INVALID")

    root = Path(__file__).resolve().parents[2]
    frozen = predecessor_computation or predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_gve_v1(root)
    reconciliation = reconciliation_result or _validate_lcy_reconciliation_v1(root)
    validated_matrix = (
        _validate_lcy_matrix_rows_v1(matrix_rows)
        if matrix_rows is not None
        else _load_and_validate_lcy_event_matrix_v1(root)
    )
    _assert_predecessor_lcy_state_v1(frozen, root)
    _verify_predecessor_bindings(root, frozen.semantic_source_bindings)
    _validate_lcy_matrix_rows_v1(validated_matrix)
    ...
    exact4 = set(LCY_EXACT4_EVENT_IDS_V1)
    ...
    if changed != exact4 or any(before[event_id] != after[event_id] for event_id in set(before) - exact4):
        _fail("PREDECESSOR_DELTA_NOT_EXACT_LCY_EXACT4")
    if any(before[event_id] != after[event_id] for event_id in GVE_EXACT4_EVENT_IDS_V1):
        _fail("GVE_EXACT4_NOT_PRESERVED")
    control = LCY_SAME_COMPONENT_CONTROL_EVENT_ID_V1
    if (
        control in exact4
        or control not in before
        or before[control]["ligand_component_id"] != "LCY"
        or before[control] != after[control]
    ):
        _fail("LCY_3A2G_SAME_COMPONENT_CONTROL_NOT_PRESERVED")
    expected_after = {
        "current_global_status": generic.COMPLETED_HUMAN_NEGATIVE,
        "current_review_status": generic.COMPLETED_HUMAN_NEGATIVE,
        "human_review_completed": "true",
        "human_review_authority_source": LCY_HUMAN_DECISION_SOURCE,
    ...
    for event_id in LCY_EXACT4_EVENT_IDS_V1:
        changed_fields = {
            field for field in CENSUS_COLUMNS_V1 if before[event_id][field] != after[event_id][field]
        }
        unchanged_authorized = {
            field for field in _AUTHORIZED_LCY_OVERLAY_FIELDS_V1 if before[event_id][field] == after[event_id][field]
        }
        if changed_fields != _ACTUAL_CHANGED_LCY_FIELDS_V1:
            _fail("LCY_CHANGED_FIELD_SET_NOT_EXACT12:" + event_id)
        if unchanged_authorized != _AUTHORIZED_BUT_UNCHANGED_LCY_FIELDS_V1:
            _fail("LCY_AUTHORIZED_BUT_UNCHANGED_SET_NOT_EXACT3:" + event_id)
        if changed_fields & _FORBIDDEN_LCY_CHANGED_FIELDS_V1:
            _fail("LCY_FORBIDDEN_FIELD_CHANGED:" + event_id)
        if any(after[event_id][field] != value for field, value in expected_after.items()):
            _fail("LCY_REFRESHED_SEMANTICS_INVALID:" + event_id)

    predecessor_legacy_task_negative = {
        row["canonical_event_id"]
    ...
    if legacy != predecessor_legacy_task_negative or len(legacy) != 90:
        _fail("LEGACY_TASK_NEGATIVE_POPULATION_NOT_EXACT90")
    if predecessor_orthogonal != set(GVE_EXACT4_EVENT_IDS_V1):
        _fail("PREDECESSOR_GVE_ORTHOGONAL_POPULATION_NOT_EXACT4")
    if orthogonal != predecessor_orthogonal | exact4 or len(orthogonal) != 8:
        _fail("TASK_NEGATIVE_CHEMISTRY_POSITIVE_POPULATION_NOT_GVE_PLUS_LCY_EXACT8")
    ...
    for row in rows:
        ...
        if event_id in legacy:
            continue
        if event_id in predecessor_orthogonal:
            continue
    ...
    expected_blockers = {
        "pair_authority_absent": {"all_1000": 860, "within_chemistry_positive": 0},
        "role_authority_absent": {"all_1000": 868, "within_chemistry_positive": 8},
        "human_training_exclusion": {"within_chemistry_positive": 72},
        "missing_split_authority": {
            "within_chemistry_positive": 99,
            "within_training_include": 35,
        },
        "missing_POST_training_authority": {
            "within_chemistry_positive": 123,
            "within_training_include": 43,
        },
        "missing_training_admission": {
            "within_chemistry_positive": 135,
            "within_training_include": 55,
        },
        "feature_semantics_pending": {"within_chemistry_positive": 140},
    }
    ...
    if (
        tensor["within_chemistry_positive"] != 99
        or tensor["within_training_include"] != 31
        or tensor["all_missing_are_training_excluded_population"] is not False
        or tensor["missing_source_composition"].get("LCY") != 4
        or summary["chemistry"]["positive_source_composition"].get("LCY") != 4
        or summary["chemistry"]["positive_source_composition"].get("GVE") != 4
        or "LCY" in summary["training_stage"]["future_candidate_source_composition"]
        or summary["reactive_pair"]["lcy_sample_authority_contribution_count"] != 4
        or summary["reactive_pair"]["lcy_training_target_contribution_count"] != 0
    ):
        _fail("SUMMARY_LCY_SOURCE_COMPOSITION_INVALID")
...
def _build_artifacts_from_computation_v1(
    ...
    manifest = {
        ...
        "predecessor_manifest_validation_binding": {
            "artifact_role": "PREDECESSOR_WITH_GVE_MANIFEST_VALIDATION_IDENTITY",
        ...
        "refresh_contract": {
            ...
            "actual_changed_field_count_per_lcy_row": 12,
            "semantic_source_binding_count": 156,
            "predecessor_semantic_source_binding_count": 150,
            ...
            "task_negative_chemistry_positive_population_count": 8,
            "task_negative_chemistry_positive_population_exactly_gve_plus_lcy_exact8": True,
...
def build_covapie_cumulative1000_current_global_readiness_artifacts_with_lcy_v1(
    repo_root: Path,
) -> dict[str, bytes]:
    ...
    root = repo_root.resolve()
    computation = compute_covapie_cumulative1000_current_global_readiness_census_with_lcy_v1(root)
...
def materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_lcy_v1(
    repo_root: Path, output_directory: Path | None = None
) -> dict[str, bytes]:
    """Write only the authorized Exact3 output files under the LCY output root."""
    ...
    artifacts = build_covapie_cumulative1000_current_global_readiness_artifacts_with_lcy_v1(root)
