"""Additive cumulative1000 readiness census refresh for published PRF Exact8.

This successor consumes the frozen ONL-refreshed census plus already-published
PRF ingestion and reconciliation authority.  It deep-copies the predecessor
rows and overlays only PRF Exact8.  It creates no human, chemistry, pair, role,
...
from . import covapie_completed_human_decision_reconciliation_with_prf_v1 as prf_reconciliation
from . import covapie_cumulative1000_current_global_readiness_census_with_onl_v1 as predecessor
from . import covapie_prf_completed_decision_ingestion_and_task_label_availability_v1 as prf_ingestion
...
SCHEMA_VERSION = "covapie_cumulative1000_current_global_readiness_census_with_prf_v1"
STAGE = "COVAPIE_CUMULATIVE1000_CURRENT_GLOBAL_READINESS_CENSUS_WITH_PRF_V1"
ERROR_TOKEN = "COVAPIE_CUMULATIVE1000_CURRENT_GLOBAL_READINESS_CENSUS_WITH_PRF_V1_ERROR"

OUTPUT_DIRECTORY_RELATIVE = Path(
    "data/derived/covalent_small/"
    "covapie_cumulative1000_current_global_readiness_census_with_prf_v1"
)
CENSUS_FILE = "covapie_cumulative1000_current_global_readiness_census_with_prf_v1.csv"
SUMMARY_FILE = "covapie_cumulative1000_current_global_readiness_summary_with_prf_v1.json"
MANIFEST_FILE = "covapie_cumulative1000_current_global_readiness_manifest_with_prf_v1.json"

PRODUCTION_RELATIVE = Path(
    "src/covalent_ext/"
    "covapie_cumulative1000_current_global_readiness_census_with_prf_v1.py"
)
...
GUIDE_RELATIVE = Path(
    "docs/covapie_cumulative1000_current_global_readiness_census_with_prf_v1_guide.md"
)
...
PRF_EXACT8_EVENT_IDS_V1 = prf_ingestion.EXPECTED_EVENT_IDS
PRF_EXACT8_RANKS_V1 = prf_ingestion.EXPECTED_RANKS
PRF_REVIEW_UNIT_ID_V1 = prf_ingestion.EXPECTED_REVIEW_UNIT_ID
PRF_DIRECT_TASK_IDS_CELL_V1 = "[0,3,4]"
...
PRF_RECONCILIATION_OWNER_RELATIVE = Path(
    "src/covalent_ext/covapie_completed_human_decision_reconciliation_with_prf_v1.py"
)
PRF_INGESTION_OWNER_RELATIVE = prf_ingestion.SOURCE_RELATIVE
PRF_EVENT_MATRIX_RELATIVE = prf_ingestion.OUTPUT_ROOT_RELATIVE / prf_ingestion.MATRIX
PRF_FORMAL_DECISION_RELATIVE_TO_REPOSITORY_PARENT = (
    prf_ingestion.FORMAL_DECISION_RELATIVE_TO_REPOSITORY_PARENT
)
PRIORITY_QUEUE_RELATIVE = predecessor.PRIORITY_QUEUE_RELATIVE

PRF_EVENT_MATRIX_SOURCE = PRF_EVENT_MATRIX_RELATIVE.as_posix()
PRF_FORMAL_DECISION_SOURCE = (
    PRF_FORMAL_DECISION_RELATIVE_TO_REPOSITORY_PARENT.as_posix()
)

_EXPECTED_GLOBAL_STATUS_COUNTS_V1 = {
    generic.CURRENTLY_UNREVIEWED: 265,
    generic.CURRENTLY_IN_PROGRESS: 0,
    generic.COMPLETED_HUMAN_POSITIVE: 49,
...
_EXPECTED_BOOLEAN_COUNTS_V1.update(
    {
        "reactive_pair_sample_authoritative": 66,
        "role_partition_sample_authoritative": 66,
        "canonical_mask_structural_labels_available": 66,
    }
)
...
_EXPECTED_REFRESHED_SEMANTIC_SOURCE_BINDINGS_SHA256_V1 = (
    "e6f3a5ae1fdc566887daa65324c6a110b439524b91c3b913105206f951694344"
)

_SHA_PATTERN = re.compile(r"^[0-9a-f]{64}$")
_STRUCTURAL_IDENTITY_FIELDS_V1 = predecessor._STRUCTURAL_IDENTITY_FIELDS_V1
_AUTHORIZED_PRF_OVERLAY_FIELDS_V1 = predecessor._AUTHORIZED_ONL_OVERLAY_FIELDS_V1

_ADDITIVE_SOURCE_SPECS_V1 = (
    ("PREDECESSOR_ONL_CENSUS_OWNER", PREDECESSOR_OWNER_RELATIVE, "repository_relative", 70095, "f74d2d486492a89c1aaeb523220e18558f80a8d50389e9deb076c86df7802f5b"),
    ("PREDECESSOR_ONL_MATERIALIZED_CENSUS", PREDECESSOR_CENSUS_RELATIVE, "repository_relative", 502004, "57be12d7ed5d4e013dbd402ccf0ed49aa3d86067a952fdd339fcc945062894e4"),
    ("PREDECESSOR_ONL_MATERIALIZED_SUMMARY", PREDECESSOR_SUMMARY_RELATIVE, "repository_relative", 14489, "3271df9a94bf72f8ae081735e07e132c4a7d0dfb389f3acfdf31789420f2c19b"),
    ("PRF_RECONCILIATION_SUCCESSOR", PRF_RECONCILIATION_OWNER_RELATIVE, "repository_relative", 11728, "ce4db4fcbf909852a6fca1a919ee50750279a8a6ca0968d4b33ae8f510bd0f74"),
    ("PRF_INGESTION_OWNER", PRF_INGESTION_OWNER_RELATIVE, "repository_relative", 76082, "52ce8a00e6e02af7a0dcec9fdccdc69f19555a49557b1b9f73fdf4d20e230264"),
    ("PRF_EVENT_TASK_LABEL_AVAILABILITY", PRF_EVENT_MATRIX_RELATIVE, "repository_relative", 14303, "40df3625bda841ffe94ee12681da5593fece9598f44a971b00154c66322ff75b"),
    ("PRF_FORMAL_HUMAN_DECISION", PRF_FORMAL_DECISION_RELATIVE_TO_REPOSITORY_PARENT, "repository_parent_relative", 26699, "2b5b81290405761acaaacc5ba0764aeed23c93ea1eebe420f2c14528045c3ce3"),
)


class Cumulative1000CurrentGlobalReadinessCensusWithPRFError(ValueError):
    """Raised unless the additive PRF refresh is exactly source-derived."""


def _fail(reason: str) -> NoReturn:
    raise Cumulative1000CurrentGlobalReadinessCensusWithPRFError(
...
def _load_and_validate_prf_event_matrix_v1(root: Path) -> tuple[dict[str, str], ...]:
    payload = _read_regular_file(root / PRF_EVENT_MATRIX_RELATIVE, "PRF_EVENT_MATRIX")
    if len(payload) != 14303 or _sha256(payload) != _ADDITIVE_SOURCE_SPECS_V1[5][4]:
        _fail("PRF_EVENT_MATRIX_BINDING_INVALID")
    source_derived = prf_ingestion.build_artifacts_v1(root)
    if source_derived[prf_ingestion.MATRIX] != payload:
        _fail("PRF_EVENT_MATRIX_NOT_SOURCE_DERIVED")
    try:
        reader = csv.DictReader(io.StringIO(payload.decode("utf-8"), newline=""))
    except UnicodeDecodeError as error:
        raise Cumulative1000CurrentGlobalReadinessCensusWithPRFError(
            f"{ERROR_TOKEN}:PRF_EVENT_MATRIX_NOT_UTF8"
        ) from error
    if tuple(reader.fieldnames or ()) != prf_ingestion.MATRIX_HEADER:
        _fail("PRF_EVENT_MATRIX_HEADER_INVALID")
    rows = tuple(dict(row) for row in reader)
    if (
        len(rows) != 8
        or tuple(row["canonical_event_id"] for row in rows) != PRF_EXACT8_EVENT_IDS_V1
        or tuple(int(row["scaleup_rank"]) for row in rows) != PRF_EXACT8_RANKS_V1
        or len({row["canonical_event_id"] for row in rows}) != 8
    ):
        _fail("PRF_EVENT_MATRIX_IDENTITY_NOT_EXACT8")
    expected_cells = {
        ...
        "role_profile": predecessor.predecessor.DIRECT_PROFILE,
        "global_canonical_task_count": "5",
        "direct_profile_applicable_task_ids_json": PRF_DIRECT_TASK_IDS_CELL_V1,
        ...
        "authority_source": prf_ingestion.AUTHORITY_SOURCE,
    ...
    for row in rows:
        ...
        if any(row[key] != value for key, value in expected_cells.items()):
            _fail("PRF_EVENT_MATRIX_SEMANTICS_INVALID:" + event_id)
        try:
            applicability = json.loads(row["canonical_task_applicability_json"])
        except json.JSONDecodeError as error:
            raise Cumulative1000CurrentGlobalReadinessCensusWithPRFError(
                f"{ERROR_TOKEN}:PRF_EXACT5_JSON_INVALID:{event_id}"
            ) from error
        if (
            len(applicability) != 5
            ...
            _fail("PRF_EVENT_MATRIX_EXACT5_INVALID:" + event_id)
    return rows


def _validate_prf_reconciliation_v1(root: Path) -> generic.ReconciliationResult:
    result = prf_reconciliation.reconcile_real_completed_human_decisions_with_prf_v1(root)
    if result.review_summary != {
        "universe_event_count": 338,
        "universe_review_unit_count": 131,
        "completed_positive_event_count": 49,
        "completed_positive_unit_count": 5,
        "completed_negative_event_count": 24,
        "completed_negative_unit_count": 4,
        "completed_total_event_count": 73,
        "completed_total_unit_count": 9,
        "in_progress_event_count": 0,
        "in_progress_unit_count": 0,
        "unreviewed_event_count": 265,
        "unreviewed_unit_count": 122,
    }:
        _fail("PRF_RECONCILIATION_SUMMARY_INVALID")
    if len(result.normalized_facts) != 49 or Counter(
        fact.training_disposition for fact in result.normalized_facts
    ) != Counter({generic.TRAINING_INCLUDE: 12, generic.TRAINING_EXCLUDE: 37}):
        _fail("PRF_RECONCILIATION_NORMALIZED_FACTS_INVALID")
    rows = [
        row for row in result.reconciled_rows
        if row["raw_review_unit_id"] == PRF_REVIEW_UNIT_ID_V1
    ]
    if (
        len(rows) != 8
        or {row["canonical_event_id"] for row in rows} != set(PRF_EXACT8_EVENT_IDS_V1)
        or any(row["current_review_status"] != generic.COMPLETED_HUMAN_POSITIVE for row in rows)
    ):
        _fail("PRF_RECONCILIATION_EXACT8_INVALID")
    return result


def _assert_predecessor_prf_state_v1(
    computation: predecessor.predecessor.Cumulative1000CurrentGlobalReadinessComputationV1,
) -> None:
    rows = [
        row for row in computation.rows
        if row["canonical_event_id"] in set(PRF_EXACT8_EVENT_IDS_V1)
    ]
    if len(rows) != 8 or tuple(int(row["scaleup_rank"]) for row in rows) != PRF_EXACT8_RANKS_V1:
        _fail("PREDECESSOR_PRF_EXACT8_IDENTITY_INVALID")
    expected = {
        "current_global_status": generic.CURRENTLY_UNREVIEWED,
        "priority_review_in_scope": "true",
        "review_unit_id": PRF_REVIEW_UNIT_ID_V1,
        "current_review_status": generic.CURRENTLY_UNREVIEWED,
        "human_review_completed": "false",
        "chemistry_disposition": predecessor.predecessor.CHEMISTRY_UNRESOLVED,
        "task_relevance_disposition": predecessor.predecessor.TASK_UNRESOLVED,
        "training_use_disposition": predecessor.predecessor.TRAINING_UNRESOLVED,
        ...
        "role_profile": predecessor.predecessor.ROLE_NOT_ESTABLISHED,
    ...
    if any(any(row[key] != value for key, value in expected.items()) for row in rows):
        _fail("PREDECESSOR_PRF_STATE_INVALID")
    if Counter(row["chemistry_disposition"] for row in computation.rows)[
        predecessor.predecessor.CHEMISTRY_POSITIVE
    ] != 58:
        _fail("PREDECESSOR_POSITIVE_COUNT_NOT_58")


def _overlay_prf_exact8_v1(
    ...
    if set(matrix_by_event) != set(PRF_EXACT8_EVENT_IDS_V1) or len(matrix_by_event) != 8:
        _fail("PRF_OVERLAY_MATRIX_SET_INVALID")
    ...
    for row in rows:
        ...
        if (
            ...
            or row["ligand_component_id"] != "PRF"
            or row["review_unit_id"] != PRF_REVIEW_UNIT_ID_V1
        ):
            _fail("PRF_MATRIX_PREDECESSOR_IDENTITY_MISMATCH:" + event_id)
        row.update(
            {
                "current_global_status": generic.COMPLETED_HUMAN_POSITIVE,
                "current_review_status": generic.COMPLETED_HUMAN_POSITIVE,
                "human_review_completed": "true",
                "human_review_authority_source": PRF_FORMAL_DECISION_SOURCE,
                "chemistry_disposition": predecessor.predecessor.CHEMISTRY_POSITIVE,
                "chemistry_authority_source": PRF_EVENT_MATRIX_SOURCE,
                "task_relevance_disposition": predecessor.predecessor.TASK_RELEVANT,
                "task_relevance_authority_source": PRF_EVENT_MATRIX_SOURCE,
                ...
                "structurally_applicable_task_ids_json": PRF_DIRECT_TASK_IDS_CELL_V1,
                "post_geometry_sample_authoritative": "false",
                "post_geometry_training_target_available": "false",
                "pre_geometry_authoritative": "false",
                "pre_geometry_training_target_available": "false",
                ...
                "positive_authority_source": PRF_EVENT_MATRIX_SOURCE,
...
def _build_summary_v1(
    ...
    chemistry_positive = event_set("chemistry_disposition", predecessor.predecessor.CHEMISTRY_POSITIVE)
    chemistry_negative = event_set("chemistry_disposition", predecessor.predecessor.CHEMISTRY_NEGATIVE)
    chemistry_not_established = event_set("chemistry_disposition", predecessor.predecessor.CHEMISTRY_NOT_ESTABLISHED)
    chemistry_unresolved = event_set("chemistry_disposition", predecessor.predecessor.CHEMISTRY_UNRESOLVED)
    task_relevant = event_set("task_relevance_disposition", predecessor.predecessor.TASK_RELEVANT)
    task_not_relevant = event_set("task_relevance_disposition", predecessor.predecessor.TASK_NOT_RELEVANT)
    task_unresolved = event_set("task_relevance_disposition", predecessor.predecessor.TASK_UNRESOLVED)
    ...
    training_unresolved = event_set("training_use_disposition", predecessor.predecessor.TRAINING_UNRESOLVED)
    ...
    summary["refresh_delta"] = {
        "frozen_predecessor_positive_count": 58,
        "prf_exact8_delta_count": 8,
    ...
    summary["global_status_distribution"]["counts"] = {
        ...
        for status in predecessor.predecessor.GLOBAL_STATUSES_V1
    }
    ...
    summary["chemistry"] = {
        ...
        "positive_source_composition": {
            "CURRENT_RUNTIME": sum(row["positive_authority_source"] == predecessor.predecessor._RUNTIME_INDEX for row in rows),
            "FFQ": sum(row["positive_authority_source"] == predecessor.predecessor._FFQ_EVENT for row in rows),
            ...
            "G3H": sum(row["positive_authority_source"] == predecessor.predecessor._G3H_EVENT for row in rows),
            "ONL": sum(row["positive_authority_source"] == predecessor.ONL_EVENT_MATRIX_SOURCE for row in rows),
            "PRF": sum(row["positive_authority_source"] == PRF_EVENT_MATRIX_SOURCE for row in rows),
        },
    ...
    summary["role"]["role_profile_counts"] = {
        predecessor.predecessor.STRICT_PROFILE: profile_counts[predecessor.predecessor.STRICT_PROFILE],
        predecessor.predecessor.DIRECT_PROFILE: profile_counts[predecessor.predecessor.DIRECT_PROFILE],
        "other": sum(value for profile, value in profile_counts.items() if profile not in {predecessor.predecessor.STRICT_PROFILE, predecessor.predecessor.DIRECT_PROFILE}),
    }
    ...
    summary["blockers"] = {
        ...
        "pair_authority_absent": {
            ...
            "within_positive_66": sum(row["reactive_pair_sample_authoritative"] == "false" for row in positive_rows),
        },
        "role_authority_absent": {
            "all_1000": sum(row["role_partition_sample_authoritative"] == "false" for row in rows),
            "within_positive_66": sum(row["role_partition_sample_authoritative"] == "false" for row in positive_rows),
        },
        "human_training_exclusion": {"within_positive_66": sum(row["human_training_excluded"] == "true" for row in positive_rows)},
        "missing_split_authority": {
            "within_positive_66": sum(row["formal_split_authoritative"] == "false" for row in positive_rows),
            "within_include_29": sum(row["formal_split_authoritative"] == "false" for row in include_rows),
        },
        "missing_tensor_integration": {
            "within_positive_66": len(missing_tensor_rows),
            ...
            "missing_source_composition": {
                "G3H": sum(row["positive_authority_source"] == predecessor.predecessor._G3H_EVENT for row in missing_tensor_rows),
                "ONL": sum(row["positive_authority_source"] == predecessor.ONL_EVENT_MATRIX_SOURCE for row in missing_tensor_rows),
                "PRF": sum(row["positive_authority_source"] == PRF_EVENT_MATRIX_SOURCE for row in missing_tensor_rows),
            },
        },
        "missing_POST_training_authority": {
            "within_positive_66": sum(row["post_geometry_training_target_available"] == "false" for row in positive_rows),
        ...
        "missing_training_admission": {
            "within_positive_66": sum(row["formal_training_admitted"] == "false" for row in positive_rows),
            "within_include_29": sum(row["formal_training_admitted"] == "false" for row in include_rows),
        },
        "feature_semantics_pending": {"within_positive_66": len(positive_rows)},
    }
    summary["top_pending_review_units_by_event_yield"] = top_pending
    summary["authority_boundary"].update(
        {
            "next_priority_review_unit": "COVAPIE_BULK_REVIEW_UNIT_5834329D9E2A1F22",
            "next_priority_review_ligand": "2VS",
            "next_priority_review_event_count": 8,
        }
    )
    return summary
...
def compute_covapie_cumulative1000_current_global_readiness_census_with_prf_v1(
    repo_root: Path,
) -> predecessor.predecessor.Cumulative1000CurrentGlobalReadinessComputationV1:
    """Compute the exact additive PRF refresh entirely from frozen sources."""
    ...
    _assert_predecessor_prf_state_v1(frozen)
    reconciliation = _validate_prf_reconciliation_v1(root)
    matrix_rows = _load_and_validate_prf_event_matrix_v1(root)
    rows = _overlay_prf_exact8_v1(frozen.rows, matrix_rows)
...
def validate_covapie_cumulative1000_current_global_readiness_census_with_prf_v1(
    computation: object,
    *,
    predecessor_computation: predecessor.predecessor.Cumulative1000CurrentGlobalReadinessComputationV1 | None = None,
) -> bool:
    """Fail closed unless refreshed rows, summary, and provenance are exact."""

    expected_type = predecessor.predecessor.Cumulative1000CurrentGlobalReadinessComputationV1
    ...
    frozen = predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_onl_v1(root) if predecessor_computation is None else predecessor_computation
    ...
    prf_set = set(PRF_EXACT8_EVENT_IDS_V1)
    if changed != prf_set or len(changed) != 8:
        _fail("PREDECESSOR_DELTA_NOT_EXACT_PRF8")
    if any(refreshed_by_event[event_id] != frozen_by_event[event_id] for event_id in seen - prf_set):
        _fail("NON_PRF_ROW_CHANGED")
    ...
    for event_id in PRF_EXACT8_EVENT_IDS_V1:
        ...
        if changed_fields != expected_changed_fields or not changed_fields <= _AUTHORIZED_PRF_OVERLAY_FIELDS_V1:
            _fail("PRF_CHANGED_FIELD_SET_INVALID:" + event_id)
        if any(before[field] != after[field] for field in _STRUCTURAL_IDENTITY_FIELDS_V1):
            _fail("PRF_STRUCTURAL_EVIDENCE_CHANGED:" + event_id)
        if any(after[field] != value for field, value in expected_prf.items()):
            _fail("PRF_REFRESHED_SEMANTICS_INVALID:" + event_id)

    previous_sets = _sets_for_algebra_v1(frozen.rows)
    current_sets = _sets_for_algebra_v1(rows)
    if previous_sets["chemistry_positive"] & prf_set:
        _fail("PRF_PREDECESSOR_POSITIVE_INTERSECTION_NOT_EMPTY")
    if not (
        current_sets["chemistry_positive"] == previous_sets["chemistry_positive"] | prf_set
        and current_sets["chemistry_unresolved"] == previous_sets["chemistry_unresolved"] - prf_set
        and current_sets["task_relevant"] == previous_sets["task_relevant"] | prf_set
        and current_sets["task_unresolved"] == previous_sets["task_unresolved"] - prf_set
        and current_sets["training_exclude"] == previous_sets["training_exclude"] | prf_set
        and current_sets["training_unresolved"] == previous_sets["training_unresolved"] - prf_set
        ...
        _fail("PRF_EXACT_SET_ALGEBRA_INVALID")
    ...
    if Counter(row["role_profile"] for row in rows if row["role_partition_sample_authoritative"] == "true") != Counter({predecessor.predecessor.STRICT_PROFILE: 31, predecessor.predecessor.DIRECT_PROFILE: 35}):
        _fail("CENSUS_ROLE_PROFILE_DISTRIBUTION_INVALID")

    for row in rows:
        event_id = row["canonical_event_id"]
        if row["role_partition_sample_authoritative"] == "true":
            expected_task_ids = [0, 1, 2, 3, 4] if row["role_profile"] == predecessor.predecessor.STRICT_PROFILE else [0, 3, 4] if row["role_profile"] == predecessor.predecessor.DIRECT_PROFILE else None
            try:
        ...
        elif row["role_profile"] != predecessor.predecessor.ROLE_NOT_ESTABLISHED or row["canonical_mask_structural_labels_available"] != "false" or row["structurally_applicable_task_ids_json"] != "null":
            _fail("ROLELESS_ROW_FALSE_APPLICABILITY_NOT_UNKNOWN:" + event_id)
        if row["reactive_pair_sample_authoritative"] == "true" and row["chemistry_disposition"] != predecessor.predecessor.CHEMISTRY_POSITIVE:
...
def build_covapie_cumulative1000_current_global_readiness_artifacts_with_prf_v1(
    repo_root: Path,
) -> dict[str, bytes]:
    """Build the deterministic Exact3 outputs without repository writes."""

    root = repo_root.resolve()
    computation = compute_covapie_cumulative1000_current_global_readiness_census_with_prf_v1(root)
...
def materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_prf_v1(
    repo_root: Path,
    ...
    artifacts = build_covapie_cumulative1000_current_global_readiness_artifacts_with_prf_v1(root)
