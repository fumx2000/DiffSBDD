"""CovaPIE unified single-rule runtime for ADMIT_001 through ADMIT_015.

The committed Exact14 runtime remains authoritative for the shared Exact13
result, dispatch error, constants, and its fourteen handlers.  This successor
adds only the ADMIT_015 adapter, an immutable Exact15 registry, and a new
...
CALLABLE_DISCOVERED_RULE_IDS = KNOWN_RULE_IDS
ADAPTER_READY_RULE_IDS = KNOWN_RULE_IDS
...
def _admit015_context_failure(reason: str) -> NoReturn:
...
def _admit015_adapter_failure(reason: str) -> NoReturn:
    _raise_dispatch_error(
...
def _admit015_candidate_invalid() -> UnifiedAdmissionRuleEvaluation:
    return UnifiedAdmissionRuleEvaluation(
        ...
        reason="ADMIT_015_CANDIDATE_RECORD_MAPPING_INVALID",
        normalized_values=(),
...
def _prevalidate_admit015_source(
    source: object,
) -> admit015.Admit015EvaluationResult:
    if type(source) is not admit015.Admit015EvaluationResult:
        _admit015_adapter_failure(
            "ADMIT_015_UNIFIED_ADAPTER_SOURCE_TYPE_INVALID"
        )
    try:
        storage = vars(source)
        if type(storage) is not dict or tuple(storage) != ADMIT_015_SOURCE_FIELDS:
            raise ValueError("Exact9 storage/order drift")
        if (
            tuple(field.name for field in fields(admit015.Admit015EvaluationResult))
            != ADMIT_015_SOURCE_FIELDS
        ):
            raise ValueError("Exact9 dataclass order drift")
        values = tuple(
            getattr(source, name) for name in ADMIT_015_SOURCE_FIELDS
        )
        exact_types = (
            str,
            str,
        ...
        reconstructed = admit015.Admit015EvaluationResult(*values)
        if (
...
def _expected_admit015_from_oracle(
    stage_authorization_context: object,
) -> admit015_oracle.Admit015EvaluationResultContractDesign:
    try:
        result = (
            admit015_oracle
        ...
        result_type = admit015_oracle.Admit015EvaluationResultContractDesign
        ...
        values = tuple(
            getattr(result, name) for name in ADMIT_015_SOURCE_FIELDS
        )
        exact_types = (
            str,
            str,
...
def _validate_admit015_oracle_equivalence(
    source: admit015.Admit015EvaluationResult,
    expected: admit015_oracle.Admit015EvaluationResultContractDesign,
) -> None:
    try:
        source_values = tuple(
            getattr(source, name) for name in ADMIT_015_SOURCE_FIELDS
        ...
        expected_values = tuple(
            getattr(expected, name) for name in ADMIT_015_SOURCE_FIELDS
        )
...
def _project_stage_authorization_record_to_exact_string_pairs(
    ...
    if (
        ...
        or canonical_record[0][0] != "current_stage_training_authorized"
...
def _evaluate_registered_admit_015(
    ...
    *,
    ...
    if batch_context is not None:
        _admit015_context_failure("ADMIT_015_BATCH_CONTEXT_MUST_BE_NONE")
    ...
    if not isinstance(candidate_record, Mapping):
        return _admit015_candidate_invalid()
    try:
        source = admit015.evaluate_admit_015(
            stage_authorization_context=stage_authorization_context
        )
    except Exception:
        _admit015_adapter_failure(
    ...
    validated_source = _prevalidate_admit015_source(source)
    expected = _expected_admit015_from_oracle(stage_authorization_context)
    _validate_admit015_oracle_equivalence(validated_source, expected)
    try:
        return _project_admit015_exact13(validated_source)
    except Exception:
        _admit015_adapter_failure(
            "ADMIT_015_UNIFIED_ADAPTER_SOURCE_INVARIANT_INVALID"
        )
...
EVALUATOR_REGISTRY = MappingProxyType(
    {
        ...
        "ADMIT_014": predecessor.EVALUATOR_REGISTRY["ADMIT_014"],
        "ADMIT_015": _evaluate_registered_admit_015,
    }
)
...
EXPECTED_BASE_COMMIT = "d70d7d8919c3ec59e0b3d864ec8e496695ab770b"
EXPECTED_BASE_PARENT = "b4a8770f27bdd263d0d3ad5c0a8b210e19954cbe"
EXPECTED_BASE_TREE = "b8b3620ed5262f95694104249d607fff8d21ec01"
EXPECTED_BASE_SUBJECT = "add CovaPIE ADMIT_015 unified adapter contract v1"
MANIFEST_SCHEMA_VERSION = (
    "covapie_unified_dispatch_runtime_with_admit_001_to_015_manifest_v1"
)
...
CONTRACT_FILENAME = "covapie_admit_001_to_015_runtime_contract.csv"
TRUTH_FILENAME = "covapie_admit_001_to_015_dispatch_truth_matrix.csv"
REGISTRY_FILENAME = "covapie_admit_001_to_015_registry_and_identity_audit.csv"
SAFETY_FILENAME = "covapie_admit_001_to_015_runtime_safety_audit.csv"
ISSUE_FILENAME = "covapie_admit_001_to_015_runtime_issue_readiness_inventory.csv"
MANIFEST_FILENAME = "covapie_admit_001_to_015_runtime_manifest.json"
OUTPUT_FILES = (
    CONTRACT_FILENAME,
    TRUTH_FILENAME,
    REGISTRY_FILENAME,
...
EXACT42_INPUT_REPRESENTATIONS = {
    ...
    "current_exact14": ("{}", "None", "None", "None", "{target:True}"),
    "future_exact15": ("{}", "None", "None", "None", "{target:True}"),
    "known_unregistered": (
        "{}", "None", "None", "None", "{target:True}",
    ),
...
def _runtime_input_representations(
    ...
    if scenario not in {
        str(spec["_scenario"])
        for spec in admit015_adapter_design._routing_case_specs()
    }:
...
def _mutate_exact9(value: object, scenario: str) -> object:
    ...
    if scenario in {"source_subclass", "oracle_subclass"}:
        ...
        for name in ADMIT_015_SOURCE_FIELDS:
...
def _execute_exact42_case(spec: Mapping[str, Any]) -> dict[str, str]:
    ...
    if scenario == "batch_object":
        ...
        stage = _StageProbe({"current_stage_training_authorized": False})
    ...
    original_formal = admit015.evaluate_admit_015
    original_oracle = (
        admit015_oracle
        .classify_admit_015_formal_evaluator_interface_design
    )
    ...
    admit015.evaluate_admit_015 = formal_wrapper
    (
        admit015_oracle
        .classify_admit_015_formal_evaluator_interface_design
    ) = oracle_wrapper
    try:
        ...
        admit015.evaluate_admit_015 = original_formal
        (
            admit015_oracle
            .classify_admit_015_formal_evaluator_interface_design
        ) = original_oracle
...
def _dispatcher_and_predecessor_rows() -> list[dict[str, str]]:
    ...
    for rule_id in KNOWN_RULE_IDS[:14]:
...
def _registry_rows() -> list[dict[str, str]]:
    ...
    for index, rule_id in enumerate(KNOWN_RULE_IDS, 1):
        if rule_id in KNOWN_RULE_IDS[:14]:
            expected_identity = (
            ...
            identity_preserved = (
                EVALUATOR_REGISTRY[rule_id]
                is _evaluate_registered_admit_015
            )
        else:
...
def _updated_issue_rows(
    ...
    coverage["integration_transition"] = (
        "unified_dispatch_runtime_with_admit_001_to_015_implemented_v1"
    )
    coverage["successor_effective_status"] = "resolved"
...
def build_runtime_state(
    ...
    design_specs = admit015_adapter_design._routing_case_specs()
    if (
        len(design_specs) != 42
    ...
    if (
        ...
        or len(contract_rows) != 45
        or len(registry_rows) != 27
        or len(safety_rows) != 32
        ...
        raise ValueError("Exact15 runtime state failed closed")
...
def _manifest_payload(
    ...
    manifest = {
        ...
        "admit_015_handler_signature": str(
            inspect.signature(_evaluate_registered_admit_015)
        ),
        "admit_015_source_fields": list(ADMIT_015_SOURCE_FIELDS),
        ...
        "output_file_count": 6,
        "output_csv_schemas": {
            CONTRACT_FILENAME: list(CONTRACT_COLUMNS),
            TRUTH_FILENAME: list(TRUTH_COLUMNS),
            REGISTRY_FILENAME: list(REGISTRY_COLUMNS),
...
class MaterializationRetentionError(RuntimeError):
    ...
    def __init__(
        ...
        *,
        ...
        detail = (
            f"failure staging retained at {authenticated_retained_path}; "
        ...
        super().__init__(
            ...
            f"frozen_retained_staging_identity={retained_staging_identity}; "
...
def _verify_destination_binding(
    ...
    parent_fd: int,
    root_fd: int,
    expected_root: tuple[int, int, int, int, int, int],
    payloads: Mapping[str, bytes],
    ...
    def binding_check() -> None:
        ...
        if (
            _full_identity(destination) != expected_root
            or _full_identity(os.fstat(root_fd)) != expected_root
...
def _rename_noreplace(
    ...
    _assert_staging_name_binding(
        plan, parent_fd, staging_name, staging_fd, staging_identity
    )
    _assert_output_parent(plan, parent_fd)
    _assert_staging_name_binding(
        plan, parent_fd, staging_name, staging_fd, staging_identity
...
def _assert_staging_name_binding(
    ...
    if (
        not stat.S_ISDIR(lexical.st_mode)
        or stat.S_ISLNK(lexical.st_mode)
        or _full_identity(lexical) != staging_identity
        or _full_identity(os.fstat(root_fd)) != staging_identity
    ):
...
def _materialize_set(
    ...
    try:
        ...
        if plan.root_identity is not None:
            ...
            if (
                _full_identity(destination) != plan.root_identity
                or not stat.S_ISDIR(destination.st_mode)
                or stat.S_ISLNK(destination.st_mode)
            ):
        ...
        for _ in range(64):
            candidate = (
                f".exact15-runtime-stage-{secrets.token_hex(16)}"
            )
            try:
        ...
        _verify_complete_set(
            ...
            payloads,
            staged,
            binding_check=lambda: _assert_staging_name_binding(
                plan,
                parent_fd,
                staging_name,
                root_fd,
...
def run_covapie_bulk_download_admission_unified_dispatch_runtime_with_admit_001_to_015_v1(
...
if __name__ == "__main__":
    run_covapie_bulk_download_admission_unified_dispatch_runtime_with_admit_001_to_015_v1()
