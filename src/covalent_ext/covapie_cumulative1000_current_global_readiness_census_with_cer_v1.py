"""Additive cumulative1000 readiness census refresh for published CER Exact4.

This successor consumes the frozen with-1N0 census plus already-published
CER ingestion and reconciliation authority. It deep-copies the predecessor
rows and overlays only CER Exact4. It creates no human, chemistry, pair, role,
...
from . import covapie_completed_human_decision_reconciliation_with_cer_v1 as cer_reconciliation
from . import covapie_cumulative1000_current_global_readiness_census_with_1n0_v1 as predecessor
from . import covapie_cer_completed_decision_ingestion_and_task_label_availability_v1 as cer_ingestion
...
SCHEMA_VERSION = "covapie_cumulative1000_current_global_readiness_census_with_cer_v1"
STAGE = "COVAPIE_CUMULATIVE1000_CURRENT_GLOBAL_READINESS_CENSUS_WITH_CER_V1"
ERROR_TOKEN = "COVAPIE_CUMULATIVE1000_CURRENT_GLOBAL_READINESS_CENSUS_WITH_CER_V1_ERROR"

OUTPUT_DIRECTORY_RELATIVE = Path(
    "data/derived/covalent_small/"
    "covapie_cumulative1000_current_global_readiness_census_with_cer_v1"
)
CENSUS_FILE = "covapie_cumulative1000_current_global_readiness_census_with_cer_v1.csv"
SUMMARY_FILE = "covapie_cumulative1000_current_global_readiness_summary_with_cer_v1.json"
MANIFEST_FILE = "covapie_cumulative1000_current_global_readiness_manifest_with_cer_v1.json"
...
CHECKER_RELATIVE = Path(
    "scripts/check_covapie_cumulative1000_current_global_readiness_census_with_cer_v1.py"
)
TEST_RELATIVE = Path(
    "tests/test_covapie_cumulative1000_current_global_readiness_census_with_cer_v1.py"
)
GUIDE_RELATIVE = Path(
    "docs/covapie_cumulative1000_current_global_readiness_census_with_cer_v1_guide.md"
)
...
CER_EXACT4_EVENT_IDS_V1 = cer_ingestion.EXPECTED_EVENT_IDS
CER_EXACT4_RANKS_V1 = cer_ingestion.EXPECTED_RANKS
CER_REVIEW_UNIT_ID_V1 = cer_ingestion.EXPECTED_REVIEW_UNIT_ID
CER_DIRECT_TASK_IDS_CELL_V1 = "[0,3,4]"
...
CER_RECONCILIATION_OWNER_RELATIVE = Path(
    "src/covalent_ext/covapie_completed_human_decision_reconciliation_with_cer_v1.py"
)
CER_INGESTION_OWNER_RELATIVE = cer_ingestion.SOURCE_RELATIVE
CER_EVENT_MATRIX_RELATIVE = cer_ingestion.OUTPUT_ROOT_RELATIVE / cer_ingestion.MATRIX
PRIORITY_QUEUE_RELATIVE = predecessor.PRIORITY_QUEUE_RELATIVE

CER_EVENT_MATRIX_SOURCE = CER_EVENT_MATRIX_RELATIVE.as_posix()
...
CER_HUMAN_DECISION_SOURCE = cer_ingestion.FORMAL_DECISION_RELATIVE.as_posix()

_EXPECTED_GLOBAL_STATUS_COUNTS_V1 = {
    generic.CURRENTLY_UNREVIEWED: 207,
    generic.CURRENTLY_IN_PROGRESS: 0,
    generic.COMPLETED_HUMAN_POSITIVE: 103,
...
_EXPECTED_BOOLEAN_COUNTS_V1.update(
    {
        "reactive_pair_sample_authoritative": 120,
        "role_partition_sample_authoritative": 120,
        "canonical_mask_structural_labels_available": 120,
        "training_use_include": 52,
        "future_training_admission_candidate": 35,
    }
)
...
_EXPECTED_REFRESHED_SEMANTIC_SOURCE_BINDINGS_SHA256_V1: str | None = (
    "4107486a5fb2e3086302f0064f0a1dff7d351697c74ffe18fe86e2a22d3f14fe"
)

_SHA_PATTERN = re.compile(r"^[0-9a-f]{64}$")
_STRUCTURAL_IDENTITY_FIELDS_V1 = predecessor._STRUCTURAL_IDENTITY_FIELDS_V1
_AUTHORIZED_CER_OVERLAY_FIELDS_V1 = frozenset(
    {
...
_EXPECTED_CER_STRUCTURAL_CELLS_V1 = {
...
_ADDITIVE_SOURCE_SPECS_V1 = (
    ...
    (
        ...
        CER_RECONCILIATION_OWNER_RELATIVE,
        "repository_relative",
        32847,
        "a3b5332654bbee87503737f6e8b077022232e4eb1a92fb48606bd761e30c6fba",
        False,
    ),
    (
        "CER_INGESTION_OWNER",
        CER_INGESTION_OWNER_RELATIVE,
        "repository_relative",
        81662,
    ...
    (
        ...
        CER_EVENT_MATRIX_RELATIVE,
        "repository_relative",
        10331,
        "420381168ef087940af6ed46584c15954a8590a0cbe760ad4a9947fa5d629899",
        False,
    ),
)
...
class Cumulative1000CurrentGlobalReadinessCensusWithCERError(ValueError):
    """Raised unless the additive CER refresh is exactly source-derived."""


def _fail(reason: str) -> NoReturn:
    raise Cumulative1000CurrentGlobalReadinessCensusWithCERError(
    ...
    try:
        verify_bound_source_v2(
            ...
            label=(
                ...
                + PREDECESSOR_MANIFEST_RELATIVE.as_posix()
            ),
            expected_executable=_PREDECESSOR_MANIFEST_SPEC_V1[2],
        )
    except SourceBindingPolicyV2Error as error:
        raise Cumulative1000CurrentGlobalReadinessCensusWithCERError(
            f"{ERROR_TOKEN}:PREDECESSOR_WITH_1N0_MANIFEST_BINDING_INVALID"
        ) from error
    return tuple(bindings)


def _validate_cer_matrix_rows_v1(
    rows: Sequence[Mapping[str, str]],
) -> tuple[dict[str, str], ...]:
    normalized = tuple(dict(row) for row in rows)
    if (
        len(normalized) != 4
        or any(tuple(row) != cer_ingestion.MATRIX_HEADER for row in normalized)
        or tuple(row["canonical_event_id"] for row in normalized)
        != CER_EXACT4_EVENT_IDS_V1
        or tuple(int(row["scaleup_rank"]) for row in normalized)
        != CER_EXACT4_RANKS_V1
        or len({row["canonical_event_id"] for row in normalized}) != 4
    ):
        _fail("CER_EVENT_MATRIX_IDENTITY_NOT_EXACT4")
    if Counter(row["pdb_id"] for row in normalized) != Counter({"1FJ8": 4}):
        _fail("CER_EVENT_MATRIX_PDB_IDENTITY_NOT_EXACT_1FJ8")
    if {row["cys_residue_id"] for row in normalized} != {"CYS:163-"}:
        _fail("CER_EVENT_MATRIX_CYS_IDENTITY_INVALID")
    if tuple(
        (row["protein_chain_or_asym"], row["ligand_chain_or_asym"])
        for row in normalized
    ) != (("A", "E"), ("B", "F"), ("C", "G"), ("D", "H")):
        _fail("CER_EVENT_MATRIX_CONTEXTS_COLLAPSED_OR_DRIFTED")
    if tuple(row["selected_connection_id"] for row in normalized) != (
        "covale1",
        "covale2",
        "covale3",
        "covale4",
    ):
        _fail("CER_EVENT_MATRIX_CONNECTIONS_COLLAPSED_OR_DRIFTED")
    expected_cells = {
        "review_unit_id": CER_REVIEW_UNIT_ID_V1,
        "pdb_id": "1FJ8",
        "model_number": "1",
        ...
        "pair_authority_scope": cer_ingestion.AUTHORITY_SCOPE,
        ...
        "direct_profile_applicable_task_ids_json": CER_DIRECT_TASK_IDS_CELL_V1,
        ...
        "future_training_admission_status": cer_ingestion.FUTURE_STATUS,
        ...
        "PRE_status": cer_ingestion.PRE_STATUS,
        ...
        "authority_source": cer_ingestion.AUTHORITY_SOURCE,
        "authority_scope": cer_ingestion.AUTHORITY_SCOPE,
    ...
    for row in normalized:
        ...
        if any(row[key] != value for key, value in expected_cells.items()):
            _fail("CER_EVENT_MATRIX_SEMANTICS_INVALID:" + event_id)
        try:
            applicability = json.loads(row["canonical_task_applicability_json"])
        except json.JSONDecodeError as error:
            raise Cumulative1000CurrentGlobalReadinessCensusWithCERError(
                f"{ERROR_TOKEN}:CER_EXACT5_JSON_INVALID:{event_id}"
            ) from error
        if (
            len(applicability) != 5
            ...
            != list(cer_ingestion.WARHEAD_ROLE)
            or json.loads(row["linker_atoms_json"])
            != list(cer_ingestion.LINKER_ROLE)
            or json.loads(row["scaffold_atoms_json"])
            != list(cer_ingestion.SCAFFOLD_ROLE)
            or json.loads(row["boundary_bonds_json"])
            != list(cer_ingestion.BOUNDARY_BONDS)
        ):
            _fail("CER_EVENT_MATRIX_EXACT5_INVALID:" + event_id)
    return normalized


def _load_and_validate_cer_event_matrix_v1(
    root: Path,
) -> tuple[dict[str, str], ...]:
    payload = _read_regular_file(root / CER_EVENT_MATRIX_RELATIVE, "CER_EVENT_MATRIX")
    if len(payload) != 10331 or _sha256(payload) != _ADDITIVE_SOURCE_SPECS_V1[5][4]:
        _fail("CER_EVENT_MATRIX_BINDING_INVALID")
    source_derived = cer_ingestion.build_artifacts_v1(root)
    if source_derived[cer_ingestion.MATRIX] != payload:
        _fail("CER_EVENT_MATRIX_NOT_SOURCE_DERIVED")
    try:
        reader = csv.DictReader(io.StringIO(payload.decode("utf-8"), newline=""))
    except UnicodeDecodeError as error:
        raise Cumulative1000CurrentGlobalReadinessCensusWithCERError(
            f"{ERROR_TOKEN}:CER_EVENT_MATRIX_NOT_UTF8"
        ) from error
    if tuple(reader.fieldnames or ()) != cer_ingestion.MATRIX_HEADER:
        _fail("CER_EVENT_MATRIX_HEADER_INVALID")
    return _validate_cer_matrix_rows_v1(tuple(dict(row) for row in reader))


def _validate_cer_reconciliation_v1(root: Path) -> generic.ReconciliationResult:
    result = cer_reconciliation.reconcile_real_completed_human_decisions_with_cer_v1(
        root
    )
    if result.review_summary != {
        ...
        _fail("CER_RECONCILIATION_SUMMARY_INVALID")
    if len(result.normalized_facts) != 107 or Counter(
        fact.training_disposition for fact in result.normalized_facts
    ) != Counter(
        {
            generic.TRAINING_INCLUDE: 35,
            generic.TRAINING_EXCLUDE: 68,
            generic.TRAINING_NOT_APPLICABLE: 4,
        }
    ):
        _fail("CER_RECONCILIATION_NORMALIZED_FACTS_INVALID")
    rows = [
        row
        for row in result.reconciled_rows
        if row["raw_review_unit_id"] == CER_REVIEW_UNIT_ID_V1
    ]
    if (
        len(rows) != 4
        or {row["canonical_event_id"] for row in rows}
        != set(CER_EXACT4_EVENT_IDS_V1)
        or any(
            row["current_review_status"] != generic.COMPLETED_HUMAN_POSITIVE
            for row in rows
        )
    ):
        _fail("CER_RECONCILIATION_EXACT4_INVALID")
    facts = [
        fact
        for fact in result.normalized_facts
        if fact.canonical_event_id in set(CER_EXACT4_EVENT_IDS_V1)
    ]
    if len(facts) != 4 or any(
        ...
        _fail("CER_RECONCILIATION_EXACT4_SEMANTICS_INVALID")
    return result


def _assert_predecessor_cer_state_v1(
    computation: base.Cumulative1000CurrentGlobalReadinessComputationV1,
) -> None:
    cer_set = set(CER_EXACT4_EVENT_IDS_V1)
    rows = [
        row for row in computation.rows if row["canonical_event_id"] in cer_set
    ]
    if len(rows) != 4 or tuple(
        int(row["scaleup_rank"]) for row in rows
    ) != CER_EXACT4_RANKS_V1:
        _fail("PREDECESSOR_CER_EXACT4_IDENTITY_INVALID")
    expected = {
    ...
    if any(any(row[key] != value for key, value in expected.items()) for row in rows):
        _fail("PREDECESSOR_CER_STATE_INVALID")
    if any(
        any(
            row[field] != value
            for field, value in _EXPECTED_CER_STRUCTURAL_CELLS_V1.items()
        )
        for row in rows
    ):
        _fail("PREDECESSOR_CER_STRUCTURAL_COVERAGE_INVALID")
...
def _overlay_cer_exact4_v1(
    predecessor_rows: Sequence[Mapping[str, str]],
    matrix_rows: Sequence[Mapping[str, str]],
) -> tuple[dict[str, str], ...]:
    validated_matrix = _validate_cer_matrix_rows_v1(matrix_rows)
    matrix_by_event = {
    ...
    for row in rows:
        ...
        if (
            ...
            or row["ligand_component_id"] != "CER"
            or row["review_unit_id"] != CER_REVIEW_UNIT_ID_V1
        ):
            _fail("CER_MATRIX_PREDECESSOR_IDENTITY_MISMATCH:" + event_id)
        row.update(
            {
                "current_global_status": generic.COMPLETED_HUMAN_POSITIVE,
                "current_review_status": generic.COMPLETED_HUMAN_POSITIVE,
                "human_review_completed": "true",
                "human_review_authority_source": CER_HUMAN_DECISION_SOURCE,
                "chemistry_disposition": base.CHEMISTRY_POSITIVE,
                "chemistry_authority_source": CER_EVENT_MATRIX_SOURCE,
                "positive_authority_source": CER_EVENT_MATRIX_SOURCE,
                "task_relevance_disposition": base.TASK_RELEVANT,
                "task_relevance_authority_source": CER_EVENT_MATRIX_SOURCE,
                ...
                "structurally_applicable_task_ids_json": CER_DIRECT_TASK_IDS_CELL_V1,
...
def _top_pending_review_units_v1(
    ...
    if len(candidates) != 111:
        _fail("CURRENT_PENDING_REVIEW_UNIT_COUNT_INVALID")
    if any(unit == CER_REVIEW_UNIT_ID_V1 for _n, _p, unit, _row, _s in candidates):
        _fail("CER_REVIEW_UNIT_STILL_PENDING")
    ...
    first = top[0]
    if not (
        first["review_unit_id"] == "COVAPIE_BULK_REVIEW_UNIT_9E98765987D25C42"
        and first["raw_priority_rank"] == 20
        and first["event_count"] == 4
        and first["ligand_component_ids"] == ["4M5"]
        and first["pdb_ids"] == ["5AZT", "5AZV"]
...
def _build_summary_v1(
    ...
    source_composition["CER"] = sum(
        row["positive_authority_source"] == CER_EVENT_MATRIX_SOURCE for row in rows
    )
    summary["chemistry"] = {
    ...
    summary["reactive_pair"].update(
        {
            ...
            "cer_sample_authority_contribution_count": 4,
            "cer_model_bound_target_contribution_count": 0,
    ...
    summary["blockers"] = {
        ...
        "missing_tensor_integration": {
            ...
            "missing_source_composition": {
                **summary["blockers"]["missing_tensor_integration"][
                ...
                "CER": sum(
                    row["positive_authority_source"] == CER_EVENT_MATRIX_SOURCE
                    for row in missing_tensor_rows
                ),
            },
        },
        ...
        "feature_semantics_pending": {"within_positive_120": len(positive_rows)},
    }
    summary["top_pending_review_units_by_event_yield"] = top_pending
    summary["authority_boundary"].update(
        {
            "next_priority_review_unit": "COVAPIE_BULK_REVIEW_UNIT_9E98765987D25C42",
            "next_priority_review_ligand": "4M5",
            "next_priority_review_event_count": 4,
            "next_priority_review_current_pending_rank": 1,
            "next_priority_review_raw_priority_rank": 20,
            "next_review_started": False,
            "NEXT_REVIEW_STARTED": False,
            "CER_REVIEW_COMPLETED": True,
            ...
            "CER_CENSUS_SOURCE_BINDING_V2_CLEAN_FROM_BIRTH": True,
            "separate_CER_census_V2_successor_required": False,
            "QUEUE_REFRESH": False,
    ...
    summary["authority_boundary"].pop("FILESYSTEM_MODE_AUTHORITY_TECH_DEBT", None)
    summary["authority_boundary"].pop("NEXT_RECOMMENDED_MAINLINE", None)
    summary["authority_boundary"].pop("CER_REVIEW_STARTED", None)
    summary["authority_boundary"].pop("1N0_REVIEW_COMPLETED", None)
    summary["authority_boundary"].pop(
        "ONE_N0_CENSUS_SOURCE_BINDING_V2_CLEAN_FROM_BIRTH", None
    )
    summary["authority_boundary"].pop(
        "separate_ONE_N0_census_V2_successor_required", None
    )
    return summary
...
def _verify_predecessor_semantic_bindings_v1(
    ...
    if len(bindings) != 120:
        _fail("PREDECESSOR_SEMANTIC_BINDING_COUNT_NOT_120")
    if (
...
def compute_covapie_cumulative1000_current_global_readiness_census_with_cer_v1(
    repo_root: Path,
) -> base.Cumulative1000CurrentGlobalReadinessComputationV1:
    """Compute the exact additive CER refresh entirely from frozen sources."""

    root = repo_root.resolve()
    additive_bindings = _verify_additive_sources(root)
    frozen = predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_1n0_v1(
        root
    )
    _assert_predecessor_cer_state_v1(frozen)
    _verify_predecessor_semantic_bindings_v1(frozen.semantic_source_bindings)
    reconciliation = _validate_cer_reconciliation_v1(root)
    matrix_rows = _load_and_validate_cer_event_matrix_v1(root)
    rows = _overlay_cer_exact4_v1(frozen.rows, matrix_rows)
...
def validate_covapie_cumulative1000_current_global_readiness_census_with_cer_v1(
    computation: object,
    *,
    ...
    frozen = (
        predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_1n0_v1(
            root
        )
    ...
    cer_set = set(CER_EXACT4_EVENT_IDS_V1)
    if changed != cer_set or len(changed) != 4:
        _fail("PREDECESSOR_DELTA_NOT_EXACT_CER_EXACT4")
    if any(
        refreshed_by_event[event_id] != frozen_by_event[event_id]
        for event_id in seen - cer_set
    ):
        _fail("NON_CER_ROW_CHANGED")
    ...
    for event_id in CER_EXACT4_EVENT_IDS_V1:
        ...
        if changed_fields != _AUTHORIZED_CER_OVERLAY_FIELDS_V1:
            _fail("CER_CHANGED_FIELD_SET_INVALID:" + event_id)
        if any(
            before[field] != after[field] for field in _STRUCTURAL_IDENTITY_FIELDS_V1
        ):
            _fail("CER_STRUCTURAL_EVIDENCE_CHANGED:" + event_id)
        if any(after[field] != value for field, value in expected_cer.items()):
            _fail("CER_REFRESHED_SEMANTICS_INVALID:" + event_id)

    previous_sets = _sets_for_algebra_v1(frozen.rows)
    current_sets = _sets_for_algebra_v1(rows)
    if previous_sets["chemistry_positive"] & cer_set:
        _fail("CER_PREDECESSOR_POSITIVE_INTERSECTION_NOT_EMPTY")
    if not (
        current_sets["chemistry_positive"]
        == previous_sets["chemistry_positive"] | cer_set
        and current_sets["chemistry_unresolved"]
        == previous_sets["chemistry_unresolved"] - cer_set
        and current_sets["task_relevant"] == previous_sets["task_relevant"] | cer_set
        and current_sets["task_unresolved"]
        == previous_sets["task_unresolved"] - cer_set
        and current_sets["training_include"]
        == previous_sets["training_include"] | cer_set
        and current_sets["training_unresolved"]
        == previous_sets["training_unresolved"] - cer_set
        ...
        == previous_sets["future_candidate"] | cer_set
        ...
        _fail("CER_EXACT_SET_ALGEBRA_INVALID")
    ...
    if Counter(row["task_relevance_disposition"] for row in rows) != Counter(
        {"RELEVANT": 121, "NOT_RELEVANT": 90, "UNRESOLVED": 789}
    ):
    ...
    if Counter(
        ...
        for row in rows
        if row["role_partition_sample_authoritative"] == "true"
    ) != Counter({base.STRICT_PROFILE: 52, base.DIRECT_PROFILE: 68}):
    ...
    if applicability_counts != Counter({0: 120, 1: 52, 2: 52, 3: 120, 4: 120}):
    ...
    if bindings != expected_bindings or len(bindings) != 126:
...
def build_covapie_cumulative1000_current_global_readiness_artifacts_with_cer_v1(
    repo_root: Path,
) -> dict[str, bytes]:
    ...
    root = repo_root.resolve()
    computation = compute_covapie_cumulative1000_current_global_readiness_census_with_cer_v1(
        root
    )
    ...
    manifest = {
        ...
        "predecessor_manifest_validation_binding": {
            "artifact_role": "PREDECESSOR_WITH_1N0_MANIFEST_VALIDATION_IDENTITY",
        ...
        "refresh_contract": {
            ...
            "changed_event_count": 4,
            "unchanged_event_count": 996,
            "changed_field_count_per_cer_row": 18,
            "semantic_source_binding_count": 126,
            "predecessor_semantic_source_binding_count": 120,
...
def materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_cer_v1(
    repo_root: Path,
    ...
    artifacts = build_covapie_cumulative1000_current_global_readiness_artifacts_with_cer_v1(
        root
    )
    try:
