"""Cumulative1000 readiness census refreshed by the published ME7 Exact3.

This metadata-only successor reproduces the published with-PYR census, consumes
the published ME7 reconciliation and its 79-column event matrix, and overlays
only the three approved ME7 events.  It does not create human, scientific,
...
from . import covapie_completed_human_decision_reconciliation_with_me7_v1 as reconciliation_owner
from . import covapie_cumulative1000_current_global_readiness_census_with_pyr_v1 as predecessor
...
BASELINE_COMMIT = "bb82cb003202785ce7b3bd1fe63199d29bf7cfae"
SCHEMA_VERSION = "covapie_cumulative1000_current_global_readiness_census_with_me7_v1"
STAGE = "COVAPIE_CUMULATIVE1000_CURRENT_GLOBAL_READINESS_CENSUS_WITH_ME7_V1"
ERROR_TOKEN = "COVAPIE_CUMULATIVE1000_CURRENT_GLOBAL_READINESS_CENSUS_WITH_ME7_V1_ERROR"

OUTPUT_DIRECTORY_RELATIVE = Path(
    "data/derived/covalent_small/"
    "covapie_cumulative1000_current_global_readiness_census_with_me7_v1"
)
CENSUS_FILE = "covapie_cumulative1000_current_global_readiness_census_with_me7_v1.csv"
SUMMARY_FILE = "covapie_cumulative1000_current_global_readiness_summary_with_me7_v1.json"
MANIFEST_FILE = "covapie_cumulative1000_current_global_readiness_manifest_with_me7_v1.json"
PRODUCTION_RELATIVE = Path(
    "src/covalent_ext/"
    "covapie_cumulative1000_current_global_readiness_census_with_me7_v1.py"
)
CHECKER_RELATIVE = Path(
    "scripts/check_covapie_cumulative1000_current_global_readiness_census_with_me7_v1.py"
)
TEST_RELATIVE = Path(
    "tests/test_covapie_cumulative1000_current_global_readiness_census_with_me7_v1.py"
)
GUIDE_RELATIVE = Path(
    "docs/covapie_cumulative1000_current_global_readiness_census_with_me7_v1_guide.md"
)
EXACT7_PATHS_V1 = (
...
ME7_EXACT3_EVENT_IDS_V1 = ingestion.EXPECTED_EVENT_IDS
ME7_EXACT3_RANKS_V1 = ingestion.EXPECTED_RANKS
ME7_REVIEW_UNIT_ID_V1 = ingestion.EXPECTED_REVIEW_UNIT_ID
ME7_HUMAN_DECISION_SOURCE = ingestion.FORMAL_DECISION_RELATIVE.as_posix()
ME7_EVENT_MATRIX_RELATIVE = ingestion.OUTPUT_ROOT_RELATIVE / ingestion.MATRIX
ME7_EVENT_MATRIX_SOURCE = ME7_EVENT_MATRIX_RELATIVE.as_posix()
...
ME7_RECONCILIATION_OWNER_RELATIVE = reconciliation_owner.SOURCE_RELATIVE
ME7_RECONCILIATION_ARTIFACT_RELATIVE = reconciliation_owner.OUTPUT_RELATIVE
ME7_INGESTION_OWNER_RELATIVE = ingestion.SOURCE_RELATIVE
PRIORITY_QUEUE_RELATIVE = predecessor.PRIORITY_QUEUE_RELATIVE

NEXT_PENDING_REVIEW_UNIT_ID_V1 = "COVAPIE_BULK_REVIEW_UNIT_79526304E5FB38A6"
NEXT_PENDING_EVENT_COUNT_V1 = 3
NEXT_PENDING_RAW_PRIORITY_RANK_V1 = 33
NEXT_PENDING_PDB_IDS_V1 = ("5ARB", "5ARC", "5ARD")

_AUTHORIZED_ME7_OVERLAY_FIELDS_V1 = frozenset(
    {
...
_AUTHORIZED_BUT_UNCHANGED_ME7_FIELDS_V1 = frozenset(
    {
        "human_training_excluded",
        "role_partition_sample_authoritative",
        "role_profile",
...
_ACTUAL_CHANGED_ME7_FIELDS_V1 = (
    _AUTHORIZED_ME7_OVERLAY_FIELDS_V1 - _AUTHORIZED_BUT_UNCHANGED_ME7_FIELDS_V1
)
...
_EXPECTED_REFRESHED_SEMANTIC_SOURCE_BINDINGS_SHA256_V1: str | None = (
    "f694380c340549e10ba83cbb506cd02be02115cbf578bc98eae2fec9c8094f6c"
)

_SHA_PATTERN = re.compile(r"^[0-9a-f]{64}$")
_ADDITIVE_SOURCE_SPECS_V1 = (
    (
        "PREDECESSOR_WITH_PYR_CENSUS_OWNER",
        PREDECESSOR_OWNER_RELATIVE,
        "repository_relative",
        73835,
        "e451e6e313e9ad90e7658804ddaf84c1e0f47b0181c002b8922f737f5ca0f3e7",
        False,
    ),
    (
        "PREDECESSOR_WITH_PYR_MATERIALIZED_CENSUS",
    ...
    (
        "PREDECESSOR_WITH_PYR_MATERIALIZED_SUMMARY",
    ...
    (
        "ME7_RECONCILIATION_OWNER",
        ME7_RECONCILIATION_OWNER_RELATIVE,
        "repository_relative",
        39360,
    ...
    (
        "ME7_EVENT_TASK_LABEL_AVAILABILITY",
        ME7_EVENT_MATRIX_RELATIVE,
        "repository_relative",
        4698,
...
_PREDECESSOR_MANIFEST_SPEC_V1 = (
    87978,
    "d8c2d8e9db282905db209e0c626f9d050a59eb638692d62cb355a586837f4bd9",
    False,
)
_ME7_RECONCILIATION_ARTIFACT_SPEC_V1 = (
    351626,
...
class Cumulative1000CurrentGlobalReadinessCensusWithME7Error(ValueError):
    """Raised unless the ME7 census is exactly source-derived."""


def _fail(reason: str) -> NoReturn:
    raise Cumulative1000CurrentGlobalReadinessCensusWithME7Error(
...
def _verify_validation_identities(root: Path) -> None:
    for role, relative, spec in (
        (
            "PREDECESSOR_WITH_PYR_MANIFEST",
            PREDECESSOR_MANIFEST_RELATIVE,
            _PREDECESSOR_MANIFEST_SPEC_V1,
        ),
        (
            "ME7_RECONCILIATION_ARTIFACT_VALIDATION_IDENTITY",
            ME7_RECONCILIATION_ARTIFACT_RELATIVE,
            _ME7_RECONCILIATION_ARTIFACT_SPEC_V1,
        ),
...
def _verify_predecessor(
    ...
    frozen = predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_pyr_v1(
        root
    )
...
def _validate_me7_matrix_rows_v1(
    ...
    if (
        len(ingestion.MATRIX_HEADER) != 79
        or len(normalized) != 3
    ...
    for row, pdb_id in zip(normalized, expected_pdbs, strict=True):
        ...
        if any(row[key] != value for key, value in expected_cells.items()):
            _fail("ME7_EVENT_MATRIX_SEMANTICS_INVALID:" + event_id)
        try:
            raw_profile = json.loads(row["role_profile_raw_json"])
            task_ids = json.loads(row["structurally_applicable_task_ids_json"])
        except json.JSONDecodeError as error:
            raise Cumulative1000CurrentGlobalReadinessCensusWithME7Error(
                f"{ERROR_TOKEN}:ME7_MATRIX_JSON_INVALID:{event_id}"
            ) from error
        if raw_profile is not None or task_ids is not None:
            _fail("ME7_UNKNOWN_ROLE_OR_TASK_IDS_NOT_JSON_NULL:" + event_id)
    if (
        ...
        _fail("ME7_CANONICAL_EXACT5_BOUNDARY_INVALID")
    return normalized


def _load_and_validate_me7_event_matrix_v1(root: Path) -> tuple[dict[str, str], ...]:
    payload = _verify_bound(
        root,
        "ME7_EVENT_TASK_LABEL_AVAILABILITY",
        ME7_EVENT_MATRIX_RELATIVE,
    ...
    _verify_repository_blob(
        root, ME7_EVENT_MATRIX_RELATIVE, payload, "ME7_EVENT_TASK_LABEL_AVAILABILITY"
    )
    try:
        reader = csv.DictReader(io.StringIO(payload.decode("utf-8"), newline=""))
    except UnicodeDecodeError as error:
        raise Cumulative1000CurrentGlobalReadinessCensusWithME7Error(
            f"{ERROR_TOKEN}:ME7_EVENT_MATRIX_NOT_UTF8"
        ) from error
    if tuple(reader.fieldnames or ()) != ingestion.MATRIX_HEADER:
        _fail("ME7_EVENT_MATRIX_HEADER_INVALID")
    return _validate_me7_matrix_rows_v1(tuple(dict(row) for row in reader))


def _validate_me7_reconciliation_v1(root: Path) -> generic.ReconciliationResult:
    result = reconciliation_owner.reconcile_real_completed_human_decisions_with_me7_v1(root)
    expected_summary = {
    ...
    if (
        ...
        or len(result.source_bindings) != 28
        or len(result.normalized_facts) != 154
        or len({binding.stable_identity for binding in result.source_bindings}) != 28
        or reconciliation_owner.PREDECESSOR_COVERAGE_SUMMARY["accepted_fact_count"] != 151
        or reconciliation_owner.SUCCESSOR_COVERAGE_SUMMARY["accepted_fact_count"] != 154
    ):
        _fail("ME7_RECONCILIATION_EXACT28_154_INVALID")
    target = set(ME7_EXACT3_EVENT_IDS_V1)
    facts = [fact for fact in result.normalized_facts if fact.canonical_event_id in target]
    if len(facts) != 3 or any(
        fact.review_unit_id != ME7_REVIEW_UNIT_ID_V1
        ...
        or fact.source_binding_path != ME7_HUMAN_DECISION_SOURCE
        for fact in facts
    ):
        _fail("ME7_RECONCILIATION_EXACT3_POSITIVE_NOT_APPLICABLE_INVALID")
    target_rows = [
    ...
    if len(target_rows) != 3 or any(
        ...
        or row["raw_review_unit_id"] != ME7_REVIEW_UNIT_ID_V1
        for row in target_rows
    ):
        _fail("ME7_RECONCILIATION_TARGET_STATUS_INVALID")
    return result


def _assert_predecessor_me7_state_v1(
    ...
    if (
        ...
        _fail("PREDECESSOR_WITH_PYR_CENSUS_IDENTITY_INVALID")
    target = set(ME7_EXACT3_EVENT_IDS_V1)
    ...
    if (
        len(rows) != 3
        or tuple(row["canonical_event_id"] for row in rows) != ME7_EXACT3_EVENT_IDS_V1
        or tuple(int(row["scaleup_rank"]) for row in rows) != ME7_EXACT3_RANKS_V1
        or tuple(row["pdb_id"] for row in rows) != ("3QVY", "3QVY", "3QVZ")
        or any(row["review_unit_id"] != ME7_REVIEW_UNIT_ID_V1 for row in rows)
        or any(any(row[key] != value for key, value in expected.items()) for row in rows)
    ):
        _fail("PREDECESSOR_ME7_EXACT3_STATE_INVALID")
    context_ranks = {
        int(row["scaleup_rank"])
        for row in computation.rows
        if row["ligand_component_id"] == "ME7" and row["canonical_event_id"] not in target
    }
    if not set(ingestion.CONTEXT_ONLY_RANKS).issubset(context_ranks):
        _fail("ME7_CONTEXT_ROWS_MISSING")


def _overlay_me7_exact3_v1(
    ...
    matrix_by_event = {
        ...
        for row in _validate_me7_matrix_rows_v1(matrix_rows)
    }
    facts = {
    ...
    if set(facts) != set(ME7_EXACT3_EVENT_IDS_V1):
        _fail("ME7_RECONCILIATION_MATRIX_IDENTITY_MISMATCH")
    ...
    for row in rows:
        ...
        if (
            ...
            or row["review_unit_id"] != ME7_REVIEW_UNIT_ID_V1
        ):
            _fail("ME7_MATRIX_PREDECESSOR_IDENTITY_MISMATCH:" + event_id)
        ...
        row.update(
            {
                ...
                "chemistry_authority_source": ME7_EVENT_MATRIX_SOURCE,
                "positive_authority_source": ME7_EVENT_MATRIX_SOURCE,
                "task_relevance_disposition": fact.task_relevance_disposition,
                "task_relevance_authority_source": ME7_EVENT_MATRIX_SOURCE,
...
def _top_pending_review_units_v1(
    ...
    if len(candidates) != 99 or any(
        unit == ME7_REVIEW_UNIT_ID_V1 for _n, _p, unit, _row, _status in candidates
    ):
...
def _build_summary_v1(
    ...
    historical_orthogonal = (
        set(predecessor.GVE_EXACT4_EVENT_IDS_V1)
        | set(predecessor.LCY_EXACT4_EVENT_IDS_V1)
        | set(predecessor.ZERO_D8_EXACT4_EVENT_IDS_V1)
        | set(predecessor.TP2_EXACT4_EVENT_IDS_V1)
        | set(predecessor.SIX_OA_EXACT4_EVENT_IDS_V1)
    )
    summary["schema_version"] = SCHEMA_VERSION
    summary["stage"] = STAGE
    summary["refresh_delta"] = {
        "frozen_predecessor_positive_count": 164,
        "ME7_exact3_positive_delta_count": 3,
        "refreshed_positive_count": len(chemistry_positive),
        "frozen_predecessor_training_include_count": 68,
        "ME7_training_include_delta_count": 0,
        "refreshed_training_include_count": len(training_include),
        "frozen_predecessor_training_exclude_count": 76,
        "ME7_training_exclude_delta_count": 0,
        "refreshed_training_exclude_count": len(training_exclude),
        "frozen_predecessor_training_not_applicable_count": 110,
        "ME7_training_not_applicable_delta_count": 3,
        "refreshed_training_not_applicable_count": 113,
        "frozen_predecessor_future_candidate_count": 51,
        "ME7_future_candidate_delta_count": 0,
    ...
    summary["chemistry"]["positive_source_composition"]["ME7"] = sum(
        row["positive_authority_source"] == ME7_EVENT_MATRIX_SOURCE for row in rows
    )
    summary["reactive_pair"].update(
        {
            "me7_sample_authority_contribution_count": sum(
                row["positive_authority_source"] == ME7_EVENT_MATRIX_SOURCE
                and row["reactive_pair_sample_authoritative"] == "true"
                for row in rows
            ),
            "me7_training_target_contribution_count": sum(
                row["positive_authority_source"] == ME7_EVENT_MATRIX_SOURCE
    ...
    summary["orthogonal_task_negative_chemistry_positive"] = {
        ...
        "historical_exact20_population_count": len(historical_orthogonal),
        "me7_orthogonal_population_count": len(set(ME7_EXACT3_EVENT_IDS_V1) & orthogonal),
        "population_equals_historical_exact20_union_me7_exact3": orthogonal
        == historical_orthogonal | set(ME7_EXACT3_EVENT_IDS_V1),
        "gve_orthogonal_population_count": len(
            set(predecessor.GVE_EXACT4_EVENT_IDS_V1) & orthogonal
        ),
        "lcy_orthogonal_population_count": len(
            set(predecessor.LCY_EXACT4_EVENT_IDS_V1) & orthogonal
        ),
        "0d8_orthogonal_population_count": len(
            set(predecessor.ZERO_D8_EXACT4_EVENT_IDS_V1) & orthogonal
        ),
        "tp2_orthogonal_population_count": len(
            set(predecessor.TP2_EXACT4_EVENT_IDS_V1) & orthogonal
        ),
        "6oa_orthogonal_population_count": len(
            set(predecessor.SIX_OA_EXACT4_EVENT_IDS_V1) & orthogonal
        ),
        "pyr_orthogonal_population_count": len(
            set(predecessor.PYR_EXACT4_EVENT_IDS_V1) & orthogonal
        ),
    }
    ...
    boundary.update(
        {
            ...
            "ME7_REVIEW_STATE_CREATED": True,
            "ME7_REVIEW_COMPLETED": True,
            "ME7_RECONCILIATION_CONSUMED": True,
            "ME7_CENSUS_SOURCE_BINDING_V2_CLEAN_FROM_BIRTH": True,
...
def _merge_semantic_bindings_v1(
    ...
    if (
        len(predecessor_bindings) != 192
        or len(additive_bindings) != 6
        or len(merged) != 198
        or len(set(identities)) != 198
...
def _compute_components_v1(
    ...
    _assert_predecessor_me7_state_v1(frozen, root)
    reconciliation = _validate_me7_reconciliation_v1(root)
    matrix_rows = _load_and_validate_me7_event_matrix_v1(root)
    rows = _overlay_me7_exact3_v1(frozen.rows, matrix_rows, reconciliation)
...
def compute_covapie_cumulative1000_current_global_readiness_census_with_me7_v1(
    repo_root: Path,
) -> base.Cumulative1000CurrentGlobalReadinessComputationV1:
    """Compute the additive ME7 successor entirely from published sources."""
...
def validate_covapie_cumulative1000_current_global_readiness_census_with_me7_v1(
    computation: object,
    *,
    ...
    if reconciliation_result is None:
        reconciliation_result = _validate_me7_reconciliation_v1(root)
    ...
    if (
        len(_AUTHORIZED_ME7_OVERLAY_FIELDS_V1) != 19
        or len(_AUTHORIZED_BUT_UNCHANGED_ME7_FIELDS_V1) != 7
        or len(_ACTUAL_CHANGED_ME7_FIELDS_V1) != 12
        or not _AUTHORIZED_BUT_UNCHANGED_ME7_FIELDS_V1.issubset(
            _AUTHORIZED_ME7_OVERLAY_FIELDS_V1
        )
    ):
        _fail("AUTHORIZED_ME7_OVERLAY_CONTRACT_INVALID")
    ...
    if changed_events != target:
        _fail("PREDECESSOR_DELTA_NOT_EXACT_ME7_EXACT3")
    matrix_by_event = {
        row["canonical_event_id"]: row
        for row in _validate_me7_matrix_rows_v1(matrix_rows)
    }
    ...
    if set(matrix_by_event) != target or set(facts) != target:
        _fail("ME7_SOURCE_MAPPING_NOT_EXACT3")
    for event_id in ME7_EXACT3_EVENT_IDS_V1:
        ...
        unchanged_authorized = {
            field
            for field in _AUTHORIZED_ME7_OVERLAY_FIELDS_V1
            if old[field] == new[field]
        }
        if changed_fields != _ACTUAL_CHANGED_ME7_FIELDS_V1:
            _fail("ME7_CHANGED_FIELD_SET_NOT_EXACT12:" + event_id)
        if unchanged_authorized != _AUTHORIZED_BUT_UNCHANGED_ME7_FIELDS_V1:
            _fail("ME7_AUTHORIZED_BUT_UNCHANGED_SET_NOT_EXACT7:" + event_id)
        expected = {
        ...
        if any(new[key] != value for key, value in expected.items()):
            _fail("ME7_REFRESHED_SEMANTICS_INVALID:" + event_id)
        if (
            ...
            _fail("ME7_MATRIX_CENSUS_SEMANTIC_MISMATCH:" + event_id)
    orthogonal = {
        row["canonical_event_id"]
        for row in rows
        if (
    ...
    if orthogonal != expected_orthogonal or len(orthogonal) != 23:
        _fail("ORTHOGONAL_POPULATION_NOT_HISTORICAL20_UNION_ME7_EXACT3")
    expected_distributions = (
        (
            ...
            Counter(
                {
                    **predecessor._EXPECTED_GLOBAL_STATUS_COUNTS_V1,
                    generic.CURRENTLY_UNREVIEWED: 160,
                    generic.COMPLETED_HUMAN_NEGATIVE: 81,
        ...
        (
            ...
            Counter({"POSITIVE": 167, "NOT_ESTABLISHED": 90, "UNRESOLVED": 743}),
        ),
        (
            "TASK",
            Counter(row["task_relevance_disposition"] for row in rows),
            Counter({"RELEVANT": 145, "NOT_RELEVANT": 113, "UNRESOLVED": 742}),
        ),
        (
            "TRAINING",
    ...
    boolean_counts = {
        "reactive_pair_sample_authoritative": 167,
    ...
    expected_blockers = {
        "chemistry_unresolved": {"all_1000": 743},
        "pair_authority_absent": {"all_1000": 833, "within_chemistry_positive": 0},
        "role_authority_absent": {"all_1000": 844, "within_chemistry_positive": 11},
        "human_training_exclusion": {"within_chemistry_positive": 76},
        "missing_split_authority": {
            "within_chemistry_positive": 126,
            "within_training_include": 43,
        },
        "missing_POST_training_authority": {
            "within_chemistry_positive": 150,
            "within_training_include": 51,
        },
        "missing_training_admission": {
            "within_chemistry_positive": 162,
            "within_training_include": 63,
        },
        "feature_semantics_pending": {"within_chemistry_positive": 167},
    }
    ...
    if (
        tensor["within_chemistry_positive"] != 126
        or tensor["within_training_include"] != 39
        or tensor["missing_source_composition"].get("ME7") != 3
        or summary["chemistry"]["positive_source_composition"].get("ME7") != 3
        or summary["reactive_pair"]["me7_sample_authority_contribution_count"] != 3
        or summary["reactive_pair"]["me7_training_target_contribution_count"] != 0
        ...
        _fail("SUMMARY_ME7_SOURCE_OR_ADMISSION_COMPOSITION_INVALID")
...
def _build_artifacts_from_computation_v1(
    ...
    manifest = {
        ...
        "predecessor_manifest_validation_binding": {
            "artifact_role": "PREDECESSOR_WITH_PYR_MANIFEST_VALIDATION_IDENTITY",
        ...
        "me7_reconciliation_artifact_validation_binding": {
            "artifact_role": "ME7_RECONCILIATION_ARTIFACT_VALIDATION_IDENTITY",
            "path": ME7_RECONCILIATION_ARTIFACT_RELATIVE.as_posix(),
            "path_namespace": "repository_relative",
            "byte_count": _ME7_RECONCILIATION_ARTIFACT_SPEC_V1[0],
            "sha256": _ME7_RECONCILIATION_ARTIFACT_SPEC_V1[1],
            "expected_executable": _ME7_RECONCILIATION_ARTIFACT_SPEC_V1[2],
        ...
        "refresh_contract": {
            ...
            "actual_changed_field_count_per_me7_row": 12,
            "semantic_source_binding_count": 198,
            "predecessor_semantic_source_binding_count": 192,
            ...
            "task_negative_chemistry_positive_population_count": 23,
            "historical_orthogonal_population_count": 20,
            "me7_orthogonal_population_count": 3,
            "census_refreshed": True,
            "census_refresh_performed": True,
            "queue_refreshed": False,
            "next_review_started": False,
            "review_state_created_by_this_refresh": False,
            "me7_review_state_exists": True,
            "me7_review_completed": True,
            "me7_reconciliation_consumed": True,
            "new_human_authority_created": False,
            "new_scientific_authority_created": False,
...
def build_covapie_cumulative1000_current_global_readiness_artifacts_with_me7_v1(
    repo_root: Path,
) -> dict[str, bytes]:
    ...
    root = Path(repo_root).resolve()
    computation = compute_covapie_cumulative1000_current_global_readiness_census_with_me7_v1(
        root
    )
...
def materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_me7_v1(
    repo_root: Path, output_directory: Path | None = None
) -> dict[str, bytes]:
    """Write only the authorized Exact3 files under the exact ME7 output root."""
    ...
    artifacts = build_covapie_cumulative1000_current_global_readiness_artifacts_with_me7_v1(
        root
    )
