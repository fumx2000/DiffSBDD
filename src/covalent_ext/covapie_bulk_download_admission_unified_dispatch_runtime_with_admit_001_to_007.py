"""CovaPIE unified single-rule runtime for ADMIT_001 through ADMIT_007.

The Exact6 public types and handlers are reused by identity.  This successor
adds only the ADMIT_007 adapter.  Public dispatch is pure in memory; the
...
from covalent_ext import (
    covapie_bulk_download_admission_admit_007_rule_logic_interface as admit007,
)
from covalent_ext import (
...
PROJECT = "CovaPIE"
STEP = "unified dispatch runtime with ADMIT_001 to ADMIT_007 v1"
STAGE = (
    "covapie_bulk_download_admission_unified_dispatch_runtime_"
    "with_admit_001_to_007_v1"
)
EXPECTED_BASE_COMMIT = "ea6678caef867bf2c4c0901bc4066f4d5e4f7520"
EXPECTED_BASE_SUBJECT = "add CovaPIE ADMIT_007 unified adapter contract design v1"
...
# Identity re-exports: the Exact6 predecessor remains the sole type authority.
...
CALLABLE_DISCOVERED_RULE_IDS = tuple(
    f"ADMIT_{index:03d}" for index in range(1, 8)
)
ADAPTER_READY_RULE_IDS = CALLABLE_DISCOVERED_RULE_IDS
LEGACY_ADAPTER_NOT_READY_RULE_IDS: tuple[str, ...] = ()
ADMIT_007_CANDIDATE_FIELDS = ("covalent_event_evidence_source",)
ADMIT_007_CONTEXT_ITEMS = ("allowed_covalent_evidence_classes",)
...
def _admit007_context_failure(reason: str) -> NoReturn:
...
def _admit007_adapter_failure(reason: str) -> NoReturn:
    _raise_dispatch_error(
...
def _admit007_candidate_invalid(reason: str) -> UnifiedAdmissionRuleEvaluation:
    return UnifiedAdmissionRuleEvaluation(
        schema_version=RESULT_SCHEMA_VERSION,
        admission_rule_id="ADMIT_007",
        admission_rule_name=RULE_NAMES["ADMIT_007"],
        outcome="invalid",
        passed=False,
        ...
        consumed_candidate_fields=ADMIT_007_CANDIDATE_FIELDS,
        consumed_context_items=ADMIT_007_CONTEXT_ITEMS,
        evaluator_io_used=False,
        adapter_id=ADAPTER_IDS["ADMIT_007"],
    )


def _prevalidate_admit007_source(
    source: object,
) -> admit007.Admit007EvaluationResult:
    """Require the committed exact type and reconstruct all Exact10 invariants."""
    if type(source) is not admit007.Admit007EvaluationResult:
        _admit007_adapter_failure("ADMIT_007_UNIFIED_ADAPTER_SOURCE_TYPE_INVALID")
    assert type(source) is admit007.Admit007EvaluationResult
    try:
        storage = vars(source)
    except TypeError:
    ...
    if type(storage) is not dict or tuple(storage) != admit007.RESULT_FIELDS:
        _admit007_adapter_failure(
            "ADMIT_007_UNIFIED_ADAPTER_SOURCE_INVARIANT_INVALID"
        )
    if (
        tuple(field.name for field in fields(admit007.Admit007EvaluationResult))
        != admit007.RESULT_FIELDS
    ):
        _admit007_adapter_failure(
            "ADMIT_007_UNIFIED_ADAPTER_SOURCE_INVARIANT_INVALID"
        )
    try:
        ordered_values = tuple(
            getattr(source, name) for name in admit007.RESULT_FIELDS
        )
        reconstructed = admit007.Admit007EvaluationResult(*ordered_values)
...
def _expected_admit007_from_oracle(
    ...
    try:
        ...
        validated = (
            ()
            if canonical == ""
            else ((ADMIT_007_CANDIDATE_FIELDS[0], canonical),)
        )
        expected = admit007.Admit007EvaluationResult(
            admission_rule_id="ADMIT_007",
...
def _validate_admit007_oracle_equivalence(
    source: admit007.Admit007EvaluationResult,
    expected: admit007.Admit007EvaluationResult,
) -> None:
    if type(expected) is not admit007.Admit007EvaluationResult or source != expected:
        _admit007_adapter_failure(
            "ADMIT_007_UNIFIED_ADAPTER_SOURCE_INVARIANT_INVALID"
        )


def _project_admit007_exact13(
    source: admit007.Admit007EvaluationResult,
...
def _evaluate_registered_admit_007(
    ...
    *,
    ...
    if batch_context is not None:
        _admit007_context_failure("ADMIT_007_BATCH_CONTEXT_MUST_BE_NONE")
    if not isinstance(evaluation_context, Mapping):
        _admit007_context_failure(
            "ADMIT_007_EVALUATION_CONTEXT_MAPPING_REQUIRED"
        )
    try:
    ...
    if not isinstance(candidate_record, Mapping):
        return _admit007_candidate_invalid(
            "ADMIT_007_CANDIDATE_RECORD_MAPPING_INVALID"
        )
    try:
        scalar_object = candidate_record["covalent_event_evidence_source"]
    except KeyError:
        return _admit007_candidate_invalid("covalent_event_evidence_missing")
    if scalar_object is None or (
        type(scalar_object) is str and scalar_object == ""
    ):
        return _admit007_candidate_invalid("covalent_event_evidence_missing")

    source = admit007.evaluate_admit_007(
        scalar_object,
        allowed_classes_object,
    )
    validated_source = _prevalidate_admit007_source(source)
    expected = _expected_admit007_from_oracle(
        scalar_object,
        allowed_classes_object,
    )
    _validate_admit007_oracle_equivalence(validated_source, expected)
    return _project_admit007_exact13(validated_source)
...
EVALUATOR_REGISTRY = MappingProxyType(
    {
        ...
        "ADMIT_007": _evaluate_registered_admit_007,
    }
)
...
STANDALONE_ROOT = Path(
    "data/derived/covalent_small/"
    "covapie_bulk_download_admission_admit_007_rule_logic_interface_v1"
)
DESIGN_ROOT = Path(
    "data/derived/covalent_small/"
    "covapie_bulk_download_admission_admit_007_unified_adapter_contract_design_gate_v1"
)
UNIFIED_ROOT = Path(
    "data/derived/covalent_small/"
...
SOURCE_PATHS = tuple(
    ...
    for value in (
        ...
        str(EXACT6_ROOT / "covapie_admit_001_to_006_runtime_manifest.json"),
        str(EXACT6_ROOT / "covapie_admit_001_to_006_runtime_contract.csv"),
        str(EXACT6_ROOT / "covapie_admit_001_to_006_runtime_truth_matrix.csv"),
        str(EXACT6_ROOT / "covapie_admit_001_to_006_registry_routing_and_oracle_audit.csv"),
        str(EXACT6_ROOT / "covapie_admit_001_to_006_runtime_safety_audit.csv"),
        "src/covalent_ext/covapie_bulk_download_admission_admit_007_rule_logic_interface.py",
        str(STANDALONE_ROOT / "covapie_admit_007_rule_logic_interface_manifest.json"),
        "src/covalent_ext/covapie_bulk_download_admission_admit_007_unified_adapter_contract_design_gate.py",
        str(DESIGN_ROOT / "covapie_admit_007_unified_adapter_contract_manifest.json"),
        str(DESIGN_ROOT / "covapie_admit_007_unified_adapter_contract.csv"),
        str(DESIGN_ROOT / "covapie_admit_007_candidate_projection_and_context_routing_matrix.csv"),
        str(DESIGN_ROOT / "covapie_admit_007_unified_result_projection_truth_matrix.csv"),
        str(DESIGN_ROOT / "covapie_admit_007_unified_adapter_safety_audit.csv"),
        str(DESIGN_ROOT / "covapie_admit_007_unified_adapter_issue_readiness_inventory.csv"),
...
EXACT6_SOURCE_PATH = SOURCE_PATHS[0]
EXACT6_MANIFEST_PATH = SOURCE_PATHS[1]
EXACT6_CONTRACT_PATH = SOURCE_PATHS[2]
EXACT6_TRUTH_PATH = SOURCE_PATHS[3]
EXACT6_REGISTRY_PATH = SOURCE_PATHS[4]
EXACT6_SAFETY_PATH = SOURCE_PATHS[5]
...
CONTRACT_FILENAME = "covapie_admit_001_to_007_runtime_contract.csv"
TRUTH_FILENAME = "covapie_admit_001_to_007_runtime_truth_matrix.csv"
REGISTRY_FILENAME = "covapie_admit_001_to_007_registry_routing_and_oracle_audit.csv"
SAFETY_FILENAME = "covapie_admit_001_to_007_runtime_safety_audit.csv"
ISSUE_FILENAME = "covapie_admit_001_to_007_runtime_issue_inventory.csv"
MANIFEST_FILENAME = "covapie_admit_001_to_007_runtime_manifest.json"
CSV_OUTPUTS = (
    CONTRACT_FILENAME,
    TRUTH_FILENAME,
    REGISTRY_FILENAME,
...
def _validate_predecessors(
    ...
    exact6_manifest = _json_document(snapshot, EXACT6_MANIFEST_PATH)
    exact6_contract = _csv_document(snapshot, EXACT6_CONTRACT_PATH)
    exact6_truth = _csv_document(snapshot, EXACT6_TRUTH_PATH)
    exact6_registry = _csv_document(snapshot, EXACT6_REGISTRY_PATH)
    exact6_safety = _csv_document(snapshot, EXACT6_SAFETY_PATH)
    ...
    exact6_tree = _ast_document(snapshot, EXACT6_SOURCE_PATH)
    ...
    route007 = tuple(
        row for row in unified_routing.rows if row["admission_rule_id"] == "ADMIT_007"
    )
    design_issue_map = {row["issue_id"]: row for row in design_issues.rows}
    checks = (
        exact6_manifest.get("all_checks_passed") is True,
        exact6_manifest.get("registered_rule_ids")
        == list(KNOWN_RULE_IDS[:6]),
        exact6_manifest.get("registered_rule_count") == 6,
        exact6_manifest.get("ready_for_training") is False,
        len(exact6_contract.rows) == 56,
        len(exact6_truth.rows) == 105,
        len(exact6_registry.rows) == 15,
        len(exact6_safety.rows) == 36,
        _literal_registry_keys(exact6_tree) == KNOWN_RULE_IDS[:6],
        standalone_manifest.get("all_checks_passed") is True,
        standalone_manifest.get("result_fields") == list(admit007.RESULT_FIELDS),
        standalone_manifest.get("truth_matrix_row_count") == 37,
        "evaluate_admit_007" in _top_level_function_names(standalone_tree),
        design_manifest.get("all_checks_passed") is True,
        design_manifest.get("future_registered_rule_order")
        == list(KNOWN_RULE_IDS[:7]),
        design_manifest.get("admit_007_unified_adapter_implemented") is False,
        ...
        len(route007) == 1,
        route007[0]["candidate_field_dependencies"]
        == ADMIT_007_CANDIDATE_FIELDS[0],
        route007[0]["evaluation_context_dependencies"]
        == ADMIT_007_CONTEXT_ITEMS[0],
    ...
    return {
        "exact6_manifest": exact6_manifest,
...
def _invoke_admit007_counted(
    ...
    *,
    ...
    formal_real = admit007.evaluate_admit_007
    oracle_real = admit007_oracle.classify_admit_006_admit_007_evidence_design
    ...
    admit007.evaluate_admit_007 = formal_wrapper  # type: ignore[assignment]
    admit007_oracle.classify_admit_006_admit_007_evidence_design = oracle_wrapper  # type: ignore[assignment]
    ...
    try:
        ...
        admit007.evaluate_admit_007 = formal_real  # type: ignore[assignment]
        admit007_oracle.classify_admit_006_admit_007_evidence_design = oracle_real  # type: ignore[assignment]
    return (
        observed,
        len(formal_args),
        len(oracle_args),
...
def _synthetic_exact37_definitions() -> tuple[tuple[str, object, object], ...]:
    exact = admit007.ALLOWED_COVALENT_EVIDENCE_CLASSES
    scalar_cases: tuple[tuple[str, object], ...] = (
        ("canonical_structure_bond", admit007.CANONICAL_ENUM_MEMBERS[0]),
        ("canonical_curated_annotation", admit007.CANONICAL_ENUM_MEMBERS[1]),
        ("canonical_distance_only", admit007.CANONICAL_ENUM_MEMBERS[2]),
        ("type_none", None),
        ("type_int", 7),
        ("type_bool", True),
    ...
    context_cases: tuple[tuple[str, object], ...] = (
        ...
        ("context_distance_only", (*exact, admit007.CANONICAL_ENUM_MEMBERS[2])),
...
def _predecessor_parity_definitions() -> tuple[dict[str, Any], ...]:
    return (
        *predecessor._predecessor_parity_definitions(),
        {
            "id": "P6_A006_EXPLICIT_PASS",
            "rule": "ADMIT_006",
            "candidate": {
                "covalent_event_evidence_source":
                predecessor.admit006.CANONICAL_ENUM_MEMBERS[0],
            },
            "kwargs": {
                "evaluation_context": {
                    "allowed_covalent_evidence_classes":
                    predecessor.admit006.ALLOWED_COVALENT_EVIDENCE_CLASSES,
                }
            },
        },
        {
            "id": "P6_A006_DISTANCE_BLOCKED",
            "rule": "ADMIT_006",
            "candidate": {
                "covalent_event_evidence_source":
                predecessor.admit006.CANONICAL_ENUM_MEMBERS[2],
            },
            "kwargs": {
                "evaluation_context": {
                    "allowed_covalent_evidence_classes":
...
def _fresh_unsafe_source(
    ...
    value = admit007.evaluate_admit_007(
        admit007.CANONICAL_ENUM_MEMBERS[0],
        admit007.ALLOWED_COVALENT_EVIDENCE_CLASSES,
    )
...
def _truth_rows() -> list[dict[str, str]]:
    ...
    missing_context = _SingleLookupMapping(
        ADMIT_007_CONTEXT_ITEMS[0], object(), present=False
    )
    context_cases: tuple[tuple[str, dict[str, object], str], ...] = (
        ...
        (
            "A007_CONTEXT_DOWNLOAD",
            {
                "evaluation_context": {
                    ADMIT_007_CONTEXT_ITEMS[0]: admit007.ALLOWED_COVALENT_EVIDENCE_CLASSES
                },
    ...
    missing_candidate = _SingleLookupMapping(
        ADMIT_007_CANDIDATE_FIELDS[0], object(), present=False
    )
    mapping_subclass_candidate = _SingleLookupMapping(
        ADMIT_007_CANDIDATE_FIELDS[0], admit007.CANONICAL_ENUM_MEMBERS[0]
    )
    identity_scalar = object()
    identity_context = object()
    identity_candidate = _SingleLookupMapping(
        ADMIT_007_CANDIDATE_FIELDS[0], identity_scalar
    )
    identity_evaluation = _SingleLookupMapping(
        ADMIT_007_CONTEXT_ITEMS[0], identity_context
    )
    candidate_cases: tuple[
        ...
        (
            "A007_CANDIDATE_NONE",
            {ADMIT_007_CANDIDATE_FIELDS[0]: None},
            admit007.ALLOWED_COVALENT_EVIDENCE_CLASSES,
            "invalid",
            "covalent_event_evidence_missing",
            0,
        ...
        (
            "A007_CANDIDATE_EMPTY_SUBCLASS",
            {ADMIT_007_CANDIDATE_FIELDS[0]: EmptyStringSubclass("")},
            admit007.ALLOWED_COVALENT_EVIDENCE_CLASSES,
            "invalid",
            admit007.SCALAR_REASONS[0],
            1,
            1,
            None,
        ),
        (
            "A007_CANDIDATE_WHITESPACE",
            {ADMIT_007_CANDIDATE_FIELDS[0]: " "},
            admit007.ALLOWED_COVALENT_EVIDENCE_CLASSES,
            "invalid",
            admit007.SCALAR_REASONS[3],
            1,
            1,
            None,
        ),
        (
            "A007_CANDIDATE_MALFORMED",
            {ADMIT_007_CANDIDATE_FIELDS[0]: 7},
            admit007.ALLOWED_COVALENT_EVIDENCE_CLASSES,
            "invalid",
            admit007.SCALAR_REASONS[0],
            1,
        ...
        (
            "A007_CANDIDATE_DISTANCE",
            {ADMIT_007_CANDIDATE_FIELDS[0]: "distance_only_inference"},
            admit007.ALLOWED_COVALENT_EVIDENCE_CLASSES,
            "blocked",
            admit007.BLOCKED_REASON,
            1,
            1,
            None,
        ),
        (
            "A007_CANDIDATE_MAPPING_SUBCLASS",
    ...
    for (
        ...
        observed, formal, oracle, formal_args, oracle_args = _invoke_admit007_counted(
        ...
        if expected_formal:
            scalar = (
                candidate.value
                if isinstance(candidate, _SingleLookupMapping)
                else candidate[ADMIT_007_CANDIDATE_FIELDS[0]]
            )
            identities = (
    ...
    for index, (name, scalar, context) in enumerate(
        ...
        candidate = {ADMIT_007_CANDIDATE_FIELDS[0]: scalar}
        observed, formal, oracle, _, _ = _invoke_admit007_counted(
            candidate, context
        )
        observed_value, observed_reason = _label(observed)
        if scalar is None or (type(scalar) is str and scalar == ""):
            ...
            expected = admit007.evaluate_admit_007(scalar, context)
    ...
    valid = admit007.evaluate_admit_007(
        admit007.CANONICAL_ENUM_MEMBERS[0],
        admit007.ALLOWED_COVALENT_EVIDENCE_CLASSES,
    )

    class SourceSubclass(admit007.Admit007EvaluationResult):
        pass

    source_subclass = object.__new__(SourceSubclass)
    for name in admit007.RESULT_FIELDS:
        object.__setattr__(source_subclass, name, getattr(valid, name))

    missing_shape = admit007.evaluate_admit_007(
        admit007.CANONICAL_ENUM_MEMBERS[0],
        admit007.ALLOWED_COVALENT_EVIDENCE_CLASSES,
    )
    object.__delattr__(missing_shape, admit007.RESULT_FIELDS[-1])
    extra_shape = admit007.evaluate_admit_007(
        admit007.CANONICAL_ENUM_MEMBERS[0],
        admit007.ALLOWED_COVALENT_EVIDENCE_CLASSES,
    )
    ...
    unsafe_sources: list[tuple[str, object, str]] = [
        (
            "SOURCE_WRONG_TYPE",
            object(),
            "ADMIT_007_UNIFIED_ADAPTER_SOURCE_TYPE_INVALID",
        ),
        (
            "SOURCE_SUBCLASS",
            source_subclass,
    ...
    for case_id, field_name, replacement in (
        ...
        ("SOURCE_REASON", "reason", admit007.SCALAR_REASONS[4]),
        (
            "SOURCE_CANONICAL",
            "canonical_covalent_event_evidence_source",
            admit007.CANONICAL_ENUM_MEMBERS[2],
        ),
    ...
    mismatch_oracle = admit007_oracle.classify_admit_006_admit_007_evidence_design(
        admit007.CANONICAL_ENUM_MEMBERS[1],
        admit007.ALLOWED_COVALENT_EVIDENCE_CLASSES,
    )
    observed, formal, oracle, _, _ = _invoke_admit007_counted(
        candidate,
    ...
    for case_id, scalar, context, expected_value, expected_reason in projection_cases:
        ...
        projection_ok = (
            ...
            and (
                ...
                == ((ADMIT_007_CANDIDATE_FIELDS[0], scalar),)
            )
        )
        rows.append(
            _truth_row(
    ...
    for case_id, candidate_value, expected_reason in (
        ...
        observed, formal, oracle, _, _ = _invoke_admit007_counted(
            candidate_value, admit007.ALLOWED_COVALENT_EVIDENCE_CLASSES
        )
        observed_value, observed_reason = _label(observed)
        projection_ok = (
            type(observed) is UnifiedAdmissionRuleEvaluation
            and observed.normalized_values == observed.validated_candidate_fields == ()
            and observed.consumed_context_items == ADMIT_007_CONTEXT_ITEMS
        )
        rows.append(
            _truth_row(
                case_id,
...
def _contract_definitions() -> tuple[tuple[str, str, str, str], ...]:
    definitions = (
        ...
        ("A007_001", "admit007_identity", "rule name", RULE_NAMES["ADMIT_007"]),
        ("A007_002", "admit007_identity", "adapter ID", ADAPTER_IDS["ADMIT_007"]),
        ...
        ("CANDIDATE_001", "candidate_projection", "field", ADMIT_007_CANDIDATE_FIELDS[0]),
...
def _registry_rows() -> list[dict[str, str]]:
    ...
    for rule_id in KNOWN_RULE_IDS:
        ...
        predecessor_reused = rule_id in KNOWN_RULE_IDS[:6]
        successor_implemented = rule_id == "ADMIT_007"
        rows.append(
            {
                "admission_rule_id": rule_id,
    ...
    *,
    ...
    try:
        ...
        checks = (
            ...
            EVALUATOR_REGISTRY["ADMIT_007"]
            is _evaluate_registered_admit_007,
            UnifiedAdmissionRuleEvaluation
...
def run_covapie_bulk_download_admission_unified_dispatch_runtime_with_admit_001_to_007_v1(
    output_root: Path = DEFAULT_OUTPUT_ROOT,
) -> dict[str, Any]:
    state = build_runtime_state()
    if state["all_checks_passed"] is not True:
        raise RuntimeError(
            "unified ADMIT_001 to ADMIT_007 runtime failed closed: "
...
if __name__ == "__main__":
    materialized = run_covapie_bulk_download_admission_unified_dispatch_runtime_with_admit_001_to_007_v1()
