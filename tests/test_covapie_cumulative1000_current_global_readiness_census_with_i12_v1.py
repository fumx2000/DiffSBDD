...
ERROR = subject.Cumulative1000CurrentGlobalReadinessCensusWithI12Error
CHECKER_SPEC = importlib.util.spec_from_file_location(
    "check_covapie_cumulative1000_current_global_readiness_census_with_i12_v1",
    REPO / "scripts/check_covapie_cumulative1000_current_global_readiness_census_with_i12_v1.py",
)
...
def frozen():
    return predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_2a2_v1(
        REPO
    )
...
@pytest.fixture(scope="session")
def computation():
    return subject.compute_covapie_cumulative1000_current_global_readiness_census_with_i12_v1(
        REPO
    )
...
@pytest.fixture(scope="session")
def matrix_rows():
    payload = (REPO / subject.I12_EVENT_MATRIX_RELATIVE).read_bytes()
    return [
        dict(row)
...
def _i12_predicate(row: dict[str, str]) -> bool:
    return row["canonical_event_id"] == subject.I12_EXACT4_EVENT_IDS_V1[0]
...
def test_frozen_predecessor_and_i12_bindings_are_exact() -> None:
...
def test_happy_path_source_derived_exact1000_and_validator(computation, frozen) -> None:
    ...
    assert subject.validate_covapie_cumulative1000_current_global_readiness_census_with_i12_v1(
...
def test_compute_architecture_call_counts(monkeypatch) -> None:
    ...
    original_predecessor = (
        subject.predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_2a2_v1
    )
    original_reconciliation = (
        subject.i12_reconciliation.reconcile_real_completed_human_decisions_with_i12_v1
    )
    original_build = subject.i12_ingestion.build_artifacts_v1
    original_validate = subject.i12_ingestion.validate_completed_decision_projection_v1
    ...
    monkeypatch.setattr(
        subject.i12_reconciliation,
        "reconcile_real_completed_human_decisions_with_i12_v1",
        wrapped_reconciliation,
    )
    monkeypatch.setattr(subject.i12_ingestion, "build_artifacts_v1", wrapped_build)
    monkeypatch.setattr(
        subject.i12_ingestion,
        "validate_completed_decision_projection_v1",
        wrapped_validate,
    )
    subject.compute_covapie_cumulative1000_current_global_readiness_census_with_i12_v1(
        REPO
    )
    assert calls == {
        "predecessor": 1,
...
def test_exact4_identity_ranks_pdbs_and_cysteines(matrix_rows) -> None:
    validated = subject._validate_i12_matrix_rows_v1(matrix_rows)
    assert tuple(row["canonical_event_id"] for row in validated) == (
        subject.I12_EXACT4_EVENT_IDS_V1
    )
    ...
    assert {row["cys_residue_id"] for row in validated} == {
        "CYS:145-", "CYS:144-",
    }


def test_i12_matrix_d1_d5_role_chemical_and_exact5_semantics(matrix_rows) -> None:
    rows = subject._validate_i12_matrix_rows_v1(matrix_rows)
    for row in rows:
        ...
        assert row["future_training_candidate_is_training_admission"] == "false"
        ...
        assert row["future_training_candidate_derived_by_ingestion"] == "true"
...
def test_exact4_delta_and_996_dict_equal(computation, frozen) -> None:
    ...
    assert changed == set(subject.I12_EXACT4_EVENT_IDS_V1)
    ...
    assert all(
        ...
        } == subject._AUTHORIZED_I12_OVERLAY_FIELDS_V1
        for event_id in changed
    )
    assert len(subject._AUTHORIZED_I12_OVERLAY_FIELDS_V1) == 18
    assert "human_training_excluded" not in subject._AUTHORIZED_I12_OVERLAY_FIELDS_V1
    assert "training_use_include" in subject._AUTHORIZED_I12_OVERLAY_FIELDS_V1
    assert "future_training_admission_candidate" in subject._AUTHORIZED_I12_OVERLAY_FIELDS_V1


def test_predecessor_exact5_state_is_proven(frozen) -> None:
    subject._assert_predecessor_i12_state_v1(frozen)
    rows = [
        row for row in frozen.rows
        if row["canonical_event_id"] in set(subject.I12_EXACT4_EVENT_IDS_V1)
    ]
...
def test_i12_refreshed_semantics_and_training_prohibitions(computation) -> None:
    rows = [
    ...
    assert all(
        ...
        and row["human_review_authority_source"] == subject.I12_HUMAN_DECISION_SOURCE
        and row["chemistry_disposition"] == "POSITIVE"
        and row["chemistry_authority_source"] == subject.I12_EVENT_MATRIX_SOURCE
        and row["positive_authority_source"] == subject.I12_EVENT_MATRIX_SOURCE
...
def test_i12_structural_fields_are_unchanged(computation, frozen) -> None:
    before = {row["canonical_event_id"]: row for row in frozen.rows}
    after = {row["canonical_event_id"]: row for row in computation.rows}
    assert all(
        before[event_id][field] == after[event_id][field]
        for event_id in subject.I12_EXACT4_EVENT_IDS_V1
        for field in subject._STRUCTURAL_IDENTITY_FIELDS_V1
    )
    for event_id in subject.I12_EXACT4_EVENT_IDS_V1:
        assert all(
            after[event_id][field] == value
            for field, value in subject._EXPECTED_I12_STRUCTURAL_CELLS_V1.items()
        )
...
def test_exact_set_algebra_keeps_include_future_admission_runtime(computation, frozen) -> None:
    ...
    i12 = set(subject.I12_EXACT4_EVENT_IDS_V1)
    assert after["chemistry_positive"] == before["chemistry_positive"] | i12
    assert after["chemistry_unresolved"] == before["chemistry_unresolved"] - i12
    assert after["task_relevant"] == before["task_relevant"] | i12
    assert after["task_unresolved"] == before["task_unresolved"] - i12
    assert after["training_include"] == before["training_include"] | i12
    assert after["training_exclude"] == before["training_exclude"]
    assert after["future_candidate"] == before["future_candidate"] | i12
    assert after["training_unresolved"] == before["training_unresolved"] - i12
    for key in (
...
def test_global_chemistry_task_training_and_status_counts(computation) -> None:
    ...
    ) == Counter({"RELEVANT": 117, "NOT_RELEVANT": 86, "UNRESOLVED": 797})
...
def test_pair_role_exact5_geometry_and_training_counts(computation) -> None:
    ...
    assert summary["reactive_pair"]["sample_level_authoritative_pair_count"] == 116
    assert summary["reactive_pair"]["published_model_bound_target_constructible_count"] == 41
    assert summary["reactive_pair"]["i12_sample_authority_contribution_count"] == 4
    assert summary["reactive_pair"]["i12_model_bound_target_contribution_count"] == 0
    assert summary["role"]["role_profile_counts"] == {
        "STRICT_LINKER_PRESENT_V1": 52,
        "DIRECT_ATTACHMENT_OPTIONAL_LINKER_V1": 64,
        "other": 0,
    }
    assert summary["role"]["unknown_role_row_count"] == 884
    assert [
    ...
    assert summary["training_stage"]["future_training_admission_candidate_count"] == 31
...
def test_blocker_counts_are_exact_and_nonexclusive(computation) -> None:
    ...
    assert blockers["human_training_exclusion"] == {"within_positive_116": 68}
    ...
    assert blockers["feature_semantics_pending"] == {"within_positive_116": 116}
    assert blockers["pair_authority_absent"] == {
        "all_1000": 884, "within_positive_116": 0,
    }
...
def test_reconciliation_and_dynamic_pending_queue_head(computation) -> None:
    result = reconciliation.reconcile_real_completed_human_decisions_with_i12_v1(REPO)
    ...
    assert expected[0]["review_unit_id"] == "COVAPIE_BULK_REVIEW_UNIT_80FE8023FD901B01"
    assert expected[0]["ligand_component_ids"] == ["1N0"]
    assert expected[0]["pdb_ids"] == ["4JWS", "4JWU", "4JX1"]
    assert expected[0]["rank"] == 1
    assert expected[0]["raw_priority_rank"] == 18
    assert expected[0]["event_count"] == 4
    assert all(item["ligand_component_ids"] != ["I12"] for item in expected)
...
def test_authority_boundary_and_non_actions(computation) -> None:
    ...
    assert boundary["I12_CENSUS_SOURCE_BINDING_V2_CLEAN_FROM_BIRTH"] is True
    assert boundary["QUEUE_REFRESH"] is False
    assert boundary["next_priority_review_ligand"] == "1N0"
    assert boundary["next_priority_review_current_pending_rank"] == 1
    assert boundary["next_priority_review_raw_priority_rank"] == 18
    ...
    assert boundary["I12_REVIEW_COMPLETED"] is True
...
def test_projection_digests_and_semantic_bindings(computation, frozen) -> None:
    ...
    assert len(computation.semantic_source_bindings) == 114
...
def test_no_direct_formal_parse_binding_or_exact_source_mode_authority(computation) -> None:
    ...
    for binding in computation.semantic_source_bindings:
        if binding["artifact_role"] in additive_roles:
            ...
            assert set(binding) == legacy_keys
...
def test_exact_posix_mode_cannot_be_added_to_semantic_binding(computation, frozen) -> None:
    ...
    with pytest.raises(ERROR, match="SEMANTIC_SOURCE_BINDING_SCHEMA_INVALID"):
        subject.validate_covapie_cumulative1000_current_global_readiness_census_with_i12_v1(
...
def test_materialization_exact3_rematerialization_is_idempotent(tmp_path: Path) -> None:
    ...
    first = subject.materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_i12_v1(
        REPO, destination
    )
    first_disk = {path.name: path.read_bytes() for path in destination.iterdir()}
    second = subject.materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_i12_v1(
        REPO, destination
    )
...
def test_candidate_untracked_lifecycle_is_accepted() -> None:
    assert checker._classify_repository_lifecycle_v1(
        ...
        behind=0,
        head_parent=None,
        baseline_to_head_commit_count=0,
        baseline_to_head_changed_paths=(),
    ) == "CANDIDATE_UNTRACKED"


def test_committed_unpushed_tracked_clean_lifecycle_is_accepted() -> None:
    assert checker._classify_repository_lifecycle_v1(
        ...
        ahead=1,
        behind=0,
        head_parent=checker.BASELINE_COMMIT,
        baseline_to_head_commit_count=1,
...
def test_materialized_exact7_inventory_and_independent_checker() -> None:
    ...
    assert result["refreshed_positive_count"] == 116
    assert result["task_relevant_count"] == 117
    assert result["training_include_count"] == 48
    assert result["training_exclude_count"] == 68
    assert result["future_candidate_count"] == 31
    assert result["formal_training_admitted_count"] == 5
    assert result["current_runtime_model_usable_count"] == 17
    assert result["semantic_source_binding_count"] == 114
...
def test_predecessor_i12_row_not_unreviewed_rejected(frozen) -> None:
    rows = [dict(row) for row in frozen.rows]
    index = next(
        i for i, row in enumerate(rows)
        if row["canonical_event_id"] == subject.I12_EXACT4_EVENT_IDS_V1[0]
    )
    rows[index]["current_review_status"] = "CURRENTLY_IN_PROGRESS"
    with pytest.raises(ERROR):
        subject._assert_predecessor_i12_state_v1(replace(frozen, rows=tuple(rows)))


def test_predecessor_i12_wrong_review_unit_rejected(frozen) -> None:
    rows = [dict(row) for row in frozen.rows]
    index = next(
        i for i, row in enumerate(rows)
        if row["canonical_event_id"] == subject.I12_EXACT4_EVENT_IDS_V1[0]
...
def test_predecessor_i12_already_authoritative_rejected(frozen, field, value) -> None:
    rows = [dict(row) for row in frozen.rows]
    index = next(
        i for i, row in enumerate(rows)
        if row["canonical_event_id"] == subject.I12_EXACT4_EVENT_IDS_V1[0]
...
def test_non_i12_output_row_change_rejected(computation, frozen) -> None:
    mutated = _mutate_row(
        computation,
        lambda row: row["canonical_event_id"] not in set(subject.I12_EXACT4_EVENT_IDS_V1),
...
def test_reconciliation_contract_drift_rejected(monkeypatch) -> None:
    ...
    with pytest.raises(ERROR):
        subject._validate_i12_reconciliation_v1(REPO)
...
def test_matrix_must_be_source_derived(monkeypatch) -> None:
    ...
    monkeypatch.setattr(subject.i12_ingestion, "build_artifacts_v1", lambda root: drifted)
    with pytest.raises(ERROR):
        subject._load_and_validate_i12_event_matrix_v1(REPO)
...
def test_matrix_event_coverage_mutations_rejected(matrix_rows, mutation) -> None:
    ...
    if mutation == "missing":
        ...
        extra = dict(rows[-1])
        extra["canonical_event_id"] = "EXTRA_I12_EVENT"
        extra["scaleup_rank"] = "674"
        rows.append(extra)
    with pytest.raises(ERROR):
        subject._validate_i12_matrix_rows_v1(rows)
...
def test_refreshed_row_semantic_mutations_rejected(computation, frozen, updates) -> None:
    mutated = _mutate_row(computation, _i12_predicate, **updates)
    with pytest.raises(ERROR):
        subject.validate_covapie_cumulative1000_current_global_readiness_census_with_i12_v1(
            mutated, predecessor_computation=frozen
        )
...
def test_summary_mutation_rejected_by_independent_checker(computation) -> None:
    summary = deepcopy(computation.summary)
    summary["blockers"]["missing_tensor_integration"]["within_positive_116"] = 74
    with pytest.raises(ValueError):
        checker.independently_verify_counts_v1(list(computation.rows), summary)


@pytest.mark.parametrize("stale_key", ["within_positive_112", "within_include_44"])
...
def test_manifest_binding_mutation_rejected(computation) -> None:
    artifacts = subject.build_covapie_cumulative1000_current_global_readiness_artifacts_with_i12_v1(
        REPO
    )
