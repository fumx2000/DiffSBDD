...
from covalent_ext import covapie_split_leakage_qa_gate as legacy


LEGACY_ROOT = Path("data/derived/covalent_small/covapie_split_leakage_qa_gate_v0")
...
CHECK_SCRIPT = Path("scripts/check_covapie_split_leakage_qa_gate_v0.py")
...
EXPECTED_STAGE = "covapie_split_leakage_qa_gate_v0"
...
EXPECTED_SUCCESS_LINE = "covapie_split_leakage_qa_gate_v0_retirement_policy_passed"
OLD_SUCCESS_LINE = "covapie_split_leakage_qa_gate_v0_passed"
...
def test_guarded_entrypoint_inventory_is_explicit_and_complete() -> None:
    ...
    assert legacy.GUARDED_ENTRYPOINTS[-1] == "run_covapie_split_leakage_qa_gate_v0"
...
def test_main_legacy_producer_fails_closed() -> None:
    with pytest.raises(retirement.LegacyStageRetiredError):
        _invoke("run_covapie_split_leakage_qa_gate_v0")
...
def test_module_import_has_no_artifact_io() -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-c", "import covalent_ext.covapie_split_leakage_qa_gate"],
        cwd=REPO_ROOT,
        env=environment,
        text=True,
        stdout=subprocess.PIPE,
...
def test_check_does_not_call_old_producer_or_report_old_success() -> None:
    text = CHECK_SCRIPT.read_text(encoding="utf-8")
    assert "run_covapie_split_leakage_qa_gate_v0" not in text
    result = _run_check()
    assert result.returncode == 0, result.stdout + result.stderr
