...
from covalent_ext import (
    covapie_bulk_download_admission_admit_007_unified_adapter_contract_design_gate as gate,
)


ROOT = Path(__file__).resolve().parents[1]
CHECKER_PATH = ROOT / "scripts/check_covapie_bulk_download_admission_admit_007_unified_adapter_contract_design_gate_v1.py"
...
def _valid_source() -> gate.Admit007EvaluationDesignRecord:
    return gate.classify_admit_007_independent_design(
...
def test_source_subclass_is_rejected() -> None:
    class Subclass(gate.Admit007EvaluationDesignRecord):
        pass
...
@pytest.mark.parametrize("storage_tamper", ["missing", "extra"])
def test_exact_type_source_storage_tamper_is_rejected(storage_tamper: str) -> None:
    source = _valid_source()
    if storage_tamper == "missing":
        object.__delattr__(source, "reason")
    else:
        object.__setattr__(source, "unexpected_field", "tamper")
    decision = gate.validate_source_shape_and_invariants_for_design(source)
    assert not decision.accepted
    assert decision.reason == gate.SOURCE_INVARIANT_REASON
...
def test_oracle_mismatch_blocks_projection() -> None:
    source = _valid_source()
    expected = gate.classify_admit_007_independent_design("explicit_curated_covalent_annotation", gate.ALLOWED_CLASSES)
...
def test_issue_inventory_is_exact11_and_byte_identical() -> None:
    predecessor = ROOT / gate.SOURCE_PATHS[2]
...
def test_coverage_issue_still_contains_admit006() -> None:
    ...
    assert "ADMIT_007" in issues["UNIFIED_ADMISSION_RULE_COVERAGE_INCOMPLETE"]["affected_rules"].split("|")


def test_current_runtime_is_exact6_and_has_no_admit007_handler() -> None:
    manifest = json.loads((ROOT / gate.SOURCE_PATHS[1]).read_text())
    assert manifest["registered_rule_ids"] == list(gate.CURRENT_REGISTERED_RULE_ORDER)
    tree = ast.parse((ROOT / gate.SOURCE_PATHS[0]).read_text())
    assert "_evaluate_registered_admit_007" not in {node.name for node in tree.body if isinstance(node, ast.FunctionDef)}
...
def test_production_is_standard_library_only_and_has_no_runtime_handler() -> None:
    ...
    assert "evaluate_admission_rule" not in functions and "_evaluate_registered_admit_007" not in functions


def test_standalone_evaluator_ast_does_not_call_design_oracle() -> None:
    snapshot = gate.build_frozen_source_snapshot()
    tree = ast.parse(gate._bytes(snapshot, gate.SOURCE_PATHS[3]).decode())
...
def test_deterministic_double_materialization(tmp_path: Path) -> None:
    ...
    gate.run_covapie_bulk_download_admission_admit_007_unified_adapter_contract_design_gate_v1(first)
    gate.run_covapie_bulk_download_admission_admit_007_unified_adapter_contract_design_gate_v1(second)
...
def test_tamper_fails_even_if_manifest_hash_is_refreshed(tmp_path: Path) -> None:
    ...
    gate.run_covapie_bulk_download_admission_admit_007_unified_adapter_contract_design_gate_v1(expected)
    gate.run_covapie_bulk_download_admission_admit_007_unified_adapter_contract_design_gate_v1(tampered)
...
def test_missing_extra_or_symlink_output_fails_closed(tmp_path: Path, mode: str) -> None:
    ...
    gate.run_covapie_bulk_download_admission_admit_007_unified_adapter_contract_design_gate_v1(expected)
    gate.run_covapie_bulk_download_admission_admit_007_unified_adapter_contract_design_gate_v1(candidate)
    if mode == "missing":
...
def test_unexpected_existing_output_entry_fails_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="unexpected output entry"):
        gate.run_covapie_bulk_download_admission_admit_007_unified_adapter_contract_design_gate_v1(tmp_path)
...
def test_manifest_overclaim_fails_checker(tmp_path: Path) -> None:
    ...
    manifest["readiness"]["admit_007_unified_adapter_implemented"] = True
...
def test_checker_passes_and_reports_exact_counts() -> None:
    ...
    assert (payload["source_count"], payload["routing_rows"], payload["truth_rows"], payload["issue_rows"]) == (14, 23, 56, 11)
...
@pytest.mark.parametrize(
    ...
    [
        ("admission_rule_id", "ADMIT_006", False),
        ("adapter_id", "covapie_admit_006_unified_adapter_v1", False),
        ("future_exact7_order", "ADMIT_001|ADMIT_002|ADMIT_003|ADMIT_004|ADMIT_005|ADMIT_006", False),
        ("future_exact7_order", "ADMIT_001|ADMIT_002|ADMIT_003|ADMIT_004|ADMIT_005|ADMIT_006|ADMIT_007|ADMIT_008", False),
...
@pytest.mark.parametrize(
    "tamper_kind",
    [
        "standalone_all_json", "blocked_rejected", "blocked_invalid",
        "blocked_admit006_reason", "blocked_historical_lowercase_reason",
        "context_canonical_cleared",
    ...
    def mutate(rows: list[dict[str, str]]) -> None:
        ...
        if tamper_kind == "standalone_all_json":
            ...
            def update(value: dict[str, object], field: str) -> None:
                value["outcome"] = "rejected"
            rewrite_json(row, ("source_exact10_json", "oracle_exact10_json", "unified_exact13_json"), update)
        elif tamper_kind == "blocked_invalid":
            row = by_id["PROJECTION_distance_blocked"]
            def update(value: dict[str, object], field: str) -> None:
                value["outcome"] = "invalid"
            rewrite_json(row, ("source_exact10_json", "oracle_exact10_json", "unified_exact13_json"), update)
        elif tamper_kind in ("blocked_admit006_reason", "blocked_historical_lowercase_reason"):
            row = by_id["PROJECTION_distance_blocked"]
            wrong = ("COVALENT_EVENT_EVIDENCE_NOT_EXPLICIT" if tamper_kind == "blocked_admit006_reason"
                     else "distance_only_inference_not_admissible")
            row["expected_reason"] = wrong
            def update(value: dict[str, object], field: str) -> None:
                value["reason"] = wrong
            rewrite_json(row, ("source_exact10_json", "oracle_exact10_json", "unified_exact13_json"), update)
            ...
            by_id["BOUNDARY_admit007_not_registered"]["behavior"] = "registered"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("real_candidate_evaluation", "true"),
        ("raw_read", "true"),
        ("training", "true"),
...
def test_checker_owned_frozen_output_hash_map_is_exact() -> None:
    ...
    assert checker.FROZEN_OUTPUT_SHA256 == {
        gate.CONTRACT_FILENAME: "9c155462b7ffe83a172eb16dccde0da0c8c6be03fff93aa1a405aa9ca7b14537",
        gate.ROUTING_FILENAME: "5cca82bac1f9e971b238735a9c8aad3c06f5876e32dad535f639645453720c59",
        gate.TRUTH_FILENAME: "0423420e0442f9d6ea7cf8b1befe319f0b6bc6527d1038a0a4f95883d982cddd",
        gate.SAFETY_FILENAME: "4641829cb8912cfbd9a84689feed03b2780baee199d13070cd0b97c8e90c9992",
        gate.ISSUE_FILENAME: "e63c1b83fe245031ecdefa6b3387086b2aba8d72ef50189be17b9874e8f18196",
        gate.MANIFEST_FILENAME: "9cb470520b666624ecbde992354c05587dd38c3006f1f21db4a2dfe3733a4eb4",
    }
...
def test_checker_owned_source_boundary_matches_exact14() -> None:
    checker = _load_checker()
    assert len(checker.SOURCE_BOUNDARY) == 14
