...
from covalent_ext import covapie_4lh_completed_decision_ingestion_and_task_label_availability_v1 as ingestion
from covalent_ext import covapie_completed_human_decision_reconciliation_with_4lh_v1 as reconciliation
from covalent_ext import covapie_cumulative1000_current_global_readiness_census_with_4lh_v1 as subject


REPO_ROOT = Path(__file__).resolve().parents[1]
ERROR = subject.Cumulative1000CurrentGlobalReadinessCensusWith4LHError
TARGET = set(subject.FOUR_LH_EXACT4_EVENT_IDS_V1)

CHECKER_SPEC = importlib.util.spec_from_file_location(
    "check_with_4lh", REPO_ROOT / subject.CHECKER_RELATIVE
)
...
def bundle():
    computation = subject.compute_covapie_cumulative1000_current_global_readiness_census_with_4lh_v1(
        REPO_ROOT
    )
    ...
    reconciled = reconciliation.reconcile_real_completed_human_decisions_with_4lh_v1(
        REPO_ROOT
    )
    matrix = _parse(REPO_ROOT / subject.FOUR_LH_EVENT_MATRIX_RELATIVE)
    return {
...
def _validate(bundle, computation) -> bool:
    return subject.validate_covapie_cumulative1000_current_global_readiness_census_with_4lh_v1(
        computation,
...
def test_predecessor_binding_and_unresolved_4lh_state(bundle) -> None:
    subject._assert_predecessor_4lh_state_v1(bundle["frozen"], REPO_ROOT)
    subject._verify_predecessor_bindings(
        REPO_ROOT, bundle["frozen"].semantic_source_bindings
    )
    assert len(bundle["frozen"].semantic_source_bindings) == 162
...
def test_reconciliation_exact23_135_and_review_summary(bundle) -> None:
    result = bundle["reconciliation"]
    assert len(result.source_bindings) == 23
    assert len(result.normalized_facts) == 135
    assert len({binding.stable_identity for binding in result.source_bindings}) == 23
...
def test_matrix_exact4_role_pair_pre_post_and_training_boundary(bundle) -> None:
    rows = subject._validate_4lh_matrix_rows_v1(bundle["matrix"])
    assert tuple(row["canonical_event_id"] for row in rows) == ingestion.EXPECTED_EVENT_IDS
    assert tuple(int(row["scaleup_rank"]) for row in rows) == (950, 951, 952, 953)
    for row in rows:
        ...
        assert (row["protein_reactive_atom"], row["ligand_reactive_atom"]) == ("SG", "CAP")
        ...
        assert row["PRE_mapping_count"] == "2"
        assert row["PRE_mapping_status"] == "PRE_SOURCE_GRAPH_MAPPING_AMBIGUOUS"
...
def test_schema_exact4_delta_and_overlay_sets(bundle) -> None:
    ...
    assert len(subject._AUTHORIZED_4LH_OVERLAY_FIELDS_V1) == 19
    assert len(subject._AUTHORIZED_BUT_UNCHANGED_4LH_FIELDS_V1) == 1
    assert len(subject._ACTUAL_CHANGED_4LH_FIELDS_V1) == 18
    for event in TARGET:
        changed = {
        ...
        unchanged = {
            field for field in subject._AUTHORIZED_4LH_OVERLAY_FIELDS_V1
            if before[event][field] == after[event][field]
        }
        assert changed == subject._ACTUAL_CHANGED_4LH_FIELDS_V1
        assert unchanged == subject._AUTHORIZED_BUT_UNCHANGED_4LH_FIELDS_V1
...
def test_target_projection_semantics(bundle) -> None:
    ...
    for row in rows:
        assert row["current_global_status"] == "COMPLETED_HUMAN_POSITIVE"
        assert row["human_review_authority_source"] == subject.FOUR_LH_HUMAN_DECISION_SOURCE
        assert row["chemistry_disposition"] == "POSITIVE"
        assert row["task_relevance_disposition"] == "RELEVANT"
        assert row["training_use_disposition"] == "INCLUDE"
...
def test_global_distributions_review_delta_and_authority_counts(bundle) -> None:
    ...
    assert Counter(row["current_global_status"] for row in rows)["CURRENTLY_UNREVIEWED"] == 179
    assert Counter(row["current_global_status"] for row in rows)["COMPLETED_HUMAN_POSITIVE"] == 119
    ...
    assert Counter(row["training_use_disposition"] for row in rows) == Counter(
        {"INCLUDE": 64, "EXCLUDE_FROM_TRAINING_ONLY": 72, "NOT_APPLICABLE": 102, "UNRESOLVED": 762}
    )
    assert sum(row["reactive_pair_sample_authoritative"] == "true" for row in rows) == 148
    assert sum(row["role_partition_sample_authoritative"] == "true" for row in rows) == 140
    assert sum(row["canonical_mask_structural_labels_available"] == "true" for row in rows) == 140
...
def test_exact5_b3_no_sixth_and_applicability_counts(bundle) -> None:
    ...
    assert applicability == Counter({0: 140, 1: 52, 2: 52, 3: 140, 4: 140})
...
def test_summary_blockers_and_4lh_source_composition(bundle) -> None:
    summary = bundle["computation"].summary
    blockers = summary["blockers"]
    assert blockers["population_sizes"] == {
        "chemistry_positive_population_count": 148,
        "training_include_population_count": 64,
    }
    assert blockers["chemistry_unresolved"] == {"all_1000": 762}
    assert blockers["pair_authority_absent"] == {"all_1000": 852, "within_chemistry_positive": 0}
    assert blockers["role_authority_absent"] == {"all_1000": 860, "within_chemistry_positive": 8}
    assert blockers["missing_split_authority"] == {
        "within_chemistry_positive": 107, "within_training_include": 39,
    }
    assert blockers["missing_tensor_integration"]["within_chemistry_positive"] == 107
    assert blockers["missing_tensor_integration"]["within_training_include"] == 35
    assert blockers["missing_tensor_integration"]["missing_source_composition"]["4LH"] == 4
    ...
    assert blockers["feature_semantics_pending"] == {"within_chemistry_positive": 148}


def test_next_pending_tp2_and_authority_boundary(bundle) -> None:
    summary = bundle["computation"].summary
    assert summary["human_review"]["completed_event_count"] == 159
    assert summary["human_review"]["completed_unit_count"] == 27
    assert summary["human_review"]["unreviewed_event_count"] == 179
    assert summary["human_review"]["unreviewed_unit_count"] == 104
    top = summary["top_pending_review_units_by_event_yield"][0]
    assert top == {
        "rank": 1, "raw_priority_rank": 27,
        "review_unit_id": subject.NEXT_PENDING_REVIEW_UNIT_ID_V1,
        "event_count": 4, "pdb_ids": ["1F4C", "1F4D"], "ligand_component_ids": ["TP2"],
    ...
    assert boundary["4LH_REVIEW_COMPLETED"] is True
    assert boundary["4LH_CENSUS_SOURCE_BINDING_V2_CLEAN_FROM_BIRTH"] is True
...
def test_semantic_lineage_exact168_without_collisions(bundle) -> None:
    bindings = bundle["computation"].semantic_source_bindings
    assert len(bindings) == 168
    assert tuple(bindings[:162]) == bundle["frozen"].semantic_source_bindings
    assert [item["artifact_role"] for item in bindings[162:]] == [
        item[0] for item in subject._ADDITIVE_SOURCE_SPECS_V1
    ]
    assert len({(item["path_namespace"], item["path"]) for item in bindings}) == 168
    predecessor_roles = {item["artifact_role"] for item in bindings[:162]}
    additive_roles = [item["artifact_role"] for item in bindings[162:]]
...
@pytest.mark.parametrize(
    ("field", "value"),
    (
        ("canonical_event_id", subject.FOUR_LH_EXACT4_EVENT_IDS_V1[0] + ":TAMPER"),
        ...
        ("post_geometry_source_evidence_available", "false"),
        ("pre_geometry_authoritative", "true"),
    ),
)
def test_target_semantic_tampers_fail_closed(bundle, field: str, value: str) -> None:
    with pytest.raises(ERROR):
        _validate(bundle, _mutate(bundle, subject.FOUR_LH_EXACT4_EVENT_IDS_V1[0], **{field: value}))
...
def test_matrix_semantic_tampers_fail_closed(bundle, field: str, value: str) -> None:
    ...
    rows[0][field] = value
    with pytest.raises(ERROR):
        subject._validate_4lh_matrix_rows_v1(rows)
...
def test_summary_next_pending_and_binding_collision_fail_closed(bundle) -> None:
    ...
    summary = deepcopy(bundle["computation"].summary)
    summary["top_pending_review_units_by_event_yield"][0]["raw_priority_rank"] = 26
    with pytest.raises(ERROR):
        _validate(bundle, replace(bundle["computation"], summary=summary))
...
def test_materialization_outside_authorized_root_fails(tmp_path: Path) -> None:
    with pytest.raises(ERROR, match="OUTPUT_DIRECTORY_NOT_AUTHORIZED"):
        subject.materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_4lh_v1(
