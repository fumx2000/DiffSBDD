...
ERROR = subject.Cumulative1000CurrentGlobalReadinessCensusWith2VSError
CHECKER_SPEC = importlib.util.spec_from_file_location(
    "check_covapie_cumulative1000_current_global_readiness_census_with_2vs_v1",
    REPO / "scripts/check_covapie_cumulative1000_current_global_readiness_census_with_2vs_v1.py",
)
...
def frozen():
    return predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_prf_v1(REPO)


@pytest.fixture(scope="session")
def computation():
    return subject.compute_covapie_cumulative1000_current_global_readiness_census_with_2vs_v1(REPO)
...
def _two_vs_predicate(row: dict[str, str]) -> bool:
    return row["canonical_event_id"] == subject.TWO_VS_EXACT8_EVENT_IDS_V1[0]
...
def test_frozen_predecessor_and_two_vs_bindings_are_exact() -> None:
...
def test_happy_path_exact1000_and_public_validator(computation, frozen) -> None:
    assert type(computation) is predecessor.predecessor.predecessor.Cumulative1000CurrentGlobalReadinessComputationV1
    assert len(computation.rows) == 1000
    assert subject.validate_covapie_cumulative1000_current_global_readiness_census_with_2vs_v1(
...
def test_exact8_delta_and_992_dict_equal(computation, frozen) -> None:
    ...
    assert changed == set(subject.TWO_VS_EXACT8_EVENT_IDS_V1)
...
def test_predecessor_exact8_state_is_proven(frozen) -> None:
    subject._assert_predecessor_two_vs_state_v1(frozen)
    rows = [row for row in frozen.rows if row["canonical_event_id"] in set(subject.TWO_VS_EXACT8_EVENT_IDS_V1)]
    assert tuple(int(row["scaleup_rank"]) for row in rows) == (848, 849, 850, 851, 859, 860, 861, 862)
...
def test_two_vs_refreshed_semantics_and_no_training_promotion(computation) -> None:
    rows = [row for row in computation.rows if row["canonical_event_id"] in set(subject.TWO_VS_EXACT8_EVENT_IDS_V1)]
    assert len(rows) == 8
    assert all(
        row["current_global_status"] == "COMPLETED_HUMAN_POSITIVE"
        and row["human_review_completed"] == "true"
        and row["human_review_authority_source"] == subject.TWO_VS_FORMAL_DECISION_SOURCE
        and row["chemistry_disposition"] == "POSITIVE"
        and row["chemistry_authority_source"] == subject.TWO_VS_EVENT_MATRIX_SOURCE
...
def test_two_vs_structural_fields_are_byte_for_field_equal(computation, frozen) -> None:
    before = {row["canonical_event_id"]: row for row in frozen.rows}
    after = {row["canonical_event_id"]: row for row in computation.rows}
    assert all(
        before[event_id][field] == after[event_id][field]
        for event_id in subject.TWO_VS_EXACT8_EVENT_IDS_V1
        for field in subject._STRUCTURAL_IDENTITY_FIELDS_V1
    )
    exact8 = [after[event_id] for event_id in subject.TWO_VS_EXACT8_EVENT_IDS_V1]
...
def test_exact_set_algebra(computation, frozen) -> None:
    ...
    two_vs = set(subject.TWO_VS_EXACT8_EVENT_IDS_V1)
    assert before["chemistry_positive"].isdisjoint(two_vs)
    assert after["chemistry_positive"] == before["chemistry_positive"] | two_vs
    assert after["chemistry_unresolved"] == before["chemistry_unresolved"] - two_vs
    assert after["task_relevant"] == before["task_relevant"] | two_vs
    assert after["task_unresolved"] == before["task_unresolved"] - two_vs
    assert after["training_exclude"] == before["training_exclude"] | two_vs
    assert after["training_unresolved"] == before["training_unresolved"] - two_vs
...
def test_refreshed_exact11_and_disposition_counts(computation) -> None:
    ...
    assert Counter(row["chemistry_disposition"] for row in computation.rows) == Counter({"POSITIVE": 74, "NOT_ESTABLISHED": 86, "UNRESOLVED": 840})
    assert Counter(row["task_relevance_disposition"] for row in computation.rows) == Counter({"RELEVANT": 75, "NOT_RELEVANT": 86, "UNRESOLVED": 839})
    assert Counter(row["training_use_disposition"] for row in computation.rows) == Counter({"INCLUDE": 29, "EXCLUDE_FROM_TRAINING_ONLY": 45, "NOT_APPLICABLE": 86, "UNRESOLVED": 840})
...
def test_pair_role_exact5_geometry_and_training_counts(computation) -> None:
    ...
    assert summary["role"]["role_profile_counts"] == {"STRICT_LINKER_PRESENT_V1": 31, "DIRECT_ATTACHMENT_OPTIONAL_LINKER_V1": 43, "other": 0}
    assert [task["structurally_applicable_authoritative_role_count"] for task in summary["canonical_exact5"]["tasks"]] == [74, 31, 31, 74, 74]
...
def test_refreshed_blockers_are_exact_and_obsolete_formulation_absent(computation) -> None:
    ...
    assert computation.summary["blockers"]["missing_tensor_integration"]["missing_source_composition"] == {"G3H": 8, "ONL": 9, "PRF": 8, "2VS": 8}


def test_reconciliation_summary_and_dynamic_full_queue_top10(computation) -> None:
    result = reconciliation.reconcile_real_completed_human_decisions_with_2vs_v1(REPO)
    assert result.review_summary == {
        "universe_event_count": 338, "universe_review_unit_count": 131,
        "completed_positive_event_count": 57, "completed_positive_unit_count": 6,
        "completed_negative_event_count": 24, "completed_negative_unit_count": 4,
        "completed_total_event_count": 81, "completed_total_unit_count": 10,
        "in_progress_event_count": 0, "in_progress_unit_count": 0,
        "unreviewed_event_count": 257, "unreviewed_unit_count": 121,
    }
    ...
    assert expected[0]["review_unit_id"] == "COVAPIE_BULK_REVIEW_UNIT_9723B0F9EC07CC81"
    assert expected[0]["ligand_component_ids"] == ["1F8"]
    assert expected[0]["pdb_ids"] == ["3ORX"]
...
def test_authority_boundary_and_non_actions(computation) -> None:
    ...
    assert boundary["next_priority_review_unit"] == "COVAPIE_BULK_REVIEW_UNIT_9723B0F9EC07CC81"
    assert boundary["next_priority_review_ligand"] == "1F8"
...
def test_three_private_derived_contract_digests(computation) -> None:
    assert subject._sha256(subject._csv_bytes(computation.rows)) == "e0e4eb86d2961e2db2ca139ffe5492cfe9675b768826be85a3d0516b532ae24a"
    assert subject._sha256(subject._json_bytes(computation.summary)) == "1b5cca68c2b81426cfae86921a666d8766dc40d31032c24ba90888f0b88588f7"
    assert subject._sha256(subject._canonical_json(list(computation.semantic_source_bindings)).encode()) == "77e22e96e118d4ebc7b468daee89694d37c4aa0a51f63ae937e4498d82724b59"
...
def test_semantic_bindings_preserve_predecessor_and_add_exact7_inputs(computation, frozen) -> None:
    ...
    assert len(frozen.semantic_source_bindings) == 53
    assert len(identities) == len(computation.semantic_source_bindings) == 60
...
def test_exact3_materialized_outputs_and_two_directory_determinism(tmp_path: Path) -> None:
    built = subject.build_covapie_cumulative1000_current_global_readiness_artifacts_with_2vs_v1(REPO)
    assert tuple(built) == (subject.CENSUS_FILE, subject.SUMMARY_FILE, subject.MANIFEST_FILE)
    one = subject.materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_2vs_v1(REPO, tmp_path / "one")
    two = subject.materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_2vs_v1(REPO, tmp_path / "two")
...
def test_materialized_exact7_inventory_and_checker() -> None:
    ...
    assert result["refreshed_positive_count"] == 74
    assert result["semantic_source_binding_count"] == 60
    assert result["ready_for_formal_training"] is False


def test_non_two_vs_row_change_rejected(computation, frozen) -> None:
    mutated = _mutate_row(computation, lambda row: row["canonical_event_id"] not in set(subject.TWO_VS_EXACT8_EVENT_IDS_V1), current_global_status="CURRENTLY_IN_PROGRESS")
    with pytest.raises(ERROR):
        subject.validate_covapie_cumulative1000_current_global_readiness_census_with_2vs_v1(mutated, predecessor_computation=frozen)


def test_two_vs_missing_rejected(computation, frozen) -> None:
    rows = [dict(row) for row in computation.rows if row["canonical_event_id"] != subject.TWO_VS_EXACT8_EVENT_IDS_V1[0]]
    with pytest.raises(ERROR):
        subject.validate_covapie_cumulative1000_current_global_readiness_census_with_2vs_v1(_replace_rows(computation, rows), predecessor_computation=frozen)


def test_two_vs_extra_rejected(computation, frozen) -> None:
    rows = [dict(row) for row in computation.rows]
...
def test_two_vs_duplicate_rejected(computation, frozen) -> None:
    rows = [dict(row) for row in computation.rows]
    first = next(index for index, row in enumerate(rows) if row["canonical_event_id"] == subject.TWO_VS_EXACT8_EVENT_IDS_V1[0])
    second = next(index for index, row in enumerate(rows) if row["canonical_event_id"] == subject.TWO_VS_EXACT8_EVENT_IDS_V1[1])
...
def test_two_vs_semantic_drift_rejected(computation, frozen, updates) -> None:
    mutated = _mutate_row(computation, _two_vs_predicate, **updates)
    with pytest.raises(ERROR):
        subject.validate_covapie_cumulative1000_current_global_readiness_census_with_2vs_v1(mutated, predecessor_computation=frozen)


def test_old_66_positive_projection_retained_rejected(computation, frozen) -> None:
    before = {row["canonical_event_id"]: row for row in frozen.rows}
    rows = [dict(before.get(row["canonical_event_id"], row)) for row in computation.rows]
    assert Counter(row["chemistry_disposition"] for row in rows)["POSITIVE"] == 66
    with pytest.raises(ERROR):
        subject.validate_covapie_cumulative1000_current_global_readiness_census_with_2vs_v1(_replace_rows(computation, rows), predecessor_computation=frozen)
...
@pytest.mark.parametrize(
    "mutator",
    [
        lambda summary: summary["top_pending_review_units_by_event_yield"].insert(0, {"rank": 1, "review_unit_id": subject.TWO_VS_REVIEW_UNIT_ID_V1, "event_count": 8}),
        lambda summary: summary["authority_boundary"].update({"next_priority_review_unit": subject.TWO_VS_REVIEW_UNIT_ID_V1}),
        lambda summary: summary["authority_boundary"].update({"next_priority_review_ligand": "2VS"}),
        lambda summary: summary["blockers"]["missing_tensor_integration"]["missing_source_composition"].update({"2VS": 0}),
    ],
    ids=("top_still_two_vs", "next_unit_not_1f8", "next_ligand_not_1f8", "missing_tensor_omits_two_vs8"),
)
def test_summary_drift_rejected(computation, frozen, mutator) -> None:
    summary = deepcopy(computation.summary)
    mutator(summary)
    with pytest.raises(ERROR):
        subject.validate_covapie_cumulative1000_current_global_readiness_census_with_2vs_v1(replace(computation, summary=summary), predecessor_computation=frozen)
...
def test_semantic_binding_drift_rejected(computation, frozen) -> None:
    ...
    with pytest.raises(ERROR):
        subject.validate_covapie_cumulative1000_current_global_readiness_census_with_2vs_v1(replace(computation, semantic_source_bindings=tuple(bindings)), predecessor_computation=frozen)


def test_predecessor_materialized_outputs_remain_exact() -> None:
    output = REPO / predecessor.OUTPUT_DIRECTORY_RELATIVE
    expected = {
        predecessor.CENSUS_FILE: (506220, "a707cb60c8f788f9ad0e94e89c4038226cfa5f94c15b0afcfa6e36adca3c1b12"),
        predecessor.SUMMARY_FILE: (14742, "82d4d36beb21efb2a588beaea9d3b9c61a6275596482e39ff45341e4cbe316f7"),
        predecessor.MANIFEST_FILE: (25586, "0e3b96b63577c536639143660361ad23744ccdf5327d62a2b4022c48001ff3d6"),
    }
