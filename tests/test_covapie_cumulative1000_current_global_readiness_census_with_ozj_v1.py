...
ERROR = subject.Cumulative1000CurrentGlobalReadinessCensusWithOZJError
CHECKER_SPEC = importlib.util.spec_from_file_location(
    "check_covapie_cumulative1000_current_global_readiness_census_with_ozj_v1",
    REPO / "scripts/check_covapie_cumulative1000_current_global_readiness_census_with_ozj_v1.py",
)
...
def frozen():
    return predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_cht_v1(
        REPO
    )
...
@pytest.fixture(scope="session")
def computation():
    return subject.compute_covapie_cumulative1000_current_global_readiness_census_with_ozj_v1(
        REPO
    )
...
@pytest.fixture(scope="session")
def matrix_rows():
    payload = (REPO / subject.OZJ_EVENT_MATRIX_RELATIVE).read_bytes()
    return [
        dict(row)
...
def _ozj_predicate(row: dict[str, str]) -> bool:
    return row["canonical_event_id"] == subject.OZJ_EXACT4_EVENT_IDS_V1[0]
...
def test_frozen_predecessor_and_ozj_bindings_are_exact() -> None:
...
def test_happy_path_source_derived_exact1000_and_validator(computation, frozen) -> None:
    ...
    assert subject.validate_covapie_cumulative1000_current_global_readiness_census_with_ozj_v1(
...
def test_compute_architecture_call_counts(monkeypatch) -> None:
    ...
    original_predecessor = (
        subject.predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_cht_v1
    )
    original_reconciliation = (
        subject.ozj_reconciliation.reconcile_real_completed_human_decisions_with_ozj_v1
    )
    original_build = subject.ozj_ingestion.build_artifacts_v1
    original_validate = subject.ozj_ingestion.validate_completed_decision_projection_v1
    ...
    monkeypatch.setattr(
        subject.ozj_reconciliation,
        "reconcile_real_completed_human_decisions_with_ozj_v1",
        wrapped_reconciliation,
    )
    monkeypatch.setattr(subject.ozj_ingestion, "build_artifacts_v1", wrapped_build)
    monkeypatch.setattr(
        subject.ozj_ingestion,
        "validate_completed_decision_projection_v1",
        wrapped_validate,
    )
    subject.compute_covapie_cumulative1000_current_global_readiness_census_with_ozj_v1(
        REPO
    )
    assert calls == {
        "predecessor": 1,
...
def test_exact4_identity_ranks_pdbs_and_cys168(matrix_rows) -> None:
    validated = subject._validate_ozj_matrix_rows_v1(matrix_rows)
    assert tuple(row["canonical_event_id"] for row in validated) == (
        subject.OZJ_EXACT4_EVENT_IDS_V1
    )
    ...
    assert {row["cys_residue_id"] for row in validated} == {"CYS:168-"}
...
def test_exact4_delta_and_996_dict_equal(computation, frozen) -> None:
    ...
    assert changed == set(subject.OZJ_EXACT4_EVENT_IDS_V1)
    ...
    assert all(
        ...
        } == subject._AUTHORIZED_OZJ_OVERLAY_FIELDS_V1
        for event_id in changed
    )
    assert len(subject._AUTHORIZED_OZJ_OVERLAY_FIELDS_V1) == 18
    assert "human_training_excluded" not in subject._AUTHORIZED_OZJ_OVERLAY_FIELDS_V1


def test_predecessor_exact5_state_is_proven(frozen) -> None:
    subject._assert_predecessor_ozj_state_v1(frozen)
    rows = [
        row for row in frozen.rows
        if row["canonical_event_id"] in set(subject.OZJ_EXACT4_EVENT_IDS_V1)
    ]
...
def test_ozj_refreshed_semantics_and_training_prohibitions(computation) -> None:
    rows = [
    ...
    assert all(
        ...
        and row["human_review_authority_source"] == subject.OZJ_FORMAL_DECISION_SOURCE
        and row["chemistry_disposition"] == "POSITIVE"
        and row["chemistry_authority_source"] == subject.OZJ_EVENT_MATRIX_SOURCE
        and row["positive_authority_source"] == subject.OZJ_EVENT_MATRIX_SOURCE
...
def test_ozj_structural_fields_are_unchanged(computation, frozen) -> None:
    before = {row["canonical_event_id"]: row for row in frozen.rows}
    after = {row["canonical_event_id"]: row for row in computation.rows}
    assert all(
        before[event_id][field] == after[event_id][field]
        for event_id in subject.OZJ_EXACT4_EVENT_IDS_V1
        for field in subject._STRUCTURAL_IDENTITY_FIELDS_V1
    )
    for event_id in subject.OZJ_EXACT4_EVENT_IDS_V1:
        assert all(
            after[event_id][field] == value
            for field, value in subject._EXPECTED_OZJ_STRUCTURAL_CELLS_V1.items()
        )
...
def test_exact_set_algebra_keeps_include_future_admission_runtime(computation, frozen) -> None:
    ...
    ozj = set(subject.OZJ_EXACT4_EVENT_IDS_V1)
    assert after["chemistry_positive"] == before["chemistry_positive"] | ozj
    assert after["chemistry_unresolved"] == before["chemistry_unresolved"] - ozj
    assert after["task_relevant"] == before["task_relevant"] | ozj
    assert after["task_unresolved"] == before["task_unresolved"] - ozj
    assert after["training_include"] == before["training_include"] | ozj
    assert after["future_candidate"] == before["future_candidate"] | ozj
    assert after["training_unresolved"] == before["training_unresolved"] - ozj
    for key in (
...
def test_global_chemistry_task_training_and_status_counts(computation) -> None:
    ...
    ) == Counter({"RELEVANT": 105, "NOT_RELEVANT": 86, "UNRESOLVED": 809})
...
def test_pair_role_exact5_geometry_and_training_counts(computation) -> None:
    ...
    assert summary["reactive_pair"]["sample_level_authoritative_pair_count"] == 104
    assert summary["reactive_pair"]["published_model_bound_target_constructible_count"] == 41
    assert summary["reactive_pair"]["ozj_sample_authority_contribution_count"] == 4
    assert summary["reactive_pair"]["ozj_model_bound_target_contribution_count"] == 0
    assert summary["role"]["role_profile_counts"] == {
        "STRICT_LINKER_PRESENT_V1": 48,
        "DIRECT_ATTACHMENT_OPTIONAL_LINKER_V1": 56,
        "other": 0,
    }
    assert summary["role"]["unknown_role_row_count"] == 896
    assert [
    ...
    assert summary["training_stage"]["future_training_admission_candidate_count"] == 23
...
def test_blocker_counts_are_exact_and_nonexclusive(computation) -> None:
    ...
    assert blockers["human_training_exclusion"] == {"within_positive_104": 64}
    ...
    assert blockers["feature_semantics_pending"] == {"within_positive_104": 104}
    assert blockers["pair_authority_absent"] == {
        "all_1000": 896, "within_positive_104": 0,
    }
...
def test_reconciliation_and_dynamic_pending_queue_head(computation) -> None:
    result = reconciliation.reconcile_real_completed_human_decisions_with_ozj_v1(REPO)
    ...
    assert expected[0]["review_unit_id"] == "COVAPIE_BULK_REVIEW_UNIT_2557BFE1E3B5C4C5"
    assert expected[0]["ligand_component_ids"] == ["F24"]
    assert expected[0]["pdb_ids"] == ["3V4X"]
    assert expected[0]["event_count"] == 4
    assert all(item["ligand_component_ids"] != ["OZJ"] for item in expected)
...
def test_authority_boundary_and_non_actions(computation) -> None:
    ...
    assert boundary["next_priority_review_ligand"] == "F24"
...
def test_projection_digests_and_semantic_bindings(computation, frozen) -> None:
    ...
    assert len(computation.semantic_source_bindings) == 95
...
def test_materialized_exact7_inventory_and_independent_checker() -> None:
    ...
    assert result["refreshed_positive_count"] == 104
    assert result["task_relevant_count"] == 105
    assert result["training_include_count"] == 40
    assert result["training_exclude_count"] == 64
    assert result["future_candidate_count"] == 23
    assert result["formal_training_admitted_count"] == 5
    assert result["current_runtime_model_usable_count"] == 17
    assert result["semantic_source_binding_count"] == 95
...
def test_predecessor_ozj_row_not_unreviewed_rejected(frozen) -> None:
    rows = [dict(row) for row in frozen.rows]
    index = next(
        i for i, row in enumerate(rows)
        if row["canonical_event_id"] == subject.OZJ_EXACT4_EVENT_IDS_V1[0]
    )
    rows[index]["current_review_status"] = "CURRENTLY_IN_PROGRESS"
    with pytest.raises(ERROR):
        subject._assert_predecessor_ozj_state_v1(replace(frozen, rows=tuple(rows)))


def test_predecessor_ozj_wrong_review_unit_rejected(frozen) -> None:
    rows = [dict(row) for row in frozen.rows]
    index = next(
        i for i, row in enumerate(rows)
        if row["canonical_event_id"] == subject.OZJ_EXACT4_EVENT_IDS_V1[0]
...
def test_non_ozj_output_row_change_rejected(computation, frozen) -> None:
    mutated = _mutate_row(
        computation,
        lambda row: row["canonical_event_id"] not in set(subject.OZJ_EXACT4_EVENT_IDS_V1),
        current_global_status="CURRENTLY_IN_PROGRESS",
    )
    with pytest.raises(ERROR):
        subject.validate_covapie_cumulative1000_current_global_readiness_census_with_ozj_v1(
...
@pytest.mark.parametrize(
    "label",
    ["PREDECESSOR_CHT_CENSUS_OWNER", "OZJ_RECONCILIATION_SUCCESSOR"],
)
...
def test_reconciliation_contract_drift_rejected(monkeypatch) -> None:
    ...
    with pytest.raises(ERROR):
        subject._validate_ozj_reconciliation_v1(REPO)
...
def test_matrix_event_coverage_mutations_rejected(matrix_rows, mutation) -> None:
    ...
    if mutation == "missing":
        ...
        extra = dict(rows[-1])
        extra["canonical_event_id"] = "EXTRA_OZJ_EVENT"
        extra["scaleup_rank"] = "674"
        rows.append(extra)
    with pytest.raises(ERROR):
        subject._validate_ozj_matrix_rows_v1(rows)
...
def test_refreshed_row_semantic_mutations_rejected(computation, frozen, updates) -> None:
    mutated = _mutate_row(computation, _ozj_predicate, **updates)
    with pytest.raises(ERROR):
        subject.validate_covapie_cumulative1000_current_global_readiness_census_with_ozj_v1(
            mutated, predecessor_computation=frozen
        )
...
def test_summary_mutation_rejected_by_independent_checker(computation) -> None:
    summary = deepcopy(computation.summary)
    summary["blockers"]["missing_tensor_integration"]["within_positive_104"] = 62
    with pytest.raises(ValueError):
...
def test_manifest_binding_mutation_rejected(computation) -> None:
    artifacts = subject.build_covapie_cumulative1000_current_global_readiness_artifacts_with_ozj_v1(
        REPO
    )
