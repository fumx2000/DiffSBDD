...
from build_training_dataset_packaging_design_gate import PLANNED_PACKAGING_DESIGN_ROOT, run  # noqa: E402
from test_training_dataset_design_gate import _read_csv, _write_csv  # noqa: E402
from test_training_dataset_design_review_qa import _make_fixture as _make_design_qa_fixture, _run as _run_design_qa  # noqa: E402
...
def _args(paths):
    return argparse.Namespace(
        ...
        output_gate_plan_csv=Path(DESIGN_ROOT) / "training_dataset_packaging_design_gate_plan.csv",
        output_report_csv=Path(DESIGN_ROOT) / "training_dataset_packaging_design_gate_report.csv",
        output_md=paths["design_qa_summary"].parent / "training_dataset_packaging_design_gate_summary.md",
    )


def _make_fixture(tmp_path):
    paths = _make_design_qa_fixture(tmp_path)
    rows, code = _run_design_qa(paths)
    assert code == 0
    assert len(rows) == 3
    paths["packaging_gate_plan"] = Path(DESIGN_ROOT) / "training_dataset_packaging_design_gate_plan.csv"
    paths["packaging_gate_report"] = Path(DESIGN_ROOT) / "training_dataset_packaging_design_gate_report.csv"
    paths["packaging_gate_summary"] = paths["design_qa_summary"].parent / "training_dataset_packaging_design_gate_summary.md"
    return paths
...
def _assert_blocked(paths, reason):
    ...
    assert code == 1
    assert len(report) == 3
    assert any(row["training_dataset_packaging_design_gate_status"] == "blocked" for row in report)
...
def test_success_outputs_three_passed_rows_without_modifying_inputs(tmp_path):
    ...
    assert len(plan) == 3
    assert len(report) == 3
    assert len(_read_csv(paths["packaging_gate_plan"])) == 3
    assert len(_read_csv(paths["packaging_gate_report"])) == 3
    assert all(row["training_dataset_packaging_design_gate_status"] == "training_dataset_packaging_design_gate_passed" for row in report)
    assert all(row["explicit_approval_required_before_training_dataset_packaging_design"] == "true" for row in report)
    assert all(row["ready_for_training_dataset_packaging_design_after_approval"] == "true" for row in report)
    assert all(row["training_dataset_packaging_design_executed"] == "false" for row in report)
    ...
    assert "Training Dataset Packaging Design Gate Summary" in paths["packaging_gate_summary"].read_text(encoding="utf-8")
    for key, before in input_hashes.items():
        assert _sha(paths[key]) == before


def test_design_qa_manifest_schema_split_and_design_report_blockers(tmp_path):
    paths = _make_fixture(tmp_path)
    paths["design_qa_report"].unlink()
    _assert_blocked(paths, "design_review_qa_row_found_once")
    paths = _make_fixture(tmp_path)
    ...
    _assert_blocked(paths, "design_review_qa_status_passed")
    paths = _make_fixture(tmp_path)
...
def test_package_manifest_label_count_and_path_blockers(tmp_path):
    paths = _make_fixture(tmp_path)
...
def test_design_dir_planned_output_and_global_forbidden_artifacts_block(tmp_path):
    paths = _make_fixture(tmp_path)
    (Path(DESIGN_ROOT) / "bad.sdf").write_text("bad", encoding="utf-8")
    _assert_blocked(paths, "design_review_only_files_valid")
    paths = _make_fixture(tmp_path)
    (Path(DESIGN_ROOT) / "bad.pt").write_text("bad", encoding="utf-8")
    _assert_blocked(paths, "design_review_only_files_valid")
    paths = _make_fixture(tmp_path)
    Path(PLANNED_PACKAGING_DESIGN_ROOT).mkdir(parents=True)
    _assert_blocked(paths, "planned_packaging_design_outputs_absent_before_gate")
    paths = _make_fixture(tmp_path)
    ...
    paths = _make_fixture(tmp_path)
    archive = Path("data/derived/covalent_small/bad.tar")
...
def test_script_has_no_disallowed_runtime_calls():
    source = Path(__file__).resolve().parents[1] / "scripts" / "build_training_dataset_packaging_design_gate.py"
