...
from covalent_ext import (
    covapie_bulk_download_admission_admit_006_rule_logic_interface as subject,
)


CHECKER_PATH = (
    subject.REPO_ROOT
    / "scripts/check_covapie_bulk_download_admission_admit_006_rule_logic_interface_v1.py"
)
CHECKER_SPEC = importlib.util.spec_from_file_location("admit006_hardening_checker", CHECKER_PATH)
...
def _assert_invalid_construction(values: dict[str, object]) -> None:
    with pytest.raises((TypeError, ValueError)):
        subject.Admit006EvaluationResult(**values)  # type: ignore[arg-type]
...
def test_public_signature_is_exact_and_both_parameters_are_required() -> None:
    signature = inspect.signature(subject.evaluate_admit_006)
...
def test_frozen_result_has_exact10_ordered_fields() -> None:
    assert tuple(field.name for field in fields(subject.Admit006EvaluationResult)) == subject.RESULT_FIELDS
    result = subject.evaluate_admit_006(
...
def test_direct_construction_rejects_exact_str_and_tuple_subclasses() -> None:
    ...
    values = _result_values()
    values["admission_rule_id"] = Text("ADMIT_006")
    _assert_invalid_construction(values)
    values = _result_values()
...
def test_scalar_validation_precedence(scalar: object, reason: str) -> None:
    result = subject.evaluate_admit_006(scalar, None)
...
def test_wrong_context_container_has_type_reason(context: object) -> None:
    result = subject.evaluate_admit_006(subject.CANONICAL_ENUM_MEMBERS[0], context)
...
def test_both_explicit_members_pass_with_empty_reason(scalar: str) -> None:
    result = subject.evaluate_admit_006(scalar, subject.ALLOWED_COVALENT_EVIDENCE_CLASSES)
...
def test_scalar_failure_wins_when_scalar_and_context_are_both_invalid() -> None:
    result = subject.evaluate_admit_006("UNKNOWN", None)
...
def test_evaluator_is_deterministic_and_does_not_mutate_inputs() -> None:
    ...
    before = list(context)
    first = subject.evaluate_admit_006(scalar, context)
    second = subject.evaluate_admit_006(scalar, context)
    assert first == second
...
def test_all_exact37_cases_match_a_separate_test_oracle() -> None:
    ...
    for _, _, scalar, _, context in definitions:
        observed = subject.evaluate_admit_006(scalar, context)
        assert tuple(getattr(observed, field.name) for field in fields(observed)) == _independent_oracle(scalar, context)
...
def test_source_boundary_row5_uses_preservation_wording_and_outputs_have_no_stale_claim(
    ...
    state = subject.build_interface_state()
    assert state["source_audit_rows"][4]["boundary_necessity"] == expected
    root = tmp_path / "outputs"
    subject.run_covapie_bulk_download_admission_admit_006_rule_logic_interface_v1(root)
    paths = tuple(root / name for name in subject.OUTPUT_FILES) + (
        subject.REPO_ROOT
        / "docs/covapie_bulk_download_admission_admit_006_rule_logic_interface_v1_summary.md",
    )
...
def test_evaluator_transitive_call_graph_is_pure_and_oracle_independent() -> None:
    ...
    closure = _assert_pure_call_graph(tree, "evaluate_admit_006")
    assert closure == {"evaluate_admit_006", "_validate_scalar", "_validate_context", "_result"}
...
def test_call_graph_guard_rejects_design_oracle_io_and_materializer_helpers(bad_call: str) -> None:
    tree = ast.parse(f"def evaluate_admit_006(x):\n    return helper(x)\ndef helper(x):\n    return {bad_call}\n")
    with pytest.raises(AssertionError):
        _assert_pure_call_graph(tree, "evaluate_admit_006")
...
def test_issue_inventory_exactly_preserves_predecessor_exact11() -> None:
    ...
    assert coverage["affected_rules"] == "|".join(f"ADMIT_{index:03d}" for index in range(6, 16))
...
def test_checker_rejects_exact37_semantic_tamper_after_manifest_hash_refresh(
    ...
    root = tmp_path / "outputs"
    materialized = subject.run_covapie_bulk_download_admission_admit_006_rule_logic_interface_v1(root)
...
def test_checker_rejects_premature_admit006_unified_coverage_removal() -> None:
    snapshot = subject.build_frozen_source_snapshot()
    predecessor = tuple(
    ...
    coverage["affected_rules"] = "|".join(f"ADMIT_{index:03d}" for index in range(7, 16))
    coverage["integration_transition"] = "admit_006_removed_too_early"
...
def test_corrected_issue_bytes_exactly_equal_committed_predecessor_inventory() -> None:
    ...
    assert hashlib.sha256(observed_bytes).hexdigest() == "d3333fd74af79b065eb136d830e730c3a636e0d632bcb735b3e841a29d548c79"
...
def test_deterministic_double_materialization_and_exact_output_set(tmp_path: Path) -> None:
    root = tmp_path / "outputs"
    result = subject.run_covapie_bulk_download_admission_admit_006_rule_logic_interface_v1(root)
...
def test_production_import_is_silent_and_has_no_materialization_side_effect(tmp_path: Path) -> None:
    command = [sys.executable, "-c", "import covalent_ext.covapie_bulk_download_admission_admit_006_rule_logic_interface"]
