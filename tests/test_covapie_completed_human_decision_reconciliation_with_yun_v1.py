...
ERROR = subject.CompletedDecisionReconciliationWithYUNError
GENERIC_ERROR = generic.CompletedDecisionReconciliationError
CHECKER_SPEC = importlib.util.spec_from_file_location(
    "check_completed_human_decision_reconciliation_with_yun_v1",
    REPO
...
def test_public_api_is_minimal_and_yun_transition_adapter_is_not_created() -> None:
    ...
    assert subject.YUN_TRANSITION_ADAPTER_CREATED is False
    ...
    assert "reconcile_real_completed_human_decisions_with_1f8_v1" not in called


def test_yun_projector_reuses_ingestion_owner_and_generic_types(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    original = yun_ingestion_owner.load_frozen_formal_decision_v1
    calls: list[Path] = []
...
def test_yun_projector_wraps_ingestion_owner_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fail(_repo_root: Path) -> dict[str, object]:
        raise yun_ingestion_owner.YUNIngestionSafetyError("SOURCE_DRIFT")
    ...
    with pytest.raises(ERROR, match="YUN_INGESTION_OWNER_VALIDATION_FAILED"):
        subject.project_yun_completed_decision_v1(repo_root=REPO)


def test_yun_projection_exact7_semantics_and_formal_binding() -> None:
    source = subject.project_yun_completed_decision_v1(repo_root=REPO)
    assert source.binding == generic.SourceBinding(
        ...
        byte_count=30722,
        sha256="b4eeebe03354e820d9658225997c34b58b41c66f4dfe126230024306816e1140",
        schema_version="covapie_yun_exact7_formal_human_decision_v1",
        review_unit_id="COVAPIE_BULK_REVIEW_UNIT_1138FFC288DFD03D",
    )
    assert tuple(fact.canonical_event_id for fact in source.facts) == (
        subject._YUN_EVENT_IDS
    )
    assert all(
        ...
        and fact.source_decision_schema == subject._YUN_FORMAL_DECISION_SCHEMA
        and fact.source_decision_sha256 == subject._YUN_FORMAL_DECISION_SHA256
...
def test_projector_does_not_reinterpret_pair_role_post_pre_or_context() -> None:
    bound = yun_ingestion_owner.load_frozen_formal_decision_v1(REPO)
    expected = subject._project_validated_yun_binding_v1(bound)
    ...
    assert subject._project_validated_yun_binding_v1(altered) == expected
...
def test_projector_rejects_formal_binding_drift(
    ...
    with pytest.raises(ERROR, match="YUN_FORMAL_DECISION_BINDING_INVALID"):
        subject._project_validated_yun_binding_v1(bound)


@pytest.mark.parametrize("new_count", (6, 8))
def test_projector_rejects_exact7_count_drift(new_count: int) -> None:
    bound = yun_ingestion_owner.load_frozen_formal_decision_v1(REPO)
    ...
    with pytest.raises(ERROR, match="YUN_NORMALIZED_EVENT_COUNT_NOT_EXACT7"):
        subject._project_validated_yun_binding_v1(bound)
...
def test_projector_rejects_missing_and_extra_event_identity() -> None:
    ...
    bound["normalized"]["events"][-1]["canonical_event_id"] = (
        "COVAPIE_CYS_SG_EVENT_V1:EXTRA:YUN:SD"
    )
    with pytest.raises(ERROR, match="YUN_NORMALIZED_EVENT_COVERAGE_NOT_EXACT7"):
        subject._project_validated_yun_binding_v1(bound)
...
def test_projector_rejects_duplicate_event() -> None:
    ...
    with pytest.raises(ERROR, match="YUN_NORMALIZED_EVENT_ID_DUPLICATE"):
        subject._project_validated_yun_binding_v1(bound)
...
def test_projector_rejects_completed_decision_semantics_drift(
    ...
    with pytest.raises(ERROR, match="YUN_NORMALIZED_EVENT_DECISION_INVALID"):
        subject._project_validated_yun_binding_v1(bound)


def test_original_historical_yun_prior_is_exact7_unreviewed() -> None:
    rows = _historical()
    subject._prove_yun_original_unreviewed_prior_v1(rows)
    yun_rows = [
        row
        for row in rows
        if row["canonical_event_id"] in set(subject._YUN_EVENT_IDS)
    ]
    assert len(yun_rows) == 7
    assert {row["raw_review_unit_id"] for row in yun_rows} == {
        subject._YUN_REVIEW_UNIT_ID
    }
    assert all(
...
def test_historical_proof_rejects_one_missing_yun_event() -> None:
    rows = [
        row
        for row in _historical()
        if row["canonical_event_id"] != subject._YUN_EVENT_IDS[0]
    ]
    with pytest.raises(ERROR, match="YUN_HISTORICAL_EVENT_MISSING"):
        subject._prove_yun_original_unreviewed_prior_v1(rows)


def test_historical_proof_rejects_duplicate_yun_event() -> None:
    rows = _historical()
    target = next(
        row
        for row in rows
    ...
    with pytest.raises(ERROR, match="YUN_HISTORICAL_EVENT_DUPLICATE"):
        subject._prove_yun_original_unreviewed_prior_v1(rows)


def test_historical_proof_rejects_yun_review_unit_drift() -> None:
    rows = _historical()
    target = next(
        row
        for row in rows
    ...
    target["raw_review_unit_id"] = "DRIFTED_YUN_REVIEW_UNIT"
    with pytest.raises(ERROR, match="YUN_HISTORICAL_REVIEW_UNIT"):
        subject._prove_yun_original_unreviewed_prior_v1(rows)


def test_historical_proof_rejects_extra_event_in_yun_unit() -> None:
    rows = _historical()
    extra = next(
        row
        for row in rows
        if row["raw_review_unit_id"] != subject._YUN_REVIEW_UNIT_ID
    )
    extra["raw_review_unit_id"] = subject._YUN_REVIEW_UNIT_ID
    with pytest.raises(
        ERROR, match="YUN_HISTORICAL_REVIEW_UNIT_EVENT_SET_NOT_EXACT7"
    ):
        subject._prove_yun_original_unreviewed_prior_v1(rows)
...
def test_historical_proof_rejects_one_non_unreviewed_yun_prior(
    status: str,
) -> None:
    rows = _historical()
    target = next(
    ...
    with pytest.raises(ERROR, match="YUN_PRIOR_STATE_NOT_EXACT7_UNREVIEWED"):
        subject._prove_yun_original_unreviewed_prior_v1(rows)
...
def test_historical_proof_rejects_whole_non_unreviewed_yun_unit(
    status: str,
) -> None:
    rows = _historical()
    for row in rows:
...
def test_onl_adapter_leaves_all_yun_rows_field_equal() -> None:
...
def test_yun_unchanged_proof_rejects_unexpected_onl_adapter_change() -> None:
    original = generic.load_real_historical_reconciliation_v1(REPO)
    adapted = [
        dict(row)
    ...
    with pytest.raises(ERROR, match="ONL_ADAPTER_CHANGED_YUN_ROW"):
        subject._prove_yun_rows_unchanged_after_onl_normalization_v1(
            original, adapted
        )


def test_yun_needs_no_adapter_and_passes_unchanged_generic() -> None:
    adapted = _adapted_historical()
    sources = subject.load_real_completed_decision_sources_with_yun_v1(REPO)
    result = generic.reconcile_completed_human_decisions_v1(adapted, sources)
    assert result.review_summary["completed_positive_event_count"] == 72


def test_one_yun_prior_drift_fails_at_unit_consistency_gate() -> None:
...
def test_yun_unit_prior_drift_reaches_generic_prior_status_protection(
    status: str,
) -> None:
    adapted = [dict(row) for row in _adapted_historical()]
    sources = subject.load_real_completed_decision_sources_with_yun_v1(REPO)
...
def test_original_historical_with_all_eight_sources_still_fails_for_onl() -> None:
...
def test_exact8_source_composition_is_collision_free() -> None:
    sources = subject.load_real_completed_decision_sources_with_yun_v1(REPO)
    assert [len(source.facts) for source in sources] == [8, 16, 8, 9, 8, 8, 8, 7]
    assert len({source.binding.review_unit_id for source in sources}) == 8
    assert len({source.binding.stable_identity for source in sources}) == 8
    event_ids = [
        fact.canonical_event_id for source in sources for fact in source.facts
    ]
    assert len(event_ids) == len(set(event_ids)) == 72


def test_source_loader_rejects_yun_collision_with_previous65(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        subject, "project_yun_completed_decision_v1", lambda *, repo_root: altered
    )
    with pytest.raises(ERROR, match="REAL_NORMALIZED_FACT_EVENT_COLLISION"):
        subject.load_real_completed_decision_sources_with_yun_v1(REPO)


def test_source_loader_rejects_yun_review_unit_mismatch(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source = subject.project_yun_completed_decision_v1(repo_root=REPO)
    altered = replace(
    ...
    with pytest.raises(ERROR, match="YUN_SOURCE_REVIEW_UNIT_MISMATCH"):
        subject.load_real_completed_decision_sources_with_yun_v1(REPO)
...
def test_real_reconciliation_counts_training_and_final_yun_authority() -> None:
    result = subject.reconcile_real_completed_human_decisions_with_yun_v1(REPO)
...
def test_exact_yun_delta_relative_to_published_1f8_chain() -> None:
    adapted = _adapted_historical()
    sources = subject.load_real_completed_decision_sources_with_yun_v1(REPO)
    ...
    assert after["completed_positive_event_count"] - before["completed_positive_event_count"] == 7
    assert after["completed_total_event_count"] - before["completed_total_event_count"] == 7
    assert after["unreviewed_event_count"] - before["unreviewed_event_count"] == -7
    ...
    assert current_training[generic.TRAINING_INCLUDE] - prior_training[generic.TRAINING_INCLUDE] == 7
    assert current_training[generic.TRAINING_EXCLUDE] - prior_training[generic.TRAINING_EXCLUDE] == 0
...
def test_production_pipeline_calls_onl_and_generic_exactly_once(
    ...
    subject.reconcile_real_completed_human_decisions_with_yun_v1(REPO)
...
def test_checker_exact4_frozen_bindings_and_all_gates() -> None:
    ...
    assert result["source_binding_count"] == 8
    assert result["normalized_fact_count"] == 72
    assert result["yun_transition_adapter_created"] is False
    assert result["yun_rows_unchanged_by_onl_adapter"] is True
    assert result["production_onl_adapter_call_count"] == 1
    assert result["production_generic_reconciler_call_count"] == 1
    assert result["published_global_positive_count"] == 82
    assert result["published_training_include_count"] == 29
    assert result["published_future_candidate_count"] == 12
    assert result["expected_next_global_positive_count"] == 89
...
def test_current_census_remains_published_82_and_expected_89_is_informational() -> None:
    result = checker.run_check_v1(REPO)
    assert result["published_global_positive_count"] == 82
    assert result["published_training_include_count"] == 29
    assert result["published_future_candidate_count"] == 12
    assert result["yun_reconciliation_local_positive_delta"] == 7
    assert result["expected_next_global_positive_count"] == 89
    assert result["expected_next_census_derivation_status"] == "INFORMATIONAL_ONLY"
    derivation = result["expected_next_census_derivation"]
    assert derivation["training_include"] == 36
    assert derivation["future_training_candidates"] == 19
    assert derivation["missing_split_within_positive"] == 48
    assert derivation["missing_split_within_include"] == 11
    assert derivation["missing_tensor_within_positive"] == 48
    assert derivation["missing_tensor_within_include"] == 7
    assert derivation["all_missing_are_training_excluded_population"] is False
    assert derivation["missing_POST_training_authority"] == 72
    assert derivation["missing_POST_training_authority_within_include"] == 19
    assert derivation["missing_admission"] == 84
    assert derivation["missing_admission_within_include"] == 31
    assert derivation["feature_semantics_pending_positive"] == 89
    assert result["expected_next_pending_head"] == "NEQ/EXACT6_INFORMATIONAL_ONLY"
