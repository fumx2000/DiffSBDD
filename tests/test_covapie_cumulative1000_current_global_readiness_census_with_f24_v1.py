...
ERROR = subject.Cumulative1000CurrentGlobalReadinessCensusWithF24Error
CHECKER_SPEC = importlib.util.spec_from_file_location(
    "check_covapie_cumulative1000_current_global_readiness_census_with_f24_v1",
    REPO / "scripts/check_covapie_cumulative1000_current_global_readiness_census_with_f24_v1.py",
)
...
def frozen():
    return predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_ozj_v1(
        REPO
    )
...
@pytest.fixture(scope="session")
def computation():
    return subject.compute_covapie_cumulative1000_current_global_readiness_census_with_f24_v1(
        REPO
    )
...
@pytest.fixture(scope="session")
def matrix_rows():
    payload = (REPO / subject.F24_EVENT_MATRIX_RELATIVE).read_bytes()
    return [
        dict(row)
...
def _f24_predicate(row: dict[str, str]) -> bool:
    return row["canonical_event_id"] == subject.F24_EXACT4_EVENT_IDS_V1[0]
...
def test_frozen_predecessor_and_f24_bindings_are_exact() -> None:
...
def test_happy_path_source_derived_exact1000_and_validator(computation, frozen) -> None:
    ...
    assert subject.validate_covapie_cumulative1000_current_global_readiness_census_with_f24_v1(
...
def test_compute_architecture_call_counts(monkeypatch) -> None:
    ...
    original_predecessor = (
        subject.predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_ozj_v1
    )
    original_reconciliation = (
        subject.f24_reconciliation.reconcile_real_completed_human_decisions_with_f24_v1
    )
    original_build = subject.f24_ingestion.build_artifacts_v1
    original_validate = subject.f24_ingestion.validate_completed_decision_projection_v1
    ...
    monkeypatch.setattr(
        subject.f24_reconciliation,
        "reconcile_real_completed_human_decisions_with_f24_v1",
        wrapped_reconciliation,
    )
    monkeypatch.setattr(subject.f24_ingestion, "build_artifacts_v1", wrapped_build)
    monkeypatch.setattr(
        subject.f24_ingestion,
        "validate_completed_decision_projection_v1",
        wrapped_validate,
    )
    subject.compute_covapie_cumulative1000_current_global_readiness_census_with_f24_v1(
        REPO
    )
    assert calls == {
        "predecessor": 1,
...
def test_exact4_identity_ranks_pdbs_and_cys111(matrix_rows) -> None:
    validated = subject._validate_f24_matrix_rows_v1(matrix_rows)
    assert tuple(row["canonical_event_id"] for row in validated) == (
        subject.F24_EXACT4_EVENT_IDS_V1
    )
    ...
    assert {row["cys_residue_id"] for row in validated} == {"CYS:111-"}


def test_f24_matrix_d1_d5_role_chemical_and_exact5_semantics(matrix_rows) -> None:
    rows = subject._validate_f24_matrix_rows_v1(matrix_rows)
    for row in rows:
        ...
        assert row["ligand_reactive_atom"] == "C8"
        assert row["role_partition_human_choice"] == "REVISE_ROLE_PARTITION"
        assert row["selected_role_candidate_index_0based"] == ""
        assert row["role_profile"] == "DIRECT_ATTACHMENT_OPTIONAL_LINKER_V1"
        assert json.loads(row["warhead_atoms_json"]) == [
            "C1", "C2", "C4", "C8", "O2", "O5", "O6",
        ]
        assert json.loads(row["chemical_warhead_atoms_json"]) == [
            "C1", "C2", "C8", "O2", "O6",
        ]
...
def test_exact4_delta_and_996_dict_equal(computation, frozen) -> None:
    ...
    assert changed == set(subject.F24_EXACT4_EVENT_IDS_V1)
    ...
    assert all(
        ...
        } == subject._AUTHORIZED_F24_OVERLAY_FIELDS_V1
        for event_id in changed
    )
    assert len(subject._AUTHORIZED_F24_OVERLAY_FIELDS_V1) == 18
    assert "human_training_excluded" not in subject._AUTHORIZED_F24_OVERLAY_FIELDS_V1


def test_predecessor_exact5_state_is_proven(frozen) -> None:
    subject._assert_predecessor_f24_state_v1(frozen)
    rows = [
        row for row in frozen.rows
        if row["canonical_event_id"] in set(subject.F24_EXACT4_EVENT_IDS_V1)
    ]
...
def test_f24_refreshed_semantics_and_training_prohibitions(computation) -> None:
    rows = [
    ...
    assert all(
        ...
        and row["human_review_authority_source"] == subject.F24_FORMAL_DECISION_SOURCE
        and row["chemistry_disposition"] == "POSITIVE"
        and row["chemistry_authority_source"] == subject.F24_EVENT_MATRIX_SOURCE
        and row["positive_authority_source"] == subject.F24_EVENT_MATRIX_SOURCE
...
def test_f24_structural_fields_are_unchanged(computation, frozen) -> None:
    before = {row["canonical_event_id"]: row for row in frozen.rows}
    after = {row["canonical_event_id"]: row for row in computation.rows}
    assert all(
        before[event_id][field] == after[event_id][field]
        for event_id in subject.F24_EXACT4_EVENT_IDS_V1
        for field in subject._STRUCTURAL_IDENTITY_FIELDS_V1
    )
    for event_id in subject.F24_EXACT4_EVENT_IDS_V1:
        assert all(
            after[event_id][field] == value
            for field, value in subject._EXPECTED_F24_STRUCTURAL_CELLS_V1.items()
        )
...
def test_exact_set_algebra_keeps_include_future_admission_runtime(computation, frozen) -> None:
    ...
    f24 = set(subject.F24_EXACT4_EVENT_IDS_V1)
    assert after["chemistry_positive"] == before["chemistry_positive"] | f24
    assert after["chemistry_unresolved"] == before["chemistry_unresolved"] - f24
    assert after["task_relevant"] == before["task_relevant"] | f24
    assert after["task_unresolved"] == before["task_unresolved"] - f24
    assert after["training_include"] == before["training_include"] | f24
    assert after["future_candidate"] == before["future_candidate"] | f24
    assert after["training_unresolved"] == before["training_unresolved"] - f24
    for key in (
        "chemistry_negative", "chemistry_not_established", "task_not_relevant",
        "training_exclude", "training_not_applicable",
...
def test_global_chemistry_task_training_and_status_counts(computation) -> None:
    ...
    ) == Counter({"RELEVANT": 109, "NOT_RELEVANT": 86, "UNRESOLVED": 805})
...
def test_pair_role_exact5_geometry_and_training_counts(computation) -> None:
    ...
    assert summary["reactive_pair"]["sample_level_authoritative_pair_count"] == 108
    assert summary["reactive_pair"]["published_model_bound_target_constructible_count"] == 41
    assert summary["reactive_pair"]["f24_sample_authority_contribution_count"] == 4
    assert summary["reactive_pair"]["f24_model_bound_target_contribution_count"] == 0
    assert summary["role"]["role_profile_counts"] == {
        "STRICT_LINKER_PRESENT_V1": 48,
        "DIRECT_ATTACHMENT_OPTIONAL_LINKER_V1": 60,
        "other": 0,
    }
    assert summary["role"]["unknown_role_row_count"] == 892
    assert [
...
def test_blocker_counts_are_exact_and_nonexclusive(computation) -> None:
    ...
    assert blockers["human_training_exclusion"] == {"within_positive_108": 64}
    ...
    assert blockers["feature_semantics_pending"] == {"within_positive_108": 108}
    assert blockers["pair_authority_absent"] == {
        "all_1000": 892, "within_positive_108": 0,
    }
...
def test_reconciliation_and_dynamic_pending_queue_head(computation) -> None:
    result = reconciliation.reconcile_real_completed_human_decisions_with_f24_v1(REPO)
    ...
    assert expected[0]["review_unit_id"] == "COVAPIE_BULK_REVIEW_UNIT_6B422BBF7FAD44F6"
    assert expected[0]["ligand_component_ids"] == ["2A2"]
    assert expected[0]["pdb_ids"] == ["3ORZ"]
    assert expected[0]["rank"] == 1
    assert expected[0]["raw_priority_rank"] == 16
    assert expected[0]["event_count"] == 4
    assert all(item["ligand_component_ids"] != ["F24"] for item in expected)
...
def test_authority_boundary_and_non_actions(computation) -> None:
    ...
    assert boundary["next_priority_review_ligand"] == "2A2"
    assert boundary["next_priority_review_current_pending_rank"] == 1
    assert boundary["next_priority_review_raw_priority_rank"] == 16
...
def test_projection_digests_and_semantic_bindings(computation, frozen) -> None:
    ...
    assert len(computation.semantic_source_bindings) == 102
...
def test_materialized_exact7_inventory_and_independent_checker() -> None:
    ...
    assert result["refreshed_positive_count"] == 108
    assert result["task_relevant_count"] == 109
    ...
    assert result["semantic_source_binding_count"] == 102
...
def test_predecessor_f24_row_not_unreviewed_rejected(frozen) -> None:
    rows = [dict(row) for row in frozen.rows]
    index = next(
        i for i, row in enumerate(rows)
        if row["canonical_event_id"] == subject.F24_EXACT4_EVENT_IDS_V1[0]
    )
    rows[index]["current_review_status"] = "CURRENTLY_IN_PROGRESS"
    with pytest.raises(ERROR):
        subject._assert_predecessor_f24_state_v1(replace(frozen, rows=tuple(rows)))


def test_predecessor_f24_wrong_review_unit_rejected(frozen) -> None:
    rows = [dict(row) for row in frozen.rows]
    index = next(
        i for i, row in enumerate(rows)
        if row["canonical_event_id"] == subject.F24_EXACT4_EVENT_IDS_V1[0]
...
def test_predecessor_f24_already_authoritative_rejected(frozen, field, value) -> None:
    rows = [dict(row) for row in frozen.rows]
    index = next(
        i for i, row in enumerate(rows)
        if row["canonical_event_id"] == subject.F24_EXACT4_EVENT_IDS_V1[0]
...
def test_non_f24_output_row_change_rejected(computation, frozen) -> None:
    mutated = _mutate_row(
        computation,
        lambda row: row["canonical_event_id"] not in set(subject.F24_EXACT4_EVENT_IDS_V1),
        current_global_status="CURRENTLY_IN_PROGRESS",
    )
    with pytest.raises(ERROR):
        subject.validate_covapie_cumulative1000_current_global_readiness_census_with_f24_v1(
...
def test_reconciliation_contract_drift_rejected(monkeypatch) -> None:
    ...
    with pytest.raises(ERROR):
        subject._validate_f24_reconciliation_v1(REPO)
...
def test_matrix_must_be_source_derived(monkeypatch) -> None:
    ...
    monkeypatch.setattr(subject.f24_ingestion, "build_artifacts_v1", lambda root: drifted)
    with pytest.raises(ERROR):
        subject._load_and_validate_f24_event_matrix_v1(REPO)
...
def test_matrix_event_coverage_mutations_rejected(matrix_rows, mutation) -> None:
    ...
    if mutation == "missing":
        ...
        extra = dict(rows[-1])
        extra["canonical_event_id"] = "EXTRA_F24_EVENT"
        extra["scaleup_rank"] = "674"
        rows.append(extra)
    with pytest.raises(ERROR):
        subject._validate_f24_matrix_rows_v1(rows)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"future_training_admission_status": "DRIFTED"},
        {"training_admitted": "true"},
        {"role_profile": "STRICT_LINKER_PRESENT_V1"},
        {"selected_role_candidate_index_0based": "3"},
        {"role_partition_human_choice": "SELECT_CANDIDATE"},
        {"direct_profile_applicable_task_ids_json": "[0,1,2,3,4]"},
        {"warhead_atoms_json": "[\"C1\",\"C2\",\"C8\",\"O2\",\"O6\"]"},
        {"chemical_warhead_atoms_json": "[\"C1\",\"C2\",\"C4\",\"C8\",\"O2\",\"O5\",\"O6\"]"},
        {"chemical_warhead_differs_from_role_region": "false"},
...
def test_matrix_exact5_mutations_rejected(matrix_rows, mutation) -> None:
    ...
    if mutation == "b3_removed":
        ...
        tasks[1]["structurally_applicable"] = True
...
def test_refreshed_row_semantic_mutations_rejected(computation, frozen, updates) -> None:
    mutated = _mutate_row(computation, _f24_predicate, **updates)
    with pytest.raises(ERROR):
        subject.validate_covapie_cumulative1000_current_global_readiness_census_with_f24_v1(
            mutated, predecessor_computation=frozen
        )
...
def test_summary_mutation_rejected_by_independent_checker(computation) -> None:
    summary = deepcopy(computation.summary)
    summary["blockers"]["missing_tensor_integration"]["within_positive_108"] = 66
    with pytest.raises(ValueError):
        checker.independently_verify_counts_v1(list(computation.rows), summary)


@pytest.mark.parametrize("stale_key", ["within_positive_104", "within_include_40"])
...
def test_manifest_binding_mutation_rejected(computation) -> None:
    artifacts = subject.build_covapie_cumulative1000_current_global_readiness_artifacts_with_f24_v1(
        REPO
    )
...
@pytest.mark.parametrize(
    ("field", "value"),
    [("timestamp", "2026-08-29T00:00:00Z"), ("hostname", "machine")],
)
