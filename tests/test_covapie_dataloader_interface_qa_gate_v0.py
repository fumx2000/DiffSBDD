...
from covalent_ext import covapie_dataloader_interface_qa_gate as legacy
from covalent_ext import covapie_legacy_pipeline_retirement_policy as retirement


LEGACY_ROOT = Path("data/derived/covalent_small/covapie_dataloader_interface_qa_gate_v0")
CHECK_SCRIPT = Path("scripts/check_covapie_dataloader_interface_qa_gate_v0.py")
...
EXPECTED_STAGE = "covapie_dataloader_interface_qa_gate_v0"
...
EXPECTED_SUCCESS_LINE = "covapie_dataloader_interface_qa_gate_v0_retirement_policy_passed"
OLD_SUCCESS_LINE = "covapie_dataloader_interface_qa_gate_v0_passed"
EXPECTED_ERROR_MESSAGE = (
    f"legacy_stage_retired:{EXPECTED_STAGE}:superseded_by=None:"
    f"recommended_next_step={EXPECTED_NEXT_STEP}"
)
IMPORT_COMPATIBILITY_MODULES = (
    "covapie_actual_dataloader_design_gate",
    "covapie_bulk_download_design_gate",
    "covapie_cys_sg_acquired_annotation_manual_review_gate",
    "covapie_cys_sg_discovery_download_manifest_gate",
    "covapie_cys_sg_discovery_download_smoke",
    "covapie_cys_sg_discovery_support_review_gate",
    "covapie_cys_sg_future_struct_conn_controlled_raw_acquisition_gate",
    "covapie_cys_sg_future_struct_conn_crosscheck_execution_gate",
    "covapie_cys_sg_future_struct_conn_crosscheck_gate",
    "covapie_cys_sg_ligand_covale_annotation_alignment_gate",
    "covapie_cys_sg_manual_review_decision_application_gate",
    "covapie_cys_sg_manual_review_decision_input_by_user",
    "covapie_cys_sg_ready_candidate_materialization_gate",
    "covapie_cys_sg_result_review_decision_application_gate",
    "covapie_cys_sg_result_review_decision_input_by_user",
    "covapie_cys_sg_struct_conn_crosscheck_result_review_gate",
    "covapie_cys_sg_targeted_annotation_acquisition_smoke",
    "covapie_cys_sg_targeted_metadata_expansion_gate",
    "covapie_cys_sg_targeted_metadata_expansion_next_batch_gate",
    "covapie_cys_sg_targeted_metadata_next_batch_acquisition_smoke",
    "covapie_dataloader_interface_design_gate",
    "covapie_dataloader_interface_qa_gate",
    "covapie_dataloader_interface_smoke",
    "covapie_dataloader_smoke_design_gate",
    "covapie_feature_semantics_resolution_design_gate",
    "covapie_feature_semantics_resolution_smoke",
    "covapie_feature_semantics_resolution_smoke_qa_gate",
    "covapie_feature_semantics_tensorization_audit_gate",
    ...
    "covapie_manual_event_identity_support_evidence_acquisition_gate",
    "covapie_metadata_dataloader_smoke",
    "covapie_metadata_dataloader_smoke_qa_gate",
    "covapie_sample_preparation_design_gate",
    "covapie_sample_preparation_execution_smoke",
    "covapie_sample_preparation_qa_gate",
    "covapie_small_pilot_candidate_expansion_gate",
    "covapie_small_pilot_download_manifest_gate",
    "covapie_small_pilot_manifest_rerun_gate",
...
def test_main_run_fails_closed() -> None:
    with pytest.raises(retirement.LegacyStageRetiredError):
        legacy.run_covapie_dataloader_interface_qa_gate_v0()
...
def test_guards_precede_filesystem_readiness_and_old_field_lookup(
    ...
    for name in ("_csv_rows", "_load_json", "_run_git"):
...
def test_module_import_has_zero_artifact_io() -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-c", "import covalent_ext.covapie_dataloader_interface_qa_gate"],
        cwd=REPO_ROOT,
        env=environment,
        text=True,
        stdout=subprocess.PIPE,
...
def test_check_is_policy_only_and_does_not_report_old_success() -> None:
    ...
    assert "run_covapie_dataloader_interface_qa_gate_v0" not in text
    for forbidden in (
        "write_text",
        "write_bytes",
        "open(",
...
def test_forty_module_import_compatibility_has_no_artifact_side_effect() -> None:
    assert len(IMPORT_COMPATIBILITY_MODULES) == 40
    guarded = (Path("data/derived/covalent_small"), *STEP14AR_FILES)
    before = _tree_hash(guarded)
    environment = os.environ.copy()
    environment["PYTHONDONTWRITEBYTECODE"] = "1"
    environment["PYTHONPATH"] = os.pathsep.join(
    ...
    code = (
        "import importlib; "
        f"mods={IMPORT_COMPATIBILITY_MODULES!r}; "
        "[importlib.import_module('covalent_ext.' + name) for name in mods]"
    )
    result = subprocess.run(
...
def test_two_independent_modules_are_not_retired() -> None:
    ...
    for module_name in INDEPENDENT_MODULES:
        module = __import__(f"covalent_ext.{module_name}", fromlist=[module_name])
