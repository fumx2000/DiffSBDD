...
from covalent_ext import covapie_dataloader_interface_design_gate as legacy
from covalent_ext import covapie_legacy_pipeline_retirement_policy as retirement


LEGACY_ROOT = Path(
    "data/derived/covalent_small/covapie_dataloader_interface_design_gate_v0"
)
...
EXPECTED_STAGE = "covapie_dataloader_interface_design_gate_v0"
...
EXPECTED_SUCCESS_LINE = (
    "covapie_dataloader_interface_design_gate_v0_retirement_policy_passed"
)
OLD_SUCCESS_LINE = "covapie_dataloader_interface_design_gate_v0_passed"
...
def test_stage_alias_guards_and_import_surface_are_exact() -> None:
    ...
    for symbol in (
        ...
        "build_input_source_rows",
        "build_feature_interface_rows",
        "build_batch_collate_rows",
        "build_checkpoint_compatibility_rows",
        "build_interface_smoke_plan_rows",
        "build_boundary_rows",
    ):
...
def test_main_run_fails_closed() -> None:
    with pytest.raises(retirement.LegacyStageRetiredError):
        legacy.run_covapie_dataloader_interface_design_gate_v0()
...
def test_module_import_has_zero_artifact_io() -> None:
    ...
    result = subprocess.run(
        [sys.executable, "-c", "import covalent_ext.covapie_dataloader_interface_design_gate"],
        cwd=REPO_ROOT,
        env=environment,
        text=True,
        stdout=subprocess.PIPE,
...
def test_check_is_policy_only_and_does_not_report_old_success() -> None:
    ...
    assert "run_covapie_dataloader_interface_design_gate_v0" not in text
    for forbidden in (
        "write_text",
        "write_bytes",
        "open(",
...
def test_historical_mapping_is_read_only_noncanonical_evidence() -> None:
    mapping = REPO_ROOT / legacy.FIELD_MAPPING_CONTRACT_CSV
    before = _tree_hash((LEGACY_ROOT,))
    with mapping.open(newline="", encoding="utf-8") as handle:
        header = next(csv.reader(handle))
    result = _run_check()
    assert result.returncode == 0, result.stdout + result.stderr
    assert "final_dataset_field_name" in header
    assert legacy.build_retirement_policy().legacy_stage_executable is False
    assert before == _tree_hash((LEGACY_ROOT,))
