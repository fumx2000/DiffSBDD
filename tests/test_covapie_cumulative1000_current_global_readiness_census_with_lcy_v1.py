...
from covalent_ext import covapie_completed_human_decision_reconciliation_with_lcy_v1 as lcy_reconciliation
from covalent_ext import covapie_cumulative1000_current_global_readiness_census_with_lcy_v1 as subject
from covalent_ext import covapie_lcy_completed_decision_ingestion_and_task_label_availability_v1 as ingestion


REPO_ROOT = Path(__file__).resolve().parents[1]
ERROR = subject.Cumulative1000CurrentGlobalReadinessCensusWithLCYError
TARGET = set(ingestion.EXPECTED_EVENT_IDS)

CHECKER_SPEC = importlib.util.spec_from_file_location(
    "check_with_lcy", REPO_ROOT / subject.CHECKER_RELATIVE
)
...
def bundle():
    computation = subject.compute_covapie_cumulative1000_current_global_readiness_census_with_lcy_v1(
        REPO_ROOT
    )
    ...
    reconciliation = lcy_reconciliation.reconcile_real_completed_human_decisions_with_lcy_v1(
        REPO_ROOT
    )
    matrix = _parse(REPO_ROOT / subject.LCY_EVENT_MATRIX_RELATIVE)
    return {
...
def _validate(bundle, computation) -> bool:
    return subject.validate_covapie_cumulative1000_current_global_readiness_census_with_lcy_v1(
        computation,
...
def test_exact4_identity_and_matrix_authority(bundle) -> None:
    assert subject.LCY_EXACT4_EVENT_IDS_V1 == ingestion.EXPECTED_EVENT_IDS
    assert subject.LCY_EXACT4_RANKS_V1 == (898, 899, 900, 901)
    assert subject.LCY_REVIEW_UNIT_ID_V1 == "COVAPIE_BULK_REVIEW_UNIT_BA488AF51EDD8ED6"
    assert subject._validate_lcy_matrix_rows_v1(bundle["matrix"]) == bundle["matrix"]
    assert len(bundle["reconciliation"].source_bindings) == 21
    assert len(bundle["reconciliation"].normalized_facts) == 127
...
def test_schema_delta_and_overlay_sets_are_exact(bundle) -> None:
    ...
    assert len(subject._AUTHORIZED_LCY_OVERLAY_FIELDS_V1) == 15
    assert len(subject._AUTHORIZED_BUT_UNCHANGED_LCY_FIELDS_V1) == 3
    assert len(subject._ACTUAL_CHANGED_LCY_FIELDS_V1) == 12
    for event_id in TARGET:
        ...
        assert actual == subject._ACTUAL_CHANGED_LCY_FIELDS_V1
        assert unchanged == subject._AUTHORIZED_BUT_UNCHANGED_LCY_FIELDS_V1
...
def test_target_semantics_and_authority_sources(bundle) -> None:
    ...
    for row in rows:
        ...
        assert row["human_review_authority_source"] == subject.LCY_HUMAN_DECISION_SOURCE
        assert row["chemistry_disposition"] == "POSITIVE"
        assert row["chemistry_authority_source"] == subject.LCY_EVENT_MATRIX_SOURCE
        assert row["positive_authority_source"] == subject.LCY_EVENT_MATRIX_SOURCE
        assert row["task_relevance_disposition"] == "NOT_RELEVANT"
        assert row["task_relevance_authority_source"] == subject.LCY_EVENT_MATRIX_SOURCE
...
def test_gve_exact4_and_lcy_3a2g_control_are_unchanged(bundle) -> None:
    before = {row["canonical_event_id"]: row for row in bundle["frozen"].rows}
    after = {row["canonical_event_id"]: row for row in bundle["computation"].rows}
    controls = set(subject.GVE_EXACT4_EVENT_IDS_V1)
    controls.add(subject.LCY_SAME_COMPONENT_CONTROL_EVENT_ID_V1)
    assert len(controls) == 5
    assert all(before[event_id] == after[event_id] for event_id in controls)


def test_cross_field_populations_are_legacy90_plus_gve4_plus_lcy4(bundle) -> None:
    ...
    assert orthogonal == set(subject.GVE_EXACT4_EVENT_IDS_V1) | TARGET
...
def test_global_authority_exact5_and_geometry_counts(bundle) -> None:
    ...
    assert sum(row["reactive_pair_sample_authoritative"] == "true" for row in rows) == 140
...
def test_summary_uses_population_neutral_keys_and_source_counts(bundle) -> None:
    ...
    assert blockers["pair_authority_absent"] == {
        "all_1000": 860, "within_chemistry_positive": 0,
    }
    assert blockers["role_authority_absent"] == {
        "all_1000": 868, "within_chemistry_positive": 8,
    }
    assert blockers["missing_tensor_integration"]["within_chemistry_positive"] == 99
    assert blockers["missing_tensor_integration"]["within_training_include"] == 31
    assert blockers["missing_tensor_integration"]["missing_source_composition"]["LCY"] == 4
    assert summary["chemistry"]["positive_source_composition"]["LCY"] == 4
    assert summary["chemistry"]["positive_source_composition"]["GVE"] == 4
    assert "LCY" not in summary["training_stage"]["future_candidate_source_composition"]
...
def test_human_review_next_pending_and_boundaries(bundle) -> None:
    ...
    assert summary["top_pending_review_units_by_event_yield"][0]["raw_priority_rank"] == 25
    assert summary["top_pending_review_units_by_event_yield"][0]["review_unit_id"] == subject.NEXT_PENDING_REVIEW_UNIT_ID_V1
    assert summary["top_pending_review_units_by_event_yield"][0]["ligand_component_ids"] == ["0D8"]
    boundary = summary["authority_boundary"]
    assert boundary["LCY_REVIEW_COMPLETED"] is True
    assert boundary["READY_FOR_NEXT_HIGH_YIELD_HUMAN_REVIEW_PREPARATION"] is True
...
def test_semantic_lineage_exact156(bundle) -> None:
    bindings = bundle["computation"].semantic_source_bindings
    assert len(bundle["frozen"].semantic_source_bindings) == 150
    assert len(bindings) == 156
    assert len({(item["path_namespace"], item["path"]) for item in bindings}) == 156
    assert tuple(bindings[:150]) == bundle["frozen"].semantic_source_bindings
    assert [item["artifact_role"] for item in bindings[150:]] == [
...
def test_lcy_semantic_mutations_fail_closed(bundle, field: str, value: str) -> None:
...
@pytest.mark.parametrize(
    "event_id",
    (
        subject.GVE_EXACT4_EVENT_IDS_V1[0],
        subject.LCY_SAME_COMPONENT_CONTROL_EVENT_ID_V1,
    ),
)
def test_published_gve_and_same_component_lcy_mutations_fail_closed(
    bundle, event_id: str
) -> None:
    mutated = _mutate(bundle, event_id, current_review_status="CURRENTLY_IN_PROGRESS")
    with pytest.raises(ERROR):
        _validate(bundle, mutated)


def test_fifth_lcy_target_identity_fails_closed(bundle, monkeypatch) -> None:
    monkeypatch.setattr(
        subject,
        "LCY_EXACT4_EVENT_IDS_V1",
        (*subject.LCY_EXACT4_EVENT_IDS_V1, subject.LCY_SAME_COMPONENT_CONTROL_EVENT_ID_V1),
    )
    with pytest.raises(ERROR):
        _validate(bundle, bundle["computation"])
...
def test_materialization_outside_authorized_root_fails(tmp_path: Path) -> None:
    with pytest.raises(ERROR, match="OUTPUT_DIRECTORY_NOT_AUTHORIZED"):
        subject.materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_lcy_v1(
