...
SPEC = importlib.util.spec_from_file_location("check_yun_v2", CHECKER_PATH)
...
def bound() -> dict[str, object]:
    return subject.load_frozen_yun_authority_v2(repo_root=ROOT)


@pytest.fixture(scope="module")
def artifacts() -> dict[str, bytes]:
    return subject.verify_published_yun_v1_projection_v2(repo_root=ROOT)
...
def _load_with_mode(
    ...
    return subject.load_frozen_yun_authority_v2(
        repo_root=ROOT,
...
def test_v1_owner_is_frozen_and_untouched() -> None:
    path = ROOT / yun_v1.SOURCE_RELATIVE
    assert path.stat().st_size == subject.YUN_V1_OWNER_BYTE_COUNT == 82003
    assert _sha(path) == subject.YUN_V1_OWNER_SHA256


def test_v1_checker_and_tests_are_frozen() -> None:
    assert (ROOT / yun_v1.CHECKER_RELATIVE).stat().st_size == 18235
    assert _sha(ROOT / yun_v1.CHECKER_RELATIVE) == subject.YUN_V1_CHECKER_SHA256
    assert (ROOT / yun_v1.TEST_RELATIVE).stat().st_size == 22323
    assert _sha(ROOT / yun_v1.TEST_RELATIVE) == subject.YUN_V1_TEST_SHA256
...
def test_verify_bound_source_v2_is_actually_called(
    ...
    subject.load_frozen_yun_authority_v2(repo_root=ROOT)
    assert len(calls) >= 20


def test_old_v1_exact_mode_source_path_is_not_called() -> None:
    result = checker._verify_production_ast(ROOT)
    checker._verify_v1_pure_call_graph(ROOT)
    assert result["v1_active_source_gate_called"] is False
    assert "_verify_payload" not in result["reused_v1_function_names"]
...
def test_no_direct_read_or_exact_numeric_mode_gate() -> None:
    result = checker._verify_production_ast(ROOT)
    assert result["v2_bound_source_helper_used"] is True
...
def test_nonexecutable_review_source_safe_modes_pass(
    ...
    binding = yun_v1.FROZEN_REVIEW_PACKAGE_BINDINGS[0]
...
def test_nonexecutable_review_source_world_writable_fails(tmp_path: Path) -> None:
    binding = yun_v1.FROZEN_REVIEW_PACKAGE_BINDINGS[0]
    with pytest.raises(
        subject.YUNSourceBindingV2Error, match="SOURCE_WORLD_WRITABLE"
    ):
...
def test_executable_validator_safe_executable_modes_pass(
    ...
    binding = yun_v1.FROZEN_REVIEW_PACKAGE_BINDINGS[-1]
    assert _load_with_mode(tmp_path, binding, mode) == bound
...
def test_executable_validator_without_executable_class_fails(
    ...
    binding = yun_v1.FROZEN_REVIEW_PACKAGE_BINDINGS[-1]
    with pytest.raises(
        subject.YUNSourceBindingV2Error,
...
def test_wrong_byte_count_fails_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        subject.YUNSourceBindingV2Error, match="SOURCE_BYTE_COUNT_MISMATCH"
    ):
...
def test_wrong_sha_same_size_fails_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        subject.YUNSourceBindingV2Error, match="SOURCE_SHA256_MISMATCH"
    ):
        subject.load_frozen_yun_authority_v2(
            repo_root=ROOT,
            repository_path_overrides={binding[0]: replacement},
...
def test_symlink_source_fails_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        subject.YUNSourceBindingV2Error, match="SOURCE_SYMLINK_FORBIDDEN"
    ):
        subject.load_frozen_yun_authority_v2(
            repo_root=ROOT,
...
def test_formal_decision_exact7_event_ids(bound: dict[str, object]) -> None:
...
def test_formal_decision_exact7_ranks(bound: dict[str, object]) -> None:
    events = bound["normalized"]["events"]  # type: ignore[index]
    assert tuple(event["scaleup_rank"] for event in events) == yun_v1.EXPECTED_RANKS
...
def test_D3_remains_confirmed_pair(bound: dict[str, object]) -> None:
    events = bound["normalized"]["events"]  # type: ignore[index]
    assert {event["D3_reactive_pair"] for event in events} == {
        "CONFIRM_OBSERVED_PAIR"
    }
    assert {(event["protein_reactive_atom"], event["ligand_reactive_atom"]) for event in events} == {
        ("SG", "CAN")
    }


def test_D4_remains_candidate4(bound: dict[str, object]) -> None:
    events = bound["normalized"]["events"]  # type: ignore[index]
    assert {event["D4_role_partition"] for event in events} == {
        "SELECT_CANDIDATE_4"
    }
    assert {event["selected_role_candidate_index_0based"] for event in events} == {4}


def test_D5_remains_include(bound: dict[str, object]) -> None:
    events = bound["normalized"]["events"]  # type: ignore[index]
    assert {event["D5_training_use"] for event in events} == {"INCLUDE"}


def test_D6_remains_exact(bound: dict[str, object]) -> None:
    context = bound["normalized"]["scientific_context"]  # type: ignore[index]
    assert context["D6_exact_choice"] == yun_v1.EXPECTED_D6
...
def test_warhead_remains_exact(bound: dict[str, object]) -> None:
    role = bound["normalized"]["role"]  # type: ignore[index]
    assert role["warhead_atoms"] == ["NAS", "CAW", "OAC", "CAO", "CAN"]


def test_linker_remains_empty(bound: dict[str, object]) -> None:
    role = bound["normalized"]["role"]  # type: ignore[index]
...
def test_direct_applicable_tasks_remain_0_3_4() -> None:
    tasks = yun_v1._canonical_task_contract()
    assert tasks["direct_profile_applicable_task_ids"] == [0, 3, 4]


def test_post_source_evidence_count_remains_7(bound: dict[str, object]) -> None:
    geometry = bound["normalized"]["geometry_boundary"]  # type: ignore[index]
    assert geometry["POST_source_evidence_count"] == 7


def test_post_training_authority_remains_zero(bound: dict[str, object]) -> None:
    geometry = bound["normalized"]["geometry_boundary"]  # type: ignore[index]
...
def test_future_candidate_remains_true(bound: dict[str, object]) -> None:
...
def test_formal_admission_remains_false(bound: dict[str, object]) -> None:
    training = bound["normalized"]["training_boundary"]  # type: ignore[index]
    assert training["training_admitted"] is False
    assert training["training_admission_created"] is False


def test_materialization_and_runtime_remain_false(bound: dict[str, object]) -> None:
    training = bound["normalized"]["training_boundary"]  # type: ignore[index]
...
@pytest.mark.parametrize(
    ("name", "expected_sha"),
    [
        (yun_v1.SNAPSHOT, subject._PUBLISHED_YUN_V1_OUTPUT_BINDINGS[0][2]),
        (yun_v1.MATRIX, subject._PUBLISHED_YUN_V1_OUTPUT_BINDINGS[1][2]),
        (yun_v1.SUMMARY, subject._PUBLISHED_YUN_V1_OUTPUT_BINDINGS[2][2]),
        (yun_v1.MANIFEST, subject._PUBLISHED_YUN_V1_OUTPUT_BINDINGS[3][2]),
    ],
)
...
def test_published_v1_projection_exact4_order(
    ...
    assert tuple(artifacts) == yun_v1.OUTPUT_FILENAMES
...
def test_independent_checker_passes_candidate() -> None:
    ...
    assert result["yun_v1_bytes_preserved"] is True
    assert result["yun_v1_artifacts_preserved"] is True
    assert result["v2_bound_source_helper_used"] is True
    assert result["exact_posix_semantic_mode_active"] is False
    assert result["ready_for_v2_b2_2"] is True
