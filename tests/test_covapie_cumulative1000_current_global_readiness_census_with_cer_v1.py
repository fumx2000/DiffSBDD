...
from covalent_ext import covapie_completed_human_decision_reconciliation_with_cer_v1 as reconciliation  # noqa: E402
from covalent_ext import covapie_cumulative1000_current_global_readiness_census_with_1n0_v1 as predecessor  # noqa: E402
from covalent_ext import covapie_cumulative1000_current_global_readiness_census_with_cer_v1 as subject  # noqa: E402
from covalent_ext import covapie_cer_completed_decision_ingestion_and_task_label_availability_v1 as ingestion  # noqa: E402


ERROR = subject.Cumulative1000CurrentGlobalReadinessCensusWithCERError
CHECKER_SPEC = importlib.util.spec_from_file_location(
    "check_covapie_cumulative1000_current_global_readiness_census_with_cer_v1",
    REPO / "scripts/check_covapie_cumulative1000_current_global_readiness_census_with_cer_v1.py",
)
...
def frozen():
    return predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_1n0_v1(
        REPO
    )
...
@pytest.fixture(scope="session")
def computation():
    return subject.compute_covapie_cumulative1000_current_global_readiness_census_with_cer_v1(
        REPO
    )
...
@pytest.fixture(scope="session")
def matrix_rows():
    payload = (REPO / subject.CER_EVENT_MATRIX_RELATIVE).read_bytes()
    return [
        dict(row)
...
def _cer_predicate(row: dict[str, str]) -> bool:
    return row["canonical_event_id"] == subject.CER_EXACT4_EVENT_IDS_V1[0]


def test_public_api_is_minimal_and_reuses_predecessor_schema() -> None:
    assert subject.__all__ == (
        "Cumulative1000CurrentGlobalReadinessCensusWithCERError",
        "compute_covapie_cumulative1000_current_global_readiness_census_with_cer_v1",
        "validate_covapie_cumulative1000_current_global_readiness_census_with_cer_v1",
        "build_covapie_cumulative1000_current_global_readiness_artifacts_with_cer_v1",
        "materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_cer_v1",
    )
...
def test_frozen_predecessor_and_cer_bindings_are_exact() -> None:
...
def test_happy_path_source_derived_exact1000_and_validator(computation, frozen) -> None:
    ...
    assert subject.validate_covapie_cumulative1000_current_global_readiness_census_with_cer_v1(
...
def test_compute_architecture_call_counts(monkeypatch) -> None:
    ...
    original_predecessor = (
        subject.predecessor.compute_covapie_cumulative1000_current_global_readiness_census_with_1n0_v1
    )
    original_reconciliation = (
        subject.cer_reconciliation.reconcile_real_completed_human_decisions_with_cer_v1
    )
    original_build = subject.cer_ingestion.build_artifacts_v1
    original_validate = subject.cer_ingestion.validate_completed_decision_projection_v1
    ...
    monkeypatch.setattr(
        subject.cer_reconciliation,
        "reconcile_real_completed_human_decisions_with_cer_v1",
        wrapped_reconciliation,
    )
    monkeypatch.setattr(subject.cer_ingestion, "build_artifacts_v1", wrapped_build)
    monkeypatch.setattr(
        subject.cer_ingestion,
        "validate_completed_decision_projection_v1",
        wrapped_validate,
    )
    subject.compute_covapie_cumulative1000_current_global_readiness_census_with_cer_v1(
        REPO
    )
    assert calls == {
        "predecessor": 1,
...
def test_exact4_identity_ranks_pdbs_and_cysteines(matrix_rows) -> None:
    validated = subject._validate_cer_matrix_rows_v1(matrix_rows)
    assert tuple(row["canonical_event_id"] for row in validated) == (
        subject.CER_EXACT4_EVENT_IDS_V1
    )
    ...
    assert {row["cys_residue_id"] for row in validated} == {"CYS:163-"}


def test_cer_matrix_d1_d5_role_chemical_and_exact5_semantics(matrix_rows) -> None:
    rows = subject._validate_cer_matrix_rows_v1(matrix_rows)
    for row in rows:
        ...
        assert row["pair_authority_scope"] == ingestion.AUTHORITY_SCOPE
        assert row["all_CER_uses_C2"] == "false"
        assert row["selected_role_candidate_index_0based"] == "3"
        assert row["role_profile"] == "DIRECT_ATTACHMENT_OPTIONAL_LINKER_V1"
        assert json.loads(row["warhead_atoms_json"]) == list(ingestion.WARHEAD_ROLE)
        ...
        assert row["new_human_authority_created_by_ingestion"] == "false"
...
def test_exact4_delta_and_996_dict_equal(computation, frozen) -> None:
    ...
    assert changed == set(subject.CER_EXACT4_EVENT_IDS_V1)
    ...
    assert all(
        ...
        } == subject._AUTHORIZED_CER_OVERLAY_FIELDS_V1
        for event_id in changed
    )
    assert len(subject._AUTHORIZED_CER_OVERLAY_FIELDS_V1) == 18
    assert "human_training_excluded" not in subject._AUTHORIZED_CER_OVERLAY_FIELDS_V1
    assert "training_use_include" in subject._AUTHORIZED_CER_OVERLAY_FIELDS_V1
    assert "future_training_admission_candidate" in subject._AUTHORIZED_CER_OVERLAY_FIELDS_V1


def test_overlay_selection_is_frozen_event_id_exact4_not_ligand_wide() -> None:
    source = inspect.getsource(subject._overlay_cer_exact4_v1)
    assert "if event_id not in matrix_by_event" in source
    assert 'if row["ligand_component_id"] == "CER"' not in source


def test_predecessor_exact5_state_is_proven(frozen) -> None:
    subject._assert_predecessor_cer_state_v1(frozen)
    rows = [
        row for row in frozen.rows
        if row["canonical_event_id"] in set(subject.CER_EXACT4_EVENT_IDS_V1)
    ]
...
def test_cer_refreshed_semantics_and_training_prohibitions(computation) -> None:
    rows = [
    ...
    assert all(
        ...
        and row["human_review_authority_source"] == subject.CER_HUMAN_DECISION_SOURCE
        and row["chemistry_disposition"] == "POSITIVE"
        and row["chemistry_authority_source"] == subject.CER_EVENT_MATRIX_SOURCE
        and row["positive_authority_source"] == subject.CER_EVENT_MATRIX_SOURCE
...
def test_cer_structural_fields_are_unchanged(computation, frozen) -> None:
    before = {row["canonical_event_id"]: row for row in frozen.rows}
    after = {row["canonical_event_id"]: row for row in computation.rows}
    assert all(
        before[event_id][field] == after[event_id][field]
        for event_id in subject.CER_EXACT4_EVENT_IDS_V1
        for field in subject._STRUCTURAL_IDENTITY_FIELDS_V1
    )
    for event_id in subject.CER_EXACT4_EVENT_IDS_V1:
        assert all(
            after[event_id][field] == value
            for field, value in subject._EXPECTED_CER_STRUCTURAL_CELLS_V1.items()
        )
...
def test_exact_set_algebra_keeps_include_future_admission_runtime(computation, frozen) -> None:
    ...
    cer = set(subject.CER_EXACT4_EVENT_IDS_V1)
    assert after["chemistry_positive"] == before["chemistry_positive"] | cer
    assert after["chemistry_unresolved"] == before["chemistry_unresolved"] - cer
    assert after["task_relevant"] == before["task_relevant"] | cer
    assert after["task_unresolved"] == before["task_unresolved"] - cer
    assert after["training_include"] == before["training_include"] | cer
    assert after["training_exclude"] == before["training_exclude"]
    assert after["future_candidate"] == before["future_candidate"] | cer
    assert after["training_unresolved"] == before["training_unresolved"] - cer
    for key in (
...
def test_global_chemistry_task_training_and_status_counts(computation) -> None:
    assert Counter(row["chemistry_disposition"] for row in computation.rows) == Counter(
        {"POSITIVE": 120, "NOT_ESTABLISHED": 90, "UNRESOLVED": 790}
    )
    assert Counter(
        row["task_relevance_disposition"] for row in computation.rows
    ) == Counter({"RELEVANT": 121, "NOT_RELEVANT": 90, "UNRESOLVED": 789})
    assert Counter(row["training_use_disposition"] for row in computation.rows) == Counter(
        {
            "INCLUDE": 52, "EXCLUDE_FROM_TRAINING_ONLY": 68,
            "NOT_APPLICABLE": 90, "UNRESOLVED": 790,
        }
    )
...
def test_human_review_counts_are_distinct_from_global_negative(computation) -> None:
    assert computation.summary["human_review"] == {
        ...
        "pending_event_count": 207,
        "current_pending_review_unit_count": 111,
    }
...
def test_pair_role_exact5_geometry_and_training_counts(computation) -> None:
    ...
    assert summary["reactive_pair"]["sample_level_authoritative_pair_count"] == 120
    assert summary["reactive_pair"]["published_model_bound_target_constructible_count"] == 41
    assert summary["reactive_pair"]["cer_sample_authority_contribution_count"] == 4
    assert summary["reactive_pair"]["cer_model_bound_target_contribution_count"] == 0
    assert summary["role"]["role_profile_counts"] == {
        "STRICT_LINKER_PRESENT_V1": 52,
        "DIRECT_ATTACHMENT_OPTIONAL_LINKER_V1": 68,
        "other": 0,
    }
    assert summary["role"]["unknown_role_row_count"] == 880
    assert [
    ...
    assert summary["training_stage"]["future_training_admission_candidate_count"] == 35
...
def test_blocker_counts_are_exact_and_nonexclusive(computation) -> None:
    ...
    assert blockers["human_training_exclusion"] == {"within_positive_120": 68}
    assert blockers["missing_split_authority"] == {
        "within_positive_120": 79, "within_include_52": 27,
    }
    assert blockers["missing_tensor_integration"] == {
        "within_positive_120": 79,
        "within_include_52": 23,
    ...
    assert blockers["missing_POST_training_authority"] == {
        "within_positive_120": 103, "within_include_52": 35,
    }
    assert blockers["missing_training_admission"] == {
        "within_positive_120": 115, "within_include_52": 47,
    }
    assert blockers["feature_semantics_pending"] == {"within_positive_120": 120}
    assert blockers["pair_authority_absent"] == {
        "all_1000": 880, "within_positive_120": 0,
    }
...
def test_refresh_delta_is_exact(computation) -> None:
    assert computation.summary["refresh_delta"] == {
        "frozen_predecessor_positive_count": 116,
        "cer_exact4_delta_count": 4,
        "refreshed_positive_count": 120,
        "frozen_predecessor_training_include_count": 48,
        "refreshed_training_include_count": 52,
        "frozen_predecessor_training_exclude_count": 68,
        "refreshed_training_exclude_count": 68,
        "frozen_predecessor_future_candidate_count": 31,
        "refreshed_future_candidate_count": 35,
...
def test_reconciliation_and_dynamic_pending_queue_head(computation) -> None:
    result = reconciliation.reconcile_real_completed_human_decisions_with_cer_v1(REPO)
    ...
    assert expected[0]["review_unit_id"] == "COVAPIE_BULK_REVIEW_UNIT_9E98765987D25C42"
    assert expected[0]["ligand_component_ids"] == ["4M5"]
    assert expected[0]["pdb_ids"] == ["5AZT", "5AZV"]
    assert expected[0]["rank"] == 1
    assert expected[0]["raw_priority_rank"] == 20
    assert expected[0]["event_count"] == 4
    assert all(item["ligand_component_ids"] != ["CER"] for item in expected)
...
def test_authority_boundary_and_non_actions(computation) -> None:
    ...
    assert "NEXT_RECOMMENDED_MAINLINE" not in boundary
    assert "FILESYSTEM_MODE_AUTHORITY_TECH_DEBT" not in boundary
    assert boundary["CER_CENSUS_SOURCE_BINDING_V2_CLEAN_FROM_BIRTH"] is True
    assert boundary["CENSUS_REFRESH"] is True
    assert boundary["QUEUE_REFRESH"] is False
    assert boundary["next_priority_review_ligand"] == "4M5"
    assert boundary["next_priority_review_current_pending_rank"] == 1
    assert boundary["next_priority_review_raw_priority_rank"] == 20
    ...
    assert boundary["NEXT_REVIEW_STARTED"] is False
    assert boundary["CER_REVIEW_COMPLETED"] is True
    assert boundary["training_started"] is False
    assert boundary["TRAINING_STARTED"] is False
    assert boundary["formal_decision_read_directly"] is False
    assert boundary["formal_decision_bound_directly"] is False
    assert boundary["formal_validator_executed"] is False
    ...
    for key in (
        ...
        "warhead_type_authority", "reusable_chemistry_authority",
        "reusable_pair_authority", "reusable_role_authority",
    ):
...
def test_projection_digests_and_semantic_bindings(computation, frozen) -> None:
    ...
    assert len(computation.semantic_source_bindings) == 126
...
def test_no_direct_formal_parse_binding_or_exact_source_mode_authority(computation) -> None:
    ...
    additive_roles = {item[0] for item in subject._ADDITIVE_SOURCE_SPECS_V1}
    legacy_keys = {"artifact_role", "path", "path_namespace", "byte_count", "sha256"}
    for binding in computation.semantic_source_bindings:
        if binding["artifact_role"] in additive_roles:
            assert set(binding) == legacy_keys | {"expected_executable"}
            assert binding["expected_executable"] is False
        else:
            assert set(binding) in (
                legacy_keys,
...
def test_exact_posix_mode_cannot_be_added_to_semantic_binding(computation, frozen) -> None:
    ...
    with pytest.raises(ERROR, match="SEMANTIC_SOURCE_BINDING_SCHEMA_INVALID"):
        subject.validate_covapie_cumulative1000_current_global_readiness_census_with_cer_v1(
...
def test_exact3_build_materialization_and_two_directory_determinism(tmp_path: Path) -> None:
    built = subject.build_covapie_cumulative1000_current_global_readiness_artifacts_with_cer_v1(
        REPO
    )
    one = subject.materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_cer_v1(
        REPO, tmp_path / "one"
    )
    two = subject.materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_cer_v1(
        REPO, tmp_path / "two"
    )
...
def test_materialization_rejects_unexpected_sentinel_before_write(tmp_path: Path) -> None:
    destination = tmp_path / "sentinel"
    destination.mkdir()
    sentinel = destination / "unexpected.txt"
    sentinel.write_bytes(b"unchanged")
    with pytest.raises(ERROR, match="OUTPUT_DIRECTORY_UNEXPECTED_ENTRY"):
        subject.materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_cer_v1(
            REPO, destination
        )
    assert sentinel.read_bytes() == b"unchanged"
    assert {path.name for path in destination.iterdir()} == {"unexpected.txt"}


def test_materialization_rejects_root_symlink_before_write(tmp_path: Path) -> None:
    real_target = tmp_path / "real"
    real_target.mkdir()
    destination = tmp_path / "destination"
    destination.symlink_to(real_target, target_is_directory=True)
    with pytest.raises(ERROR, match="OUTPUT_ROOT_SYMLINK_FORBIDDEN"):
        subject.materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_cer_v1(
            REPO, destination
        )
...
def test_materialization_rejects_allowed_output_symlink_before_write(tmp_path: Path) -> None:
    destination = tmp_path / "allowed_symlink"
    destination.mkdir()
    external = tmp_path / "external"
    external.write_bytes(b"unchanged")
    (destination / subject.CENSUS_FILE).symlink_to(external)
    with pytest.raises(ERROR, match="OUTPUT_ENTRY_NOT_REGULAR"):
        subject.materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_cer_v1(
            REPO, destination
        )
    assert external.read_bytes() == b"unchanged"
    assert {path.name for path in destination.iterdir()} == {subject.CENSUS_FILE}


def test_materialization_rejects_unexpected_directory_before_write(tmp_path: Path) -> None:
    ...
    assert {path.name for path in destination.iterdir()} == {"unexpected"}


def test_materialization_exact3_rematerialization_is_idempotent(tmp_path: Path) -> None:
    destination = tmp_path / "idempotent"
    first = subject.materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_cer_v1(
        REPO, destination
    )
    first_disk = {path.name: path.read_bytes() for path in destination.iterdir()}
    second = subject.materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_cer_v1(
        REPO, destination
    )
    second_disk = {path.name: path.read_bytes() for path in destination.iterdir()}
    assert first == second
    assert first_disk == second_disk == first
    assert set(first_disk) == {subject.CENSUS_FILE, subject.SUMMARY_FILE, subject.MANIFEST_FILE}
...
def test_candidate_untracked_lifecycle_is_accepted() -> None:
    assert checker._classify_repository_lifecycle_v1(
        placement="CANDIDATE_UNTRACKED",
...
def test_future_tracked_clean_allows_multiple_commits_unrelated_and_origin_between() -> None:
    assert checker._classify_repository_lifecycle_v1(
        placement="TRACKED_CLEAN",
        head="later-head",
        origin="origin-between-baseline-and-head",
        ahead=3,
        behind=0,
        baseline_is_ancestor_of_head=True,
        baseline_is_ancestor_of_origin=True,
        origin_is_ancestor_of_head=True,
        baseline_to_head_changed_paths=(
            *subject.EXACT7_PATHS_V1,
            "src/covalent_ext/unrelated_future_successor_v1.py",
        ),
    ) == "TRACKED_CLEAN"


def test_pushed_tracked_clean_lifecycle_is_accepted() -> None:
    assert checker._classify_repository_lifecycle_v1(
        placement="TRACKED_CLEAN",
        head="direct-successor",
        origin="direct-successor",
        ahead=0,
        behind=0,
        baseline_is_ancestor_of_head=True,
        baseline_is_ancestor_of_origin=True,
        origin_is_ancestor_of_head=True,
        baseline_to_head_changed_paths=subject.EXACT7_PATHS_V1,
    ) == "TRACKED_CLEAN"


def test_future_lifecycle_simulations_are_fail_closed() -> None:
    result = checker.check_lifecycle_simulations_v1()
    assert result["candidate_untracked"] is True
    assert result["tracked_clean"] is True
    assert result["multiple_commits_allowed"] is True
    assert result["unrelated_successors_allowed"] is True
    assert result["origin_between_baseline_and_head_allowed"] is True
    assert result["missing_path_fails"] is True
    assert result["baseline_rewind_fails"] is True
    assert result["origin_divergence_fails"] is True
    assert result["behind_remote_fails"] is True
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"behind": 1},
        {"baseline_to_head_changed_paths": subject.EXACT7_PATHS_V1[:-1]},
    ],
)
def test_invalid_future_tracked_relations_fail_closed(updates) -> None:
    values = {
        "placement": "TRACKED_CLEAN",
        "head": "future-head",
    ...
    with pytest.raises(ValueError):
        checker._classify_repository_lifecycle_v1(**{**values, **updates})
...
def test_materialized_exact7_inventory_and_independent_checker() -> None:
    ...
    assert result["refreshed_positive_count"] == 120
    assert result["task_relevant_count"] == 121
    assert result["training_include_count"] == 52
    assert result["training_exclude_count"] == 68
    assert result["future_candidate_count"] == 35
    assert result["formal_training_admitted_count"] == 5
    assert result["current_runtime_model_usable_count"] == 17
    assert result["semantic_source_binding_count"] == 126
...
def test_predecessor_cer_row_not_unreviewed_rejected(frozen) -> None:
    rows = [dict(row) for row in frozen.rows]
    index = next(
        i for i, row in enumerate(rows)
        if row["canonical_event_id"] == subject.CER_EXACT4_EVENT_IDS_V1[0]
    )
    rows[index]["current_review_status"] = "CURRENTLY_IN_PROGRESS"
    with pytest.raises(ERROR):
        subject._assert_predecessor_cer_state_v1(replace(frozen, rows=tuple(rows)))


def test_predecessor_cer_wrong_review_unit_rejected(frozen) -> None:
    rows = [dict(row) for row in frozen.rows]
    index = next(
        i for i, row in enumerate(rows)
        if row["canonical_event_id"] == subject.CER_EXACT4_EVENT_IDS_V1[0]
...
def test_predecessor_cer_already_authoritative_rejected(frozen, field, value) -> None:
    rows = [dict(row) for row in frozen.rows]
    index = next(
        i for i, row in enumerate(rows)
        if row["canonical_event_id"] == subject.CER_EXACT4_EVENT_IDS_V1[0]
...
def test_non_cer_output_row_change_rejected(computation, frozen) -> None:
    mutated = _mutate_row(
        computation,
        lambda row: row["canonical_event_id"] not in set(subject.CER_EXACT4_EVENT_IDS_V1),
...
@pytest.mark.parametrize(
    "label",
    [
        "PREDECESSOR_WITH_1N0_CENSUS_OWNER",
        "PREDECESSOR_WITH_1N0_MATERIALIZED_CENSUS",
        "PREDECESSOR_WITH_1N0_MATERIALIZED_SUMMARY",
        "PREDECESSOR_WITH_1N0_MANIFEST",
        "CER_RECONCILIATION_OWNER",
        "CER_INGESTION_OWNER",
        "CER_EVENT_TASK_LABEL_AVAILABILITY",
    ],
)
def test_frozen_source_byte_drift_rejected(monkeypatch, label) -> None:
    ...
    def corrupt(**kwargs):
        if kwargs["label"].startswith(label + ":"):
            raise subject.SourceBindingPolicyV2Error("tampered")
        return original(**kwargs)

    monkeypatch.setattr(subject, "verify_bound_source_v2", corrupt)
    with pytest.raises(ERROR):
...
def test_reconciliation_contract_drift_rejected(monkeypatch) -> None:
    ...
    with pytest.raises(ERROR):
        subject._validate_cer_reconciliation_v1(REPO)
...
def test_matrix_must_be_source_derived(monkeypatch) -> None:
    ...
    monkeypatch.setattr(subject.cer_ingestion, "build_artifacts_v1", lambda root: drifted)
    with pytest.raises(ERROR):
        subject._load_and_validate_cer_event_matrix_v1(REPO)
...
def test_raw_priority_queue_byte_drift_rejected(monkeypatch) -> None:
    ...
    def drift(**kwargs):
        if kwargs["label"].startswith("FROZEN_PRIORITY_QUEUE:"):
            raise subject.SourceBindingPolicyV2Error("tampered")
        return original(**kwargs)

    monkeypatch.setattr(subject, "verify_bound_source_v2", drift)
    with pytest.raises(ERROR):
...
def test_matrix_event_coverage_mutations_rejected(matrix_rows, mutation) -> None:
    ...
    if mutation == "missing":
        ...
        extra = dict(rows[-1])
        extra["canonical_event_id"] = "EXTRA_CER_EVENT"
        extra["scaleup_rank"] = "674"
        rows.append(extra)
    with pytest.raises(ERROR):
        subject._validate_cer_matrix_rows_v1(rows)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"human_training_use_disposition": "EXCLUDE_FROM_TRAINING_ONLY"},
        {"training_use_human_authoritative": "false"},
...
def test_refreshed_row_semantic_mutations_rejected(computation, frozen, updates) -> None:
    mutated = _mutate_row(computation, _cer_predicate, **updates)
    with pytest.raises(ERROR):
        subject.validate_covapie_cumulative1000_current_global_readiness_census_with_cer_v1(
            mutated, predecessor_computation=frozen
        )
...
def test_summary_mutation_rejected_by_independent_checker(computation) -> None:
    summary = deepcopy(computation.summary)
    summary["blockers"]["missing_tensor_integration"]["within_positive_120"] = 78
    with pytest.raises(ValueError):
        checker.independently_verify_counts_v1(list(computation.rows), summary)


@pytest.mark.parametrize("stale_key", ["within_positive_116", "within_include_48"])
...
def test_manifest_binding_mutation_rejected(computation) -> None:
    artifacts = subject.build_covapie_cumulative1000_current_global_readiness_artifacts_with_cer_v1(
        REPO
    )
