...
from covalent_ext import covapie_cumulative1000_current_global_readiness_census_with_nwj_v1 as subject


REPO_ROOT = Path(__file__).resolve().parents[1]
ERROR = subject.Cumulative1000CurrentGlobalReadinessCensusWithNWJError
TARGET = set(subject.NWJ_EXACT4_EVENT_IDS_V1)

CHECKER_SPEC = importlib.util.spec_from_file_location(
    "check_with_nwj", REPO_ROOT / subject.CHECKER_RELATIVE
)
...
def _validate(bundle, computation) -> bool:
    return subject.validate_covapie_cumulative1000_current_global_readiness_census_with_nwj_v1(
        computation,
        repo_root=REPO_ROOT,
...
def test_predecessor_binding_and_unresolved_nwj_state(bundle) -> None:
    subject._assert_predecessor_nwj_state_v1(bundle["frozen"], REPO_ROOT)
    subject._verify_predecessor_bindings(
        REPO_ROOT, bundle["frozen"].semantic_source_bindings
    )
    assert len(bundle["frozen"].semantic_source_bindings) == 174
...
def test_reconciliation_exact25_143_and_review_summary(bundle) -> None:
    result = bundle["reconciliation"]
    assert len(result.source_bindings) == 25
    assert len(result.normalized_facts) == 143
    assert len({binding.stable_identity for binding in result.source_bindings}) == 25
...
def test_matrix_exact4_role_pair_pre_post_and_training_boundary(bundle) -> None:
    rows = subject._validate_nwj_matrix_rows_v1(bundle["matrix"])
    assert tuple(row["canonical_event_id"] for row in rows) == ingestion.EXPECTED_EVENT_IDS
    assert tuple(int(row["scaleup_rank"]) for row in rows) == (674, 675, 676, 677)
    for row in rows:
        assert row["human_review_completed"] == "true"
        assert row["human_task_relevance_decision"] == "IN_DOMAIN"
        assert row["human_chemistry_decision"] == "POSITIVE"
        assert (row["protein_reactive_atom"], row["ligand_reactive_atom"]) == ("SG", "CAV")
        assert row["selected_role_candidate"] == "CANDIDATE_A_DIRECT_FORMYL"
        assert row["role_profile"] == "DIRECT_ATTACHMENT_OPTIONAL_LINKER_V1"
        assert row["direct_profile_applicable_task_ids_json"] == "[0,3,4]"
        assert json.loads(row["warhead_atom_ids_json"]) == ["CAV", "OAE"]
        assert json.loads(row["linker_atom_ids_json"]) == []
        assert json.loads(row["minimal_seed_atom_ids_json"]) == ["CAX", "CAI", "CAK"]
        assert row["primary_anchor"] == "CAX"
        ...
        assert row["final_PRE_reaction_status"] == "PRE_SOURCE_GRAPH_MAPPING_INCOMPATIBLE"
...
def test_schema_exact4_delta_and_overlay_sets(bundle) -> None:
    ...
    assert len(subject._AUTHORIZED_NWJ_OVERLAY_FIELDS_V1) == 19
    assert len(subject._AUTHORIZED_BUT_UNCHANGED_NWJ_FIELDS_V1) == 1
    assert len(subject._ACTUAL_CHANGED_NWJ_FIELDS_V1) == 18
    for event in TARGET:
        changed = {
        ...
        unchanged = {
            field for field in subject._AUTHORIZED_NWJ_OVERLAY_FIELDS_V1
            if before[event][field] == after[event][field]
        }
        assert changed == subject._ACTUAL_CHANGED_NWJ_FIELDS_V1
        assert unchanged == subject._AUTHORIZED_BUT_UNCHANGED_NWJ_FIELDS_V1
...
def test_target_projection_semantics(bundle) -> None:
    ...
    for row in rows:
        ...
        assert row["human_review_authority_source"] == subject.NWJ_HUMAN_DECISION_SOURCE
...
def test_global_distributions_review_delta_and_authority_counts(bundle) -> None:
    ...
    assert Counter(row["current_global_status"] for row in rows)["CURRENTLY_UNREVIEWED"] == 171
    ...
    assert sum(row["reactive_pair_sample_authoritative"] == "true" for row in rows) == 156
    assert sum(row["role_partition_sample_authoritative"] == "true" for row in rows) == 148
    assert sum(row["canonical_mask_structural_labels_available"] == "true" for row in rows) == 148
...
def test_exact5_b3_no_sixth_and_applicability_counts(bundle) -> None:
    ...
    assert applicability == Counter({0: 148, 1: 56, 2: 56, 3: 148, 4: 148})
    assert bundle["computation"].summary["role"]["direct_profile_A_B3_C_count"] == 92
...
def test_summary_blockers_and_nwj_source_composition(bundle) -> None:
    summary = bundle["computation"].summary
    blockers = summary["blockers"]
    assert blockers["population_sizes"] == {
        "chemistry_positive_population_count": 156,
        "training_include_population_count": 68,
    }
    assert blockers["chemistry_unresolved"] == {"all_1000": 754}
    assert blockers["pair_authority_absent"] == {"all_1000": 844, "within_chemistry_positive": 0}
    assert blockers["role_authority_absent"] == {"all_1000": 852, "within_chemistry_positive": 8}
    assert blockers["missing_split_authority"] == {
        "within_chemistry_positive": 115, "within_training_include": 43,
    }
    assert blockers["missing_tensor_integration"]["within_chemistry_positive"] == 115
    assert blockers["missing_tensor_integration"]["within_training_include"] == 39
    assert blockers["missing_tensor_integration"]["missing_source_composition"]["NWJ"] == 4
    ...
    assert blockers["feature_semantics_pending"] == {"within_chemistry_positive": 156}
    assert summary["training_stage"]["future_candidate_source_composition"]["NWJ"] == 4


def test_next_pending_6oa_and_authority_boundary(bundle) -> None:
    summary = bundle["computation"].summary
    assert summary["human_review"]["completed_event_count"] == 167
    assert summary["human_review"]["completed_unit_count"] == 29
    assert summary["human_review"]["unreviewed_event_count"] == 171
    assert summary["human_review"]["unreviewed_unit_count"] == 102
    top = summary["top_pending_review_units_by_event_yield"][0]
    assert top == {
        "rank": 1, "raw_priority_rank": 29,
        "review_unit_id": subject.NEXT_PENDING_REVIEW_UNIT_ID_V1,
        "event_count": 4, "pdb_ids": ["4OU2"], "ligand_component_ids": ["6OA"],
    ...
    assert boundary["NWJ_REVIEW_COMPLETED"] is True
    assert boundary["NWJ_CENSUS_SOURCE_BINDING_V2_CLEAN_FROM_BIRTH"] is True
...
def test_semantic_lineage_exact180_without_collisions(bundle) -> None:
    bindings = bundle["computation"].semantic_source_bindings
    assert len(bindings) == 180
    assert tuple(bindings[:174]) == bundle["frozen"].semantic_source_bindings
    assert [item["artifact_role"] for item in bindings[174:]] == [
        item[0] for item in subject._ADDITIVE_SOURCE_SPECS_V1
    ]
    assert len({(item["path_namespace"], item["path"]) for item in bindings}) == 180
    ...
    assert materialized_manifest["nwj_reconciliation_artifact_validation_binding"][
...
@pytest.mark.parametrize(
    ("field", "value"),
    (
        ("canonical_event_id", subject.NWJ_EXACT4_EVENT_IDS_V1[0] + ":TAMPER"),
...
def test_target_semantic_tampers_fail_closed(bundle, field: str, value: str) -> None:
    with pytest.raises(ERROR):
        _validate(bundle, _mutate(bundle, subject.NWJ_EXACT4_EVENT_IDS_V1[0], **{field: value}))
...
def test_duplicate_nwj_event_fails_closed(bundle) -> None:
    rows = deepcopy(list(bundle["computation"].rows))
...
def test_matrix_semantic_tampers_fail_closed(bundle, field: str, value: str) -> None:
    ...
    rows[0][field] = value
    with pytest.raises(ERROR):
        subject._validate_nwj_matrix_rows_v1(rows)
...
@pytest.mark.parametrize(
    ("path", "value"),
    (
        (("top_pending_review_units_by_event_yield", 0, "review_unit_id"), subject.NWJ_REVIEW_UNIT_ID_V1),
        (("top_pending_review_units_by_event_yield", 0, "raw_priority_rank"), 30),
        (("authority_boundary", "QUEUE_REFRESH"), True),
...
def test_published_nwj_source_digest_tampers_fail_closed(monkeypatch) -> None:
    spec = list(subject._ADDITIVE_SOURCE_SPECS_V1)
    matrix_index = next(
        index for index, item in enumerate(spec)
        if item[0] == "NWJ_EVENT_TASK_LABEL_AVAILABILITY"
    )
...
def test_published_nwj_reconciliation_digest_tamper_fails_closed(monkeypatch) -> None:
    byte_count, _digest, executable = subject._NWJ_RECONCILIATION_ARTIFACT_SPEC_V1
    monkeypatch.setattr(
        subject, "_NWJ_RECONCILIATION_ARTIFACT_SPEC_V1",
        (byte_count, "0" * 64, executable),
    )
    with pytest.raises(ValueError, match="NWJ_RECONCILIATION_ARTIFACT_BINDING_INVALID"):
        checker.verify_frozen_bindings_v1(REPO_ROOT)
...
def test_materialization_outside_authorized_root_fails(tmp_path: Path) -> None:
    with pytest.raises(ERROR, match="OUTPUT_DIRECTORY_NOT_AUTHORIZED"):
        subject.materialize_covapie_cumulative1000_current_global_readiness_artifacts_with_nwj_v1(
